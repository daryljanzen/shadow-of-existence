...
| **⛔ every asymptotically-flat optics formula must be re-derived on $f=1-2M/r-r^2/\alpha^2$, because this construction is never asymptotically flat** | ***LANDED r3577*** — *placed in P7, at the deflection derivation with its `\ldg` marker.* | *`never asymptotically flat` and `not asymptotically flat` appear **nowhere**; `asymptotically flat` occurs only in P1 and P2, neither of them at an optics formula.* **The bake walked into this trap itself** — the Schwarzschild shadow $b=\sqrt{27}M$ gave the clean, wrong $b=\alpha$ at Nariai — *and recorded it as the field's characteristic error.* **P7 is where the optics lives and where the fence belongs** |
