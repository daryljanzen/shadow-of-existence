...
# THE DOCUMENT LEDGER
*Generated by `scripts/classify_documents.py` at fork front **c54.194**. **Do not hand-edit** — declare `kind:` in a file's own frontmatter, or add it to the tables in the script.*
...
| `THE_WEAVE.md` | STATE | How the parts are one object — the per-paper orchestration grid and the record of what joins to what | — | c54.194 | 0 |
| `THE_WORK.md` | STATE | The working fork's OWN front-level view of the register — fronts, not rows. Absorbed by this line, never edited by it (ARC 15's prime directive). | — | c54.193 | 1 |
...
| `FOR_54.md` | RECORD | THE INBOX — what the observer line has found in the working fork's text and routed rather than edited. Read on arrival, and after each absorption. Items are dropped from this file the revision they are applied. | c54.194 | c54.194 |  |
| `FOR_56.md` | RECORD | THE RETURN INBOX — what the working fork has found in the observer line's instruments and registers, routed rather than edited. The mirror of FOR_54.md. Items are dropped from this file the revision they are applied. | c54.194 | c54.194 |  |
