...
| **`GRAV`** | whether species enter **the potential source AND $\Psi$'s anisotropic stress** through the rate or by their own density *(second site added r3089)* | ⚠ **OPEN — currently `0`** | *`0` routes every species through $(3H_c^2/2)\Omega_i$, which **presumes the rate is sourced by them**; `1` uses each species' physical $4\pi Ga^2\rho_i$, **symmetric between matter and radiation as Daryl's reading requires**. **The abstract's "rate fixed by the cosmological constant alone" argues for `1`; running it does not fix the fit.*** | *r3046 abstract; r3047 run* |
| **`PHZ`** | whether the mode inherits the branch-point phase $k\,r_*$ or is reset at entry | ⛔ **OPEN — currently `0` (reset), and the reset's stated reason is WRONG** | *the code says "unbounded tortoise phase leaves nothing to inherit"; **r2154 ① derived $r_*=0.2247\alpha$, FINITE**, with the phase **linear in $k$** and therefore **a sound-horizon renormalization** — which is the shape of the measured defect. **Wired and live: the comb moves hard with it.*** | *r2154 ①; r3092* |
| **`SLIP`** | the baryon-photon slip damping in the photon momentum equation | **DETERMINED — `1`** | *ordinary tight-coupling physics on the layer; belongs. **Wired r3090 only to test it, and it is eliminated: peaks byte-identical, $P_1/P_2$ $1.447\to1.442$.*** | *r3090* |
