---
name: owed
kind: STATE
current: r2735
description: Work this programme's turns CREATED — the register every other register here lacks, since all of them track work discharged.
sources: [chat]
---

# OWED — work a turn created

> ⛭ *Daryl, r2735: "what then becomes owed work or still todos sitting there newly as a
> result." Every other register tracks work DISCHARGED. This one tracks what a finding
> PUT THERE — and it is the half that evaporates at a compaction unless it is written.*

- [ ] (416) PO-4: a COMPACT one-parameter subgroup, if one exists -- r2733 exhausted what the substrate supplies and found only a non-compact rapidity (SL(2,R))
- [ ] (416) PO-5: where can a gauge connection on this substrate FAIL TO BE FLAT? -- r2729's restatement, never worked
- [ ] (416) PO-6: which OPERATOR ORDERING does the corpus commit to? r2728 computed both branches and the corpus fixes neither
- [ ] (416) PO-10: what would a comparison look like that does NOT import a parameter vector the corpus does not posit? -- r2726, asked nowhere yet
- [ ] (416) P14 edit owed: the holonomy wall's stated reason ('only phases') is too narrow -- r2734 -- the paper still carries it
- [ ] (416) WITHDRAWN r2735: the P14 edit is not owed -- P14 never said 'only phases'; the narrow reason was this line's, written into PO-5's row at r2666/r2667 and corrected there at r2734
- [ ] (424) PO-6 counterterm half: compute the shear deficit 4 sigma^2 + O(sigma^4) -- cc54's c54.215 named it; nobody has run it
- [ ] (424) PO-4: the two ABSENT su(2) directions -- cc54 c54.218 shows <T,w> = N(T) is dimension 1 against 3; r2718's 'one factor short' framing is withdrawn
- [ ] (424) PO-11: re-verdict cc54's c54.214 -- the omega != 0 problem was never posed, and posing it builds the object; my r2717 strike used the omega = 0 reading
- [ ] (424) PO-5: cc54 c54.216 bounds the residue to ONE TEST -- a fixed pure number -- which contradicts nothing in r2729 and sharpens it
- [ ] (428) PO-11 (REOPENED r2739): the quantised field, mode completeness, and the JOIN between the static region's continuum and the wall -- P14's own words for what remains; the radial continuum is delivered and is NOT the sector
- [ ] (429) cc54 FOR_56 items 26/28/30/32 (the stale-receipt class): converted A7 and C32 at r2740; the class recurs every time a finding is acted on, and the standing habit is in the wisdom ledger rather than a gate
- [ ] (430) PO-5 + the constant ledger are ONE QUESTION (c54.216): does the corpus commit that ell_P is a GAUGE? p0 states it; the test is whether any paper treats it as a second scale
- [ ] (432) PO-6: compute the counterterm basis at SECOND order in the shear -- r2743 established the degeneracy survives to first order and breaks at two gravitons; what the second-order basis CONTAINS is unrun
- [ ] (433) PO-11: do JanzenGeometricCore / JanzenBoundary already carry the horizon matching P14 cites them for? r2744 named the KIND of problem; whether the two papers supply it is UNCHECKED
- [ ] (435) P14 EDIT: the join sentence cites JanzenGeometricCore/JanzenBoundary for a horizon crossing neither carries; JanzenSlicing and janzen_circle carry the C-infinity continuation through r=0 (r2745). A citation aimed at the wrong two papers.
- [ ] (436) PO-10: (a) the published measurement uncertainties for each derived quantity -- data, legitimately imported; (b) the list of which corpus quantities are DERIVED not fitted, from the papers' own statements. r2746 specified the comparison and refused to score it on invented sigmas.
- [ ] (438) PO-10: check the OTHER THREE pairs (r_s 145.4, 1+z_eq, the onset ratio) against the receipts that assert them, as r2748 did for theta_* -- the prose value and the asserted value differed by a factor of 2.7 on the first one
- [ ] (441) P15 DISCREPANCY (r2749): the damping-scale signature is +10.83% by C8's analytic baryon-weighted derivation and +8.2% by P15_damping_ratio_clean on CAMB's exact x_e(z). C8 flags it; nothing resolves it. P15 calls it 'a real, computed effect' nine times.
- [ ] (443) P15 DAMPING: compute the x_e(a) RESPONSE to the radiation-free rate. r2750 shows C8's cancellation assumes x_e identical and the CAMB receipt does not -- neither has computed the response, and it is smaller than either since both agree the effect is real and of order ten per cent.
- [ ] (445) P15 DAMPING (r2751): fix the CAMB arm's r_D integration -- P15_damping_ratio_clean's own GATE reports pi*r_D/r_s = 0.1434 against CAMB's thetad/thetastar = 0.1544, a 7.1% miss on the LCDM arm. That defect, not the x_e response, is the size of the 10.83-vs-8.97 gap.
- [ ] (446) P15 DAMPING RESOLVED (r2752): the gap is truncation. Converged ratio +9.94% vs C8's +10.83%; the 0.9pp residual is C8's baryon weighting plus its seam-carried radiation density, both measured. What remains: decide whether P15's ~8% should become ~10%.
- [ ] (447) P15: decide the damping signature's value now that the discrepancy is resolved. Candidates: +5.66% (full Hu-Sugiyama, converged), +8.37% (x_e weighted, no baryon term), +8.97% (the receipt's own), +10.83% (C8, now known to omit the x_e weight). P15 says ~8% nine times.
- [ ] (448) P15 CARRIES THREE VALUES FOR ONE DEFINED OBSERVABLE (r2753): sec:tensions says theta_D/theta_* is 'larger by 9.4%'; nine places say '~8%'; P15_damping_ratio_clean computes that exact ratio as 1.0816 = +8.16%. The observable is NAMED, so this is a numbers problem, not a definitional one.
- [ ] (448) P15 CARRIES THREE VALUES FOR ONE NAMED OBSERVABLE (r2754): sec:tensions says theta_D/theta_* is 'larger by 9.4%'; nine places say '~8%'; P15_damping_ratio_clean computes that exact ratio as 1.0816 = +8.16%. The observable is NAMED, so this is a numbers problem.
