...
| **6** | **`L-230`** | `L-165` `L-221` | 3/3 | 1/3 | the `COMPUTES:` convention — uptake ⛔ **FALLING, re-measured r2551**: 39/309 (12.6%) at r2447 → 40/357 (11.2%) → **40/395 (10.1%)** — **one adoption while eighty-six receipts were written**, and this line wrote **84 this session and used it once**. ⇒ **The decision: require the header's PRESENCE (mechanical, and ~84 receipts would fail) or stop calling it a convention** — **the number is unmeasured since**, and a convention whose uptake nobody re-counts is a convention nobody is adopting while the corpus grew a sixth; a receipt that declares what it computed is a receipt a vein can be excavated through |
