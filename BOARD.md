...
>
> *11 live rows: **4 veins**, **7 leads**. 260 struck.*
...
| **4** | **`L-230`** | `L-165` `L-221` | 3/3 | 1/3 | the `COMPUTES:` convention — uptake ⛔ **FALLING, re-measured r2551**: 39/309 (12.6%) at r2447 → 40/357 (11.2%) → **40/395 (10.1%)** — **one adoption while eighty-six receipts were written**, and this line wrote **84 this session and used it once**. ⇒ **The decision: require the header's PRESENCE (mechanical, and ~84 receipts would fail) or stop calling it a convention** — **the number is unmeasured since**, and a convention whose uptake nobody re-counts is a convention nobody is adopting while the corpus grew a sixth; a receipt that declares what it computed is a receipt a vein can be excavated through |
| **5** | **`L-207`** | `L-165` | 1/3 | 3/3 | ⓶ the confined/branch-point exhibition — **gated on PO-6 and must not be opened without it** (⓵ discharged r2450) |
| **6** | **`L-218`** | *— instrument work* | 3/3 | 0/3 | ✔ ⓶ done r2478 (`COMPANION_SPEC`) · ⓵ the reader-package formats via pandoc in CI · ⓷ the contribution, last |
| **7** | **`L-228`** | *— instrument work* | 3/3 | 0/3 | `check_loci` extended to the possessive and compound-noun forms node 52 declined to claim |
