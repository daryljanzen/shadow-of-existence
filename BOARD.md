...
|---|---|---|---|---|---|
| **1** | **`L-174`** | `L-165` `L-175` | 3/3 | 3/3 | **exhibit a bend carrying the graviton's TWO transverse polarizations** — the one unworked stratum, and everything with $\ge1$ Killing vector is worked (closed form / LTB r2450 / Gowdy in P11). ⛭ c54.198: the free shear is **two, not five** — the momentum constraint fixes $W$ under the York split — and `cor:radiation` already names them: **'the transverse degrees of freedom a sweep cannot carry'**, so the question is whether anything other than a sweep is available. ⛔ c54.199: and the two the constraint LEAVES are the ones that carry the geometry — **Schwarzschild's ADM shear is $3M/r^3$ in Painlevé–Gullstrand and $0$ in static slices, the whole difference longitudinal, so $\sigma^{TT}=0$ in both** — so **the three the constraint owns carry the FOLIATION and the two it leaves carry the GEOMETRY**, and ⛔ **Goldberg–Sachs bears on the OPTICAL shear, not on $\sigma^{TT}$** (r2505’s attribution withdrawn) |
