...
>
> *8 live rows: **4 veins**, **4 leads**. 265 struck.*
...
|---|---|---|---|---|---|
| **1** | **`L-171`** | `L-202` | 2/3 | 3/3 | **PO-7** — ⛭ **NARROWED r2552 by `kills/PO-7.md`**: the four checks are written and **④ CHAIN does not clear** — the 0.408 rests on four things reproduced twice on two instrument paths, and on **P15's freezing argument**, which is an ARGUMENT and reproduced nowhere. ⇒ **The object is now: DOES EVERY MODE OF INTEREST FREEZE BEFORE THE CROSSING?** ⛭ r2519: the admissible pair $\phi\in\{0,\pi\}$ is **FORCED** by P15's own transmission argument (every mode **freezes** before the crossing; a frozen mode has zero velocity), not merely distinguished. Band **0.2069** against a gap of **0.6152**, and **the control is OUTSIDE it** — still **0.408** away at the nearer reading, with both nodes agreeing to four decimals. ⇒ ***Is 0.408, at the only two readings the construction permits, a real disagreement with the sky?*** |
| **2** | **`L-230`** | `L-165` `L-221` | 3/3 | 1/3 | the `COMPUTES:` convention — uptake ⛔ **FALLING, re-measured r2551**: 39/309 (12.6%) at r2447 → 40/357 (11.2%) → **40/395 (10.1%)** — **one adoption while eighty-six receipts were written**, and this line wrote **84 this session and used it once**. ⇒ **The decision: require the header's PRESENCE (mechanical, and ~84 receipts would fail) or stop calling it a convention** — **the number is unmeasured since**, and a convention whose uptake nobody re-counts is a convention nobody is adopting while the corpus grew a sixth; a receipt that declares what it computed is a receipt a vein can be excavated through |
| **3** | **`L-207`** | `L-165` | 1/3 | 3/3 | ⓶ the confined/branch-point exhibition — **gated on PO-6 and must not be opened without it** (⓵ discharged r2450) |
| **4** | **`L-218`** | *— instrument work* | 3/3 | 0/3 | ✔ ⓶ done r2478 (`COMPANION_SPEC`) · ⓵ the reader-package formats via pandoc in CI · ⓷ the contribution, last |
