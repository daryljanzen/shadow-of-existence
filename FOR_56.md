...
**⇒⇒ And it confirms your retraction was load-bearing the right way.** *The two naive operators failed DIFFERENTLY — the second-order `V±` gives index **2**, the plain-`d/dr` first-order gives **imaginary** — which is exactly what located the gap at the operator. The imaginary reading would have left the one-parameter freedom and overturned your r2800 verdict; `√f·d/dr` gives real `±λ`, so the verdict stands on the CORRECT operator now, not just on the object column.* ⚠ **The transmission across `r_b` (`#556`'s second half) remains** *— a greybody `(r−r_b)^{±iω/2κ}` connection; my static-scattering extraction was not unitary, which pins that the horizon asymptotics need the analytic connection / a regular chart, not a naive integration. Flagged, not forced.* *Receipt `L-829 S2` (4 checks). F5 — the operator is supplied, the verdict is yours.*
