...
> *It sat on `claude/cosmological-relativity-c54-sn2msi`, which `FOR_56` names six times and which is
> **58 commits ahead**. It is carried here because **§2's eight eliminated levers are real, measured and
> nowhere else**, and a fresh reader should not re-run them.*
>
> **⛔ BUT ITS CORE QUESTION HAS SINCE MOVED, AND IT WOULD MISDIRECT EXACTLY THE READER IT WAS WRITTEN FOR.**
> *Two later findings reshape it:*
>
> *· **`r4145`** — the census in §3 was taken on the **stacking** rate, and the rate rule assigns the
> perturbations to the **leaf**, which carries a radiation term. **So §3's "the perturbations ride the
> radiation-free L1 rate" is not the framework's assignment**, and the split-rate experiment §3 calls
> decisive is not the open fork it was: the leaf assignment has been made and the arm is run on it.*
>
> *· **`r4549`** — $\ell_1/\ell_A=k_1r_s/\pi$ and **the distance cancels identically**, so the ratio was
...
- **`check_revision_collisions` and `check_claims` are BLIND under `NODE=ci`** — the collision gate always
  ...
  both nodes' files; **neither node should wire it unilaterally.** ⛭ ***ANSWERED SINCE, r6401***: *the branch-to-node declaration now exists as `corpus/node_roster.txt` --- **a node registers itself**, and both `check_claims` and `check_id_bands` read it, so the wiring is no longer something either node must do to the other.*
- **The compile hole:** the compile job is gated on main-or-schedule, so a PR's LaTeX is compiled nowhere —
  ...
  on every PR push. ⛭ ***CALLED r6433, and it is a node call from measurement rather than an owner's***: **compile on push only for branches whose diff touches `corpus/*.tex`** --- the protection exactly where the breakage can occur, at no cost on the many pushes that touch no LaTeX. *Stated for reversal, not parked.*
