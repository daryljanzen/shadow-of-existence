> **⌗ RETIRED r2380 under `RG-1` — ITS CONTENT IS THE ONTOLOGY MAP'S §1·LEVELS, AND ITS PAPER NUMBERING IS TWO
> RENAMES STALE.**
> *Assembled r822 as "the prerequisite the four failed transfer attempts (r818–r821) skipped: CR's actual
> perturbation structure, held in one place."* ***Its central content — the LEFTWARD reading — is now stated in
> the corpus's own voice and generalised beyond it***: *"the rate is **set** geometrically and $\rho$ is
> inherited content read off that clock, not a term that sources the rate," with the "a radiation fluid must
> gravitate and so alter the rate" objection named a **category error** — P15 §properframe/§flatlcdm verbatim,
> and the general form is `ONTOLOGY_FOUNDATION_INDEX` §1·LEVELS, **which does what this file could not**: it
> makes the which-rate answer **read off the geometry** at each computation (L1 the foliation stacking rate ·
> L2 the leaf's local dynamics · L3 the projection) rather than re-adjudicated and collapsed every time.*
>
> ⚠ *It refers throughout to **P13** for what is now **P15**, so a node following its section pointers lands in
> the wrong paper. **That is why a held picture is retired once the corpus holds it**: a picture is a snapshot of
> where things were written, and the writing moves.*
>
> **⌗ WHAT SURVIVES IT, and it is not the picture:** *the reason it was assembled at all — **"you need to read
> the papers… it's good to hold all the relevant stuff"** — is `THE_WISDOM_LEDGER` C's read-don't-grep discipline
> and `THE_PLAN`'s **GREP TO FIND, READ TO CONFIRM**. **Four failed transfer attempts bought that rule, and the
> rule outlived the picture.***
...
## 1. The background rate — radiation-free, and WHY (§properframe, §flatlcdm)
...
- r_s=∫_{z_rec}^{z_onset}(c_s/H)dz on the radiation-free H, depending on ONE early parameter z_onset≈z_onset≈6.8×10³ (T_onset~1.6 eV). Standard radiation-governed r_s is the forbidden z_onset→∞ limit.
- c_s the ordinary **baryon-loaded** value — the seam matter is pressureless (w=0); the throat ratio √3 does NOT set it.
- θ*=r_s/D_M **fixed by Ω_m alone**: the radiation-free rate carries H₀ out of both r_s and D_M, so the acoustic scale meets the data at the *directly measured* H₀ — no second H₀ (Hubble tension dissolved), the one z_onset the same at every H₀.
...
- **prop:subhorizon (decisive for the transfer):** at z_onset≈6850, k_hor=H₀E(z_onset)/[(1+z_onset)c]≈0.010/Mpc, with **E(z)=√(Ω_m(1+z)³+Ω_Λ)** (the radiation-free rate). The acoustic peaks are at k~π/r_s≈0.022/Mpc **and above** — so the acoustic modes are **inside the horizon at the branch point by ≳2×**.
...
- **Background:** the radiation-free rate H(z)=H₀√(Ω_m(1+z)³+Ω_Λ). Matter-like throughout; **no radiation-dominated epoch**.
...
- **The sharp CR prediction & refutation edge:** the radiation-free rate → little driving → the peaks sit near the *undriven* pattern (higher P1/P2 asymmetry than the driven ΛCDM), UNLESS the seam-handover ICs shift it. Whether the undriven-plus-seam pattern lands the observed heights is the test. This is a *specific, computable* prediction, not a free knob.

## 6. Why the four attempts failed (diagnosed)
All four fed **super-horizon adiabatic ICs driven through radiation domination** — the standard story. CR has neither: the modes start sub-horizon at the branch point, and the background is radiation-free (matter-like). The causal arrow was backwards (rate *sourced* by radiation, vs CR's rate *set* and radiation carried). The r817 skim-not-read error, one level down — now corrected at source.

## 7. The build, correctly scoped
A transfer from the **seam handover** (z_onset≈6850, sub-horizon coherent ICs, inherited amplitude) on the **radiation-free background** (matter-like, so Ψ≈const, little driving) to recombination — baryon loading, Silk damping, flat projection. Validate the *machinery* on ΛCDM (standard ICs+background) as a code check, then run the CR setup (seam ICs + radiation-free background) for the prediction. This is now a well-posed, bounded computation with the physics held correctly — the honest next build.
