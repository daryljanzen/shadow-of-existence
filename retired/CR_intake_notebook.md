> **⌗ RETIRED r2380 under `RG-1` — a single-session intake notebook whose session ended at r251.**
> *Its own scope line settles it: **"light intake notes, single-purpose — gather only what's needed to help with
> the map restructure… No verdicts, no running audit. The restructure itself is the first task AFTER intake —
> not performed here."*** *A document that declares itself preparatory for one named task, and names the task as
> happening elsewhere, is a record the moment that task runs.*
>
> ⚠ *And it is stale in the way that misleads a spin-up specifically: its confirmed reading order routes through
> **`CR_flatLCDM (P7, cosmology)`** — a bibkey and a role that no longer exist (P7 is the framework paper; the
> cosmology is P15), and the order it lists stops at eight documents where the `README` now runs ten steps over
> seventeen papers. **A stale reading order is the worst kind of stale card, because it is read first.***
