> **⌗ RETIRED r2380 under `RG-1`. WHERE ITS LIVE CONTENT WENT — and both its deliverables landed.**
> *Charted r828 to work "the open depth r826 named exactly: the detailed worldline dynamics for a concrete
> matter model," carrying two deliverables and a stated refutation test.*
> · ***The deuterium test — its own "real refutation test" — WAS RUN AND PASSED.*** *P16 returns
>   $D/H\simeq2.5\times10^{-5}$ at the observed value, and $^4$He, from a single inherited datum at
>   $-0.5\sigma$ and $+0.5\sigma$. **The arc said it would report whichever way it went; it went favourably and
>   the corpus states it as such.***
> · *The **exact peak heights** are now `L-171` at a sharper place (two internal routes, not CR against the
>   sky) and `L-147` for the likelihood, **fenced**: falsifier fixed before data, control run first.*
> · *The **worldline depth** it existed for is disposed twice: the **branch-point crossing dynamics closed at
>   c54.113** (there is no worldline crossing to compute — a comoving worldline terminates and what continues is
>   the analytic continuation), and what remains of "a concrete matter model" is `L-150`'s leading-order
>   interior.*
> ***What it uniquely held and what survives it: the governing discipline.*** *"The heights arc failed four
> times before r822 stopped and read the source" · **validate the machinery on ΛCDM before trusting a CR
> number** · prefer a robust method to a fragile ODE · **we do not steer toward the favourable branch**. Those
> are `THE_WISDOM_LEDGER` D and `THE_PLAN`'s per-turn list, and they are why this arc is a record and not a
> loss.*
