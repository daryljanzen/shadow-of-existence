> **⌗ RETIRED r2380 under `RG-1` — the audit ran, the bead is a theorem, and the audit's DISCIPLINE outlived it.**
> *An audit of r973→r988 whose object is now **`thm:bead`** in P7 — the closed contour as one analytic object,
> with `thm:antimatter-progenitor` built on it.*
>
> **⌗ WHAT SURVIVES IT is its opening discipline, and it is worth quoting because it is the whole method of the
> r2377–r2380 audit in four lines:** ***"No trusting the node's writeups. Every claimed edit checked against the
> current paper text AND against the geometry/source it rests on."*** *· the named failure it was auditing —
> **collapsing the DOUBLE ruling of the one-sheeted hyperboloid into "one bundle read under a sign"** — which is
> now `THE_WISDOM_LEDGER`'s partial-symmetry-flatten scrap · and* ***"when a real result is verified → cut a
> bundle + real changelog entry. Every time there's progress."*** *That last line is the four-step state advance,
> written before it was a rule and now gated by `check_changelog`.*
