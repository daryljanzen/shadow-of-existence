> **⌗ RETIRED r2380 under `RG-1` — IT LANDED, AND THE THING IT WAS GATING IS IN THE PAPER.**
> *Built r656 as T6 of the constant-ledger plan, with its own gate stated: **"the p0 write (T7) is gated on this
> landing."*** ***T7 landed.*** *p0 §sec:ledger carries the reframe in the paper's own voice: the traditional
> Planck length, mass and time are **cross-register gauge-combinations** mixing the thermal $\hbar$ with the
> real-geometric $c$ and $G$, so* ***"a Planck value is NOT a physical scale but what unit-counting yields when
> it has only gauges to count"*** *— the one physical length is $\alpha$, and $\alpha/\ell_P\sim10^{61}$ is the
> size of the universe in gauge-units, **a number, not a tuning**.*
>
> **⌗ AND ITS SECOND QUESTION LANDED TOO:** *whether the cosmological-constant problem reframes or dissolves —
> p0 §sec:ledger dissolves it on two premises the framework rejects, **with the honest residue stated there**
> that $\Lambda$'s *value* is the ledger's one input scale and is not predicted.* *Its sibling receipt,
> `retired/CONSTANT_LEDGER_receipt.md`, retired the same revision.*

---

# The Planck values from CR — T6 of the constant-ledger plan (the Planck work)

### Built with the gate at r656 (T6, Daryl-directed). A working build doc, not yet in a paper; every finding carries its source; the conjecture limbs are tagged and stated without being claimed; nothing here overrides a paper. Builds on the constant-ledger receipt (`retired/CONSTANT_LEDGER_receipt.md`) and P13 §176–179 (the Planck-units computation already in the corpus). The p0 write (T7) is gated on this landing.
...
- **The centerpiece conjecture (face 23 — prominent, not buried):** CR conjecturally dissolves the cosmological-constant problem *and* the coincidence problem by the one-scale fact. Grounded: the Planck-scale-is-not-physical half (CC) and the coincidence dissolution (P7 §515). **conjecture** and the striking conjecture: matter vacuum energy is a bend off the Λ-substrate, not a source of Λ (grounding path = the matter sector, A5). Stated boldly *as a conjecture*, honest on its open half.
- **T7 (the p0 Planck discussion):** write the Planck-values-as-gauge-combinations reading AND the **centerpiece conjecture, prominently** — CR's one scale dissolving the two deepest fine-tuning problems — with the bold conjecture stated strongly and the proof-status tagged honestly (Planck-scale half + coincidence grounded; CC-sourcing half conjecture, matter sector the grounding path; P7 §515 cross-ref). Do *not* bury it; do *not* assert the open half as proven. Eyes open, hands steady.
...
**Net:** half-(ii) moves striking-conjecture → grounded; the CC dissolution is now the two grounded halves (Planck scale not physical; no bare-$\Lambda$-vs-$\rho_{\rm vac}$ split), with $\Lambda$'s value the one honest residue. Dispersed r671 → p0 §sec:ledger (body + abstract + tag), the runway, the map. Stated for reversal.
