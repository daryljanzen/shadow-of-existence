...
> ⌗ *Its register — **do-not-assert, held both ways: colour-from-geometry neither asserted nor refuted** — is
...
- **NOT a constraint — the open hinge [do-not-assert, both ways; this is test 1, not a fact]:** that the real-substrate $\sigma$ *is* the Wick rotation bridging $SO(5,1)\leftrightarrow SO(6)$, and hence that the discrete $A_2$ skeleton *is* the real-Lorentzian shadow of the Wick-face $\mathfrak{su}(3)$. Source tags this do-not-assert (`colour_frontier_dS6.md` l.197); it identifies two signature-flips by shared flavor and is precisely what test 1 decides. **Do not hand a fresh head this clause as given** — taking it as fact starts test 1 with the answer baked toward "genuine lift." (Caught by c17 on relay; an earlier draft of this platter wrongly listed it as source-verified C7.)

---

## 4. The do-not-assert boundary (do not cross it from gist)
