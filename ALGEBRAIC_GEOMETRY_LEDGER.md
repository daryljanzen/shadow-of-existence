---
name: algebraic-geometry-ledger
kind: FORWARD
current: r3546
job: The algebraic-geometry field-bake ledger — what bit, what bounced, and the boundary. One of the three fields listed but never thrown (the r3505 overnight order: algebraic geometry ×57, discriminant ×28 / genus ×21). `OWED` 622.
sources: [cowork]
---

> **▣ FORWARD — the algebraic-geometry bake, and part of the corpus.** *Three registers kept apart:
> **what bit**, **what bounced**, **what the boundary is**. The overnight order named it —* ***"the
> three never thrown: … algebraic geometry ×57"*** *— and its two live sub-terms are `discriminant`
> ×28 and `genus` ×21. The corpus's spine is a CUBIC, $r^3-\alpha^2 r+2M\alpha^2$, and everything an
> algebraic geometer would ask of a cubic — its discriminant locus, its branch points, the cross-ratio
> of its roots, the genus of the curve it cuts, its equianharmonic point — the corpus already computes,
> under other fields' names. This bake's finding is expected to be a BOUNCE of a particular shape: the
> content is here and is receipted, but under `quadric`, `combinatorics`, and `complex-analysis` heads,
> not as "algebraic geometry".*


## ⌗ THE LANDING TABLE — where each register stands against the corpus, checked r3546

> **▣ THE `\ldg` GATHER FOR THIS LEDGER.** *Live prose only, `corpus/reach_baseline.py`, and every
> term searched in the CORPUS's spelling. **The distinction this field turns on is between an OBJECT
> being in a paper and its NAME being there**, and it is not a quibble here: this bake's own thesis is
> that the content is worked under other fields' labels.*

### ★★★ THE BOUNCE IS THE RESULT, AND IT IS A CLEAN ONE

*The forward predicted it: "the content is here and is receipted, but under `quadric`, `combinatorics`
and `complex-analysis` heads, not as algebraic geometry."* ⇒ ***Measured, that prediction holds
exactly.*** *`algebraic variety`, `scheme`, `Zariski`, `elliptic curve`, `Riemann surface`,
`j-invariant`, `canonical bundle`, `plane cubic`, `discriminant locus`, `branch locus` — **every one
×0 across seventeen bodies** — while the objects those words name are computed in five papers and
carry running receipts.*

⌗ **So this field's register is almost entirely LANDED, and what it is owed is almost entirely NAMES.**
*That is a different debt from the one the first nine gathers found, and it is a smaller one — but it
is not nothing, because a paper that computes a discriminant locus without saying so cannot be found
by anyone looking for one.*

### ⌗ THE TABLE

| register | its paper | disposition |
|---|---|---|
| **the horizon cubic's discriminant $\Delta=4-27(2M)^2$ and its zero-locus = the two Nariai values** | `P05`, `P12`, `P03` | ***LANDED.*** *`P05` states $\Delta$ and its simple zero; `P12` carries $-4\alpha^4(27M^2-\alpha^2)$ vanishing at $\Lambda M^2=1/9$ with the isotropy jump $4\to6$* |
| **the reducible cubic factors as line ∪ conic, and its SINGULAR POINT is Nariai** | `P03`, `P12` | ***LANDED, and in the field's own term.*** *`singular point` ×5 corpus-wide, and `P03`'s two are this; `P12` gives the factoring $(r-r_0)(r^2+rr_0+r_0^2-1)$ with the quadratic factor the fundamental ellipse* |
| **the 3-sheeted branched cover, monodromy $=$ Galois $=S_3$, $\Delta$ not a square** | `P05` | ***LANDED, and freshly repaired.*** *r3528–r3530 swept 21 sites in seven papers so the corpus now says ONE thing about deck-vs-monodromy; this row would have been PART-LANDED three revisions ago* |
| **the cross-ratio of the marked points, its equianharmonic collapse, $j=0$ / CM by $\omega$ kept apart from the horizon cover** | `P05` | ***LANDED, with its fence.*** *`equianharmonic` ×6 and `complex multiplication` ×1, both `P05`'s, and the paper's own "not here shown to be one thing" bound is intact* |
| **`P09`'s Petrov speciality discriminant $I^3-27J^2$ — a SECOND discriminant, independent of the horizon cubic's** | `P09` | ***LANDED.*** *`speciality` ×11, all `P09`'s; the object is the self-dual Weyl operator's characteristic cubic and it is genuinely not the horizon cubic* |
| ⚠ **`SP_S4` the two discriminants are the SAME ALGEBRAIC EVENT on different cubics** | `P09` ← spectral | **NOT THIS FIELD'S ROW.** *The spectral gather at r3542 holds it as `S4`, PART-LANDED.* ⇒ *Recorded here so the two ledgers do not both claim it and neither carries it — **this bake CREDITS and EXTENDS `SP_S4`, and the extension is that the two discriminants are the field's own subject, which makes the connection an algebraic-geometry statement rather than a coincidence of formulas*** |
| ⛔ **`L8` the family is a PENCIL of plane cubics, and its three critical loci are three of its members** | `P07` | ***POINTER OWED, and sharply.*** *`P07` cites `\rcpt{L8_the_pencil}` in its own prose — and **the word "pencil" occurs in `P07` exactly once, inside that receipt key**. `pencil` ×6 in live prose is `p0`'s five (pencils of PLANES and QUADRICS) and `P03`'s one; `plane cubic` ×0.* ⇒ ***A receipt landed in a paper that has no word for what it establishes.*** ⌗ *And `P03`'s only statement on the matter — "NOT a pencil" — is in a `%` masthead, is about the reducible-plane-cubic sense, and is invisible to every instrument that reads bodies* |
| **`genus` is TAXONOMIC corpus-wide, never the genus of a curve** | — | ⚠ **PREMISE WRONG, LANDED ANYWAY.** *The work order counted `genus` ×21 as this field's reach; all ~21 are the genus/species metaphor, glossed by `P02` at zeroth order as "the interval". **The correction is the finding** and it belongs on the record rather than in a paper* |
| **`P07`'s `blow-up` is a CURVATURE blow-up, not a resolution of a singularity** | — | ⚠ **PREMISE WRONG, LANDED ANYWAY.** *`blow-up` ×1 corpus-wide and it is the spacetime homonym; **no algebraic-geometry blow-up occurs anywhere**, so the field's most characteristic operation is genuinely absent* |
| ⛭ **`AG1` the two discriminants are one classical formula apart, and `resultant` is ×0** | `P03`, `P12` | ***POINTER OWED, and it is the sharpest instance of this field's whole thesis.*** *The corpus states the reducible factoring in two papers and computes BOTH of its degenerations — the conic's own discriminant $4-3r_0^2$ (which is `P03`'s slicing discriminant) and the line-meets-conic condition $3r_0^2-1$ — and their product with the second squared IS $\mathrm{disc}(C)$, the textbook discriminant formula for a reducible plane curve.* ⇒ ***Two papers hold the two halves of a named identity and neither names it; `resultant` ×0, `discriminant locus` ×0.*** ⌗ *Found because the catastrophe gather flagged the row that conflates them and 59 then found the same conflation live in its own `P3` landing — so this is the one place where the field's missing NAME had already cost a real sentence.* `AG1`, ALL PASS |
| **the nine CHECKED-NEGATIVE readings** | — | **HELD.** *`P06`'s `projective type` and `moduli-free` are genuine AG language CITED to `p0` and not worked there; `P16`'s `singular point` is an apparent pole; `p0` is negative for CURVE-AG while carrying substantial quadric content* |

### ⌗ WHAT THIS GATHER FOUND

***One pointer owed, two premises corrected, and every object landed.*** *No substance is owed by this
field to any paper — which, across ten gathers, makes it the first register with no substance debt at
all, and the reason is structural rather than creditable: **the objects were landed by other fields
before this one was thrown.***

⛭ **THE POINTER THAT IS OWED IS WORTH MORE THAN IT LOOKS.** *`L8_the_pencil` was, until r3537, the
corpus's ONE unresolvable `\rcpt{}` key — cited by `P07` and registered nowhere.* ⇒ ***So the same
structure was missing from the index AND missing from the paper's vocabulary, and the two absences have
the same cause: nothing in the corpus was calling it a pencil.*** *The index half is closed. The paper
half is one clause, and it buys the thing the receipt actually proves — that the three critical loci
are not three facts but one family read at three values of its parameter.*

⌗ **AND THE FIELD'S OWN BOUNDARY, stated plainly.** *An algebraic geometer reading this corpus would
find a cubic surface's worth of work and no variety, no scheme, no Riemann surface and no blow-up. **The
corpus does algebraic geometry of exactly one curve, thoroughly, and no algebraic geometry at all.***
*That is a true sentence about the boundary and it is not a debt: nothing the corpus claims needs the
apparatus it does not have.*

---

## ⛭ THE BASELINE, MEASURED FIRST

**Nothing below was called a hole before the corpus was asked what it holds.** *Seventeen paper bodies,
comments and bibliography stripped, de-macroed, via `corpus/reach_baseline.py`.*

| present, and genuine | homonym / thin | absent across all seventeen |
| --- | --- | --- |
| `discriminant` ×28 (P03×6, P05×6, P07×4 — the cubic's $\Delta=4-27(2M)^2$) · `genus` ×21 (P01×6, P02×6, p0×5) · `cross-ratio` ×5 (P05×4, p0×1) · `moduli` ×28 (parameter moduli) · `singular point` ×5 · `cusp` ×1 (P15) · `blow-up` ×1 (P07) · `divisor` ×1 (P15) | `variety` ×1 (P14, English) · `ideal` ×0 word-bounded · `resolution` ×39 (mostly "resolution" = detail/settling, not blow-up) | `algebraic variety` · `scheme` · `elliptic curve` · `projective/affine variety` · `Zariski` · `canonical bundle` · `Riemann surface` (as a NAMED term) · `intersection number` · `Weierstrass` · `j-invariant` |

⚑ **THE FIELD'S OBJECTS ARE ALREADY RECEIPTED — the finding is WHERE, not WHETHER.** *Before this
bake: the discriminant $\Delta=4-27(2M)^2$ and its zero-locus (`sweep2_discriminant_is_interval`,
`SP_S4_two_discriminants_one_event`, `Q5_nariai_on_the_locus`); the cross-ratio of the four marked
points and its equianharmonic collapse to $\{e^{\pm i\pi/3}\}$ (`Q4_equianharmonic_vantages`); the
cubic pencil and its critical values (`L8_the_pencil`, `cubic_spacing`). So the algebraic-geometry
content is worked; it wears `quadric`/`combinatorics`/`complex-analysis` labels. The bake READS all
seventeen and names where a genuinely algebraic-geometry statement (the discriminant as a plane curve /
branch locus; the genus) is or is not the corpus's own.*

---

## ⛭⛭⛭ REACH CLOSE — all seventeen papers WORKED or CHECKED-NEGATIVE by name (r3521)

*Two readers swept the seventeen bodies (P01/P02/P03/P05/p0 carriers; the other twelve). The field is
DEEP — the corpus's spine is a cubic, and everything an algebraic geometer asks of a cubic (its
discriminant locus, its reducible factoring into line ∪ conic, its branched cover and monodromy, the
cross-ratio and equianharmonic value of its roots, the Petrov speciality discriminant of the Weyl
operator's cubic) the corpus computes. Two crux findings governed the reading:*

- ***"genus" is TAXONOMIC corpus-wide, not topological.*** *Every one of its ~20 occurrences (P01, P02,
  P07, p0, P03) is the genus/species metaphor — "two metric singularities of one genus", glossed at
  zeroth order as "the interval" (P02 l.382) — NEVER the genus of a curve or Riemann surface. The
  order's "genus ×21" is not algebraic-geometry reach.*
- ***P07's "blow-up" is a CURVATURE blow-up (spacetime homonym), not an AG blow-up.*** *No genuine
  algebraic-geometry blow-up (resolution of a singularity) occurs anywhere — the work-order's premise
  is corrected on the record.*

| paper | status | where / why |
|---|---|---|
| `P01` | **CHECKED-NEGATIVE** | `BH_causality`: `genus` = taxonomic (genus/species); `affine` = affine parameter; `branch point` deferred to companions; no discriminant/cross-ratio worked |
| `P02` | **CHECKED-NEGATIVE** | `janzen_circle`: `genus` = taxonomic ("the interval, the genus", l.382); `projective` = map projection; the cubic is mentioned, not classified |
| `P03` | WORKED | `SdS-slicing`: the reducible plane cubic = **line ∪ conic**, the conic a 45°-tilted ellipse; discriminant $4-3r_0^2$ and its sign-strata; the **singular point of the reducible cubic** at Nariai (line meets conic); two cubics compared by $\operatorname{disc}H,\operatorname{disc}T$ (`P03_cubic_factor_ellipse_locus`, `Q5_nariai_on_the_locus`, `P03_cubic_spacing`, `P03_ellipse_foci`) |
| `P04` | **CHECKED-NEGATIVE** | `modern_parallax`: zero matches for any AG term in the whole file |
| `P05` | WORKED | `groupoid`: the deepest curve apparatus — discriminant $\Delta=4-27(2M)^2$ + branch locus; 3-sheeted branched cover; monodromy = deck = **Galois $S_3$** ($\Delta$ not a square); **equianharmonic** cross-ratio $\lambda^2-\lambda+1=0$, orbit-of-6 → 2; **$j=0$ / CM by $\omega$** explicitly kept distinct from the horizon cover (`Q4_equianharmonic_vantages`, `P05_deck_group_S3`, `X5_monodromy_group`) |
| `P06` | **CHECKED-NEGATIVE** | `shadow_of_existence`: `projective type` and `moduli-free` are genuine AG language but **cited** to p0, not worked here; `branch point` = cosmological |
| `P07` | WORKED | `CR_framework`: the worked hub — a **pencil of plane cubics** $r^3+(E^2-1)\alpha^2r+2M\alpha^2$, its **discriminant locus** $\Delta(E)=4\alpha^4(\alpha^2(1-E^2)^3-27M^2)$, and the **monodromy of the cover** ($S_3$, branching where two roots collide) (`two_realisations`, `order3_bridge`, `L8_the_pencil`). ⚠ `blow-up` = curvature blow-up (homonym); `genus` = taxonomic |
| `P08` | WORKED | `slicing_operator`: the same family read as **3-sheeted covers of the mass line**, discriminant $-4p^3-27q^2$ a square iff $p=0$, branch points where roots collide; the horizon-locus **ellipse's foci** at scale $2/\sqrt3$ (`order3_bridge`, `P08_nariai_signed_roots`). ⚠ one `discriminant` = "empirical discriminant" (criterion homonym) |
| `P09` | WORKED | `range_paper`: a genuinely **distinct** AG discriminant — the **Petrov speciality discriminant $I^3-27J^2$** of the self-dual Weyl operator's characteristic cubic, vanishing ⟺ two of three eigenvalues coincide (Type D) vs distinct (Type I) (`SP_S4_two_discriminants_one_event`, `P09_bianchiI_typeI`, `V1_carter_chain`); `moduli` of distinct vacuum cuts (mass, rotation, NUT) genuine |
| `P10` | **CHECKED-NEGATIVE** | `canonical_time`: horizon-cubic content only in a `%` comment (out of scope); `branch point` = cosmological; `degree of freedom` = physics DOF |
| `P11` | **CHECKED-NEGATIVE** | `dynamics_paper`: `conical` defect/strut (not a conic); `branch point` = cosmological; `degree of freedom` = physics DOF |
| `P12` | WORKED | `algebroid`: horizon-cubic **discriminant $-4\alpha^4(27M^2-\alpha^2)$** vanishing at $\Lambda M^2=1/9$ (double root, isotropy jump 4→6); the **reducible cubic = line ∪ conic** $(r-r_0)(r^2+rr_0+r_0^2-1)$, the quadratic factor the fundamental ellipse (l.321, l.327) |
| `P13` | WORKED (light) | `boundary_paper`: the reducible-cubic factoring restated, $(r-r_0)(r^2+rr_0+r_0^2-1)$, the three zero-sum roots a Cartan element with $S_3$ Weyl group (l.378) |
| `P14` | WORKED (light) | `matter_sector`: the reducible cubic (l.378); the **horizon cubic through the three hinges** (a cubic through three points, l.620); the "root variety" $(\omega r)^3-\omega r+2M=r(1-\omega)$ (l.883). ⚠ `discriminant` = the quark/lepton $\lambda\bmod3$ classifier (homonym) |
| `P15` | **CHECKED-NEGATIVE** | `CR_cosmology`: `discriminant` = "discriminant of principle" (a discriminating statistic); `divisor` = the quantity divided by (arithmetic); `cusp` = a **projection artifact** the smooth curve does not carry; `moduli`-vs-invariants a genuine aside but nothing worked |
| `P16` | **CHECKED-NEGATIVE** | `cosmogenesis`: `singular point` = an apparent **pole** of a complex variable (no monodromy); `singular` = spacetime metric singularity; `branch point` = cosmological (the $\mathbb{Z}_3$ cube-root cover deployed physically, worked as Fuchsian-ODE monodromy, not an AG branch locus) |
| `p0`  | **CHECKED-NEGATIVE** *(for curve-AG)* | `geometric_core`: `genus` = taxonomic; `cross-ratio` = Cayley–Klein **metric** log-cross-ratio (`Q3_cayley_klein`, quadric field); pencil-of-quadrics classification returns **null** (`D3_segre_no_pencil`); `node` = Dynkin node — substantial projective/quadric content, but no curve/variety/discriminant-of-a-cubic claim of its own |

⇒ ***Seventeen of seventeen accounted for: EIGHT WORKED, NINE CHECKED-NEGATIVE.*** *Reach: **17/17 ACCOUNTED FOR** — 8 **WORKED**, 9 **CHECKED-NEGATIVE by name** (the reading standard; the worked-only figure is $8/17$) — deep,
and the genuine content is concentrated in the horizon-cubic family (P03/P05/P07/P08/P12 the discriminant,
cover, monodromy, reducible factoring; P13/P14 restating it) plus one independent object, **P09's Petrov
speciality discriminant $I^3-27J^2$**. **No new receipt was needed at close**: every WORKED claim already
carries a `\rcpt{}` that runs — spot-checked this pass (`P03_cubic_factor_ellipse_locus`,
`SP_S4_two_discriminants_one_event` both exit 0). Cross-field: the horizon-cubic discriminant / cross-ratio
/ equianharmonic content is the SAME object the quadric, complex-analysis, and combinatorics bakes work —
one receipt set, four fields' vocabulary; the algebraic-geometry framing (discriminant LOCUS, reducible
cubic's singular point, branched cover) is the reading this bake adds by name, and it duplicates no
receipt. The two crux corrections — `genus` taxonomic, `blow-up` a curvature homonym — are on the record.*

