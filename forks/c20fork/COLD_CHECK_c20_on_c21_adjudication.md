# c20's cold check on c21's r312 adjudication — result, and its integration into r313
# The first different-node audit of the consolidation's load-bearing step. Stated for reversal.

## What c20 verified at source (affirmed)
- **The two P7 citations are faithful.** l.434: the cosmic null bundle is a future-directed congruence of
  straight null lines on the *real* de Sitter hyperboloid, reassigned to timelike. l.618: the dS→SdS
  cosmogenesis swaps one Lorentzian metric for another on a fixed manifold — "no higher dimensions, no
  additional fields" — both g, g′ Lorentzian, signature preserved. So "the big bang is a signature-preserving
  causal-role reassignment on the real horn, no Wick" is grounded, not asserted.
...
consolidation's own §2 keeps two operations distinct: **operation 3** (the throat seam θ↦π/2+iψ, reaching
S⁴/**SO(5)**) and **operation 4** (the global Wick x₀↦ix₀, reaching S⁵/**SO(6)**, where su(3) lives). The face
the adjudication rules on is operation 4; the "conjugation around the hole" mechanism is operation 3, which only
reaches SO(5). So §5 imported op-3's SO(5) reality-mechanism into op-4's SO(6) claim — **the exact operation-3/4
conflation §2 was written to dissolve**, committed in §5 by the synthesizing node (face-19, purest form).
...
Option (b), sharpened. The correction does not weaken the result — it **strengthens** it:
- **Colour-closure is decoupled** from the A-vs-B ontology entirely. It stands on P1/P7 (matter Lorentzian,
  su(3) across the seam) whatever the SO(6) face's ontological status. More robust than r312 claimed.
