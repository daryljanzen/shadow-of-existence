...
VEINS = {
 ...
 'L-175': ("PO-9 \u00b7 WHAT FIXES THE SUBSTRATE'S DIMENSION",
   ...
   "**\u2714 **ANSWERED r2572 by c54.207's `L-533`, and with a THIRD branch** \u2014 *whether a second slicing could be non-arbitrary*: **Rule 2 neither forbids it nor forces it \u2014 it EMPTIES it.** The routed test (*\u201cis any of it a modulus?\u201d*) could not run as written, because **it needs P6\u2019s three-sense guard and the offset IS the mass**, which P6 names as the sense the criterion does *not* exclude. \u21d2 With the guard: **every rung above the last must be maximally symmetric \u21d2 a plane section \u21d2 scale-only; the scale map has rank 1 over two choices; and the cut fixes $\\alpha$ from $\\Lambda$ and $r_0$ from the mass and is BLIND above.** \u21d2\u21d2 ***So the substrate\u2019s dimension is unbounded above because NOTHING BELOW CAN SEE THE DIFFERENCE \u2014 which makes CUT\u2192SUBSTRATE a CONSEQUENCE rather than a rule of conduct.*** \u26a0 **What stays dark on this vein is now only what the emptying leaves: nothing above the cut is decidable from below, which is a RESULT and not a question**** \u2014 and the substrate "
...
LEADS = {
 ...
 'L-171': ("**PO-7** \u2014 \u26ed **NARROWED r2552 by `kills/PO-7.md`**: the four checks are written and "
   "\u26d4 **\u2461 DOES NOT CLEAR \u2014 corrected r2599/r2602**: check \u2461 asks what would have to be true for the item to stay open, and `kills/PO-7.md`'s own closing line answers it \u2014 ***\u201cwhat remains of \u24b7 is a PROGENITOR derivation of the datum's phase\u201d***. **That is live, so nothing is pending in the register and none ever was.** \u21d2 ***And \u24b7's derivation is `PO-seam`'s dark half: the ORDER is `PO-seam` first, this second.*** \u2714 **\u2463 CHAIN CLEARS r2559** (cc54 `L-805`) \u2014 the 0.408 rested on four things "
