...
EST = {
    'PO-13': ('the misplaced phase — WHY the propagated comb runs short', 1, 1, 3, None,
        'r3049: the rate is generated by a deparametrized TRUE HAMILTONIAN on the layer, not sourced by a content budget. And the frameworks established reach is the VACUUM sector -- the CMB fit is a matter-sector claim it marks not-yet-built'),
...
SINCE = 0
LASTFIND = ("r3049: **the axioms prose supplies what the model needed.** The canonical remark: the "
            "constraint deparametrized along cosmic time is solved for **a TRUE HAMILTONIAN THAT "
            "GENERATES THE LAYERS ADVANCE**, and **the deparametrized generator reproduces the "
            "flat-LCDM expansion**. ⇒ **The rate is what the true Hamiltonian generates, not what "
            "a content budget sources** — the positive statement I have been missing. ⌗ And the "
            "sections opening names its reach: **the VACUUM sector**, with matter and SM **not yet "
            "built**.")
