"""
A1.4-ext :: CR radiation-free rate vs DESI DR2 BAO (arXiv:2503.14738, Table IV) -- the
state-of-the-art BAO dataset (7 tracers, 13 distance measurements with correlations),
superseding A1.4's 3 SDSS DR12 points.

CR claim (A1.4, established): the radiation-free rate factors H0 out of D_M, D_H, and r_d
alike, so the BAO ratios D_M/r_d, D_H/r_d, D_V/r_d depend on the DIMENSIONLESS Omega_m
ALONE (H0-independent). With the sound horizon CMB-calibrated (z_onset fixed by theta_*),
CR is a ONE-PARAMETER (Omega_m) fit to all of DESI DR2, at ANY H0 including the local 73.
LCDM's r_d is pinned in physical Mpc by the radiation-era w_m, so its BAO fit forces H0~67.4;
at H0=73 it breaks. That contrast is the Hubble tension and its CR resolution, now tested
against DESI DR2 rather than DR12.
"""
import numpy as np
