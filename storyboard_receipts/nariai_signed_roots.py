"""nariai_signed_roots.py (r1612) -- the SIGNED Nariai root structure, and the four
distinct quantities of magnitude 2/sqrt3 that must not be conflated.

WHY THIS EXISTS. P8 sec:open ("smaller remainders") and THE_PLAN's B.2 both stated the
forced scale as "the larger of the two Nariai roots -- the lone root r=2/sqrt3 against the
merged pair at -/+1/sqrt3 -- the backward-radial root the lap closes on". Three things are
wrong with that as written, and this receipt establishes each:
  (i)  it reads a SLICING OFFSET r_0 as a HORIZON ROOT r;
  (ii) it carries P3 sec:ellipse's "(r_0, r) = (+/-2/sqrt3, -/+1/sqrt3)" pairing across as
       if both entries were roots, so a DOUBLED root is written at "-/+1/sqrt3", i.e. at
       two values at once;
  (iii) on the M>0 member -- the one the cosmology selects -- the lone/backward-radial root
       is NEGATIVE (-2/sqrt3), as P2 sec:ring and P3 sec:lap both state.
AND THE CAVEAT THAT FORBIDS A NAIVE FIX. P2 sec:ring: "These signs are one vantage's, and
the enumeration must not privilege them ... the backward-radial reflection R carries the
whole set to its conjugate reading -- the merged horizon to -alpha/sqrt3, the closing root
to +2alpha/sqrt3." So the correction is NOT "flip the sign"; it is "name the vantage, and
stop calling an offset a root."

Gauge alpha=1 throughout. Horizon cubic: r^3 - r + 2M = 0 (roots sum to zero).
Offset relation: 2M = r_0 - r_0^3, so r_0 is ALWAYS itself a root.
"""
import numpy as np
