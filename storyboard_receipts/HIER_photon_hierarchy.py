...
#   ARM=cr            -- the stacking rate, radiation-free: H^2 = H0^2(Om a^-3 + OL), which is the
#                        rate the foliation stacks at; radiation is content, not in the kernel.
# Running the CR arm THROUGH this file rather than porting this file's machinery into the CR
# receipts means the CR arm inherits every correction made here -- the conformal-time seed
# (r2349), the k-range default (r2367), the viscosity coefficient (r2361), polarisation (r2369).
# The CR receipts have none of the photon hierarchy, the E-mode tower, or a visibility function.
_ARM = os.environ.get('ARM','lcdm').lower()
if _ARM == 'cr':
    H0 = float(os.environ.get('H0_CR','73.0'))
    Om = float(os.environ.get('OM_CR','0.3066'))*float(os.environ.get('OMF','1.0')); OL = 1-Om
    ombh2 = float(os.environ.get('OMBH2_CR','0.0224'))*float(os.environ.get('OBF','1.0'))
    Rb_rec = float(os.environ.get('RBF','1.0'))*31500*ombh2/(2.7255/2.7)**4/(1+z_rec)
    Or_content = float(os.environ.get('ORF','1.0'))*4.1833e-5/(H0/100)**2
    Hub=lambda a: H0*np.sqrt(Om/a**3+OL)              # STACKING rate: radiation-free
else:
...
# r2349 (second part): the grid began at a=1e-5, i.e. z=99999, while the intended start is
# z=1e6 -- so eta_onset (correctly computed by quadrature) fell OFF the grid and the spline
# lookups silently handed back z=90768, an order of magnitude adrift.  Extended to 1e-7 so
# the intended start is ON grid; verified to recover z=1000000 exactly.
ag=np.logspace(-7,0,20000)
# r2349: eta was SEEDED AT ZERO at a=1e-5, omitting the conformal time already elapsed from
# a=0.  In radiation domination the integrand c/(a^2 H) is CONSTANT (H ~ a^-2), so the missing
# piece is a/(H0 sqrt(Or)) = 4.596 Mpc exactly.  Negligible at eta_0 ~ 14000 (0.03%) but 12% at
# z=10000 -- which distorted every early-time comparison while leaving l_A and the peaks right.
# Verified against CAMB: z=10000 38.348 -> 42.944 (CAMB 42.947); z=1090 275.505 -> 280.101
# (CAMB 280.113).
_eta_seed = float(quad(lambda a: c/(a**2*Hub(a)), 1e-12, ag[0], limit=200)[0])
...
zs=float(os.environ.get('ZS','1e6'))   # start deep in the radiation era, adiabatic growing mode; the pole is the LIMIT not a start point
a_onset=1/(1+zs)
...
# r2369: polarisation ON by default.  The standard damping rate's 16/15 term INCLUDES
# polarisation (without it the coefficient is 8/9), and the analytic receipts -- C8,
# damping_ratio_clean, the r_D decomposition -- all use 16/15.  With POL off the instrument was
# running 8/9 physics against a 16/15 analytic side, and against CAMB, which polarises.  Matching
# it closed the last residual in the peak ratios: P1/P3 3.4% -> 0.79%, P1/P2 0.6% -> 0.42%.
_POL = os.environ.get('POL','1') == '1'          # photon polarisation (E-mode hierarchy G_l)
...
def source_terms(ee, Yv, kk, g, tau):
    """Public: return (Sm, Sd, Si, Sq) from the module's own assembly, knobs honoured."""
    _SRC_ONLY[0]=True
    try: return _assemble_source(ee, Yv, kk, g, tau)
    finally: _SRC_ONLY[0]=False
...
def _rhs(e, y, kk, tight):
    ...
    # r2361: the coefficient here multiplies sigma_g = A theta/tau', which enters theta' as a
    # FRICTION term -k^2 sigma_g/(1+R).  The amplitude damping rate is HALF the friction
    # coefficient -- as the slip term below states and correctly applies.  Matching the standard
    #     Gamma = (k^2/[6(1+R)tau'])[R^2/(1+R) + 16/15]
    # on its R->0 part gives  A/2 = 8/45,  i.e.  A = 16/45.  r2163 derived 8/45 correctly as the
    # RATE coefficient and then assigned it to the FRICTION coefficient, dropping that factor 2;
    # the slip term two blocks down doubles correctly, so the two were inconsistent WITHIN one
    # equation.  Independently, the band-power residual after the k-range fix (r2358) required
    # exactly VISCF=2 against the old value -- derivation and measurement agreeing.
    _visc = float(os.environ.get('VISCF','1.0'))*((16.0/45.0) if _POL else (8.0/27.0))
    ...
    # r2375 DIAGNOSTIC: NOBARYGRAV=1 drops the baryon source from the constraint, which is what
    # the CR receipts do structurally -- their state has no delta_b at all, so baryons load the
    # photons through R but never gravitate.  Removing it here measures what that omission costs.
    _OBG = 0.0 if os.environ.get('NOBARYGRAV','0')=='1' else 1.0
    Php = -Hc*Ps - kk**2*Ph/(3*Hc) - (Hc/2)*(Ogv*dg+Onv*dn+Ocv*dc+_OBG*Obv*db)
    out = np.zeros_like(y)
    out[:,0] = -tc + 3*Php
    out[:,1] = -Hc*tc + kk**2*Ps
...
def evolve(kk):
    ...
    if _ARM == 'cr':
        # r2373: CR's CHARACTERISTIC data, transplanted from ROBUST_p1p2_scan unchanged.
        # The branch point is a NULL surface, so the initial-value problem is characteristic:
        # ONE free function per mode plus regularity, not a field and an independent momentum.
        # The modes are already sub-horizon there (prop:subhorizon), so the super-horizon
        # adiabatic data below does NOT apply -- at eta_onset=180 the l=200 mode has k eta = 2.8.
        #   Psi envelope: 3(sin x - x cos x)/x^3 -> 3/x^2 at large x, matched to 1 at x->0
        #   amplitude flat in k (C4); phase RESET (A.126) -- the tortoise phase leaves nothing
        #   to inherit -- so dThat = 0, i.e. velocities zero: a common phase at an extremum.
        xs = (kk/Hcs)/np.sqrt(3.0)
        Pl = 1.0/(1.0 + xs**2/3.0)
        Ph0 = -Pl
        xe = 1.0/np.sqrt(3.0)
        A_flat = -(3*(np.sin(xe)-xe*np.cos(xe))/xe**3)/2
        That = A_flat*np.ones_like(kk); dThat = np.zeros_like(kk)
        hh = 1e-4
        _env = lambda x: 3*(np.sin(x)-x*np.cos(x))/np.maximum(np.abs(x),1e-12)**3
        dPl = np.where(xs<1e-6, 0.0, (_env(xs+hh)-_env(xs-hh))/(2*hh))
        dPh0 = -dPl*(kk/np.sqrt(3.0))
        dg0 = 4*(That - Ph0)
        th0 = -3*dThat + 6*dPh0
        Ps0 = Ph0
    else:
    ...
    # r2373: the seed must be CONSISTENT with the branch's own Psi_0, since it is just the
    # constraint Phi - Psi = 6 H^2 Omega_nu sigma_nu / k^2 solved for sigma (F_2 = 2 sigma).
    # The LambdaCDM branch has Psi_0 = Phi_0/(1+0.4 fnu); the CR branch has Psi_0 = Phi_0, so its
    # seed is ZERO.  Deriving it from (Ph0 - Ps0) rather than hard-coding the LambdaCDM form
    # makes both branches correct by construction.
    y[:,IN]=2.0*(Ph0-Ps0)*kk**2/(6.0*Hcs**2*Ons)
    ...
    def march(grid, tight):
        ...
        for n in range(len(grid)-1):
            ...
            if grid[n+1] > _ESTORE and (n % _STRIDE)==0:
                ...
                # r2367: this MUST match the coefficient the evolution uses (16/45, derived at
                # r2361 -- the amplitude rate is half the friction coefficient).  It was left at
                # 8/45 when line 224 was corrected, so the STORED sigma_gamma carried half the
                # viscosity the DYNAMICS used, and the source assembly read the wrong value.
                _vs = float(os.environ.get('VISCF','1.0'))*((16.0/45.0) if _POL else (8.0/27.0))
    ...
    # SNAPSHOT HOOK (r2179): the compact store keeps 6 columns, so the FULL state vector is not
    # recoverable from it.  SNAPETA dumps y at the first grid point past that eta, which is what a
    # whole-system implicit reference needs to start from.
    _SNAP=float(os.environ.get('SNAPETA','0')); snapshot={}
    e_sw_marker[0]=e_sw
    _orig_march=march
    def march(grid,tight,_m=_orig_march):
        if _SNAP>0 and grid[0]<=_SNAP<=grid[-1]:
            for n in range(len(grid)-1):
                _m(grid[n:n+2],tight)
                if grid[n+1]>=_SNAP and 'y' not in snapshot:
                    snapshot['y']=y.copy(); snapshot['eta']=grid[n+1]; snapshot['tight']=tight
            return
        return _m(grid,tight)
    march(g1, True)
    ...
    if _SNAP>0: globals()['SNAPSHOT']=snapshot
    return np.array(store_e), np.array(store_y), e_sw

def spectrum(lmax=900):
    # r2367: KMAXL default raised 900 -> 2500.  The projection kernel has support ABOVE
    # k = l/D_M, so a ladder cut at k_max truncates C_l worse as l -> k_max D_M: at 900 it
    # retained 61% at l=800 and NOTHING at l=1150 (the fourth peak simply did not appear).
    # The rule is k_max >= 2 l_max/D_M; 2500 recovers every multipole to <0.5%.
    stretch=2.75; Lmax=int(float(os.environ.get('KMAXL','2500'))/stretch)+4
    Ls=np.arange(2,Lmax); lL=np.sqrt(Ls*(Ls+2))*stretch
    _OS=int(os.environ.get('OVERSAMPLE','1'))
    # LLMIN extends the grid DOWNWARD.  CR's discrete set starts at L=2 -> l=7.78 (its low-multipole
    # floor), but the LambdaCDM CONTROL should have a continuous measure reaching k->0, which is what
    # the Sachs-Wolfe plateau needs (r2147).
    _LLMIN=float(os.environ.get('LLMIN','0'))
    if _LLMIN>0: lL=np.concatenate([np.linspace(_LLMIN,lL[0],int(os.environ.get('NLOW','40')),endpoint=False),lL])
    if _OS>1: lL=np.linspace(lL[0],lL[-1],len(lL)*_OS)
    kk=lL/DM; keep=lL<=float(os.environ.get('KMAXL','2500')); kk=kk[keep]; lL=lL[keep]
    ...
    return _assemble_source(ee, Yv, kk, g, tau, lmax=lmax)

def _assemble_source(ee, Yv, kk, g, tau, lmax=900):
    # r2352: lmax was referenced here but never passed -- the __main__ spectrum path raised
    # NameError, so this instrument has never emitted its own C_l.  It was not worth completing
    # while the transfer carried an unexplained 4.4% error; with that resolved (r2349) it is.
    """THE ONE source assembly (r2196).  Extracted so external harnesses call the SAME code the
    spectrum uses -- previously `abs.py` reimplemented it and therefore silently ignored every
    diagnostic knob (ISWF, DPF, NOPSG, NODAMP), manufacturing null results."""
    # compact store columns: 0 Theta_0, 1 theta_b, 2 Phi, 3 sigma_nu, 4 sigma_gamma, 5 Pi
    Ph=Yv[:,:,2]; sgn=Yv[:,:,3]
    Hcv=np.array([Hc_of(e) for e in ee])[:,None]; Onv=np.array([On_of(e) for e in ee])[:,None]
    _sgg_s = Yv[:,:,4]
    ...
    DOPP = float(os.environ.get('DPF','1.0'))*Yv[:,:,1]/kk[None,:]   # BARYON velocity; DPF for diagnosis
    ISWs = np.gradient(Ph,ee,axis=0)+np.gradient(Ps,ee,axis=0)
    # r2375 DIAGNOSTIC: INSTANT=1 replaces the visibility function by a delta at recombination,
    # which is what the CR receipts do.  Run on the LambdaCDM arm -- where the right answer is
    # known -- it measures what that approximation costs, and so whether it explains the two
    # implementations' disagreement about CR's first peak.
    if os.environ.get('INSTANT','0')=='1':
        _gd = np.zeros_like(g); _j = int(np.argmin(np.abs(ee-eta_rec)))
        _gd[_j] = 1.0/(ee[_j+1]-ee[_j] if _j+1 < len(ee) else ee[_j]-ee[_j-1])
        g = _gd
    ...
    # r2376: LMIN defaulted to 120, which is FINE for LambdaCDM (first peak 220) and WRONG for the
    # CR arm, whose first peak sits near 140 -- with DL=30 that leaves too few points below it to
    # register as a local maximum, so the finder labelled the SECOND peak P1 and every ratio was
    # off by one.  That is what produced an apparent 332-vs-150 disagreement between the two CR
    # implementations; they in fact agree (band-power peaks at 140/320/500/700).
    _LMIN = int(os.environ.get('LMIN', '40' if _ARM=='cr' else '120'))
    _DL=int(os.environ.get('DL','15')); ls=np.arange(_LMIN,lmax,_DL); Cl=np.empty(len(ls))
    x0m = eta_0-ee
...
if __name__ == "__main__":
    ...
    # r2376: argrelextrema with order=2 cannot see a maximum within 2 samples of either end, so a
    # peak near the start of the range is silently dropped.  Detect edge maxima explicitly.
    idx=[q for q in _ar(Dl,np.greater,order=2)[0]]
    if len(Dl)>3 and Dl[0]>Dl[1] and (not idx or idx[0]>2):
        idx=[0]+idx
        print('    NOTE: a maximum sits at or below the first sampled multipole (l=%d);'%ls[0])
        print('          lower LMIN to resolve it properly.')
    def _refine(q):
    ...
    print("\n  *** PHOTON-HIERARCHY CONTROL (LambdaCDM) — NO DAMPING ENVELOPE ANYWHERE ***")
