"""PK4 — THE PEAK HEIGHTS, WITH A CHARACTERISTIC MATCHING.  On the ground of A.0: one integral over the set rate.
...
print("="*80); print("PK6 — ONE DATUM PER MODE, ENVELOPE THROUGHOUT"); print("="*80)
...
def Psi_leg(x):
    """ONE DATUM PER MODE: the seam supplies an amplitude, not an amplitude AND a phase.  So what
    crosses is the ENVELOPE of the leg's potential, 3/x^2 for x>>1, matched smoothly to 1 at x->0."""
    x=np.asarray(x,float)
    return 1.0/(1.0+(x**2)/3.0)          # -> 1 at x=0, -> 3/x^2 at large x: the envelope, no zeros
...
def leg_state(k):
    ...
    # *** ONE FREE FUNCTION PER MODE = THE AMPLITUDE; THE PHASE IS THE SEAM'S AND IS COMMON. ***
    # C4: that amplitude is FLAT -- Psi(x_entry)/2, the same for every k.  So the leg supplies the
    # ENVELOPE, not an instantaneous value, and there is no k-dependent phase to carry across.
    That  = A_leg*np.ones_like(np.atleast_1d(k))
    dThat = np.zeros_like(That)
...
# *** CHARACTERISTIC, NOT CAUCHY.  P15 sec:coherence: the seam is NULL, so the data are ONE free
# function per mode plus regularity along the generators -- NOT a field and an independent momentum.
# So there is no second coefficient to set: B = 0, and the phase is the seam's, common to all modes. ***
B = 0.0*k
...
print(f"\n  {'n':>3s} {'l (PK6)':>9s} {'l (measured)':>13s}   {'height':>12s}")
