...
Inputs, all derived in C4/C7a/C8/C9 except H0 and Omega_m, which are observations:
  ...
  * r_s matched (+0.76%);  r_D +10.09%;  theta_D/theta_* +9.26%.
...
# against the measured ~301", and "C_l(CR)/C_l(LCDM) = exp[-(l/l_D)^2 (r^2-1)] with r = 1.093, so
# the ratio is 0.82 at l_D and 0.65 at 1.5 l_D."
#
# NOTE, recorded and not re-litigated here: th_ratio above is HARDCODED 1.0926 with the comment
# "DERIVED in C9", while C9 itself produces 1.0941.  The gate pins what THIS file computes, and
# separately pins it against the paper's printed r = 1.093, so the two cannot drift apart
...
assert abs(th_ratio - 1.093) < 0.001, \
    (f"this receipt's theta_D/theta_* is {th_ratio:.4f} but CR_cosmology.tex "
     f"sec:envelope-consequence prints r = 1.093")
assert abs(th_ratio**2 - 1 - 0.193775) < 1.0e-6, \
    f"the damping exponent r^2-1 = {th_ratio**2-1:.6f}, expected 0.193775"
assert abs(lD_CR - 1281.347) < 0.01, f"l_D(CR) = {lD_CR:.3f}, expected 1281.347 (= 1400/{th_ratio:.4f})"
# the two suppression figures the paper quotes.
assert abs(np.exp(-1.0**2*(th_ratio**2-1)) - 0.8238) < 1.0e-4, \
    (f"C_l ratio at l_D is {np.exp(-(th_ratio**2-1)):.4f}; CR_cosmology.tex says 0.82")
assert abs(np.exp(-1.5**2*(th_ratio**2-1)) - 0.6466) < 1.0e-4, \
    (f"C_l ratio at 1.5 l_D is {np.exp(-2.25*(th_ratio**2-1)):.4f}; CR_cosmology.tex says 0.65")
