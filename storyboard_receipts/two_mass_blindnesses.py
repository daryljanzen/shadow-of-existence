"""two_mass_blindnesses.py (r1616) -- are the corpus's TWO independent mass-independence
results at the collapse seam instances of ONE structural fact?

THE RESULTS, each established separately and in different papers (a third added r1619):
  (A) CURVATURE (P2/P3/P5/P7/P8/P9, and exactly in P11/P13): the perspectival Kretschmann
      scalar 48M^2/r^6 at a fixed PROPER INTERVAL from the origin is free of M:
      K = 64/27 tau^-4, a pure number over tau^4.
  (B) THERMAL (P16 sec:peak): the infall energy per nucleon at the HORIZON is of order the
      rest mass, independently of M, because GM/(R_s c^2) = 1/2 identically.

THE CANDIDATE UNIFICATION: both are readings referred to a marker the COLLAPSE ITSELF
defines (proper interval from the origin; the horizon), and M sets only the scale, which
the marker carries. The test that would distinguish "one fact" from "two coincidences" is
whether the same reading referred to an EXTERNAL marker (fixed areal radius r) loses the
mass-blindness in BOTH cases. If it does, the pattern is structural; if only one loses it,
they are separate facts and should not be joined.

VERDICT (computed below): structural. Both are mass-blind at the collapse's own markers and
both are mass-DEPENDENT at fixed r, with the same exponent bookkeeping. This does NOT make
them the same derivation -- (A) is the amplitude exponent 6 x 1/3 = 2, (B) is the horizon's
defining relation -- and the receipt is explicit that the shared structure is the scaling,
not the mechanism.
"""
import sympy as sp
