"""lap_timeline.py (r1631) -- A2.11: THE LAP'S TIMELINE. The finite intervals between the
landmarks of the cosmogenetic lap, exactly in phase and then in SI units.

WHY IT IS COMPUTABLE AND HAS NEVER BEEN DONE. Both constants are FIXED, not fitted:
alpha = sqrt(3/Lambda) and M = c^2/(3 sqrt(Lambda) G) (eq:Nariai-mass). The law is closed form,
r = A sinh^(2/3)(w) on the expansion leg with A = (2GM alpha^2/c^2)^(1/3) and w = 3 c tau/(2 alpha).
The corpus names every landmark and states no interval between any two of them: 'seconds', 'Gyr',
'years' and 'Myr' return ZERO occurrences in P7, P15 and P16 (re-checked r1631).

WHAT THIS ESTABLISHES
  (A) the landmark PHASES, exact and in closed form -- including a relation that is a result in
      its own right: the front seam sits at HALF the back seam's phase, w_back = 2 w_front
      = arccosh 2 = ln(2+sqrt3). The 2:1 in RADIUS is a 1:2 in PHASE, inverted.
  (B) the finite intervals in SI, on the corpus's own constants.
...
HELD, do-not-assert (and written into the placement too):
  * WHICH landmark is 'the beginning of the observable cosmology' is NOT settled here. P7
    ...
    receipt computes intervals along the law; it does not adjudicate that distinction.
  * The imaginary interval is NOT a duration and NOT recurrence (the r1622 guards).
  * H_0 enters only as a scale. CR's invariant is Omega_m; H_0 is left to the local ladder, so
    every SI number below scales as 1/H_0 and is quoted at the directly measured value the
    corpus fits at.
"""
import sympy as sp
