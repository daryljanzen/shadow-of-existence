...
import os as _os
RR=float(_os.environ.get('RR','0'))
Hub=lambda a: H0*np.sqrt(RR*Om/3403.6/a**4+Om/a**3+OL)                    # *** NO RADIATION TERM ***
...
DM=eta_0-eta_rec
zs=6761.0   # FIXED, not refitted -- the control varies only the rate's radiation
a_onset=1/(1+zs); eta_onset=float(np.interp(a_onset,ag,eg)); rs=rs_f(zs); lA=np.pi*DM/rs
print("="*80); print("CRRUN22 — FIXED SEAM, RADIATION IN THE RATE TOGGLED"); print("="*80)
