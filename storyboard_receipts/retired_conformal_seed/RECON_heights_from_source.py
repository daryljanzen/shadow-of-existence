...
def spectrum(nk=None, lmax=900):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    print(f"\n  RECONSTRUCTING P1/P2 FROM THE SOURCE  (spectrum peaks at 150, 360, 585)")
    print(f"    {'l':>6s} {'SW':>12s} {'damping':>9s} {'tilt':>8s} {'SW^2 x d x t':>14s}")
    prod={}
    for lt in (150,360,585):
        j=int(np.argmin(np.abs(lL-lt)))
        k=kk[j]; dmp=np.exp(-2*(k*rD)**2); tilt=lt**(0.965-1)
        p=SW[j]**2*dmp*tilt; prod[lt]=p
        print(f"    {lt:>6d} {SW[j]:>12.5f} {dmp:>9.4f} {tilt:>8.4f} {p:>14.5e}")
    print(f"\n    predicted P1/P2 from the source = {prod[150]/prod[360]:.3f}   (measured 1.471)")
    print(f"    predicted P1/P3                 = {prod[150]/prod[585]:.3f}   (measured 1.461)")
