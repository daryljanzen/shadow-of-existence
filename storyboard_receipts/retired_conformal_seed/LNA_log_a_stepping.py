...
def modes_all(kk):
    ...
    def rhs(e,Y):
        ...
        return out.ravel()
    NS=1400
    a_lo=float(np.interp(eta_onset,eg,ag)); a_hi=float(np.interp(eta_end,eg,ag))
    lna=np.linspace(np.log(a_lo),np.log(a_hi),NS+1); h=lna[1]-lna[0]
    eg2=np.array([float(np.interp(np.exp(x),ag,eg)) for x in lna])
    def rhs_lna(x,Y):
        e=float(np.interp(np.exp(x),ag,eg))
        return rhs(e,Y)/Hc_of(e)
    Y=y0.ravel().copy(); store=np.empty((NS+1,Y.size)); store[0]=Y
    for n in range(NS):
        x=lna[n]
        k1=rhs_lna(x,Y); k2=rhs_lna(x+h/2,Y+h/2*k1); k3=rhs_lna(x+h/2,Y+h/2*k2); k4=rhs_lna(x+h,Y+h*k3)
