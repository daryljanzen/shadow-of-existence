...
def spectrum(nk=None, lmax=900):
    ...
    krs=kk*rs/np.pi
    ext=sorted(list(_ar(SW,np.greater,order=3)[0])+list(_ar(SW,np.less,order=3)[0]))
    print(f"\n  *** THE DRIVING PHASE SHIFT THE BUILD PRODUCES ***")
    print(f"    an UNDRIVEN cos(k r_s) has extrema at k r_s/pi = 1, 2, 3, ...")
    print(f"    {'n':>3s} {'k r_s/pi at extremum':>21s} {'shift vs n':>11s} {'l':>6s}")
    for n,q in enumerate(ext[:4],1):
        print(f"    {n:>3d} {krs[q]:>21.3f} {krs[q]-n:>11.3f} {lL[q]:>6.0f}")
    if ext:
        print(f"\n    first extremum at l = {lL[ext[0]]:.0f}, i.e. {lL[ext[0]]/lA:.3f} of l_A = {lA:.1f}")
        print(f"    A.26's transfer argument requires 0.73;  LambdaCDM's first peak sits at 220/301.6 = 0.729")
