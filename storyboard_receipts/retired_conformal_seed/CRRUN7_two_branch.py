...
print("="*80); print("CRRUN7 — TWO-BRANCH SEAM DATA"); print("="*80)
...
def mode(k):
    ...
    # *** TWO-BRANCH SEAM DATA (A.50).  The corpus's low-l structure is the closed-S^3 discreteness
    # floor at l <~ 8; above that the plateau is ORDINARY.  So a mode that has never oscillated
    # receives ORDINARY ADIABATIC SUPER-HORIZON DATA -- the validated ΛCDM form -- and only modes
    # already inside the horizon at the seam take the constraint form. ***
    S = (Ogs+Ons) + 0.75*Ocs
    if k < Hcs:
        # SUPER-HORIZON AT THE SEAM: the ordinary adiabatic mode, normalised to primordial Phi = 1
        Ph0 = 1.0; Ps0 = Ph0/(1+2*fnu/5)
        dg0 = -2*Ps0
        y0=[-1.5*Ps0, k**2*eta_onset*Ps0/2, dg0, k**2*eta_onset*Ps0/2,
            dg0, k**2*eta_onset*Ps0/2, Ph0]+[0.0]*(LMAX-1)
    else:
        # SUB-HORIZON AT THE SEAM: the leg's handover, amplitude flat (C4), Phi from the constraint
        A=0.4835
...
Dn=Dl/np.max(Dl)
print(f"\n  CR spectrum, two-branch data (l_A={lA:.1f}, l_seam={Hc_of(eta_onset)*DM:.0f}):")
for j in range(0,len(ls),3):
    print(f"    l={ls[j]:>5d}  {Dn[j]:6.3f}  {'#'*int(60*Dn[j])}")
