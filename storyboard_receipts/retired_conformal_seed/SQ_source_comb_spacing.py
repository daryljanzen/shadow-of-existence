...
def spectrum(nk=None, lmax=900):
    ...
    krs=kk*rs/np.pi
    print(f"\n  *** SW vs SW^2: DOES SQUARING DOUBLE THE COMB? ***  (l_A={lA:.1f}, l_A/2={lA/2:.1f})")
    e1=[q for q in _ar(np.abs(SW),np.greater,order=3)[0]]
    e2=[q for q in _ar(SW**2,np.greater,order=3)[0]]
    print(f"    |SW| extrema at l_L = {[round(lL[q]) for q in e1[:6]]}")
    print(f"        their k r_s/pi = {[round(krs[q],2) for q in e1[:6]]}")
    print(f"    SW^2 maxima  at l_L = {[round(lL[q]) for q in e2[:6]]}")
    print(f"    spacing of SW^2 maxima: {[round(lL[e2[j+1]]-lL[e2[j]]) for j in range(min(4,len(e2)-1))]}")
    print(f"\n    zeros of SW (sign changes) at l_L = ", end="")
    zs_=[round(lL[q]) for q in range(1,nk) if SW[q-1]*SW[q]<0][:6]
    print(zs_)
