...
def modes_all(kk):
    ...
    for n in range(NS):
        ...
        if not np.all(np.isfinite(Y)):
            bad=np.where(~np.isfinite(Y))[0]
            yy=Y.reshape(nk,NV); prev=store[n].reshape(nk,NV)
            mi=bad[0]//NV; vi=bad[0]%NV
            names=['delta_c','theta_c','delta_g','theta_gb','delta_nu','theta_nu','Phi']+[f'F{j+2}' for j in range(LMAX-1)]
            print(f"\n  *** FIRST NON-FINITE AT STEP {n+1} of {NS} (eta={eg2[n+1]:.1f}) ***")
            print(f"      mode index {mi} of {nk}:  k={kk[mi]:.6f}  ->  l = k D = {kk[mi]*DM:.0f}")
            print(f"      variable {vi}: {names[vi]}")
            print(f"      how many modes affected: {len(np.unique(bad//NV))} of {nk}")
            print(f"\n      state of mode {mi} one step earlier (eta={eg2[n]:.1f}):")
            for j,nm in enumerate(names):
                v=prev[mi,j]
                if abs(v)>1e-30: print(f"        {nm:10s} {v:14.5e}")
            print(f"\n      and the step before that:")
            p2=store[max(n-1,0)].reshape(nk,NV)
            for j,nm in enumerate(names):
                if abs(p2[mi,j])>1e-30: print(f"        {nm:10s} {p2[mi,j]:14.5e}")
            raise SystemExit
    class _S:
