...
print("="*80); print("CRRUN11 — DISCRETE CLOSED-S3 SOURCE SPECTRUM"); print("="*80)
...
def modes_all(kk):
    ...
    S=(Ogs+Ons)+0.75*Ocs
    coef=-(1.5)*Hcs**2*S/(kk**2+3*Hcs**2)
    dg0=np.ones_like(kk)          # THE DATUM -- one free function per mode (AA.56)
    Ph0=coef*dg0                  # DERIVED from the constraint, no pole
    y0=np.zeros((nk,NV))
...
def spectrum(nk=None, lmax=900):
    # *** THE SOURCE MODES ARE DISCRETE (sec:largescale, AA.58). ***
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    # *** SUBTRACT THE BASELINE.  SW = -Rb Psi(k) + AA(k) cos(phi).  The zeros of SW are NOT the
    # zeros of the cosine; the displaced baseline moves them, by most where the baseline is
    # largest -- which is low k, exactly the observed direction (AA.89). ***
    krs=kk*rs/np.pi
    # running mean over one acoustic period in k, as an estimate of the baseline
    from numpy.lib.stride_tricks import sliding_window_view as _sw
    per = np.pi/rs                       # one radian-pi of k
    BB=np.array([SW[(kk>k-per/2)&(kk<k+per/2)].mean() for k in kk])
    SWc = SW - BB
    zr_raw=[q for q in range(1,nk) if SW[q-1]*SW[q]<0][:5]
    zr_cor=[q for q in range(1,nk) if SWc[q-1]*SWc[q]<0][:5]
    print(f"\n  *** ZEROS BEFORE AND AFTER SUBTRACTING THE BASELINE ***  (cos zeros sit at 0.5,1.5,2.5,...)")
    print(f"    raw SW zeros,  k r_s/pi = {[round(krs[q],3) for q in zr_raw]}")
    print(f"    baseline-corrected      = {[round(krs[q],3) for q in zr_cor]}")
    print(f"    spacings raw       = {[round(krs[zr_raw[j+1]]-krs[zr_raw[j]],3) for j in range(len(zr_raw)-1)]}")
    print(f"    spacings corrected = {[round(krs[zr_cor[j+1]]-krs[zr_cor[j]],3) for j in range(len(zr_cor)-1)]}")
