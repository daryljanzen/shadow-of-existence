...
kk=np.linspace(4/D,1600/D,400)
SWv=np.empty(len(kk)); PH=np.empty(len(kk))
for j,k in enumerate(kk):
    a,b,ee,src,phr=mode(k); SWv[j]=a; PH[j]=phr
hi=[q for q in _ar(SWv,np.greater,order=3)[0]]; lo=[q for q in _ar(SWv,np.less,order=3)[0]]
ext=sorted(hi+lo)
Rbr=31500*0.0224/(2.7255/2.7)**4/(1+1089.9)
print(f"\n  *** LambdaCDM: ITS OWN ENVELOPE AND B/A ***  (Rb_rec={Rbr:.4f})")
print(f"    {'l':>6s} {'SW':>11s} {'A local':>10s} {'Psi_rec':>10s} {'B=-Rb Psi':>10s} {'|B/A|':>8s}")
for j in range(min(5,len(ext)-1)):
    q=ext[j]; A=abs(SWv[ext[j+1]]-SWv[q])/2; B=-Rbr*PH[q]
    print(f"    {kk[q]*D:>6.0f} {SWv[q]:>11.5f} {A:>10.5f} {PH[q]:>10.5f} {B:>10.5f} {abs(B/A):>8.4f}")
print(f"\n    *** the sky needs |B/A| ~ 0.195 at the first peak; CR gave 1.284 (A.102) ***")
