...
print("="*80); print("CRRUN14 — THE COMPLETE TWO-CONSTRAINT SYSTEM"); print("="*80)
...
DRIVE=float(__import__('os').environ.get('DRIVE','1.0'))
def modes_all(kk):
    ...
    def rhs(e,Y):
        ...
        out[:,1]=-Hc*tc+kk**2*Ps
        out[:,2]=-(4/3)*tg+4*Php
        out[:,3]=-(Hc*Rb/(1+Rb))*tg+(kk**2/(1+Rb))*dg/4+kk**2*Ps*DRIVE
        out[:,4]=-(4/3)*tn+4*Php
