...
print("="*80); print("CRRUN10 — EXACT CONSTRAINT, ONE PRESCRIPTION"); print("="*80)
...
def mode(k):
    ...
    # *** THE EXACT CONSTRAINT, and it needs NO BRANCHES (A.56). ***
    # Eliminating Phi' with the momentum constraint gives, exactly and at all k:
    #     k^2 Phi = -(3/2)H^2 [ sum Om_i d_i + 3(H/k^2) sum Om_i (1+w_i) th_i ]
    # Super-horizon this does NOT fix Phi -- Phi is the datum and THETA adjusts to satisfy it.
    # So: ONE datum (Phi, the primordial normalisation), species adiabatic, theta DERIVED.
    Ph0 = 1.0; Ps0 = Ph0                       # sigma_nu = 0 initially so Psi = Phi
    dg0 = -2*Ps0; dc0 = -1.5*Ps0
    sum_d = (Ogs+Ons)*dg0 + Ocs*dc0
    C = (4/3)*(Ogs+Ons) + Ocs
    th0 = (-(2/3)*(k**2*Ph0/Hcs**2) - sum_d)*k**2/(3*Hcs*C)
    y0=[dc0, th0, dg0, th0, dg0, th0, Ph0]+[0.0]*(LMAX-1)
...
print(f"  CR spectrum (l_A={lA:.1f}), ONE prescription at all k:")
