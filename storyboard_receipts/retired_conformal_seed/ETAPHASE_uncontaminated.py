...
def modes_all(kk):
    ...
    dg0=np.ones_like(kk)          # THE DATUM -- one free function per mode (A.56)
...
def spectrum(nk=None, lmax=900):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    # *** THE UNCONTAMINATED MEASUREMENT: phase in eta at FIXED k.  B(k) cannot enter because
    # it is a function of k alone -- at fixed k it is a constant offset in eta. ***
    sol2,nk2,NV2=modes_all(kk)
    ee=np.linspace(eta_onset,eta_rec,1200)
    Yg=np.array([sol2.sol(e).reshape(nk2,NV2) for e in ee])
    print(f"\n  *** PHASE IN eta AT FIXED k -- the uncontaminated statistic (A.48's method) ***")
    print(f"    {'l_L':>8s} {'k r_s/pi':>10s} {'delta_g zero crossings':>23s} {'implied phi/pi':>15s}")
    for tgt in (180,302,450,603,760):
        j=int(np.argmin(np.abs(lL-tgt)))
        d=Yg[:,j,2]; d=d-d.mean()
        nz=int(np.sum(np.diff(np.sign(d))!=0))
        print(f"    {lL[j]:>8.1f} {kk[j]*rs/np.pi:>10.3f} {nz:>23d} {nz:>15d}")
    print(f"\n    (crossings of the mean-subtracted delta_g count half-periods, so phi/pi ~ crossings)")
