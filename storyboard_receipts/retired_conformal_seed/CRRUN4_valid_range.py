...
pk=[int(ls[i]) for i in idx]
print(f"\n  l_seam = H D = {Hc_of(eta_onset)*DM:.0f}  -- below this the modes are super-horizon at the seam")
print(f"  ALL peaks found: {pk}")
val=[p for p in pk if p>Hc_of(eta_onset)*DM]
print(f"  peaks above l_seam (where the seam data are valid): {val}")
if len(val)>=3:
    d=np.diff(val); print(f"  spacings: {list(d)}   mean {np.mean(d):.0f}   l_A = {lA:.1f}")
    print(f"  implied effective r_s = pi D/mean = {np.pi*DM/np.mean(d):.1f} vs computed {rs:.2f}")
    hh=[Dl[i] for i in idx if ls[i]>Hc_of(eta_onset)*DM][:3]
    print(f"  phi (from the first valid peak) = {round(val[0]/np.mean(d))-val[0]/lA:.3f}")
    print(f"  P1/P2 = {hh[0]/hh[1]:.3f}   P1/P3 = {hh[0]/hh[2]:.3f}")
