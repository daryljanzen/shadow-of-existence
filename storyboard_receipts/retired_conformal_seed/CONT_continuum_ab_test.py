...
def spectrum(nk=None, lmax=900):
    ...
    # *** CONTINUUM GRID (A/B test).  Same source, same projection, hybrid spacing per A.70 --
    # only the k-sampling differs from the discrete closed-S^3 one. ***
    kk=np.unique(np.concatenate([np.geomspace(2.0/DM,80.0/DM,90),
                                 np.linspace(80.0/DM,899.0/DM,230)]))
    lL=kk*DM; nk=len(kk)
    print(f"  *** CONTINUUM grid: {nk} modes, l range {lL[0]:.1f}..{lL[-1]:.0f} ***")
