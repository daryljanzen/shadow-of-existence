...
def spectrum(nk=None, lmax=900):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    # *** THE DIRECT CHECK: does the SOURCE peak where k r_s = n pi? ***
    krs = kk*rs
    print(f"\n  SOURCE vs ACOUSTIC PHASE  (r_s={rs:.2f}, l_A={lA:.1f}):")
    print(f"    {'l_L':>8s} {'k r_s/pi':>10s} {'SW':>13s}")
    for j in range(0,nk,4):
        if lL[j]>700: break
        print(f"    {lL[j]:>8.1f} {krs[j]/np.pi:>10.4f} {SW[j]:>13.4e}")
