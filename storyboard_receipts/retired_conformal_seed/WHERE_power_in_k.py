...
def spectrum(nk=None, lmax=900):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    for ltest in (300,360):
        x=kk*DM
        jl=spherical_jn(int(ltest),x)
        contrib=P*(SW*jl)**2
        top=np.argsort(contrib)[::-1][:5]
        print(f"\n  WHERE l={ltest} GETS ITS POWER:  (l_A={lA:.1f}, so a peak should draw from k r_s/pi ~ integer)")
        print(f"    {'l_L':>8s} {'k r_s/pi':>10s} {'j_l(kD)':>12s} {'contribution':>14s}")
        for j in sorted(top):
            print(f"    {lL[j]:>8.1f} {kk[j]*rs/np.pi:>10.3f} {jl[j]:>12.4e} {contrib[j]:>14.4e}")
