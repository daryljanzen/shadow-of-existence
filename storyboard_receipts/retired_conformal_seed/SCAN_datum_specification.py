...
import os as _o
ENV=_o.environ.get('ENV','pade'); AF=float(_o.environ.get('AF','1.0')); KS=float(_o.environ.get('KS','1.0'))
def modes_all(kk):
    ...
    ksm=Hcs*np.sqrt(1.0+rr)*KS               # THE LEG's horizon wavenumber (r2056)
    xs=(kk/ksm)/np.sqrt(3)
    ...
    if ENV=='pade':    Pl=1.0/(1.0+xs**2/3.0)
    elif ENV=='sharp': Pl=np.where(xs<np.sqrt(3.),1.0,3.0/np.maximum(xs,1e-9)**2)
    elif ENV=='soft':  Pl=(1.0+(xs**2/3.0)**2)**-0.5
    Ph0=-Pl                                   # potential a well
    # C4: Theta_hat oscillates freely on the leg from entry at x_e, amplitude flat in k
    A_flat=-(3*(np.sin(xe)-xe*np.cos(xe))/xe**3)/2*AF
    ...
    dPh0=np.zeros_like(kk)   # GATED: envelope value implies zero derivative (r2078)
