...
for isw in [True]:
    ls,Dl=spectrum(200, lmax=800, isw=isw)
    Dn=Dl/np.max(Dl)
    print("\n  LambdaCDM CONTROL, same assembly, l from 10:")
    for j in range(0,len(ls),3):
        print(f"    l={ls[j]:>5d}  {Dn[j]:6.3f}  {'#'*int(50*Dn[j])}")
    i10=int(np.argmin(np.abs(ls-10))); ipk=int(np.argmax(Dl))
    print(f"\n  peak at l={ls[ipk]}, plateau at l={ls[i10]}: ratio D_peak/D_plateau = {Dl[ipk]/Dl[i10]:.2f}")
    print(f"  the sky has D(220)/D(10) ~ 5.7")
