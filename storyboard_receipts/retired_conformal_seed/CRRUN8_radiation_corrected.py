...
print("="*80); print("CRRUN8 — RADIATION CONTENT CORRECTED"); print("="*80)
...
Dn=Dl/np.max(Dl)
print(f"\n  z_eq(CR) = {Om/Or_content-1:.0f}   rho_r/rho_m at seam = {(1+zs)/(Om/Or_content):.3f}")
print(f"  CR spectrum (l_A={lA:.1f}, l_seam={Hc_of(eta_onset)*DM:.0f}):")
for j in range(0,len(ls),4):
    print(f"    l={ls[j]:>5d}  {Dn[j]:6.3f}  {'#'*int(55*Dn[j])}")
