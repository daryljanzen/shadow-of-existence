...
print("="*80); print("CRRUN6 — FULL CONSTRAINT, NO SUB-HORIZON LIMIT"); print("="*80)
...
# stop hunting extrema -- print the spectrum and let the shape speak
Dn=Dl/np.max(Dl)
print(f"\n  CR spectrum, normalised (l_A={lA:.1f}, l_seam={Hc_of(eta_onset)*DM:.0f}):")
for j in range(0,len(ls),3):
    bar="#"*int(60*Dn[j])
    print(f"    l={ls[j]:>5d}  {Dn[j]:6.3f}  {bar}")
