...
def spectrum(nk=None, lmax=950):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    ls=np.arange(10,lmax,20); Cl=np.empty(len(ls))
...
ls,Dl,rD=spectrum(lmax=700)
print(f"  r_D(CR) = {rD:.2f} Mpc   ->  l_D = {DM/rD:.0f}")
Dn=Dl/np.max(Dl)
print(f"\n  lowest source multipole l_2 = sqrt(8) x 2.75 = {np.sqrt(8)*2.75:.2f}  (corpus: 7.8)")
print(f"  CR spectrum, DISCRETE source (l_A={lA:.1f}):")
for j in range(0,len(ls),3):
    print(f"    l={ls[j]:>5d}  {Dn[j]:6.3f}  {'#'*int(52*Dn[j])}")
