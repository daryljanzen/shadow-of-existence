...
print("="*80); print("CRRUN16 — CORRECTED LEG HORIZON SCALE"); print("="*80)
...
def spectrum(nk=None, lmax=900):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    print(f"\n  *** WHAT THE PEAKS ARE MADE OF (peaks at 195, 405, 615) ***")
    print(f"    {'l':>6s} {'SW':>11s} {'SW^2':>11s} {'damping':>9s} {'SW^2 x damp':>13s}")
    prod={}
    for lt in (195,405,615):
        j=int(np.argmin(np.abs(lL-lt)))
        d=np.exp(-2*(kk[j]*rD)**2); p=SW[j]**2*d; prod[lt]=p
        print(f"    {lt:>6d} {SW[j]:>11.5f} {SW[j]**2:>11.5e} {d:>9.4f} {p:>13.5e}")
    print(f"\n    source-only P1/P2 = {(prod[195]/prod[405])/np.exp(0):.3f}")
    print(f"    and damping alone contributes {np.exp(-2*((195/DM)*rD)**2)/np.exp(-2*((405/DM)*rD)**2):.3f}")
    print(f"    *** SO IF |SW| IS LARGER AT THE SECOND PEAK, THAT IS THE WHOLE PROBLEM. ***")
