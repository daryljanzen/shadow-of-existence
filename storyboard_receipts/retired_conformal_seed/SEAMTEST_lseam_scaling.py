...
DM=eta_0-eta_rec
import os
zs=float(os.environ.get('ZSEAM_OVERRIDE', brentq(lambda z: np.pi*DM/rs_f(z)-301.6, 1500., 60000.)))
...
def spectrum(nk=None, lmax=900):
    ...
    krs=kk*rs/np.pi
    zr=[q for q in range(1,nk) if SW[q-1]*SW[q]<0][:4]
    sp=[round(krs[zr[j+1]]-krs[zr[j]],3) for j in range(len(zr)-1)]
    print(f"\n  RESULT  z_onset={zs:.0f}  l_seam={Hc_of(eta_onset)*DM:.0f}  r_s={rs:.1f}  l_A={lA:.1f}")
    print(f"          SW zeros at k r_s/pi = {[round(krs[q],3) for q in zr]}")
    print(f"          spacings = {sp}   (uniform 0.5 would mean phase == k r_s)")
