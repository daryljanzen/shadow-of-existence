...
def spectrum(nk=None, lmax=900):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    # *** THE INDEPENDENT CHECK: extract Psi at recombination and test BB = -Rb Psi against AA. ***
    yy=sol.sol(eta_rec).reshape(nk,NV)
    sg=yy[:,7]/2
    Psi_rec=yy[:,6]+6*Hc_of(eta_rec)**2*On_of(eta_rec)*sg/kk**2
    A_env=0.134
    print(f"\n  *** Psi AT RECOMBINATION, AND BB/AA ***   (AA ~ {A_env}, Rb_rec = {Rb_rec:.4f})")
    print(f"    {'l_L':>8s} {'Psi_rec':>11s} {'BB=-Rb Psi':>11s} {'|BB/AA|':>9s} {'asymmetry it gives':>19s}")
    for lt in (150,360,585,840):
        j=int(np.argmin(np.abs(lL-lt)))
        BB=-Rb_rec*Psi_rec[j]; r=abs(BB/A_env)
        asym=((1+r)/(1-r))**2 if r<1 else float('inf')
        print(f"    {lL[j]:>8.1f} {Psi_rec[j]:>11.5f} {BB:>11.5f} {r:>9.4f} {asym:>19.3f}")
    print(f"\n    (asymmetry = ((1+|BB/AA|)/(1-|BB/AA|))^2, the odd/even power ratio a baseline BB gives)")
    print(f"    *** the sky needs 2.212, so |BB/AA| ~ 0.195 ***")
