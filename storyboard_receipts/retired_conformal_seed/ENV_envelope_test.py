...
def spectrum(nk=None, lmax=900):
    ...
    def peaks_with(Pw, dampw):
        lsx=np.arange(60,760,10); out=np.empty(len(lsx))
        for j,l in enumerate(lsx):
            x=kk*DM
            D_=SW*dampw*spherical_jn(int(l),x)
            out[j]=np.sum(Pw*D_**2)
        out=out*lsx*(lsx+1)
        idx=[q for q in _ar(out,np.greater,order=2)[0]]
        return [int(lsx[q]) for q in idx[:4]]
    rD0=rD
    dampfull=np.exp(-(kk*rD0)**2)
    print("\n  WHICH ENVELOPE MOVES THE PEAKS?  (source extrema project to 184, 486, 787)")
    print(f"    {'configuration':38s} {'peaks'}")
    print(f"    {'measure + damping (as built)':38s} {peaks_with(P, dampfull)}")
    print(f"    {'measure only, NO damping':38s} {peaks_with(P, np.ones_like(kk))}")
    print(f"    {'damping only, FLAT measure':38s} {peaks_with(np.gradient(kk), dampfull)}")
    print(f"    {'neither: flat measure, no damping':38s} {peaks_with(np.gradient(kk), np.ones_like(kk))}")
