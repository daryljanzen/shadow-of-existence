...
def spectrum(nk=None, lmax=900):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    # *** THE CORRECT STATISTIC: extremum spacing.  Extrema of B + A cos sit at phi = n pi
    # REGARDLESS of B, so their spacing IS the acoustic period. ***
    from scipy.signal import argrelextrema as _ar
    krs=kk*rs/np.pi
    for od in (1,2,3):
        hi=[q for q in _ar(SW,np.greater,order=od)[0]]
        lo=[q for q in _ar(SW,np.less,order=od)[0]]
        ext=sorted(hi+lo)
        ph=[round(krs[q],3) for q in ext]
        gaps=[round(ph[j+1]-ph[j],3) for j in range(len(ph)-1)]
        print(f"\n  order={od}: {len(ext)} extrema at k r_s/pi = {ph}")
        print(f"           gaps = {gaps}   (each should be 1.000 if phase == k r_s)")
        print(f"           l_L  = {[round(lL[q]) for q in ext]}")
