...
print("="*80); print("CRRUN20 — Phi ON ITS OWN EQUATION, NOT THE CONSTRAINT'S DERIVATIVE"); print("="*80)
...
def modes_all(kk):
    ...
    def rhs(e,Y):
        ...
        Php=np.zeros_like(Ph)   # Phi'' + 3H Phi' = 0 on the SET RATE -> growing mode constant (C11)
        out=np.empty_like(y)
        out[:,0]=-tc+3*Php
        out[:,1]=-Hc*tc+kk**2*Ps
        out[:,2]=-(4/3)*tg+4*Php
