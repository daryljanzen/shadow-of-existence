...
# extract Psi at recombination from the LCDM control, same l values as A.102
print("\n  *** Psi AT RECOMBINATION IN THE LambdaCDM CONTROL ***")
print(f"    {'l':>6s} {'k':>9s} {'Psi_rec':>11s} {'|B/A| at A=0.134':>18s}")
Rbr=31500*0.0224/(2.7255/2.7)**4/(1+1089.9)
for lt in (151,360,586,839):
    k=lt/D
    a,b,ee,src,phr=mode(k)
    # Psi at recombination is the potential returned by mode()'s bookkeeping
    print(f"    {lt:>6d} {k:>9.5f} {phr:>11.5f} {abs(-Rbr*phr/0.134):>18.4f}")
print(f"\n  CR gave: -0.276, -0.075, -0.028, -0.011  ->  |B/A| = 1.284, 0.347, 0.131, 0.053")
