...
def spectrum(nk, lmax=1200, isw=True):
    ...
    for i,k in enumerate(kk):
        a,b,ee,src=mode(k); SW[i]=a; DP[i]=b; ISWs.append(src); PHIrec[i]=src[0]*0+1.0
    ...
    for j,l in enumerate(ls):
        ...
        if isw:
            add=np.empty(nk)
            for i,k in enumerate(kk):
                early=np.trapezoid(ISWs[i]*spherical_jn(int(l),k*(eta_0-ee)), ee)
...
ls,Dl=spectrum(170, lmax=900, isw=True)
