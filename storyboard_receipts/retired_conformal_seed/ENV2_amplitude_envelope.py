...
def spectrum(nk=None, lmax=900):
    ...
    dk = np.gradient(kk)
    P = kk**(0.965-1)/kk * dk
    # *** IS A(k) SLOWLY VARYING?  Extract the envelope from consecutive extrema. ***
    from scipy.signal import argrelextrema as _ar
    krs=kk*rs/np.pi
    hi=[q for q in _ar(SW,np.greater,order=3)[0]]
    lo=[q for q in _ar(SW,np.less,order=3)[0]]
    ext=sorted(hi+lo)
    print(f"\n  *** THE ENVELOPE A(k) FROM CONSECUTIVE EXTREMA ***")
    print(f"    {'l_L':>8s} {'k r_s/pi':>10s} {'SW':>12s} {'|half-range to next|':>21s}")
    for j in range(min(6,len(ext)-1)):
        q=ext[j]; A=abs(SW[ext[j+1]]-SW[q])/2
        print(f"    {lL[q]:>8.1f} {krs[q]:>10.3f} {SW[q]:>12.4e} {A:>21.4e}")
    if len(ext)>2:
        As=[abs(SW[ext[j+1]]-SW[ext[j]])/2 for j in range(len(ext)-1)]
        print(f"\n    A varies {As[0]:.3e} -> {As[min(4,len(As)-1)]:.3e} across the first few half-periods")
        print(f"    ratio between consecutive half-periods: {[round(As[j+1]/As[j],2) for j in range(min(4,len(As)-1))]}")
