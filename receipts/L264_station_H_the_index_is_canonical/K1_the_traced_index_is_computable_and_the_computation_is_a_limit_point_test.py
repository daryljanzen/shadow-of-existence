...
#: ** ⛭ THE BASELINE AT THE THROW COMMIT (r3972), mirroring `reach_baseline.bodies()` exactly --
#: ** `%` comments and bibliography stripped, whitespace flattened -- because a baseline compared
#: ** under a different preprocessing is not a comparison.  *A station's baseline is a fact about
#: ** the corpus BEFORE the throw; reading it from the live papers asserts that nobody answered.*
_AT_BUILD = '46b81482'          # r3150 -- this station's own throw
_AT_BUILD_CACHE = {}
...
def main():
    ...
    # ** ⛭⛭⛭ RE-PINNED r3972, AND THE CORPUS ASKS THE QUESTION NOW -- WHICH IS THIS STATION'S OWN
    # ** FINDING LANDING. **  Thrown at r3150 (`46b81482`), this file's point was that index theory
    # ** begins with *is your operator Fredholm, and on what domain*, and that the corpus asked it
    # ** nowhere.  *** r3722 -- "index theory's half of the shortfall closed: six clauses landed" --
    # ** put it into P08, and not as a label: "a kernel of dimension one over a vanishing cokernel
    # ** --- a Fredholm index of $1$, generated by the constant of integration $-2M$", with the
    # ** stability consequence drawn and receipted. ***
    # **   ⇒ ** The zero is pinned at the throw and the discharge asserted, so the file records the
    # **     closure rather than dying of it. **  *A station exists to be answered; asserting that
    # **     nobody has answered it yet is asserting that the station failed.*
    _fred_then = sum(_at_build_counts('Fredholm').values())
    check(f'⛔ ⓶ and the field\'s FIRST question was one the corpus asked NOWHERE when this station '
          f'was thrown: `Fredholm` occurred {_fred_then} times in seventeen papers at {_AT_BUILD}',
          _fred_then == 0)
    check(f'⛭⛭ AND IT ASKS IT NOW ({n_fred} occurrences, r3722): P08 states "a kernel of dimension '
          f'one over a vanishing cokernel---\\emph{{a Fredholm index of $1$, generated by the '
          f'constant of integration $-2M$}}", and draws the stability consequence rather than '
          f'stopping at the word',
          n_fred >= 1
          and 'a Fredholm index of $1$, generated by the constant of integration $-2M$' in B['P08']
          and 'an index is stable under deformation' in B['P08'])
    # ** ⛔⛭⛭ AND THIS AMENDMENT OUTLIVED ITS PREMISE BY TWO REVISIONS (repaired r3972). **  It was
    # ** amended at r3339 to assert the JOIN -- "P14 carries `limit-point` at the branch point since
    # ** r3339" -- and *** r3341 REVERTED that P14 edit ***: "the error traces back to r3140, and
    # ** P14's original passage is restored verbatim".  The r3339 text was a paraphrase that had
    # ** replaced a correct passage removed at r3140 as an artefact; r3341 put the original back and
    # ** says the restoration is the better text -- it carries the closed form, the leaf measure, the
    # ** normalizability threshold and which branch fails, where the paraphrase carried none of them.
    # **   ⇒ ** So the routing gap this check originally recorded is REAL again, and the check is
    # **     returned to its original sense rather than chased. **  *An amendment written to follow a
    # **     change must be checked against whether the change survived; this one asserted a sentence
    # **     the corpus had withdrawn two revisions after it was written.*
    check(f'⓶ᵇ the apparatus is P10\'s alone: limit-point {lp["P10"]}×P10 / {lp["P14"]}×P14, '
          f'deficiency index {RB.counts("deficiency ind")["P10"]}×P10 / '
          f'{RB.counts("deficiency ind")["P14"]}×P14 -- r3339 briefly put it in P14 and r3341 '
          f'restored P14\'s original passage verbatim, the restoration carrying the closed form, '
          f'the measure and the threshold that the paraphrase did not',
