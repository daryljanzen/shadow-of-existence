"""
bead_conjugate.py -- verifies: for the bead's cosmic-time law r=A*sinh^{2/3}(3 tau~/2) (A=(2M)^{1/3},
  2M=2/(3 sqrt3) Nariai), (i) the two cosmic-time legs are COMPLEX CONJUGATES -- r(tau~*)=conj(r(tau~))
  (Schwarz reflection), so a real r<0 on the signed lap is read by the conjugate pair tau~, tau~*;
  (ii) the imaginary shift tau~->tau~+i(2pi/3) rotates r by exactly e^{2pi i/3} (the order-3/A2 structure),
  because 1.5*(2pi/3)=pi sends sinh->-sinh; (iii) the turnaround at r=-A sits at Im tau~=pi/3, half the
  full Wick period 2pi/3. [P7 CR_framework -- "the two cosmic-time legs are complex conjugates"]
STATUS: OK   RUN: python3 bead_conjugate.py   RUNTIME: ~1s
CONTROL: a generic imaginary shift (Delta != 2pi/3) is NOT a tau~-independent phase rotation.
ORIGIN: computations/bead_conjugate.py, verified + strengthened + canonicalized r1353.
"""
import numpy as np
