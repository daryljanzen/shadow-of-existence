...
# r2376+c54.158 -- pins the two computed facts the verdict is built on, plus the printed figures
# section [1] and [2] state without computing.
#   [1] THE GAUGE SIDE.  All 8 realified su(3) generators are antisymmetric on R^6 -- max|R+R^T|
#       is EXACTLY 0.0, not merely small -- so su(3) subset so(6); and they move ALL SIX real
#       coordinates {0,...,5}, so the real rep is faithful on R^6 and cannot be squeezed into
#       so(5)/S^4's five.  That "6, not 5" is the whole of "gauge NEEDS the full S^5".  The
#       dimensions the receipt prints are enumerated here rather than typed: dim su(3) = 8,
#       dim so(5) = 10, dim so(6) = 15.  CONTROL: only 5 of the 8 generators are needed before the
#       action has already left R^5, so the count is not an artefact of taking all eight.
#   [2] THE hbar SIDE.  beta = 2 pi alpha = 6.283185307 for dS_4 and dS_5 ALIKE (kappa = 1/alpha
#       is n-independent), which is exactly why beta alone cannot fork S^4 from S^5 -- the
#       receipt's negative, and the reason the residue is a choice rather than a computation.
assert maxasym == 0.0, maxasym
assert realdim_used == set(range(6)), realdim_used
assert len(gen) == 8, len(gen)
assert len({(i, j) for i in range(5) for j in range(i+1, 5)}) == 10
assert len({(i, j) for i in range(6) for j in range(i+1, 6)}) == 15
_five = set()
for g in gen[:5]:
    _five |= set(np.where(np.abs(realify(g)).sum(0) > 1e-9)[0].tolist())
assert len(_five) > 5, _five
assert abs(beta - 6.283185307179586) < 1e-12, beta
