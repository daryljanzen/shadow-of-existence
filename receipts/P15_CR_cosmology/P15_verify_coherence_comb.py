...
P15_verify_coherence_comb.py -- verifies P15 sec:coherence: the sharp acoustic comb requires a COMMON phase
  ...
  Magueijo applied to the seam): integrate the tightly-coupled oscillator Theta(k)~cos(k r_s+phi_k)*D(k) with
  (a) a common seam phase vs (b) random phase per mode (400 realisations), compare C_ell~Theta^2 at k=ell/D_C.
  RESULT: coherent -> sharp comb at Delta-ell = pi D_C/r_s = 296 (peaks 296,592,888), peak/trough ~1e5-1e6;
  incoherent -> washed out, contrast ~1. Matches the paper (Delta-ell~296, contrast ~1 vs many orders).
  Honest bound: shows WHY one-datum-per-mode suffices; does NOT replace the full seam-to-recombination
  Boltzmann transfer, which the paper flags open. (Original docstring said "P12"; this paper is P15.)
STATUS: ✔✔ (discriminating control fires: coherent comb vs washed-out; matches paper)   RUN: python3 P15_verify_coherence_comb.py   RUNTIME: ~3s
ORIGIN: computations/perturbation_verify/verify_coherence_comb.py, verified r1391.
"""
import numpy as np
