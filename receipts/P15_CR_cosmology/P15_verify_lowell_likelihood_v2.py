"""
P15_verify_lowell_likelihood_v2.py -- verifies the P15 sec:largescale/sec:scope low-ell cosmic-variance
  likelihood, CR vs LCDM, with the CORRECTED genuine-Boltzmann depths (0.47/0.41/0.36/0.68/0.92/0.98 for
  l=2..7). Exact low-ell likelihood: each hat C_l ~ scaled chi^2_{2l+1} about the model C_l; the absolute
  baseline cancels in the CR/LCDM ratio, Delta(-2lnL)_l = (2l+1)[r_l(1/depth_l - 1) + ln(depth_l)].
  RESULT (matches paper exactly): l=2 rewards CR at -2.63 (milder dip nearer observed low power); l=3,4
  penalise +1.31,+2.10; central total Delta(-2lnL)=+1.8; octopole estimator range +0.3 (WMAP) to +3.8 (Efst).
  A wash, well inside cosmic variance.
STATUS: ✔✔ (central +1.8, range +0.3..+3.8, per-ell -2.63/+1.31/+2.10 all match paper)
RUN: python3 P15_verify_lowell_likelihood_v2.py   RUNTIME: ~1s
ORIGIN: computations/perturbation_verify/verify_lowell_likelihood_v2.py, verified r1395.
"""
import numpy as np
