...
ORIGIN: storyboard_receipts/C6_neutrino_term.py -- registered r2376 (c54); edit the origin, not this copy."""
import numpy as np
# ** r2376+c54.155: this file typed f_nu = 0.4052 (the N_eff = 3 value) one line above a
#    'check' that computed 0.4089 (N_eff = 3.046) and called the second a check of the first.
#    With no assertion the disagreement in the third digit shipped silently.  Now 0.4089 and
#    asserted against its own recomputation. **
...
print(f"   f_nu = rho_nu/rho_rad = {fnu:.4f} for N_eff = 3.046 with the (4/11)^(4/3) ratio:")
...
# ** THE CHECK (r2376+c54.155): the typed f_nu must equal its own recomputation. **
_check = (N * (7.0 / 8.0) * r_nu) / (1.0 + N * (7.0 / 8.0) * r_nu)
print()
print(f"  ** CHECK: typed f_nu = {fnu:.4f} against recomputed {_check:.4f} at N_eff = {N} **")
assert abs(fnu - _check) < 1e-3, f"f_nu {fnu} != {_check}"
print("  CHECK PASSES")
