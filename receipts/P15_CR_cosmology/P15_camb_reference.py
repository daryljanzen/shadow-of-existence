"""
P15_camb_reference.py -- verifies the P15 sec:coherence Boltzmann-reference peak ratios the CR peaks are
  claimed to reduce to. Runs CAMB (Planck-2018 params) and finds the first three TT acoustic peaks:
  P1 (ell=220, 5734 uK^2), P2 (ell=536, 2594), P3 (ell=813, 2542) -> ratios P1/P2=2.211, P2/P1=0.452,
  P3/P1=0.443. Matches the paper's cited P1/P2~2.2, P2/P1~0.45, P3/P1~0.44 digit-by-digit. This is the
  LCDM reference; CR matching it is carried by the driving equality + shared plasma + coherent phase
  (cr_collapse_driving, time_reversal_driving, verify_coherence_comb).
  DEPENDENCY: requires CAMB (pip install camb --break-system-packages; Linux wheel, no Fortran build).
STATUS: ✔✔ (CAMB Boltzmann reference; peak ratios match paper's cited values)
RUN: python3 P15_camb_reference.py   RUNTIME: ~20s (needs camb)
ORIGIN: computations/peak_heights/camb_reference.py, verified r1392.
"""
