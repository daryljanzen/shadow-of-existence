...
COMPUTES: scope.  Symbolic throughout (sympy exact arithmetic), so no value is fitted and
none is imported from data.  The one pinned number is THRESHOLD = 1/8, taken from r6497
rather than derived here; alpha is carried as a symbol and every length is a ratio to it,
so the family is scanned in a/alpha rather than evaluated at one rotation.  lambda is a
PROXY, named in the receipt's own part (4): the ratio AT THE POLE, where the S^2 radius is
r_N -- the rotating near-horizon S^2 is warped through (r_N^2 + a^2 cos^2 theta), so one
ratio does not describe it.  Bounds are the receipt's own closing words: ESTABLISHED, that
the rotating Nariai locus recovers prop:throat at a = 0, that the family is bounded by
a_max = (2 - sqrt3) alpha at the ultracold point, and that lambda = 1 at a = 0 rising
monotonically with a; NOT CLAIMED, that the warped treatment cannot reverse this.
  # ⌗ HEADER ADDED r6502 (node 60), and NOT a reading of this computation: every clause
  #   above is transcribed from the receipt's own text -- its part (4) proxy sentence and
  #   its ESTABLISHED/NOT CLAIMED close.  Added because `check_computes` has been red on
  #   the trunk since r6499 for want of it, and a red base branch blocks every node's PR.
  #   *The receipt is otherwise untouched and this header is reversible in one edit by its
  #   author, who is the one who knows whether the scope is wider than its own summary.*
