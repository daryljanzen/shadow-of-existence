"""
P03_the_weld_and_the_two_folds.py -- P3 sec:temporal-threeness / P8 prop:cosmo /
COMBINATORICS L8.1-a: whether the weld can split, and why BOTH of the corpus's threes are D-1.

THE CONFLICT.  L8.1-a welds roots = hinges = walls = GENERATIONS into "one 3", and P14 seats a
generation at each hinge wall.  But the causal trichotomy forces a bound triple to take ONE
PUNCTURE PER HINGE, so a proton is one of each COLOUR -- three generations in one proton is
wrong.  The hinge three must be colour, hence not also generation.  ** And the D = 4 result
runs through "the fold is D-1" and "the fold IS the generation count" VIA that weld. **

PART 1, THE TURNAROUND LAW AT GENERAL D, DERIVED.  With (A1) discharged (receipt
P03_operator_at_general_D), f = 1 - 2M/r^(D-3) - r^2/alpha^2 at every D, so the marginally-bound
condition (dr/dtau)^2 = 1 - f needs no further input.  Verified by substitution at D = 4..8:

    ** r^(D-1) = 2 M alpha^2 sinh^2( (D-1) tau / (2 alpha) ) **

which at D = 4 is P8 prop:cosmo's own r^3 = 2 M alpha^2 sinh^2(3 tau/2 alpha).

PART 2, ITS DECK GROUP.  sinh^2 has period i pi, so the shift tau -> tau + 2 pi i alpha/(D-1)
leaves r^(D-1) invariant and sends r -> e^{2 pi i/(D-1)} r (invariance verified at each D).
** The turnaround's deck group is Z_{D-1}: Z_3 at four, Z_4 at five.  THE TURNAROUND FOLD IS
D-1, exactly as the horizon fold is. **

PART 3, WHY BOTH ARE D-1 -- AND THIS ANSWERS L-07, open since c54.16 ("two distinct threes
with one cause; why?").  Clearing denominators, f = 0 is a polynomial of degree D-1 in r at
every D.  The two threes are the two things one does with that one polynomial: the HORIZON
three is its ZERO SET; the TURNAROUND three is the DECK ACTION of the integral of 1 - f.
** Distinct objects, one cause -- and lem:twoturnings' proof that no affine map identifies the
root sets is not in tension with that, since a common cause is not an identity. **  L-07's
third clause ("is there a third?") acquires a shape: any fold read off f is read off a
degree-(D-1) object, so the prediction is that there is no independent one.

PART 4, THE VERDICT ON THE FOLD STEP.  ** The fold is D-1 at EITHER seat, so the dimension
result's fold step survives the split unchanged -- and structurally rather than by luck, since
the fold is a property of f rather than of which structure reads it. **  The D = 5 statement is
also unchanged: four generations, now read off the turnaround's Z_4.

PART 5, WHAT THE SPLIT COSTS, and it is real.  The horizon three carries S_3 (the Weyl group /
monodromy of the cubic over the mass plane, P5); the turnaround three carries Z_3 (the cyclic
deck action).  Z_3 < S_3 and they are not the same group.  ** So on the split, P14's sentence
"the Weyl S_3 IS the relation among the three hinges" becomes a statement about COLOUR, and
FLAVOUR carries only the cyclic Z_3 -- a consequence with content, since an S_3 and a Z_3
family symmetry are different physics. **  (L-91.)

** THE SPLIT'S FALSIFIER, UNRUN: whether R = gamma^5 -- the OFFSET parity, which acts on the
horizon/offset structure -- reaches the TURNAROUND three at all.  P14 needs the generations
chiral.  If R does not reach the new seat, the split costs the chirality grading, which would
be worse than the conflict it resolves. **  (L-92.)

WHAT IS NOT DONE: the revision to P14.  The weld is shown SPLITTABLE without cost to the
dimension result; it is not SPLIT in the papers, and should not be until L-92 is run.

ORIGIN: computations/dimension_question/L67_the_weld_must_split.py -- built r2376 (c54.30);
