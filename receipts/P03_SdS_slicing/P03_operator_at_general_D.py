"""
P03_operator_at_general_D.py -- P3 rem:dimension / P8 thm:kernel: ** ASSUMPTION (A1)
DISCHARGED. **  CR's own operator at general D returns Tangherlini-de Sitter and nothing else.

THE DEBT.  Every site of the dimension result carried (A1): "the D-dimensional metric function
is the standard Tangherlini-de Sitter one -- the natural codimension-one extension, but an
EXTENSION, not a rederivation of CR's operator at general D."  It was the oldest lead in the
register and the most load-bearing.

THE POINT MISSED.  CR's operator is NOT a metric function.  P8 thm:kernel: "the condition
T_{mu nu} = 0 is the first-order linear ODE r f' + f - 1 + Lambda r^2 = 0, whose ENTIRE
solution space is f = 1 - 2M/r - Lambda r^2/3 ... the vacuum sector is exactly the kernel of
the matter functional, DERIVED, NOT MATCHED."  ** The operator is 'T = 0 on a cut in the
construction gauge', and f is its kernel -- which one can simply run at general D. **

COMPUTED HERE, from the metric rather than quoted: for
ds^2 = -f dt^2 + dr^2/f + r^2 dOmega^2_{D-2}, the Einstein tensor gives

    G^t_t = (D-2)/(2 r^2) [ r f' + (D-3)(f - 1) ]         (verified at D = 4, 5, 6)

so the kernel condition G^t_t + Lambda = 0 is the first-order linear ODE

    r f' + (D-3)(f - 1) + 2 Lambda r^2/(D-2) = 0          (P8's equation at D = 4)

whose general solution, at D = 4, 5, 6, 7, 8, is

    ** f = 1 - 2M/r^(D-3) - 2 Lambda r^2 / ((D-1)(D-2)) **

with ONE constant of integration -- verified identical to Tangherlini-de Sitter at every D
tested.  Writing alpha(D) = sqrt((D-1)(D-2)/(2 Lambda)), which returns sqrt(3/Lambda) at four,
this is f = 1 - 2M/r^(D-3) - r^2/alpha^2 at every D, and the dimension result's input
2M = r_0^(D-3) - r_0^(D-1) is that f evaluated at a root.

** SO (A1) IS DISCHARGED COMPLETELY.  The D-dimensional metric function is the entire kernel
of CR's own matter functional at dimension D, with the mass the single constant of
integration.  The dimension result STRENGTHENS: the collapse argument, the fold being D-1 and
the mass parity all run on a function the construction GENERATES rather than borrows. **
...
untouched and stands as stated.

METHOD NOTE: this took one computation, and the operator was written out in P8's abstract.
The debt sat twelve revisions because nobody read P8's kernel theorem against the arc's (A1).

ORIGIN: computations/dimension_question/L01_CR_own_operator_at_general_D.py -- built r2376
(c54.29); edit the origin, not this copy."""
