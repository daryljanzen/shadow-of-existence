"""
P03_batch2_runnable.py -- eight owed leads cleared together (L-89, L-72, L-84, L-86, L-87,
L-93, L-94, L-29).  None needed a turn of its own; none needed Daryl.

L-89, THE LOCK AT GENERAL D -- the last survivor of (A1).  For the UNLOCKED metric
-A(r)dt^2 + dr^2/f + r^2 dOmega^2_{D-2}, G^t_t - G^r_r is computed at D = 4, 5, 6 and is in
every case proportional to (A'/A - f'/f), i.e. to d_r ln(A/f) -- the same object P8's
prop:lapse carries at four -- and vanishes identically under A = c f.  ** So 'A = f' and
'p_r = -rho' are the same condition at EVERY D: the lock's characterisation is dimension-
independent and L-01's kernel is the right object at every D. **  (Scope: this shows the
equivalence, not a re-derivation from the swing that ONE curve fixes both functions; that
argument is P3's and is dimension-independent in its own terms.)

L-72, CLOSURE -- A PROPERTY OR A REQUIREMENT?  Two different closures were being run
together.  The VACUUM lap closes because the signed radius passes through r = 0 -- a property
of the coordinate holding for every member, including the overcritical, so it discriminates
nothing.  ** What confinement needs is the MATTER configuration's Z_3 SHEET INDEX returning,
and that IS required, and not by stipulation: a physical field on the branched bead must be
SINGLE-VALUED. **  A configuration whose total sheet advance is non-zero is not a field on the
manifold.  The overcritical, named by the lead as the place to look for a counterexample,
bears on the wrong closure.

L-84, LAP vs BEAD.  5/5 on the criterion: one closed curve under two names, "lap" naming the
TRAVERSAL and "bead" the OBJECT it makes.  ** This settles what the lead said depended on it:
P14 builds the wall ON THE BEAD, P3 puts r = 0 ON THE LAP, and L-75 identifies the wall WITH
r = 0 -- all one statement, no ambiguity about the wall's home. **

L-86 / L-94, THE GROUPS.  The mode carries chirality = sign(lambda) [Z_2] and triality =
-lambda mod 3 [Z_3], generating Z_6 -- and L-92 found the TURNAROUND three carries exactly
Z_6 while the HORIZON three carries D_6.  ** So the mode's own Z_6 is the turnaround's, which
is evidence FOR the weld split rather than a coincidence of orders. **  And L-94 dissolves:
L-92 showed the turnaround-side and horizon-side conditions coincide at every D, so there were
never two R's to choose between.
...
L-93, IS THERE A THIRD D-1?  Every three the corpus names -- horizon roots, turnaround roots,
hinges, walls, graze points, generations, the A_2 hexad's fundamental -- descends from f's
degree-(D-1) numerator.  ** NO independent third: the corpus has ONE D-1 read at several
places, so L-07's "two distinct threes" are two READINGS of one source. **  Bounded: an
enumeration of the threes the corpus names, not a proof that no construction could make
another.

L-29, THE UNRUN FALSIFIER OF ⊢58.  Casey's form with a LINE among the four, computed on the
corpus's own figure (throat alpha = 1, the three walls, the hinge tangent): the identity holds
to machine precision and is SIGNATURE-BLIND, every quantity entering being a bitangent among
the throat's tangent objects with none of them the waist.  ** The ledger's prediction is
confirmed and ⊢58 is NOT refuted -- the falsification test was run and did not land, which is
the only way "one privileged circle" earns anything. **

ORIGIN: computations/baryon_edge/BATCH2_clear_the_runnable.py -- built r2376 (c54.32); edit
