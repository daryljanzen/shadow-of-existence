"""
P03_thirds_from_closure.py -- P3 sec:lap / sec:tour: the fractional charges DERIVED from
the closure of the lap, with no Standard Model input.

THE DEFINITION.  The three graze points sit on the throat at 60/180/300 degrees, one
directly between each pair of hinges (receipt P03_twelve_null_legs), cutting the throat
circle into three equal arcs.  Define a constituent's WINDING as the signed number of
graze points its equatorial route crosses, in thirds of a lap.  The graze points are the
only marked points on the throat and there are exactly three because there are three hinges.

PART 1.  Between two punctures at different hinges there are exactly TWO equatorial routes,
crossing 1 and 2 graze points in opposite senses.  Since 1 + 2 = 3, ** the two routes differ
by exactly ONE FULL LAP. **  (Daryl: "the 1/3 vs 2/3 of the equator ... they're even the
opposite direction.")

PART 3, THE DERIVATION.  Let a triple's HEAD carry winding x and each PARTNER y.  Impose
only geometry:
    (i)   x - y = 1        the two routes differ by one lap                    [PART 1]
    (ii)  x + 2y in Z      the upper-headed triple CLOSES                      [P3 sec:lap]
    (iii) y + 2x in Z      the lower-headed triple closes; the horns are symmetric
Substituting y = x - 1 gives totals 3x - 2 and 3x - 1, so both are integers iff 3x is.
** x = k/3.  THE THIRDS ARE FORCED, and the 3 is the number of constituents = the number of
hinges = the number of graze points -- one 3 entering three ways, and it is the corpus's own. **

PART 4.  Modulo a whole lap (a relabelling) the distinct classes are k mod 3, and there are
exactly three:
    k = 2 : (x, y) = (+2/3, -1/3)  ** = (u, d) **
    k = 1 : (x, y) = (+1/3, -2/3)  ** = the antiquarks -- the same solution under overall
                                      sign reversal, which is C **
    k = 0 : (x, y) = (0, -1)       ** triality zero: the leptons, and every colour singlet **
** So the geometry does not MATCH the quark charges; it PRODUCES them as one of exactly
three triality classes. **  NO Standard Model number appears in (i), (ii) or (iii).

PART 5.  On the physical class, the upper-headed triple (u,d,d) totals 0 and the lower-headed
(d,u,u) totals +1, exchanged by T.  ** T changes the total by EXACTLY ONE LAP -- the nucleon
doublet, with T costing the unit the W carries. **  Lead L-71 asked whether T moves the
winding by 0, 1 or 2 and said two answers refute the isospin reading; the answer is 1.
And there are three of each because there are three hinges: three COLOUR labels of one
doublet, not six objects.
...
(e) Nothing here reaches the continuous SU(3); what is supplied is the DISCRETE part, which
is the position the corpus already accepts for charge (L-68).

Registered with refutation clauses in THE_FERMION_SECTOR_GEOMETRY.md.

ORIGIN: computations/baryon_edge/L73_thirds_derived_from_closure.py -- built r2376 (c54.22);
