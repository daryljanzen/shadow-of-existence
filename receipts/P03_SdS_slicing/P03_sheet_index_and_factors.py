"""
P03_sheet_index_and_factors.py -- P3 sec:temporal-threeness / sec:lap, P14 prop:wall: what
actually transforms as a wall-crossing count, and how the geometry's labels factorise.

** A RETRACTION FIRST. **  A prior pass argued "wall <-> hinge is a bijection, so walls and
hinges are the same three, so walls cannot carry a label different from hinges."  That is
INVALID.  Colour and generation in the Standard Model are both threes, both permuted, and
completely independent; if a bijection between index sets forced labels to coincide, the
Standard Model could not exist.  ** A bijection between index sets is not an identity of
structures, and the test for whether two structures carry independent labels is whether the
LABELS can vary independently -- not whether the objects are unrelated. **

THE CRITERION, run on wall vs hinge (five tests: LOCUS, TYPE, DEFINITION, EQUIVARIANCE,
SEPARABILITY).  Four say DIFFERENT -- a hinge is a timelike LINE at transverse 2 alpha
carrying two punctures and no legs; a wall is a POINT on the throat at radius alpha carrying
no punctures and four legs; one is the swing axis, the other the r=0 flip.  Only
EQUIVARIANCE says "same", and equivariance of a bijection is exactly the property that does
not force labels to coincide.  ** Wall and hinge are tightly related and completely
distinct, which is what a factorisation looks like. **

THE INCIDENCE IS NOT THE FREE PRODUCT.  Verified: a hinge's legs cross the other two hinges'
walls and never its own, so the realised (hinge, wall) incidences number 6 of 9 -- the full
product minus the antipodal diagonal.  A constraint, hence a prediction.

THE FACTORISATION.  A leg is (hinge, horn, which-of-the-other-two-walls): 3 x 2 x 2 = 12.
** And the third factor is DERIVED, not assigned: a hinge's two legs graze at -60 and +60,
each a Nariai cut at w = -30 and +30, so r0 = -+1/sqrt3 -- and r0 -> -r0 is R, which P14
establishes is gamma^5.  The leg pair IS the chirality. **

THE ANSWER TO "WHAT TRANSFORMS AS A CROSSING COUNT".  Three established pieces, none
introduced here: (1) a wall IS the r=0 locus relative to its hinge [P14 header; receipt
P03_wall_is_the_graze_point]; (2) r=0 is a BRANCH POINT the slicing passes through onto the
conjugate branch [P3 sec:lap]; (3) the turnaround law r^3 = 2M a^2 sinh^2(3 tau/2a) carries a
Z_3 deck action, tau -> tau + 2 pi i a/3 leaving r^3 invariant and sending r -> e^{2 pi i/3} r
[P3 sec:temporal-threeness].  ** So r carries a cube-root SHEET INDEX in Z_3 which advances by
one per passage through the branch point and returns after three.  That is exactly the
transformation law a crossing count must have, and it was in the corpus before any of this,
derived from cosmic time's own imaginary period. **  The closure rule then means something
rather than stipulating it: a composite is admissible iff its total sheet advance vanishes in
Z_3, i.e. iff it returns to the SAME BRANCH -- an unclosed object is a path ending on a
different sheet from the one it began on.

** WHAT IS NOT DONE. **  prop:wall's mode exp(-int W) chi_+ has NOT been carried through r=0
to read off the sheet advance it picks up (L-78) -- one computation on an existing solution.
The weld: READING A (hinge = colour) is forced by the causal trichotomy, since a bound triple
takes one puncture per hinge and a proton is one of each colour, not one of each generation;
so P14's "three walls = three generations" must be revised and generation sought on the
turnaround three (L-67, and it reaches the dimension result).  And the 15 = 12 + 3 count is
REGISTERED, NOT ASSERTED, with its refutation stated (if the three wall states carry a horn
label they are 6, not 3 -- unchecked, L-80).

ORIGIN: computations/baryon_edge/L76_the_label_and_the_criterion.py -- built r2376 (c54.24);
