"""
P03_reach_probe_deepening.py -- the reach programme's own probes, re-run at general D.

The field and domain bakes were run against a four-dimensional cut, at a time when
nothing in the corpus distinguished "structural" from "true at D=4".  P3 rem:dimension
supplies that distinction, so every probe whose verdict rests on a D=4 number is owed a
re-derivation.  Six are re-run here, and they do not all go one way:

  OPTICS O1   STRENGTHENED  -- r_photon = r_Nariai in every D tested (sqrt3/3, sqrt2/2,
                               sqrt15/5, sqrt6/3, sqrt35/7 at D=4..8), so "the tangency
                               condition and the photon-sphere condition are ONE
                               condition" is dimension-independent.
  CATEGORY K8b  GROUNDED    -- its bound is so(4,1), the stabiliser of a FOUR-dimensional
                               cut (dim 10 in every dS_D), so the emptiness proof stops
                               being a coincidence of counting.
  CATEGORY K5   NARROWED    -- the near-horizon isotropy enhancement is +2 in every D;
                               the pair (6,4) is D=4 (D=5 gives 9 against 7).
                               ** CALIBRATION, and it caught an error in this script: **
                               an earlier pass used so(D-2) for the near-horizon sphere
                               S^(D-2) where its isometry is so(D-1), and returned (4,2)
                               at D=4 -- contradicting K5's own stated (6,4).  Checked
                               against the ledger's numbers before propagating; the +2
                               conclusion was unaffected, the table was not.
  OPTICS O6     SCOPE WIDER THAN COMPUTATION -- 2f - r^2 f'' is the constant 2 only at
                               D=4; at D>=5 it is 2 + 2(D-2)(D-3)M/r^(D-3).
  QUADRIC Q4    SELF-DISCOUNT TOO STRONG -- a cross-ratio needs FOUR points and the
                               vantage set has (D-1)+1 = D, so it is a cross-ratio
                               configuration precisely at D=4; the D=5 four-point subset
                               is HARMONIC, not equianharmonic.
  QUADRIC 4d    ANSWERED (was untried) -- the hinge triangle is NOT self-polar w.r.t. the
                               throat (polar at alpha/2, opposite side at alpha), and a
                               plane simplex needs three points, so the question exists
                               only at D=4.

** NONE of these is a second vote for D=4. ** Every dimension statement descends from the
one fold, and counting them again is the trap FIGURE_THEOREM_LEDGER's own note names:
"the count matches and the match means nothing.  A count is not a map."

ASSUMED throughout, as in rem:dimension: the D-dimensional metric function is the standard
Tangherlini-de Sitter form.

ORIGIN: computations/dimension_question/reach_probe_deepening.py -- built r2376 (c54.11);
