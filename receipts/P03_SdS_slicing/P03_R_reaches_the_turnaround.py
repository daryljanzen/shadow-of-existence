"""
P03_R_reaches_the_turnaround.py -- P3 sec:ellipse / sec:temporal-threeness, P14 prop:wall:
does the mass reflection R = gamma^5 reach the TURNAROUND three?  ** The weld split's
falsifier, and it answers in the POSITIVE. **

THE LAW (receipt P03_the_weld_and_the_two_folds): r^(D-1) = 2 M alpha^2 sinh^2((D-1) tau/2 alpha).
R is M -> -M.  For real tau, sinh^2 >= 0, so under R the right side is non-positive and there
is a REAL image iff r^(D-1) < 0 has a real root -- iff D-1 is ODD, iff ** D IS EVEN **, and the
image is r -> -r, the SIGNED RADIUS, which is the corpus's own conjugate branch.

** AND THAT IS THE SAME CONDITION THE DIMENSION RESULT ALREADY USES ON THE HORIZON SIDE. **
2M = r0^(D-3) - r0^(D-1) is odd in the signed offset iff D is even; verified here that the two
conditions agree at D = 4, 5, 6, 7, 8.

=> ** THE CHIRALITY GRADING SURVIVES THE WELD SPLIT, AND VERBATIM. **  R acts on the turnaround
three exactly where it acts on the horizon three, so P14's parity argument transfers to
generation's new seat without modification, including its D = 5 conclusion (no mass-reflection
Z_2, hence no gamma^5, hence four generations and no chirality).  ** The split is SAFE. **

THE GROUPS.  -1 is not in the deck orbit {1, omega, omega^2}, so R is genuinely outside the deck
Z_3 and the two generate Z_3 x Z_2 = Z_6.  On the split the turnaround three carries Z_6, where
the horizon three carries D_6 = S_3 x Z_2.  ** Z_6 < D_6 at index 2, and the missing part is
exactly the TRANSPOSITIONS -- which the split assigns to COLOUR, where the causal trichotomy
needs them for the singlet. **

WHAT IS NOT SHOWN: that the wall-bound zero mode's chirality is graded by the TURNAROUND-side R
rather than the horizon-side one.  The two coincide at D = 4 by the agreement above, so nothing
is wrong, but the identification is a further step (L-94).

ORIGIN: computations/dimension_question/L92_does_R_reach_the_turnaround.py -- built r2376
(c54.31); edit the origin, not this copy."""
import sympy as sp
