"""
P03_slate_worked.py -- the nine inherited bake queues, WORKED rather than triaged.

L-24 CONFOCAL FAMILIES.  A confocal family of an EQUILATERAL quadric is its own alpha-family
(all semi-axes equal, so the confocal parameter only rescales).  ** And the corpus already
carries it: P5 sec:alpha-action / C1 gives the absolute's stabiliser as O(5,1) x R+, the R+
being that dilation. **  So the dilation is CONFOCAL rather than merely scaling -- an
identification worth having, since confocal families carry an orthogonality theorem the
corpus has never used.

L-25 THE CONFORMAL BOUNDARY.  de Sitter's J^+ is spacelike and at INFINITE affine parameter;
the seam and r=0 are at FINITE proper distance.  ** So neither is a conformal boundary -- and
P8 sec:synchronous already says exactly this, calling the mistake 'the time-face of the same
category error whose radius-face is taking r=0 for the source'. **  The queue entry asks for
what P8 supplies.

L-26 THE PHOTON RING'S FINE STRUCTURE.  ** THE LEAD'S PREMISE WAS WRONG AND THE TRUE ANSWER
IS STRONGER. **  Sub-ring spacing is exp(-2 pi lambda/Omega).  O5 shows lambda -> 0 at the
forced member -- but Omega -> 0 with it, both proportional to f(r_ph), so the RATIO survives.
And the ratio is not merely finite: for f = 1 - 2M/r - r^2/alpha^2 one has the identity
f'' = 2(f-1)/r^2, whence lambda^2 - Omega^2 = 0 IDENTICALLY, at every radius and every member.

    ** lambda / Omega = 1 exactly, so the sub-ring demagnification is exp(-2 pi) = 0.0018674
       -- the Schwarzschild value, INDEPENDENT OF M AND OF Lambda, and unchanged at the
       degenerate member.  What degenerates there is the TIMESCALE, not the spacing. **

A sharp, falsifiable observational statement, and the strongest thing in this batch.

L-27 THE COMOVING OBSERVER.  The E=1 congruence has v = sqrt(1-f) relative to static, so
every static optical result of O1-O5 transfers with ONE universal aberration factor
sqrt((1-v)/(1+v)), which -> 0 at either horizon: a comoving observer approaching a horizon
sees the shadow beamed to a point.  ** The comoving reading is a factor, not a redo. **

L-28 THE ACTION GROUPOID'S ORBIT SPACE.  For an action groupoid G |x X the orbit space is
X/G; here X = cuts, G = SO(5,1), so ** X/G is the space of GEOMETRIES -- which is P9's range
theorem -- and the orbit-type stratification is P12's stratification. **  Two papers already
carry it under other names.

L-30 THE EXCENTRES at X0 = sqrt(15) alpha.  Computed: ** ZERO null connections among the six
excentres and ZERO between excentres and hinges (0 of 36 pairs). **  So the hinge
configuration's null connectivity is CLOSED and does not reach the overhead points: the
excentres are real substrate points the construction's causal structure does not touch.  A
computed negative, not an open queue item.

L-31 / L-33, AND THE BATCH'S REAL FINDING.  L-33: CONFORMAL C5 records the SCTs as RUN at
r1888 ('closed, and there was never a remainder'); the queue quotes the pre-run text.  L-31:
the figure ledger's PART XI names the Euclid question a programme, and ** PART XII (r1110) is
'THE EUCLID PROTOCOL, RUN -- all five candidates Part XI named', with PART XIV a full-corpus
sweep. **  ** So three of the nine slate items (L-25, L-31, L-33) are STALE QUEUE TEXT
quoting a question the ledger later answered.  The queues were never pruned after the runs. **

L-32 U1-U6.  U3 is a FIGURE, not a question -- nothing unknown, only undrawn -- and after the
twelve legs, the three walls and the causal trichotomy there is far more to draw than when it
was staged.

ORIGIN: computations/baryon_edge/BATCH4_the_slate_worked.py -- built r2376 (c54.34); edit the
