"""
P03_transverse_space_is_round.py -- P3 sec:slicing / P14 sec:chirality: the transverse
two-space of a CR cut is the ROUND S^2, forced twice, and the mode tower therefore does not
truncate.

WHAT THIS RECEIPT ESTABLISHES.
  1. CR'S OPERATOR WITH THE TRANSVERSE CURVATURE LEFT FREE.  Replacing the round dOmega^2 by
     a transverse 2-space of constant curvature k and running P8 thm:kernel's condition
     T_{mu nu} = 0 in the construction gauge gives, as the ENTIRE solution space,
        f(r) = k - 2M/r - Lambda r^2/3,
     so the operator alone does NOT fix k.  That is exactly what the register's L-90 said.
  2. BUT THE FAMILY DOES.  k enters f as an additive constant independent of M, so one member
     fixes it for all.  CR's family passes through 2M = 0 six times per swing
     (w = 0,60,...,300 deg), where the cut IS the substrate -- pure de Sitter, f = 1 - r^2/alpha^2,
     constant term 1.  ** Hence k = +1, as an OUTPUT of the construction. **  Verified: that f
     solves the ODE at M = 0 with Lambda = 3/alpha^2, which is CR's own definition of alpha.
  3. AND THE GLOBAL FORM IS FIXED INDEPENDENTLY, BY SPIN STRUCTURE.  Curvature +1 leaves
     S^2/Gamma.  Every non-identity element of SO(3) is a rotation about an axis meeting S^2
     in two fixed points, so no non-trivial subgroup of SO(3) acts freely; the only free
     finite action is the antipodal map, which is not in SO(3), and whose quotient RP^2 is
     non-orientable and carries NO SPIN STRUCTURE.  ** So Gamma = 1: the round S^2. **
  4. AND THE TOWER DOES NOT TRUNCATE, UNDER ANY OPTION.  Even allowing an ORBIFOLD quotient
     S^2/Z_n -- which a construction full of branch points might claim -- lambda still runs
     over ALL positive integers; only the degeneracy thins from 2 lambda to about 2 lambda/n.
     A quotient does not change the LOCAL geometry, and lambda is set by the local geometry,
     so a quotient can delete states within a rung and cannot remove a rung.

** WHY THIS IS RECORDED AS A LOSS AND NOT A WIN.  The register's L-15 observed that the
geometry's 12 null legs + 3 walls = 15 matches one Standard Model generation, and bet that
the mode tower was a redundancy of the description -- a bet resting entirely on roundness
being a free choice.  It is not a free choice, and no available alternative truncates the
tower anyway.  L-15's own falsifier fires: on this computation the 12+3 agreement is not
explained, and the route judged most likely to explain it is closed. **

WHAT IT DOES ESTABLISH POSITIVELY.  P14's recorded negative result -- "the angular
decomposition does not supply multiplicity, since lambda = j+1/2 labels partial waves and
each contributes exactly one bound mode" -- was stated as a fact about the round sphere the
paper had written down.  It is now a fact about the only transverse space CR admits, so the
source is closed by exhaustion rather than by inspection.

ORIGIN: computations/baryon_edge/L107_is_the_transverse_space_round.py -- built r2376
