"""
P03_the_adjoint_is_entailed.py -- P3 prop:factor / sec:cubic: the corpus's factorisation
ellipse is the Killing form on the Cartan, its hexad satisfies the root-system axioms, and
the adjoint of su(3) is therefore ENTAILED rather than owed.

WHAT THIS RECEIPT ESTABLISHES, IN ORDER.
  1. P3 prop:factor factors the horizon cubic on the ellipse r^2 + r r_0 + r_0^2 = 1 and calls
     it "the A_2 norm form" without asking what the form IS.  Vieta puts the three horizon
     roots in the plane { x in R^3 : sum x = 0 }.  ** Parametrising that plane by its first two
     entries, the induced Euclidean norm is EXACTLY TWICE the corpus's form: |x|^2 = 2(a^2+ab+b^2).
     So P3's ellipse is the unit circle of the Killing form on the Cartan subalgebra of su(3). **
  2. The six differences e_i - e_j -- which are the corpus's own DESIGNATION TRANSITIONS, the
     step to a neighbouring hinge, whose reflections are sigma -- satisfy all four axioms of a
     reduced crystallographic root system in that form: spanning and 0-free; reduced (only
     +-alpha proportional); closed under its own reflections; and ** INTEGRAL, the Cartan
     integers coming out {-2,-1,1,2} **, which is the axiom that can fail and does not.  The
     angles are 0/60/120/180 with no 90, and the reflection group has order 6 = |S_3|.
  3. ** ALL SIX ROOTS HAVE CORPUS-NORM EXACTLY 1, so they LIE ON P3's OWN FACTORISATION
     ELLIPSE. **  The corpus drew the A_2 root hexagon and recorded the drawing as a
     factorisation identity.
  4. Six roots, rank two, reduced and irreducible: the classification of rank-two systems
     (A_1 x A_1 with 4, A_2 with 6, B_2 with 8, G_2 with 12) leaves exactly A_2; and by the
     isomorphism/existence theorems a reduced irreducible root system determines a complex
     simple Lie algebra uniquely -- sl(3,C), compact real form su(3), of dimension
     |Phi| + rank = 6 + 2 = 8.  ** The adjoint is not a further datum: it is the hexad plus
     the sum-to-zero plane, counted. **
  5. AND THE QUESTION AS POSED WAS MIS-POSED.  |Z(SU(3))| = |P/Q| = det(Cartan matrix) = 3 is
     computed from the SAME root system.  ** The centre and the adjoint are one datum, not
     two gifts; one cannot have the centre of A_2 without having A_2. **

** THE BOUND, AND IT IS THE LARGER HALF.  A Lie algebra being present is not a gauge symmetry
being present.  Nothing here supplies (i) a bundle for su(3) to act on, (ii) a connection
making the action gauged, or (iii) a reason the content sits in the fundamental.  The
corpus's existing walls stand: su(3) is not a subalgebra of so(5,1) (P13), and the su(3) on
the conjugate real form S^5 is reached by the global Wick, which the slicing never performs
(P3 sec:seam).  What changes is the SHAPE of the debt: the corpus does not owe the adjoint of
su(3); it owes the object su(3) acts on -- which is precisely P14's own open bundle question,
so the two are one question. **

ORIGIN: computations/baryon_edge/L96_what_supplies_the_adjoint.py -- built r2376 (c54.36);
