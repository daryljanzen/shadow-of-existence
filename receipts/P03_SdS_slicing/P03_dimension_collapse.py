"""
P03_dimension_collapse.py -- P3 rem:dimension: for which spacetime dimensions does the
horizon relation collapse to a single multiple-angle in the sky angle, as prop:triple
shows it does at D=4?

In D-dimensional Schwarzschild-de Sitter, 2M = r0^(D-3) - r0^(D-1) (alpha=1).  With
r0 = varrho sin w the harmonics present are of order D-1, D-3, ..., so the number BELOW
the top is exactly one at D=4 and D=5 and two or more from D=6 -- against one available
scale.  Hence:

    D >= 6 : no scale collapses it (checked for a single harmonic of ANY order, not
             just the top one), so no pure multiple-angle and no forced fold;
    D = 5  : collapses at varrho = 1 to 2M = (1 - cos 4w)/8, a four-fold;
    D = 4  : collapses at varrho = 2/sqrt3 to 2M = (2/(3 sqrt3)) sin 3w -- prop:triple.

TWO CHECKS THAT COULD HAVE FAILED.  (i) The hinge placement generalises to a regular
(D-1)-gon with the throat as incircle; its vertex distance alpha/cos(pi/(D-1)) returns
the FORCED 2*alpha at D=4, a value P3 fixes by an unrelated route (the hole's own
circle, midpoint tangency, the nine-point circle).  (ii) In both surviving dimensions
the tangent from a hinge lands on the Nariai radius computed independently from the
double root of f.

ASSUMED: the D-dimensional f is the standard Tangherlini-de Sitter form -- the natural
codimension-one extension, not a rederivation of the operator at general D.  This is the
single load-bearing assumption and the paper states it.

This receipt is the adversarial pass run before the result was propagated: five attacks,
none lands.  It also records one defect it caught in ITSELF -- an early version forced an
odd extension of 2M to negative offsets, which is wrong at odd D and produced a spurious
D=5 residual.

ORIGIN: computations/dimension_question/step0_adversarial.py -- built r2376 (c54.10);
