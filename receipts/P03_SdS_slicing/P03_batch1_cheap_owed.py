"""
P03_batch1_cheap_owed.py -- P3 sec:tour / P14 prop:wall: six owed leads worked together.

L-53  THE CAUSAL TRICHOTOMY AT GENERAL D.  With the hinge figure a regular (D-1)-gon at
transverse R = alpha/cos(pi/(D-1)), X.Y = -eps eps'(R^2 - alpha^2) + R^2 cos(dtheta), and
null iff = alpha^2.  Same horn forces dtheta = 0 at EVERY D (no same-horn nulls anywhere);
opposite horn gives cos dtheta = 2cos^2(pi/(D-1)) - 1 = cos(2pi/(D-1)), i.e. dtheta = +-ONE
STATION STEP, at every D.  ** So degree two is a SUBSTRATE fact and carries no dimensional
weight. **  BUT what the colour reading needs is EXHAUSTION -- that a puncture plus its two
partners uses up ALL the stations, so a bound triple is one-per-station and hence a singlet
-- and exhaustion holds at D = 4 ALONE (three stations, and a triple is three; from D = 5 the
triple is a proper subset).  ** So the colour reading IS dimension-selected, by exhaustion
rather than by degree -- a SECOND selector, independent of the harmonic collapse. **

L-58  IS THE CAUSAL 2+1 THE DESIGNATION 2+1?  The two AGREE as partitions under P3's
canonical hinge -> root map.  The five-test criterion scores 2/5: roots are values of r on the
horizon locus, stations are LINES in the substrate, and the two are graded by different
quantities (causal character vs which-root-is-mine).  ** Two structures carrying one partition
-- which is what two INDEPENDENT LABELS look like. **  The c54.18 claim of three distinct 2+1s
stands; the reason first given for it (that the partitions differ) was wrong.

L-62  THE SIGN TEST, and P14 answers it in its own sentence: "exchanging the two rulings is a
motion of it -- an isometry, ACTING ON THE CUT SPINOR AS gamma^5".  The two legs at a puncture
ARE the two rulings (receipt P03_twelve_null_legs).  ** So leg handedness and wall chirality
are the same Z_2.  One sign.  The gnomonic sign, the mass-reflection R and gamma^5 are one
fact. **  No computation was required -- only reading two sentences against each other.

L-63  THE TWO TWELVES.  The twelve legs carry the REGULAR representation of D_3 x Z_2 (orbit
12, stabiliser trivial), and D_3 x Z_2 = D_6, the dial's group on its twelve designations.
** So the two twelves are ISOMORPHIC as G-sets -- and by this corpus's own criterion that is
NOT sufficient to identify them.  BOUNDED, not closed: what would settle it is a natural map,
not a count. **

L-66  Corollary of L-62: 12 legs = 6 punctures x 2 CHIRALITY, the third factor confirmed
rather than assumed.

L-80  DO THE WALL STATES CARRY A HORN LABEL?  A wall sits at X0 = 0 and T is X0 -> -X0, so a
wall is a FIXED POINT of T: three walls, not six.  ** The count survives its own falsifier. **
And the consequence worth keeping: the punctures are the T-ORBIT and the walls are the T-FIXED
LOCUS, so "quarks are horn-doubled and leptons are not" reads as "the quark seats are the
orbit and the lepton seats are the fixed points".

TWO OF THE SIX WERE FALSIFIERS OF CLAIMS THIS CORPUS HAD MADE (L-62, L-80) AND BOTH CLAIMS
SURVIVED.  None needed a turn of its own.

ORIGIN: computations/baryon_edge/BATCH1_the_cheap_owed.py -- built r2376 (c54.28); edit the
