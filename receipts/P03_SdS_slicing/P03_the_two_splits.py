"""
P03_the_two_splits.py -- P3 sec:cubic: the DESIGNATION split (the slicing singles its own root
against the pair) and the SIGN split of the horizon triplet agree exactly when $|r_0|>1$ -- a
condition on the OFFSET, not on the mass -- and at Nariai they not only differ but FAIL IN
DIFFERENT PLACES.

WHAT WAS ASKED.  A standing lead observed two $2{+}1$ splits of the horizon roots and asked
whether they coincide at Nariai or stay distinct, requesting both computed along the whole dial.

WHAT IS ESTABLISHED.
  1. FROM THIS PAPER'S OWN FACTORISATION $(r-r_0)(r^2+r r_0+r_0^2-1)=0$: the designated root is
     $r_0$ and the pair is $(-r_0\pm\sqrt{4-3r_0^2})/2$, real while $|r_0|\le 2/\sqrt3$.
  2. ** THE SPLITS COINCIDE EXACTLY WHEN $|r_0|>1$ **, and the argument is two lines: they
     coincide iff the designated root is the sign-odd one, i.e. iff both pair members share a
     sign; the pair's product is $r_0^2-1$ and its sum is $-r_0$, so they share a sign iff
     $r_0^2>1$, and that shared sign is automatically opposite to $r_0$'s.  Checked against
     every sampled point of the dial.
  3. ** AND $|r_0|=1$ IS THE $M=0$ MEMBER **, since $2M=r_0-r_0^3$.  So the boundary between
     agreement and disagreement is exactly the de~Sitter member of the family.
  4. ** THE DISCRIMINATING QUANTITY IS THE OFFSET, NOT THE MASS.  A first reading of the table
     gave "they differ on the positive-mass branch and agree on the negative-mass branch"; the
     table refutes it -- agreement occurs at $2M=+0.385$ and at $2M=-0.385$ alike.  The
     agreement region $|r_0|>1$ is INVARIANT under $R:r_0\mapsto-r_0$ rather than exchanged by
     it, $R$ mapping the region to itself while flipping the sign of $M$ within it. **
  5. ON THE DIAL that is a plain angle: with $r_0=(2/\sqrt3)\sin w$, the condition reads
     $|\sin w|>\sqrt3/2$, the sky angle exceeding $60^\circ$ -- and $w=60^\circ$ is where
     $\sin 3w=0$, which is the $M=0$ designation.  Nariai sits at $w=30^\circ$, well inside.
  6. ** AT NARIAI THE SPLITS DIFFER, AND THEY FAIL IN DIFFERENT PLACES, which is sharper than
     the question asked for: the designated root merges with a pair member there (asserted), so
     the designation split has no content, while the sign split stays perfectly sharp isolating
     $-2/\sqrt3$.  A structure that survives Nariai and one that does not are not two readings
     of one thing. **

** AND A CAUTION THAT BELONGS WITH IT: the matter sector now uses a THIRD $1{+}2$ split -- own
wall against the other two, verified twice from independent directions.  Whether that is a third
structure or one of these two read on the hinges is NOT settled here and should not be assumed
either way; this corpus has a standing habit of conflating threes. **

ORIGIN: computations/baryon_edge/L012w_the_two_splits.py -- built r2376 (c54.73); edit the
