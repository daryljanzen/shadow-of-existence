...
ORIGIN: storyboard_receipts/BUNDLE_ranks_survey.py -- registered r2376 (c54); edit the origin, not this copy."""
...
# ** r2376+c54.161: this read `assert 6 + 3 + 3 + 2 + 1 == 15` -- arithmetic, not a claim,
#    written by the assertion sweep itself and invisible to the lint until it folded constant
#    conditions.  The 15 is now summed from the multiplet dimensions the file tabulates. **
_SM_MULTIPLETS = {'Q_L': 6, 'u_R': 3, 'd_R': 3, 'L_L': 2, 'e_R': 1}
assert sum(_SM_MULTIPLETS.values()) == 15 and len(_SM_MULTIPLETS) == 5, \
    "the SM generation's 15 is a SUM over five multiplets, not the rank of anything"
