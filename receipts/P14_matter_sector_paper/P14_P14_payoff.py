"""
P14_P14_payoff.py -- verifies THE PAYOFF (sec:cosmogenesis): A3's geometric conjugation R o K lands on
  P14's ACTUAL fermion zero-modes as charge conjugation's kinematic face. chi_+ = cosh^{-a} normalizable on
  the matter wall (cosh^{+a} rejected); chi_+ has sigma_y=+1, chi_- has sigma_y=-1 (computed +/-1.000);
  R (wall reversal) maps the bound matter mode to a bound mode of OPPOSITE chirality; the bound wall-mode is
  real on the real-tau~ leaf (K-fixed, self-conjugate photon axis); the seam continuation W runs real(bound)
  ->imaginary(propagating), the structure K conjugates. Connects P13 A3 to P14 zero-modes. [P14 sec:cosmogenesis]
STATUS: ✔✔   ORIGIN: storyboard_receipts/P14_payoff.py, verified r1385.
