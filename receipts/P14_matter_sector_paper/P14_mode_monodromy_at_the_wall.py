"""
P14_mode_monodromy_at_the_wall.py -- P14 prop:wall / sec:chirality, P3 sec:lap: the wall IS
the branch point, and the wall-bound zero-mode's sheet advance, computed.

** THE IDENTITY, RUN THROUGH THE FIVE-TEST CRITERION AND RETURNING 5/5. **  The wall (the
point on the throat antipodal to its hinge, where the signed radius flips) and the cube-root
BRANCH POINT (r = 0) agree on LOCUS, TYPE, DEFINITION, EQUIVARIANCE and SEPARABILITY: one
locus with two properties, not two objects.  ** So prop:wall's zero-mode is bound AT the
branch point, and a sheet advance is not something that might happen to it -- it is what
crossing the locus it is bound to MEANS. **

THE NEAR-WALL BRANCHING, DERIVED FROM f AND NOT ASSUMED.  With f = 1 - 2M/r - r^2/alpha^2,
f -> -2M/r as r -> 0, so the leaf measure dl = dr/sqrt|f| gives l = sqrt(2) r^(3/2)/(3 sqrt M),
hence ** r^3 = (9/2) M l^2 **.  And the turnaround law r^3 = 2M alpha^2 sinh^2(3 tau/2 alpha)
expands to ** r^3 = (9/2) M tau^2 + O(tau^4) **.  Two independent routes, one exponent: r is a
cube root of a quantity with a simple zero at the wall, and the cube-root branch point sits
exactly where the wall is.

THE MONODROMY.  r ~ l^(2/3), so continuing l around zero: a full LOOP gives r -> omega^2 r; a
CROSSING (half-loop, l -> e^{i pi} l) gives ** r -> omega r exactly **.  One wall crossing
advances the sheet by one; three crossings return it -- the winding law of receipt
P03_thirds_from_closure, obtained from the branch structure rather than posited.  (The full
loop giving omega^2 is why P14's insistence that the slicing passes THROUGH r=0 rather than
around it is load-bearing.)

** THE MODE, CARRIED THROUGH. **  P14: W = lambda sqrt(f)/r and psi = exp(-int W dx) chi_+,
with x the LEAF's proper distance (the leaf frame's e^1 = dr/sqrt f; P14 fixes the norm as
dl = dr/sqrt|f|).  So W dl = (lambda sqrt(f)/r)(dr/sqrt f) = lambda dr/r -- the sqrt(f)
cancels exactly -- and

    int W dl = lambda log r        =>     ** psi ~ r^(-lambda) **

a pure power of the signed areal radius.  Under r -> omega r the mode picks up
** omega^(-lambda) per wall crossing **, and omega^(-3 lambda) = 1 identically, so the Z_3
label is well defined on every mode of the tower.

    ** triality(mode) = -lambda mod 3, lambda the Dirac angular eigenvalue. **

lambda = 0 mod 3 gives triality zero -- colourless, the lepton class; lambda != 0 mod 3 gives
the two coloured classes, which are the two the corpus reads as exchanged by overall sign (C).
A composite is admissible iff sum(-lambda_i) = 0 mod 3: an isolated mode with lambda not
divisible by 3 returns to a different sheet than it left, so it is not a slicing of the one
manifold.

** WHAT IS NOT COMPUTED. **  The SPECTRUM of lambda -- which angular eigenvalues the wall
problem admits on this leaf, and with what multiplicity.  Until that is done the result fixes
a GRADING but not a CONTENT: it does not say how many modes sit in each class, so it does not
by itself deliver the 12 + 3 count (L-81).

ORIGIN: computations/baryon_edge/L78_the_mode_through_the_branch_point.py -- built r2376
(c54.25); edit the origin, not this copy."""
