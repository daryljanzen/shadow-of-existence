"""
P14_the_wall_is_a_wall_of_a_hinge.py -- P14 sec:chirality / sec:openquestions: the signed areal
radius is defined RELATIVE TO A VANTAGE, so there are three of them and three branch loci, the
monodromy is NON-ABELIAN, and the generator P14_the_bundle_is_the_branching found missing is
the ratio of two vantages' wall monodromies.

THE FORK, AND BOTH SIDES OF IT ARE ALREADY IN THE CORPUS.
  (i)  ONE RADIUS.  P14_route_or_point, as computed: "EACH graze-point crossing sends
       r -> omega r".  That treats r as one function vanishing at all three walls, which makes
       the cover CYCLIC and the monodromy ABELIAN.
  (ii) THREE RADII.  P14's own header: "r=0 is ... the back, X_1 = -alpha, FIXED RELATIVE TO
       THE HINGE about which the slicing plane swings", and P03_wall_is_the_graze_point's
       verification of it: "the wall is a wall OF a hinge -- 'relative to one specified hole'
       is NOT A HEDGE, IT IS THE DEFINITION."  Three vantages, three branch loci.
** The two agree on the LABEL -- both give triality = -lambda mod 3 -- and part on the GROUP,
which is exactly what was missing.  That is why the fork survived unnoticed. **

WHAT IS ESTABLISHED.
  1. THE DECISION TABLE, because a re-reading that breaks the winding sector is worth nothing.
     Re-run against both readings: triality = -lambda mod 3 holds under both; the thirds and
     the three classes are insensitive (they come from closure and the 1+2 horn split, and the
     Z_3 enters only as "the two routes differ by one lap"); and ROUTE-DEPENDENCE HOLDS UNDER
     BOTH -- verified for every hinge pair and every vantage, the ratio being omega^(-lambda)
     or omega^(+lambda) and hence trivial exactly when lambda = 0 mod 3, which is the only
     feature P14_route_or_point uses.
  2. ** AND ONE ESTABLISHED SENTENCE SEPARATES THEM, IN (ii)'s FAVOUR. **  The corpus states
     "a lone coloured constituent has no well-defined value at a point".  Under (i) a full lap
     gives omega^(-3 lambda) = 1 identically, so a lone coloured mode DOES return to itself and
     the sentence is false as written.  Under (ii) the lap acts by omega^(-lambda) and only
     triality zero returns.  ** (i) cannot support the sentence the corpus wrote; (ii) can. **
  3. ** UNDER (ii) A FULL LAP ACTS BY THE CENTRE OF SU(3) AND THE INDIVIDUAL WALLS DO NOT. **
     The product of the three wall monodromies is omega^(-lambda) times the identity, verified
     central with determinant 1; each single crossing is a non-central diagonal.  Under (i) the
     lap is the identity for every mode and says nothing.
  4. ** THE MISSING GENERATOR IS SUPPLIED, AND NOT BY ADJOINING ANYTHING. **  A single wall
     monodromy has determinant omega, so the SU(3) content is in the RATIOS: gamma_A gamma_B^-1
     is a determinant-one non-central diagonal.  With the hinge 3-cycle -- the S_3 that permutes
     the vantages, whose hinge<->wall pairing is antipodal and therefore canonical -- the pair
     does not commute, its commutant is the scalars (IRREDUCIBLE on C^3), and it preserves no
     symmetric bilinear form (only B = 0), so it is not conjugate into SO(3), the only proper
     connected irreducible subgroup.  ** Hence the smallest connected group containing the three
     wall monodromies and the hinge 3-cycle is SU(3) itself. **
  5. ** AND THE TWO GENERATORS ARE THE CORPUS'S TWO THREES DOING DIFFERENT JOBS -- the first
     time they have been needed TOGETHER rather than kept apart: the DIAGONAL generator is the
     three WALLS (branch loci) and the PERMUTATION generator is the three HINGES (vantage to
     vantage).  Neither alone is more than Z_3 or S_3; the pair is SU(3). **
  6. ** THE DECIDING PREMISE, COMPUTED RATHER THAN LEFT OWED. **  Eliminating -X_0^2 + X_1^2
     between static de Sitter's embedding relation alpha^2 - r^2 and the equatorial section's
     -X_0^2 + X_1^2 + X_2^2 = alpha^2 gives r^2 = X_2^2: the areal radius IS the section
     direction transverse to the static pair.  The vantage rotates which direction that is, so
     on the throat circle r_j(phi) = alpha sin(phi - theta_j) -- and THAT RETURNS P14's OWN
     "the back, X_1 = -alpha" rather than being fitted to it, a test it could have failed.
     Evaluated at all three walls: ** each vantage's radius vanishes at its own wall and at
     NEITHER of the other two.  Reading (i) is not available. **

** WHAT IS NOT SETTLED, EXACTLY. **  Step 1 of (6) is derived from the embedding relations
alone; the step that the vantage rotates the areal direction is a READING of P14's sentence --
the one non-computed link -- and the evaluation is at the M = 0 member, where the cut IS the
substrate.  A full settlement evaluates r on the slicing curve at M != 0.  ** AND ONE
ESTABLISHED RESULT IS PUT AT RISK: P03_wall_is_the_graze_point found that a hinge's own null
legs graze the OTHER TWO hinges' walls and never its own, so under (ii) a vantage-j mode
carried along hinge j's own leg crosses no branch point of r_j and picks up no phase.  Route
dependence on the equatorial ARCS survives (1); whether the LEG count and the ARC count are the
same count is now a real question that reading (i) never forced anyone to ask. **

ORIGIN: computations/baryon_edge/L128_three_walls_or_one_radius.py -- built r2376 (c54.52);
