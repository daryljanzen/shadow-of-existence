"""
P14_quark_lepton_frontier.py -- P14 prop:wall / sec:correspondence: an attempt to land the
quark/lepton distinction, with its conflicts named rather than smoothed.

** A RETRACTION FIRST. **  c54.28 recorded "quarks are the T-ORBIT (the six punctures) and
leptons the T-FIXED locus (the three walls)".  That is wrong as a statement about FIELDS:
P14's superpotential W = lambda sqrt(f)/r changes sign at r = 0 and nowhere else, so ** the
only place a mode binds is a WALL **.  The punctures are where the hinges meet the substrate
and no field binds there.  I read a fact about SEATS as a fact about STATES.  (And T's fixed
set is the whole equatorial plane, hence the entire throat circle -- the three walls are three
MARKED POINTS on a fixed circle, not the fixed set.)

** SO THE QUESTION IS NOT WHERE A LEPTON SITS.  Every mode sits at a wall.  It is what
distinguishes them there -- and the corpus supplies exactly two gradings, both off one
number: triality = -lambda mod 3 and chirality = sign(lambda). **

THE PROPOSAL.  Triality is exactly the Standard Model's own quark/lepton discriminant: a field
is coloured iff it carries non-trivial charge under Z(SU(3)), and Z(SU(3)) is cyclic of order
three.  Hence:

    ** A LEPTON IS A WALL-BOUND MODE WHOSE ANGULAR EIGENVALUE IS DIVISIBLE BY THREE.
       A QUARK IS ONE WHOSE IS NOT. **

Not a placement and not an added structure: the quark/lepton distinction becomes a property of
the PARTIAL WAVE, computed (receipt P14_mode_monodromy_at_the_wall) before it was asked to
mean anything.

THE CONSEQUENCE.  P14 puts ONE Dirac spinor on the slicing structure, so ** quarks and leptons
are not two fields but the same field in different partial waves, sorted by lambda mod 3 **,
in the exact periodic pattern two-coloured-then-one-colourless.  That ratio is the Standard
Model's own at the level of multiplets (two quark multiplets by isospin carrying colour, one
lepton doublet not) -- recorded as a RESONANCE, since the corpus's 2:1 is over partial waves
and the SM's over multiplets and nothing shows those are one counting.

AND IT EXPLAINS AN OBSERVATION P14 COULD NOT.  P14 records that the observed content is
anomaly-free "on every count AND ONLY AS A COMPLETE SET" and has no account of why the set
should be complete.  ** On this reading it is forced: you cannot take the coloured waves
without the colourless ones, because they are one field's tower.  Anomaly-freedom only as a
complete set is then a statement that the spectrum is ONE OBJECT. **

** WHAT IS NOT LANDED: THE COUNT. **  The SEAT count (12 legs + 3 walls = 15) and the MODE
count (one per lambda, multiplicity 2|lambda|, infinite) do not agree; both are computed and
they answer different questions.  Three ways out are stated: something truncates the tower; the
tower IS the content (L-88); or the seats carry the states and the tower is a redundancy --
the last being new here and the one the geometry favours, since the 3 walls and 12 legs are
FORCED while the degeneracy 2|lambda| is the ROUND sphere's and L-90 records that roundness is
a choice.  ** If the transverse space at the wall is not round, the degeneracy changes and the
tower may truncate.  That is the next computation (L-107). **

FALSIFIERS: the reading predicts a 2:1 ratio of coloured to colourless partial waves (because
3 is prime and two residues are non-zero), so ** a departure from 2:1, once multiplet and
partial-wave counting are ever tied together, kills it **; and if the modes at one seat cannot
be identified, the seat count is decorative and the 12+3 is a coincidence of small integers.

ORIGIN: computations/baryon_edge/L15_the_quark_lepton_frontier.py -- built r2376 (c54.35);
