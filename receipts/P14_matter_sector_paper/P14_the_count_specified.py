"""
P14_the_count_specified.py -- P14 sec:correspondence: the count problem, specified, and the
factor-by-factor match of the twelve that nobody had written down.

WHY IT EXISTS.  The corpus had been carrying "the count is not landed" in four documents
without once stating what LANDING IT WOULD REQUIRE.  A debt stated without its discharge
condition cannot be worked.  This states it.

** AND IN STATING IT, ONE HALF OF THE PROBLEM TURNS OUT TO BE BETTER THAN RECORDED AND THE
   OTHER HALF WORSE. **

  THE MATCH.  One Standard Model generation is 15 Weyl fermions splitting 12 COLOURED + 3
  COLOURLESS -- a split that is not a convention here, since P14's own grading makes triality
  the quark/lepton discriminant.  The geometry's twelve null legs factor, by the corpus's own
  sheet index, as 3 (graze point / colour) x 2 (horn) x 2 (ruling, graded by R).  Putting the
  two factorisations side by side:
        the 3        graze point            <->  colour, the SU(3) fundamental
        the first 2  HORN                   <->  WEAK ISOSPIN
        the second 2 RULING (+-60 deg)      <->  CHIRALITY
  ** and the multiplicities agree state by state, not merely in total: four legs per graze
  point against four coloured Weyl states per colour (u_L, d_L, u_R, d_R), 3 each. **
  The horn/isospin leg is NOT a guess: P03_winding_and_closure already records
  'sigma -> colour, T -> weak isospin, R -> chirality and species', and 'the horn swap costs
  exactly one lap -- the nucleon doublet'.

  THE MISMATCH, AND IT IS SHARPER THAN '12+3=15 IS UNEXPLAINED'.  The remaining three states
  are colourless, and the three WALLS cannot supply them: ** P14 has already spent the walls.
  sec:count binds one mode per wall and reads the three as the three GENERATIONS -- 'the index
  counts FAMILIES, not the states within them'. **  Reading the three walls as one generation's
  three leptons is a category error, and it is the error the phrase "12 + 3 = 15" was making.

  THE DISCHARGE CONDITION, as five testable requirements (S1-S5): three objects PER HINGE,
  triality zero, disjoint from the legs and from the walls-as-generations, carrying a
  chirality grading splitting 3 as 2+1 and an isospin grading making a doublet plus a singlet,
  the whole assignment equivariant under the order-twelve group that acts transitively on the
  six punctures.  ** And S3 is the sharp one: the colourless triple is chirality-ASYMMETRIC,
  while every object the geometry has offered so far comes in R-conjugate PAIRS.  So the
  colourless three must sit on a locus where R does NOT act freely -- a FIXED set. **

WHAT THIS IS NOT.  No correspondence is asserted.  The 12 <-> 12 match is exhibited as a
...
# ** r2376+c54.161: this read `assert 3*2*2 == 12` -- arithmetic, not a claim.  The lint could
#    not see it until it was taught to fold constant conditions, and it was written BY the
#    assertion sweep itself.  The factorisation is now read off the enumeration. **
assert (len({c for c, _, _ in legs}), len({h for _, h, _ in legs}), len({u for _, _, u in legs})) \
    == (3, 2, 2), "the twelve legs factor 3 (colour) x 2 (horn) x 2 (ruling) BY ENUMERATION"
