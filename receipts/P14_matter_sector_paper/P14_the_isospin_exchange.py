"""
P14_the_isospin_exchange.py -- P14 sec:correspondence / P3 sec:charge: what exchanges uud
and udd, where it is seated, and the one number it does not get to choose.

THE QUESTION.  The configuration's symmetry group is transitive on the six punctures, so no
invariant distinguishes the two readings -- that is proved (P03_hexagon_null_triple) and it
is the STRUCTURE, not a gap.  The live question is therefore the other one: what RELATION
EXCHANGES them, and where does it sit?

WHAT IS ESTABLISHED.
  1. ** A CLEAN NEGATIVE FIRST.  Charge conjugation ALONE cannot be the exchange. **  On the
     winding line w -> -w sends u = +2/3 to -2/3 and d = -1/3 to +1/3: it carries the quark
     class to the ANTIQUARK class, i.e. uud to the antibaryon, not to udd.  Nor does any lap
     shift.  The reason is structural: C reflects through the ORIGIN and the (u,d) pair is
     not centred there.
  2. ** THE UNIQUE INVOLUTION EXCHANGING THEM IS AFFINE:  w -> 1/3 - w. **  Solved for, not
     posited: an involution of a line exchanging two points is the reflection in their
     midpoint, and c = u + d = 1/3.
  3. ** AND THAT IS THE STANDARD MODEL'S OWN MAP.  With Q = I_3 + Y/2, the isospin Weyl
     reflection at fixed Y is Q -> Y - Q, and for the quark doublet Y = 1/3. **  The constant
     was solved for and came out equal to the doublet's hypercharge; nothing was fitted.
  4. ** ITS TWO FACTORS ARE BOTH THE CORPUS'S OWN AND NEITHER IS IN D_6. **
     w -> -w is P3 sec:charge's independent Z_2; w -> w + 1/3 is ONE graze-point crossing,
     i.e. one wall, one third of a lap.  And since Aut(A_2) = D_6 acts LINEARLY -- every
     element fixes the origin -- a map with nonzero offset cannot lie in it.  ** P3 sec:charge
     had already established, as a bookkeeping remark, that the charge Z_2 is adjoined to D_6
     'rather than sitting within it'.  That is the structural precondition for this exchange
     to exist at all. **
  5. ** THE OFFSET IS 1/N WITH N THE WALL COUNT, AND THE BARYON CHARGE 3/(2N) + 1/2 IS
     INTEGRAL AT N = 1 AND N = 3 AND NOWHERE ELSE. **  The forcing is to a PAIR, not a point,
     and that is stated rather than hidden; CR excludes N = 1 by the one-hinge-truncation
     argument it already runs on independent grounds (P14 sec:count, Rule 2).

** A CORRECTION IS CARRIED IN THE BODY AND IS NOT HIDDEN.  The first version of PART 4
concluded that the exchange 'requires a symmetry adjoined from outside D_6', arguing that D_6
acts linearly and an affine map cannot lie in it.  THE ARGUMENT COMPARES OBJECTS IN TWO
DIFFERENT SPACES: D_6 acts on the ROOT SPACE, while w lives on the WINDING LINE, an affine
line whose origin (the lepton class) is a choice.  PART 4b states the corrected result:
** the exchange IS T, the horn swap -- a substrate isometry, inside the configuration's own
order-twelve symmetry group, and ALREADY READ BY THE CORPUS AS WEAK ISOSPIN **
(P03_winding_and_closure: 'the horn swap costs EXACTLY ONE LAP -- the nucleon doublet';
'sigma -> colour, T -> weak isospin, R -> chirality and species').  Verified here: applying
w -> 1/3 - w constituent-wise carries (u,d,d) to (d,u,u) and the total from 0 to +1. **

WHAT IS ACTUALLY NEW, THEN.  The corpus had T at the level of the TRIPLE.  Constituent-wise
T is the affine reflection whose FIXED POINT is 1/6 = Y/2, the quark doublet's hypercharge.
And since C is the reflection at 0 and T the reflection at 1/6, their composite is a
TRANSLATION by 1/3 -- one graze-point crossing, one colour step:  ** T o C = sigma on the
winding line, so the corpus's three involution-readings are NOT independent there. **

THE OTHER BOUND, ON CHARGE, STATED AGAINST P9 RATHER THAN AGAINST ONE SENTENCE OF P3.  P3
sec:charge establishes that Q != 0 replaces the r=0 branch point with a timelike RN
singularity behind an inner horizon -- and states in the same breath that the charged closed
loop is 'an interior reassignment tied to the matter side'.  P9 already carries exactly that
as its ONE named open remainder ('the irreducible interior reassignments (Kerr-inner,
Reissner-Nordstroem-interior)').  ** So this is an instance of a scoped frontier, not a new
tension. **  What P9 does supply is the division that matters: CHARGE IS THE BEND -- electrovac,
m(r) = M - Q^2/2r, the vacuum kernel unchanged.  ** So the winding charge lives on the VACUUM
KERNEL and the Maxwell charge is the MATTER BEND: opposite sides of the axis P9's whole
classification is built on. **  The corpus nowhere claims they are the same object, and the
winding derivation appears in no paper body, its own receipt marking the antecedent unshown.
What is owed is a positive account, and its shape is constrained: not a Q -> 0 limit (there is
no lap at Q != 0 to take a limit of), so a CORRESPONDENCE -- structurally P13's
C = (Q -> -Q)_field o (R o K)_geometric pattern one level down, with the winding supplying the
quantisation and the field the sign and scale.  Registered, not discharged.

ORIGIN: computations/baryon_edge/L59_the_exchange_is_an_affine_reflection.py -- built r2376
(c54.36); edit the origin, not this copy."""
import itertools
import sympy as sp
