"""
P14_charge_or_colour.py -- the winding sector: the route-dependent Z_3 is not a rival to
electric charge but the OBSTRUCTION to electric charge being observable.

** NOT A PAPER RESULT.  The winding derivation appears in no paper body, and this does not
   change that.  It is registered because it dissolves the last standing objection to the
   sector's remaining reading, and because it produces a check that could have failed. **

THE OBJECTION IT ANSWERS.  P14_route_or_point established that a matter field here is
route-labelled iff its triality is non-zero.  A route-dependent quantity is not a function of
position, so it is not something an apparatus at a place can read -- while electric charge is.
So either the winding is route-dependent and cannot be a charge, or it is a charge and the
monodromy result is wrong about it.

WHAT IS COMPUTED.
  1. ** THE WINDING SPLITS, AND THE CORPUS SPLIT IT FIRST. **  P03_winding_and_closure records
     that u and d share fractional part 2/3 while ubar and dbar share 1/3, so every winding is
     w = n + phi with n an integer LAP COUNT and phi a Z_3 class.  Tabulated here for all six
     species.
  2. ** THE TWO PIECES TRANSPORT DIFFERENTLY. **  One graze-point crossing multiplies the field
     by omega^(triality); a full LAP is three crossings and omega^3 = 1.  ** So the integer part
     is route-INDEPENDENT and the fractional part IS the monodromy. **
  3. ** HENCE w IS LOCALLY READABLE IFF IT IS AN INTEGER IFF TRIALITY VANISHES. **  The
     dichotomy of the objection is false: the winding is not all of one kind.
  4. ** SO THE Z_3 IS THE OBSTRUCTION TO THE CHARGE BEING OBSERVABLE, not a competitor to it:
     a fractionally charged object is exactly one whose charge is route-dependent, hence not
     readable at a place, hence not free. **
  5. THE CHECK, WHICH COULD HAVE FAILED.  "Is a field on the manifold" (total triality zero)
     and "has integer total charge" are tested against each other on ten contents -- proton,
     neutron, both pions, Delta++, antiproton, electron, and the rejected lone quark, diquark
     and four-quark states.  ** They agree on every one **, and then symbolically: a colour
     singlet has n_q - n_qbar = 0 mod 3, and the total charge's fractional part is
     (2 n_q + n_qbar)/3 = (3 n_q - (n_q - n_qbar))/3, an integer exactly under that condition.

** SO THE STANDARD MODEL'S COLOUR-SINGLET/INTEGER-CHARGE CORRELATION -- which there follows
from the hypercharge assignments anomaly cancellation fixes -- is here the same statement as
"only single-valued combinations are fields".  Two routes to one fact, and the second is a
monodromy. **

** WHAT REMAINS A READING, AND THE ABOVE MAKES IT EASY TO FORGET.  That w IS the electric
charge is still not proved.  What is proved is that w has exactly the STRUCTURE electric charge
has -- an integer observable part and a confined fractional part in thirds, correlated with a
Z_3 -- and a quantity with the right structure is not thereby the quantity.  The
deck/graze-Z_3-is-colour identification is unchanged, and nothing here supplies a gauge field,
a coupling, or a scale: the winding has no units and electric charge does. **

ORIGIN: computations/baryon_edge/L123w_charge_or_colour.py -- built r2376 (c54.47); edit the
