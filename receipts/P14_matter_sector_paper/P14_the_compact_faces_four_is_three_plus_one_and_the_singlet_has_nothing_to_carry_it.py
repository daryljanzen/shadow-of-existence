...
commutant.  ** The order-27 SU(3) part is Delta(27): non-abelian, exponent 3, centre the Z_3
generated by omega times the identity -- the Heisenberg group mod 3, generated in SU(3) by
diag(1, omega, omega^2) and the hinge three-cycle. **  And the extra Z_3 of phases is NOT the
trace of the compact face's u(1): that u(1)'s Z_3, restricted to C^3, is omega times the
identity -- central, inside SU(3) -- and shows itself only on the fourth, colourless component
of the 4, which a three-dimensional holonomy does not have.  P14's extra phase is a non-central
element of U(3).  *The compact face's su(3) would be an isometry
