"""
P14_lambda_spectrum.py -- P14 prop:wall: the angular spectrum of the wall-bound mode, and why
it fixes a GRADING but not a CONTENT.  ** An honest negative. **

Receipt P14_mode_monodromy_at_the_wall computed triality = -lambda mod 3 for the wall-bound
zero mode.  This asks which lambda occur.

PART 1.  lambda is the eigenvalue of the angular Dirac operator on the cut's two-sphere, whose
spectrum is standard and not CR-specific: lambda = +-1, +-2, ..., multiplicity 2|lambda|.
lambda = 0 is excluded BY THE CONSTRUCTION rather than by hand -- W = 0 there, so there is no
wall and nothing to bind.

PART 2.  Normalizability at the wall: psi ~ l^(-2 lambda/3), so |psi|^2 dl converges iff
lambda < 3/4.  Hence for lambda > 0 the exp(-int W) branch fails and the conjugate binds; for
lambda < 0 the reverse.  ** So WHICH SPINOR BRANCH BINDS IS FIXED BY sign(lambda) -- the
chirality is a property of the ANGULAR problem. **  The mode therefore carries two gradings,
both read off one number:  chirality = sign(lambda) [Z_2],  triality = -lambda mod 3 [Z_3].

PART 3.  ** THE TOWER DOES NOT TERMINATE. **  Both gradings are well defined on every rung and
every class is infinitely occupied.  P14 establishes the leaf is COMPACT in the proper measure,
which makes the spectrum DISCRETE -- but discrete is not finite, and compactness does not bound
the angular quantum number.

PART 4, THE NEGATIVE.  ** The 12 + 3 count is NOT delivered by the mode spectrum. **  The leg
counting counts PLACES (12 null legs + 3 walls); the mode counting counts STATES (one bound
mode per lambda, multiplicity 2|lambda|, infinitely many).  These are different questions and
a prior reading slid between them.  The seat count is not refuted but its status changes: a
seat carries a whole tower, so the reading owes a principle selecting one rung per seat and
does not have one.  Three live ways out are stated and not chosen: (a) something selects
lambda; (b) the tower IS the content and 15 is its lowest level; (c) only the GRADING
transfers, so the corpus gets colour/isospin/chirality and NOT the multiplet count.  ** (c) is
what the evidence currently supports. **

PART 5.  P14 already said this and was right: "the index counts WALLS, NOT PARTIAL WAVES";
"the zero-mode is one Weyl mode per wall, NOT fifteen".  ** The paper drew its boundary
correctly and a later reading crossed it without saying so. **

WHAT STANDS UNTOUCHED: the causal trichotomy and the twelve legs; wall = graze point = branch
point; the winding law and the thirds derivation; triality = -lambda mod 3; chirality =
sign(lambda) (new here); confinement as failure to close the lap.  ** What is weaker: the
12 + 3 read as a STATE count.  Downgraded, not withdrawn.  And this forecloses nothing -- a
missing selection principle is a gap in a construction, not a verdict on it. **

ORIGIN: computations/baryon_edge/L81_the_lambda_spectrum.py -- built r2376 (c54.26); edit the
