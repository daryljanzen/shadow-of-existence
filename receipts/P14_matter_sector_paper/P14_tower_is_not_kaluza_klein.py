"""
P14_tower_is_not_kaluza_klein.py -- P14 sec:chirality / sec:correspondence: the lambda-tower
is a PARTIAL-WAVE decomposition and not a Kaluza-Klein tower, and the section's normal bundle
carries nothing new.

WHAT IT ESTABLISHES.
  1. ** A KK TOWER NEEDS AN INTERNAL SPACE.  lambda indexes the Dirac operator on the
     transverse S^2, and that S^2 is the r^2 dOmega^2 of the FOUR-metric -- it is spacetime. **
     2 (the section) + 2 (the sphere) = 4 exhausts the cut, so there is nothing to reduce on
     and the levels are not distinct 4D fields.  ** L-88 answered: NO. **
  2. ** AND THE ONE GENUINE EXTRA DIMENSION CANNOT SUPPLY A TOWER EITHER, for the opposite
     reason: the cut-normal is ONE dimension and NON-COMPACT, giving a continuum rather than
     a discrete ladder ** -- which is the same non-compactness the boundary paper's escape uses.
  3. ** THE RANK-3 NORMAL BUNDLE IS NOT A THIRD THREE.  It splits 2 + 1: two directions are
     the transverse S^2 (the partial waves), one is the CUT-NORMAL, which is the direction R
     acts along ** -- the corpus's own resolution of the R-vs-T tension states this
     ("R reflects the transverse cut-normal, fixing all four spacetime legs").  So the two
     summands are exactly the fermion sector's two existing gradings, angular and chirality,
     and a bundle whose summands the operator already uses adds no rank.  ** P14 listed the
     wall's normal bundle among its bundle candidates; this eliminates it. **
  4. CONSEQUENCE: all three routes of the register's L-15 trichotomy are closed -- (a) and (c)
     by P03_transverse_space_is_round, (b) here.  ** The 12+3 = 15 agreement has no surviving
     explanation inside the present structure, and that is recorded as the result. **

WHAT SURVIVES UNTOUCHED: the factor-by-factor match of the TWELVE (P14_the_count_specified),
which is a statement about the invariant sector alone; the triality grading and its account of
"anomaly-free only as a complete set"; and P14's own position, which was right throughout --
"the index counts walls, not partial waves".  ** The sector never claimed the tower was
content, and every attempt to make it one has now failed. **

ORIGIN: computations/baryon_edge/L088w_the_tower_is_not_kaluza_klein.py -- built r2376
(c54.42); edit the origin, not this copy."""
import sympy as sp
...
# ** r2376+c54.161: this read `assert 12 + 3 == 15` -- arithmetic, not a claim, written by the
#    sweep itself and invisible to the lint until it folded constants.  What is checkable here
#    is that the file supplies no route to the 15, which its own _ROUTES dict records. **
assert not any("LIVE" in v or "OPEN" in v for v in _ROUTES.values()), \
    "no route to the 12 + 3 = 15 agreement is left live in this file"
