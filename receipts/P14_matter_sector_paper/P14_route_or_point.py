"""
P14_route_or_point.py -- P14 prop:wall / sec:chirality: a matter field on this geometry is
labelled by a ROUTE if and only if it is coloured, and colour singlet-ness is single-valuedness.

THE QUESTION.  The winding derivation (P03_thirds_from_closure) is CONDITIONAL, and its own
receipt says so: "IF constituents carry windings and bound objects close, THEN the thirds
follow ... the antecedent is NOT shown."  Everything the sector produces -- the thirds, the
three classes, the isospin exchange, the hypercharge agreement -- rests on it.

** AND NOTHING BELOW IS NEW PHYSICS.  Every ingredient is already in the corpus and predates
   the question.  What is new is recognising that P14_mode_monodromy_at_the_wall ANSWERS it. **

WHAT IS ESTABLISHED.
  1. THE CRITERION, so the question is decidable rather than verbal: a field is POINT-labelled
     iff its monodromy around every closed loop is trivial, and ROUTE-labelled otherwise.  The
     corpus supplies the test case: between two punctures at different hinges there are exactly
     TWO equatorial routes, crossing ONE and TWO of the three graze points.
  2. WHAT THE FIELD IS.  P14's superpotential W = lambda sqrt(f)/r in the leaf measure gives
     W dl = lambda dr/r exactly -- the square roots cancel -- so psi ~ r^(-lambda); and near the
     wall r^3 = (9/2) M l^2, so a crossing is a half-loop sending r -> omega r.
  3. ** TRANSPORTED ALONG THE TWO ROUTES THE FIELD PICKS UP omega^(-lambda) AND
     omega^(-2 lambda), whose RATIO IS omega^(-lambda) = omega^(triality). **  So the two routes
     agree iff triality vanishes.
  4. ** THE ANSWER, AND IT IS A DISTINCTION RATHER THAN A YES OR A NO: a matter field here is
     ROUTE-labelled IF AND ONLY IF ITS TRIALITY IS NON-ZERO -- that is, iff it is COLOURED.
     A quark is route-labelled; a lepton is an ordinary single-valued function on the base. **
     So the antecedent holds for exactly the objects the derivation applies it to, and fails
     exactly where the winding is an integer and no route-dependence is needed.

** AND BOTH OF THE DERIVATION'S ANTECEDENTS ARE NOW DISCHARGED, BY ONE FACT READ TWO WAYS.
The other is closure, which P03_batch3_hot_front struck at c54.32 on the same principle -- "a
field on the branched bead must be single-valued, so a configuration with non-zero total sheet
advance IS NOT A FIELD ON THE MANIFOLD."  Read forward: a coloured field is multivalued, hence
route-labelled.  Read backward: only total-triality-zero combinations are single-valued, hence
closure is required.  ** The winding derivation's conditional is therefore unconditional. **

AND CONFINEMENT BECOMES A STATEMENT RATHER THAN A READING.  P03_winding_and_closure recorded
"confinement becomes FAILURE TO CLOSE THE LAP" as a resonance.  With both halves proved: a lone
coloured constituent has no well-defined value at a point, and only a combination of vanishing
total triality is a field on the manifold at all.  ** COLOUR SINGLET-NESS IS SINGLE-VALUEDNESS. **

** THE ONE REMAINDER, AND IT IS REAL.  The Z_3 established here is COLOUR's -- it is the centre,
by construction.  That this same label is the ELECTRIC charge you can measure is a further step
and this receipt is not it; that identification remains a reading. **

ORIGIN: computations/baryon_edge/L074w_route_or_point.py -- built r2376 (c54.46); edit the
