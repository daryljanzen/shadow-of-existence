...
_sig={}                                 # r2376+c54.161: capture only; computes nothing new
...
for label,mfun in [("m=sin x (2 zeros)", np.sin),
    ...
    _sig[label]=s                       # r2376+c54.161: capture only; computes nothing new
...
# (0) the checks above compute a PASS/FAIL boolean and only PRINT it.  Assert it.
assert ok, "a check in even_crossing_index FAILED -- see the [FAIL] line above"
# (1) PART (A), PINNED.  The Jackiw-Rebbi amplitude on the tanh wall is exp(-int tanh) = 1/cosh:
#     0.00495747 at |x| = 6 on BOTH sides (it decays symmetrically, so the mode is bound), while
#     the conjugate branch is cosh(6) = 201.7156 and is rejected.  Fires if the wall profile or
#     the sign in the exponent moves.
assert abs(amp_bound(6) - 0.00495747) < 1e-7, amp_bound(6)
assert abs(amp_bound(-6) - 0.00495747) < 1e-7, amp_bound(-6)
assert abs(amp_conj(6) - 201.71564) < 1e-4, amp_conj(6)
assert wall_chirality(m_wall, 0) == +1
assert wall_chirality(lambda x: -np.tanh(x), 0) == -1   # the reversed wall binds the other sign
# (2) PART (B), PINNED -- the even-crossing constraint, computed and not asserted: on each
#     single-valued periodic profile the crossing signs ALTERNATE, so their number is even and
#     their sum is zero.  sin 2x gives four crossings, [1,-1,1,-1]; that 4 is the pin.  The
#     lists asserted are the ones the loop above actually built, not a recomputation.
assert sorted(len(v) for v in _sig.values()) == [2, 2, 4], _sig
assert _sig["m=sin x (2 zeros)"] == [1, -1], _sig
assert _sig["m=sin 2x (4 zeros)"] == [1, -1, 1, -1], _sig
assert _sig["m=sin x + 0.3 sin 3x (still single-valued)"] == [1, -1], _sig
assert all(sum(s) == 0 and len(s) % 2 == 0 for s in _sig.values()), _sig
# (3) PART (C), and the ONE thing to be careful about: cr_wall_signs is P14 prop:wall's INPUT
#     (all three walls same-chirality), not something computed here -- so what is asserted is
#     the INCOMPATIBILITY, which IS computed: a net of 3 is not among the net indices any
#     single-valued loop can produce, all of which are 0.  That incompatibility is exactly the
#     receipt's conclusion that the r=0 BRANCH POINT is required.
_nets = {sum(s) for s in _sig.values()}
assert _nets == {0}, _nets
assert sum(cr_wall_signs) == 3 and 3 not in _nets
