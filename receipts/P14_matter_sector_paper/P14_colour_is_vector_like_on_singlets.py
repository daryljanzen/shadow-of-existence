"""
P14_colour_is_vector_like_on_singlets.py -- P14 sec:whichthree: a colourless sector that splits
$2+2$ under every $\mathbb{Z}_2$ grading is not a shape FAILURE -- it is the correct shape, because
colour is vector-like on singlets in the Standard Model too.  Two recorded failures are re-read
as correct behaviour, and the discharge condition they were measured against is re-specified.

WHAT PROMPTED IT.  The sector's standing discharge condition contains a clause -- called "the
sharp one" -- reading: "the colourless triple is chirality-ASYMMETRIC while every object the
geometry has offered comes in R-conjugate PAIRS, so the three must sit on an R-FIXED locus."

WHAT IS ESTABLISHED.
  1. ** COLOUR DOES NOT SEE THE CHIRALITY OF A COLOUR SINGLET, IN THE STANDARD MODEL EITHER. **
     $SU(3)$ acts as the trivial representation on $\nu_L$, $e_L$, $e_R$ and $\nu_R$ alike; it
     does not distinguish $e_L$ from $e_R$.  The asymmetry that makes $\nu_L,e_L$ a doublet and
     $e_R$ a singlet is $SU(2)_L$ and nothing else.
  2. ** SO THE CLAUSE ASKS THE WRONG STRUCTURE.  Its premise is true -- the triple IS
     chirality-asymmetric -- but the inference is addressed to colour.  A colour structure
     producing a chirality-asymmetric colourless triple would not match the Standard Model; it
     would EXCEED it. **
  3. ** AND THAT REFRAMES TWO RECORDED FAILURES AS NON-FAILURES. **  The turnaround's colourless
     sector having dimension four with every $\mathbb{Z}_2$ grading splitting it $2+2$ was
     recorded as a shape failure; against colour, $2+2$ is correct.  The colour bundle's
     invariant line being one-dimensional was recorded as a limitation; it is also correct,
     colour offering one trivial representation rather than a chirality-graded family.  ** Both
     were measured against a condition the Standard Model does not impose on colour. **
  4. WHERE THE ASYMMETRY LIVES, AND WHETHER THIS CONSTRUCTION HAS THE OPERATOR: it lives in
     isospin acting on chirality, and this paper names both -- $T$, the horn swap, read as weak
     isospin; and $R$, the offset parity, realised on the cut spinor as $\gamma^5$.
  5. ** THE COMMUTATOR THAT DECIDES WELL-POSEDNESS, COMPUTED: $T$ and $R$ act on independent
     data and COMMUTE (asserted).  And commuting is the RIGHT answer, against the naive
     expectation: "the weak interaction is chiral" is not the claim that isospin fails to commute
     with $\gamma^5$ -- it is the claim that isospin acts non-trivially on ONE chirality
     eigenspace and trivially on the other, which REQUIRES simultaneous diagonalisability. **
     So the structure is not excluded.  A necessary condition met is not a result, and that is
     exactly as much as a commutator can establish.

** THE RE-SPECIFICATION.  The clause is withdrawn as written and replaced by the question it was
reaching for, addressed to the operator that carries it: does $T$ act on one $R$-eigenspace of
the wall mode and trivially on the other?  That needs Proposition prop:wall's mode and is a real
computation, not done here. **

ORIGIN: computations/baryon_edge/L110w_s3_asks_the_wrong_operator.py -- built r2376 (c54.78);
