"""
P17_the_cut_is_planar_only_at_M_zero.py -- p0 (ladder) / P3 sec:slicing / P9 sec:pd: what
the relation "the four-geometries are CUTS of dS_5" can and cannot mean, computed.

WHY IT EXISTS.  The corpus asserts the cut relation three times -- P9's sweep-subgroup
criterion, P12's orientation-plus-vantage datum, P3 sec:slicing -- and never writes the map.
OPEN_PROBLEMS_MAP C.8 filed the explicit embedding as one whose "value would be pedagogical".
It is not: the fermion sector's count is a question about which transverse sector an object
sits in, and that cannot be posed without it.

WHAT IS ESTABLISHED, all symbolically.
  1. ** EVERY HYPERPLANE SECTION OF dS_5 HAS CONSTANT CURVATURE. **  Splitting X = c n + Y with
     eta(n,Y) = 0 turns the substrate condition into eta(Y,Y) = alpha^2 - c^2, a de Sitter
     four-space of radius sqrt(alpha^2 - c^2), for every admissible n and c.  A quadric cut by
     a linear space is a quadric; there is no third case.
  2. ** SdS IS NOT CONSTANT CURVATURE UNLESS M = 0. **  K = 48 M^2/r^6 + 24/alpha^4, whose
     radial derivative -288 M^2/r^7 vanishes identically iff M = 0.
     => ** A PLANE SECTION OF dS_5 IS SdS IF AND ONLY IF M = 0. **  This CONFIRMS p0 rather
     than correcting it: p0's polar hyperplane (Q6r_polar_is_the_background) is exactly the
     c = 0 row, and this shows it is not one example among many but THE only one.
  3. ** SO MATTER MUST BEND THE CUT, AND THAT IS NOW A THEOREM RATHER THAN A READING. **
     P9's "charge is the bend", P3's "matter the bend of the cut", P8's "bends off the planar
     vacuum profile" are all forced by (1)-(2): a planar cut is constant-curvature.
  4. ** AND THE BENT CUT DOES NOT CLOSE EITHER. **  Imposing only what P3's construction has --
     a round areal sphere in a spacelike 3-block, and staticity as the ambient boost the pencil
     already turns on -- gives X = (F sinh kt, F cosh kt, G, r n) with F = sqrt(f)/kappa forced
     by the dt^2 coefficient.  Then lying ON the substrate fixes G, and inducing SdS fixes G':
     two determinations of one function.  They disagree, and solving for kappa^2 returns an
     r-DEPENDENT expression, contradicting kappa's definition as the rate of a Killing flow.
     => ** THERE IS NO STATIC, SPHERICALLY SYMMETRIC HYPERSURFACE OF dS_5 INDUCING SdS AT
     M != 0. **  Consistent with the classical embedding class: Schwarzschild is class TWO in
     flat space, and dS_5 supplies only ONE dimension above four.
  5. ** AND THE FORM OF THE OBSTRUCTION IS THE RESULT.  The closure defect factors with an
     OVERALL FACTOR OF M: -M alpha^4 (-M alpha^2 + 9 M r^2 + 2 alpha^2 kappa^2 r^3 - 2 r^3).
     The failure of a cut to close as a hypersurface IS the mass it carries, linearly, with no
     other source.  The construction's central metaphor is here a computed identity. **
  6. CONTROL: at M = 0 the residual vanishes identically and G = 0, so the cut is the
     hyperplane X_2 = 0 -- the vacuum member is planar, as (1) requires.

** WHAT THIS DOES NOT DO, AND MISREADING IT WOULD BE THE ERROR THE RECEIPT EXISTS TO PREVENT.
It does NOT refute the corpus's cut relation, because that relation is not isometric embedding:
P9 defines a cut by its isometry group containing a sweep-subgroup of so(5,1), P12 by an
orientation datum plus a causal-vantage datum, and the framework paper calls SdS "a DERIVATIVE
metric" on the substrate rather than an induced one.  What it establishes is that the
ISOMETRIC-EMBEDDING reading is available at M = 0 and nowhere else -- so the corpus owes a
sentence saying which relation "cut" names. **

AND THE LOAD-BEARING CONSEQUENCE.  Item (4)'s hypothesis (i) holds whether or not an embedding
exists: if a cut carries a round areal 2-sphere at all, its SO(3) acts on a 3-dimensional
spacelike block and the equatorial section is that block's zero set.  That is exactly what
L-113 needed and could not get, so L-113's conclusion follows -- the twelve null legs are the
transversally-INVARIANT sector, and the infinite mode tower of P03_transverse_space_is_round is
everything else.  They were never rival counts of one thing.

ORIGIN: computations/dimension_question/L114_write_the_cut.py -- built r2376 (c54.41); edit
