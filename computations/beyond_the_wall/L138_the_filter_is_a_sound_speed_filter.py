#!/usr/bin/env python3
"""
L-138 / `A·4` — MATTER BRANCH-POINT CROSSING DYNAMICS.  THE SELECTION RULE THE CORPUS DERIVED IS
READ AS A SCALE FILTER; IT IS THE SAME INEQUALITY AS A SPECIES FILTER, AND ON THAT READING THE
MATTER SECTOR'S CROSSING IS EXACT RATHER THAN APPROXIMATE.

What the corpus has (ledger family 1, r2209): *"Re$\\,\\tilde\\tau$ is frozen across the lift, so its
conformal time is imaginary: an oscillating mode acquires $e^{-kc_s|\\Delta\\eta|}$ and is annihilated
($e^{-152}$ at $P_1$), a frozen super-horizon mode is a constant the shift cannot touch and passes
unchanged.  The crossing inherits what is frozen and destroys what oscillates."*  And what stays
open: *"the DETAILED worldline-and-field dynamics for a concrete model."*

** THE EXPONENT IS $k\\,c_s\\,|\\Delta\\eta|$ AND IT HAS TWO FACTORS.  The corpus varies $k$ at fixed
$c_s$ and reads a SCALE criterion -- frozen against oscillating.  Vary $c_s$ at fixed $k$ and the
same inequality is a SPECIES criterion, and the matter sector sits at $c_s\\equiv0$: transparent at
EVERY wavenumber, with no super-horizon assumption anywhere. **

** THAT IS THE DIFFERENCE BETWEEN AN APPROXIMATE INHERITANCE AND AN EXACT ONE.  The frozen-mode
argument is scale-limited and adiabatic; the pressureless argument is neither -- at $c_s=0$ the
segment's mode equation is $\\psi''=0$, whose constant solution is untouched identically. **

  PART 1  The segment, rebuilt from the corpus's own geometry.
  PART 2  ** THE TRANSMISSION AS A FUNCTION OF BOTH $k$ AND $c_s$. **
  PART 3  ** THE MATTER SECTOR: transmission exactly one, at every $k$, with no approximation. **
  PART 4  ** THE ONE INEQUALITY READ TWO WAYS, and what each reading is good for. **
  PART 5  What `L-138` becomes.

Run: python3 L138_the_filter_is_a_sound_speed_filter.py      (numpy)
"""
import numpy as np
