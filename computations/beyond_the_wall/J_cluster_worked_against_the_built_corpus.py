#!/usr/bin/env python3
"""
CLUSTER J, WORKED AGAINST THE BUILT CORPUS.  TEN SPECULATIVE GERMS FROM r469, HELD OFF-RUNWAY BY
CONSTRUCTION, NOW ASKED OF MACHINERY THAT DID NOT EXIST WHEN THEY WERE WRITTEN.

The rule for this pass is the plan's: a probe that turns up a question SETTLES it in the same turn or
the question does not enter the corpus.  So nothing speculative is promoted here.  What is looked for
is the opposite: places where a germ was groping at a structure the corpus has since BUILT, and where
the built object settles the germ -- confirming it, killing it, or replacing it with something
sharper than it asked for.

** THE ONE THAT PAYS IS `J·9`, AND IT PAYS EXACTLY. **  The germ guessed that the conjugate lap's exit
at the branch point "is not unique -- three options", and ordered them by naturalness.  ** The branch
point's monodromy is computable, it is $\\mathbb{Z}_3$, and the count is therefore forced rather than
guessed.  Two of the three sheets carry real $r$, and the corpus already uses exactly those two. **

** AND ONE DIES CLEANLY: `J·8` proposed the seat/operator interlock is SEMIDIRECT.  The corpus's own
$T$ and $R$ commute, so it is DIRECT -- $S_3\\times\\mathbb{Z}_2$.  A guess with a definite answer, and
the answer is no. **

  PART 1  ** THE MONODROMY OF THE BRANCH POINT, COMPUTED. **
  PART 2  ** THE THREE SHEETS, AND WHICH CARRY REAL $r$. **
  PART 3  ** `J·9` SETTLED: three sheets, two real legs, and a correction to the germ. **
  PART 4  ** IS IT THE CORPUS'S $\\mathbb{Z}_3$?  NO -- and the corpus's own history says why that
          matters. **
  PART 5  ** `J·8` SETTLED: direct, not semidirect. **
  PART 6  The block verdict on all ten.

Run: python3 J_cluster_worked_against_the_built_corpus.py      (sympy)
"""
import sympy as sp
