#!/usr/bin/env python3
"""
A2 / `L-136` — THE WALL IS NOT WHERE P9 SAYS IT IS, AND THE PAPER'S OWN BEYOND-THE-WALL EXEMPLAR IS
THE WITNESS.

P9 `thm:range`: *"The boundary of the range is the loss of continuous symmetry."*
P9 `cor:wall`:  *"A geometry with no continuous isometry admits no sweep-subgroup of $\\so$ to anchor
                 the construction... A geometry with no Killing symmetry is precisely one whose
                 matter is genuinely inhomogeneous.  The boundary of the range and the emergence of
                 dynamical (inhomogeneous) matter are therefore one statement read two ways."*
P9 `cor:radiation`: *"...only the loss of all confining symmetry frees the graviton's two transverse
                 polarizations (**the type-N plane wave, beyond the wall**)."*

** THE TYPE-N PLANE WAVE HAS FIVE KILLING VECTORS.  That is MORE continuous symmetry than
Schwarzschild's four, and it is VACUUM -- no matter at all, inhomogeneous or otherwise.  So the
paper's own chosen exemplar of "beyond the wall" falsifies both readings `cor:wall` gives of the
boundary, and it does so on the corpus's own definitions. **

** AND THE REPAIR IS ALREADY WRITTEN, ONE LINE ABOVE, IN `thm:bound`'s OWN PROOF: *"A geometry with
no continuous isometry, OR WHOSE ISOMETRY GROUP CONTAINS NO $\\so$-SUBGROUP ACTING AS A SWEEP, is not
of this form."*  The theorem states two failure modes and the corollary uses one. **

  PART 1  ** THE KILLING ALGEBRA, DERIVED: five vectors, and the structure is HEISENBERG. **
  PART 2  ** THE ORBITS ARE THREE-DIMENSIONAL AND NULL. **
  PART 3  ** SO(5,1)'s UNIPOTENT RADICAL IS ABELIAN -- so the wave's algebra is not a sweep. **
  PART 4  ** BUT AN ABELIAN R^3 SITS INSIDE IT AND DOES EMBED -- so `thm:bound`'s STATED condition
          is SATISFIED by a geometry the paper places beyond the wall. **
  PART 5  ** WHAT THE WALL ACTUALLY IS, and it is sharper than what the paper says. **
  PART 6  ** AND THE CONSEQUENCE FOR CHIRALITY, which runs in the construction's favour. **
  PART 7  ** THAT CONSEQUENCE MADE CONSTRUCTIVE: the unpolarized Gowdy cut, checked. **

Run: python3 A2_the_wall_is_not_the_loss_of_symmetry.py      (sympy)
"""
import sympy as sp
