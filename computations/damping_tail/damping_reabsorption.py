...
concluding) is whether refitting omega_b (and omega_m) can pull it back -- because if it can,
there is no tension; if it cannot within what BBN and the CMB peak heights already allow, the
+8.9% is a real, quantifiable residual.

METHOD. Hold the framework honest: theta_D/theta_* is computed on each rate with CAMB's
recombination thermodynamics recomputed for each (omega_b, omega_m). We measure
  d ln(r_D/r_s) / d ln(omega_b)   and   d ln(r_D/r_s) / d ln(omega_m)
(the standard-rate response -- the reabsorption levers), then ask how large a shift in each is
needed to cancel the CR +8.9%, and compare that to the range BBN (deuterium) and the CMB peak
heights independently ALLOW omega_b, omega_m to move. Reabsorbable iff the needed shift fits.
"""
import numpy as np
import camb
...
Z_ONSET = 6803.0   # CR cosmogenesis onset: rho_r/rho_m=2 -> z_onset=2*z_eq-1 (z_eq~3402). r_s clock
                   # starts at the onset (P15 sec:tensions / damping_ratio_clean.py), NOT at z=inf.
def scales(ombh2, omch2, rate_radfree, H0=67.36, mnu=0.06, tau=0.0544):
    ...
    starts at the onset z_onset (anchored to the observed acoustic scale), not
    ...
    # sound horizon: LCDM to high z (radiation era converges); CR from onset z_onset (anchored)
