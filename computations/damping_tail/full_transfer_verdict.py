"""
*** r966 CORRECTION BANNER (READ FIRST) ***
...
transfer and GENUINELY OPEN. See crossed_wire_hunt.py for the r_s seam-cut asymmetry the +8% rides.
Keep this script as "IF peaks=LCDM at high-l, THEN this deficit" -- an isolation of what the real
transfer must compute, not a claim about CR.
------------------------------------------------------------------------------------------------
THE FULL SEAM-TO-RECOMBINATION TRANSFER (leading-order, CONDITIONAL on the peaks-match assumption).

The corpus's two stated commitments, taken together, fix CR's predicted high-l TT spectrum:
  (A) PEAK HEIGHTS match flat-LCDM -- the acoustic modes are driven on the progenitor's
...
Under (A)+(B), CR's TT spectrum is LCDM's peak structure times CR's own (stronger) damping:
  C_l^CR = C_l^LCDM * exp[ -2 (l/D_M)^2 ( r_D,CR^2 - r_D,LCDM^2 ) ]
The diffusion damping is EXPONENTIALLY sensitive to r_D, so an 8.9% shift is a large high-l effect.
This script computes it against a real CAMB LCDM spectrum, validates the damping envelope, and
renders the chi^2 tension vs cosmic-variance-limited (optimistic) errors -- the SIGN of the verdict.
"""
