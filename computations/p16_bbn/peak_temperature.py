"""
B-9 (P16-D): the collapse peak temperature -- get the number.

P16 sec:peak establishes the ROBUST, M-independent LOWER BOUND: the infall delivers kinetic energy
of order the rest mass per nucleon at the horizon (GM/R_s c^2 = 1/2 identically), so the adiabatic
peak clears the deuterium bottleneck by ~4 orders for every progenitor, T_pk >~ O(10^2) MeV.
This script computes the actual infall-thermalization number and states honestly what is bounded
vs open. (Downstream-irrelevant: once dissociation is total the peak's memory is erased and the
abundances are fixed on the cooling leg -- so this is precision on an established scale, not a
result anything hangs on.)
"""
import numpy as np
