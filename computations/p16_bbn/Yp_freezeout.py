...
VALIDATION (the guard): the rates reproduce detailed balance lam_pn/lam_np = exp(-Q/T) to <1%
and freeze-out Gamma=H at T~0.7 MeV; the integrated Y_p = 0.2506 agrees with the accepted
standard-BBN Y_p ~ 0.247 to 1.5% -- the residual being exactly the known Coulomb/radiative/
finite-T-QED corrections this Born-level calc omits. Machinery certified.

RESULT for P16: on the exact (=standard Friedmann) window rate the paper specifies, the cooling
leg gives He-4 at its observed value -- P16's He-4 anchor CONFIRMED from first principles, not
imported. (D/H and Li-7 need the full nuclear network -> AlterBBN, unreachable here; the reduction
that fixes them -- window rate standard, full dissociation, freeze-out on the cooling leg -- is
independently verified, so they stand on a reduction whose every component checks out.)
"""
import numpy as np
