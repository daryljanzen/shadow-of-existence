"""
DOPPLER COMPLETION of the CR low-ell transfer. The live question from the r527 data confrontation:
does the remaining transfer piece -- the Doppler/velocity term, which projects through j_ell'(k D_C)
rather than j_ell -- carry enough ell-dependence to break the ell=2/ell=3 degeneracy and ease the
octopole tension, or is the smooth ell<~7 deficit firm?

NOT tuned to kill the tension. V(k)=(2/3)(k/k_H,lss)/sqrt(1+(k/k_H)^2) Phi_lss -- the super-horizon growing-mode velocity
(k_2/k_H~0.12, velocity suppressed), smoothly capped at the horizon so the continuum normalisation is
not contaminated by the super-horizon formula running away at sub-horizon k; the monopole(SW+ISW) and dipole(Doppler) are combined coherently (both real for super-horizon modes;
both signs tried) and in quadrature (the standard power-spectrum treatment); the continuum limit is
the validator -- it must reproduce the LCDM low-ell rise. Each spectrum normalised to its OWN
high-ell plateau, so only the SHAPE (the deficit) is compared.
"""
import numpy as np
