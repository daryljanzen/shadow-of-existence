#!/usr/bin/env python3
"""
L-50b — THE SAME GEOMETRY ON THE RIGHT OBJECT.  ** SUPERSEDES L50_the_hexagon_triple.py. **

Daryl, correcting c54.18:

    "There are 3x2 hinges, not six.  There are 3 hinge STATIONS and the slicing plane
     swings on the HINGE, not on a manifold point.  Get that straight."

    "The structure is just as well-posed as udd as it is uud.  They are complementary
     things.  The orientation and all that is no more well posed than species are: our
     universe emerged from an antimatter black hole because it's 'antimatter' RELATIVE to
     our matter.  That's the C thing.  There is no geometric distinction of one versus the
     other in an absolute sense.  These things are relative.  And I'd be surprised if
     there wasn't a connection THERE between charge and uud vs udd."

TWO CORRECTIONS, and the second one changes what the result IS.

(1) THE OBJECT.  The hinge is the LINE the door swings on -- P3 sec:slicing, and
    `P3_SWING_ONTOLOGY`: "a vertical line, parallel to the hole's axis, at distance 2 alpha
    ... it does not translate.  It is a hinge in the plain sense -- the line a door hangs
    on."  There are THREE of them.  A vertical line at transverse radius 2 alpha is not
    contained in the substrate: it PIERCES it, twice, once per horn.  ** So the six are
    3 hinges x 2 ends, and the corpus's standing phrase "the six hinges" names the
    piercings after the thing that makes them. **

(2) WHAT THE 2+1 IS.  Once the six are read as 3x2, the null connectivity says something
    different and better, and the thing I filed at c54.18 as a mysterious exclusion turns
    out to be the hinge itself.

  PART 1  The three hinges as LINES.  Where each meets the substrate, and of what causal
          character the line itself is.
  PART 2  The complete causal classification of the fifteen pairs.  ** A trichotomy, and
          each causal character encodes exactly one shared attribute. **
  PART 3  The projection, two-to-one: each triangle side carries both rulings.
  PART 4  The 2+1 restated on HINGES -- and what c54.18's "excluded relation" really was.
  PART 5  ** DARYL'S POINT, MADE INTO A THEOREM: there is no absolute distinction. **
          The configuration's symmetry group acts transitively on the six ends AND on each
          causal class, so causal character is a COMPLETE invariant of a pair and no
          vantage-independent structure singles any end out.  The 2+1 is real and its
          assignment is RELATIVE -- which is the uud/udd complementarity, not a gap.

Run: python3 L50b_three_hinges_six_ends.py     (numpy, sympy)
"""
