#!/usr/bin/env python3
"""
L-65 — CHARGE AS WINDING ON THE EQUATOR, AND THE WHOLE FERMION SECTOR, NOT BARYONS.

Daryl, and he had already said the substantive part twice before this:

  "I've already told you about the 2/3 vs 1/3 asymmetry.  How can you possibly imagine that
   is not related to the charge distinction?  THEY'RE EVEN THE OPPOSITE DIRECTION!  I noted
   the lap's asymmetry and you ignored it while talking about charge and were like but we
   don't know why one is -1/3 and the other is +2/3 ...
   WHAT ABOUT THE REST OF THE FERMIONIC CONTENT THAT IS JUST SITTING THERE READY TO BE
   IDENTIFIED?"

** THE THING I MANUFACTURED AND THEN CALLED A TENSION. **  At c54.20 I wrote that charge
cannot come from the equatorial thirds "because |-1/3| != |+2/3| and the symmetry group is
transitive on the punctures, so it would average the assignment away."  ** That argument is
wrong, and it is wrong in the same way three turns running: it demands an ABSOLUTE label
where the structure supplies a RELATIVE, DIRECTED one. **  An arc between two punctures is
not a label on a puncture.  It has a direction.  And the two directions round the same arc
differ by exactly one full lap.

  1/3 one way, 2/3 the other, and 2/3 - (-1/3) = 1.

  PART 1  The arithmetic of the thirds, exactly.  What the fractional part distinguishes
          and what the integer part distinguishes.  ** They are not what I assumed. **
  PART 2  The closure rule, tested against EVERY hadron type -- observed and excluded.
  PART 3  What that makes confinement.
  PART 4  The three involutions against the three discrete Standard Model structures.
  PART 5  ** THE WHOLE FERMION SECTOR **, axial against azimuthal: why leptons carry
          integer charge and no colour, in one statement.
  PART 6  The count of Weyl degrees of freedom, honestly -- what lands and what does not.
  PART 7  What this does to the COLOUR PROBLEM, which is the corpus's standing open item.

Run: python3 L65_the_winding_and_the_whole_sector.py     (sympy)
"""
