#!/usr/bin/env python3
"""
L-131 — THERE ARE TWO RANK-THREE BUNDLES IN THIS SECTOR AND P14 CURRENTLY CALLS BOTH 'THE
BUNDLE'.  Found while discharging `L-127` under rule 4.  ** They index different things, both
are right, and the corpus has been one paragraph away from a three-threes collision of exactly
the kind that cost this session four revisions earlier. **

  PART 1  ** THE TWO OBJECTS, AND THEY ARE NOT THE SAME. **
  PART 2  What the pushforward's three actually are -- decomposed, not asserted.
  PART 3  ** THE TEST THAT SEPARATES THEM: does one mode occupy one line or three? **
  PART 4  ** AND THE RESOLUTION IS THE STANDARD MODEL'S OWN: triality is the CENTRE's grading,
          shared by all three colours; colour is the FUNDAMENTAL's index. **
  PART 5  What must change in P14, and what `L-127` is worth after it.

Run: python3 L131_two_rank_threes.py
"""
import sympy as sp
