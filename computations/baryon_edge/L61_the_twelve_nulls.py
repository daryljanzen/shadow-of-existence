#!/usr/bin/env python3
"""
L-61 — THE STRUCTURE BY THE NUMBERS, AS DARYL STATES IT, CHECKED TERM BY TERM.

His statement, the one this file exists to verify and not to paraphrase:

  "Three hinges at X0=0 and X=2alpha, with a hole of size alpha.  The hinges run parallel
   in the embedding space to the hole's axis.  Each hinge defines a slicing plane that
   swings about it, so the swing axis runs parallel to the hole axis.  And with that we get
   the hinges puncturing the substrate at six points, 3 on each horn.  The hinge
   intersection points all connect to each other via null lines.  ** That makes 12 nulls
   connecting six substrate points. **  And those 12 nulls all tangent the hole at
   equatorial points that lie directly between the two hinges.  The horns are perfectly
   symmetric and what amounts to up or down is set by relative convention.  You can take
   one of those points and call it u, and it connects to two d's on the other horn, via
   null lines that pass two tangent points on the equator.  You can also take one and call
   it d and that d connects via the same two substrate points to two u's.  Both udd and duu
   triplets has a corresponding pair of tangent points that are related by handedness as
   the Nariai points to positive or negative gnomonic angle.  That works equivalently
   whether the two oppositely handed points are read from udd or duu.  And all three hinges
   do their own thing like this, symmetrically, carrying the whole structure."

** THE COUNT I DID NOT HAVE.  ** c54.18-19 counted SIX null chords (pairs of punctures).
Daryl counts TWELVE.  Both are right and they count different things, and his is the one
that sees the tangency points: through each puncture run the substrate's TWO rulings, so
there are 6 x 2 = 12 null LEGS, each running from a puncture to the equatorial point where
it grazes the hole; two legs meeting at a tangency point make one chord, so 12 legs = 6
chords, and 3 tangency points x 4 legs = 12.  ** The leg is the object with the tangency on
it.  The chord is the object with the two endpoints on it.  A ledger that carries only the
chord count cannot see the handedness pair, which is carried by the legs. **

  PART 1  The three hinges as Daryl sites them, and the six punctures.
  PART 2  THE TWELVE.  Both rulings through every puncture, and where each grazes the hole.
  PART 3  The tangency points lie DIRECTLY BETWEEN two hinges.  Four legs at each.
  PART 4  The triple, and the two tangent points it comes with -- MIRROR IMAGES in the
          hinge's own azimuth.  The handedness.
  PART 5  The handedness IS the sign of the gnomonic angle: each tangent from a hinge is a
          Nariai cut at sin w = 1/2, one at +30 degrees and one at -30.
  PART 6  Up/down, u/d: convention.  What is invariant and what is not, by orbit count.
  PART 7  Times three hinges.  The whole structure, tabulated.

Run: python3 L61_the_twelve_nulls.py     (numpy, sympy)
"""
