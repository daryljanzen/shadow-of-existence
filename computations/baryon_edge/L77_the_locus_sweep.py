#!/usr/bin/env python3
"""
L-77 — THE LOCUS SWEEP.  Every descriptively-named object against the place it actually sits.

Daryl, registering it two turns ago:

  "We have all these related structures that are fitted together with geometric things that
   are SHADOWS of one or the other definition and have DESCRIPTIVE NAMES and ANALYTIC
   DEFINITIONS we've not drawn identically to geometric features."

and then, when I deferred it a third time:

  "Run the L-77 sweep first darn it.  Don't leave things like that dangling and buried under
   an 'I want to run this other thing, and this other, and this other'."

** THE SWEEP HAS ALREADY PAID TWICE WITHOUT BEING RUN. **  c54.24 caught a FALSE identity
(wall = hinge, asserted from a bijection) and c54.25 caught a TRUE one that had been missed
for a turn (wall = branch point, 5/5).  Both were found by accident, one pair at a time.
This runs it over the inventory.

  PART 1  The inventory: every named object with a definite locus, and where it sits.
  PART 2  Group by locus.  ** How many names does each place carry? **
  PART 3  The corpus's own count of faces against the sweep's.
  PART 4  The five-test criterion on the pairs the grouping throws up.
  PART 5  PROVED NON-IDENTITIES -- the places two names must NOT be merged.
  PART 6  Named objects with NO locus pinned.  ** The other half of the audit. **
  PART 7  What the sweep found.

Run: python3 L77_the_locus_sweep.py     (sympy)
"""
