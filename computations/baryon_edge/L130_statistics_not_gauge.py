#!/usr/bin/env python3
"""
L-130 — WHEN ARE TWO CONSTITUENTS AT DIFFERENT VANTAGES THE SAME PARTICLE?  `L-129b` closed on
this and said it is a question about the OPERATOR, not the bundle.  ** Asking it resolves the
whole thing, and it resolves UNIQUELY: exactly one of the three candidate configuration groups
survives once Fermi statistics is included, and under it both channels come out exactly as
SU(3) requires. **

  PART 1  The question, and why the corpus already has the answer without having used it.
  PART 2  ** WHAT SECOND QUANTISATION DOES TO THE COUNT -- the spurious invariants of `L-129`
          are not forbidden by a symmetry, they DO NOT EXIST. **
  PART 3  ** THE TABLE REDONE WITH STATISTICS.  Exactly one row is right. **
  PART 4  ** AND IT SELECTS THE CONFIGURATION SPACE, which is more than was asked for. **
  PART 5  ⚠ A WORRY RAISED AND THEN ANSWERED -- it is phase versus PERMUTATION, not
          phase versus phase, and `L-74` comes out stronger than it was written.

Run: python3 L130_statistics_not_gauge.py
"""
import sympy as sp
import itertools
