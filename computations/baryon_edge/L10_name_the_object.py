#!/usr/bin/env python3
"""
L-10 — NAMING PO-1's OBJECT, and L-17 riding along.

PROTECTED_OPEN PO-1 (`FIGURE` R3, "1+2 -> uud / generations") is protected absolutely
because its object is NOT NAMED: the corpus carries two 2+1s and the row names neither.
P14, one sentence:

    "the 2+1 of a DESIGNATED ROOT against the ellipse pair is the diagonal designation
     sigma, and the SIGN 2+1 (two horizons of one sign against one) the reflection
     through the origin."

This does not answer R3.  It works out what the two objects ARE, how they sit against
each other, and what each could possibly be a candidate FOR -- which is the work the
register says is owed before the question can be asked at all.

  PART 1  The two partitions, computed along the whole dial.  Do they coincide, or cut
          across each other?
  PART 2  What acts on each.  S_3 permutes designations; what does it do to the signs?
  PART 3  The degenerate member, where the corpus says the two designations of one
          geometry meet.
  PART 4  L-17 (rides along): is the locus still line-union-conic at general D?  The
          designation 2+1 IS the line/conic split, so if the factorisation fails the
          object does not exist there.
  PART 5  What each object is a candidate FOR -- stated as candidacy, not as a match.

Run: python3 L10_name_the_object.py     (numpy, sympy)
"""
