#!/usr/bin/env python3
"""
L-126 / L-125 — THE WINDING HAS NO UNITS AND ELECTRIC CHARGE DOES.

THE LEAD.  `L-124` dissolved the last objection to the charge reading and left one thing:
the geometry supplies a dimensionless lap-count-plus-class, while electric charge additionally
carries a COUPLING -- a measured strength with which a field talks to a gauge potential.  The
registered first move was: ** ask whether the substrate has any dimensionless constant besides
alpha; if not, a coupling cannot come from the geometry and must close from the field, which
is P13's pattern again. **

** AND THE ANSWER IS SHARPER THAN THAT MOVE ANTICIPATED.  It is not that the substrate happens
   to lack a suitable constant.  It is that a TOPOLOGICAL label CANNOT carry a strength, by
   what it is -- so the gap is structural and permanent rather than an unfinished search. **

  PART 1  What a coupling IS, structurally, so the question is decidable.
  PART 2  What dimensionless quantities the corpus has.  ** MODULI, not couplings. **
  PART 3  ** THE WINDING IS TOPOLOGICAL: quantised by construction, unscaled by construction. **
  PART 4  ** SO ELECTRIC CHARGE HAS THREE PARTS AND THE GEOMETRY SUPPLIES EXACTLY ONE. **
  PART 5  ** AND L-96, L-125 AND P14's BUNDLE QUESTION ARE ONE QUESTION.  Tested, not asserted. **

Run: python3 L125w_quantisation_not_strength.py     (sympy)
"""
import sympy as sp
