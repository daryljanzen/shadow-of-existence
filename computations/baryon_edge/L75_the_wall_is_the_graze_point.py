#!/usr/bin/env python3
"""
L-75 — THE WALL, NAMED FROM THE GEOMETRY SIDE.  ** AND IT IS THE GRAZE POINT. **

Daryl asked whether the corpus had ever named the wall geometrically, and offered:

  "the slicing planes of the two adjacent hinges, aligned on the back side of the hole,
   where they tangent to the hole at r=0, as defined relative to one specified hole.
   Is that right?  Or am I confused about things?"

** HE IS RIGHT, AND HALF OF IT IS ALREADY IN THE CORPUS. **  P14's own header, stating what
sec:chirality and prop:wall are about:

  "r=0 is NOT the throat's CENTRE but a point ON the throat circle -- the back, X_1 = -alpha,
   fixed relative to the hinge about which the slicing plane swings.  ...  THE Z_3-FIXED
   CENTRE CARRIES NO WALL; the walls lie on the circle at polar 180/300/60, a Z_3-ORBIT,
   ** each ANTIPODAL to its hinge. **"

And L-61 found, from an entirely different direction -- the twelve null legs and where they
graze the throat:

  "the twelve legs graze the hole at only THREE points: 60, 180, 300 degrees, each DIRECTLY
   BETWEEN two hinges, four legs apiece."

** TWO INDEPENDENT DERIVATIONS, ONE SET OF THREE POINTS, AND NOBODY HAS PUT THEM TOGETHER. **
P14 reaches 180/300/60 from "r=0 is at the back, antipodal to the hinge".  L-61 reaches
60/180/300 from "where do the substrate's null generators through the punctures touch the
throat".  This file checks they are the same three points, works out the pairing, and then
says what that does to L-74 -- which is currently the gate on the whole fermion reading.

  PART 1  The walls from P14's side: r=0 at the back, antipodal to the hinge.
  PART 2  The graze points from L-61's side: null tangency.
  PART 3  ** SAME SET. **  And the pairing, which is the part Daryl named.
  PART 4  Whose walls does a hinge's own null leg cross?  ** Never its own. **
  PART 5  ** WHAT THIS DOES TO L-74: the winding is a WALL-CROSSING COUNT. **
  PART 6  What is still owed, and one thing this newly puts at risk.

Run: python3 L75_the_wall_is_the_graze_point.py     (sympy)
"""
