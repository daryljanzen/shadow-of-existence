#!/usr/bin/env python3
"""
L-116 — DRAW THE TURNAROUND.  The corpus's other three, depicted for the first time.

WHY.  Two independent lines reach the same recommendation.  `L-112`: the corpus has a hinge
three (colour, within-state) and a turnaround three (generation, deck Z_3), it has drawn one
and never the other, and every confusion of the last several turns has been a collision
between them.  `L-115`: with all three of `L-15`'s routes closed, the only remaining seat for
the colourless states is somewhere OTHER than the hinge figure -- and the turnaround is the
corpus's only other three, is where P14 sec:whichthree already MOVED the generation index, and
has never been depicted.

** EVERY FACT DRAWN IS VERIFIED BEFORE IT IS DRAWN, as with U3.  The figure asserts four
   things and all four are checked symbolically here. **

Run: python3 L118w_draw_the_turnaround.py     (sympy, numpy, matplotlib)
"""
import os
import numpy as np
import sympy as sp
