#!/usr/bin/env python3
"""
L-120 / L-117 — IS A COLOURLESS STATE AN INVARIANT OF THE TURNAROUND?

WHERE THIS COMES FROM.  `L-115` closed all three routes of `L-15`'s trichotomy, leaving the
option the trichotomy never contained: ** the colourless states are not seated on the hinge
figure at all. **  Two things make that the corpus's own reading rather than a dodge -- P14
sec:whichthree has already MOVED one index off that figure (the generations, to the
turnaround), and `L-114` showed the hinge figure is the fixed set of the transverse SO(3), so
it sees only what is transversally invariant.  `L-116` then drew the turnaround, which makes
the question computable instead of speculative.

** AND IT IS A REPRESENTATION COUNT.  Colour is the hinge/graze index; colourless means
   TRIVIAL under it; the turnaround's symmetry group is finite.  So "how many colourless
   states does the turnaround carry" is "what is the total dimension of the irreps trivial on
   the colour factor", which is arithmetic. **  (PART 1 begins from P14's abelian Z_6 because
   that is what P14 gives; PART 3 computes that the group is larger and NOT abelian.)

  PART 1  What the turnaround carries, taken from P14's own sentence and checked.
  PART 2  The character count, and the triality-zero sector.
  PART 3  ** DOES T DESCEND?  RUN IT -- and it does, inverting the deck.  D_6, and FOUR. **
  PART 4  Tested against L-110's S1-S5.
  PART 5  What this establishes, and the longer list of what it does not.
  PART 6  ** THE SHAPE FAILURE OF PART 4 WAS AN ARTEFACT OF ANCHORING ON FIFTEEN. **

Run: python3 L117w_the_colourless_sector.py     (sympy)
"""
import itertools
import sympy as sp
...
assert 4*1**2 + 2*2**2 == 12, "the D_6 irrep dimensions square-sum to the group order twelve"
