#!/usr/bin/env python3
"""L-106 / U3 — THE FIGURE, BUILT FOR THE PAPER.

Staged at r1110 as 'not now'; drafted at c54.35 and NOT LANDED (it was written to the repo
root's figures/, which pdflatex never reads, and inserted into no paper).  Rebuilt here to
paper standard and written to corpus/figures/, where the other two P3 figures live.

What the draft got wrong, and what this fixes:
  * the 'from above' panel was a 78-degree 3D view, not a projection: the incircle relation
    and the antipodal hinge-wall map were both illegible.  Panel (b) is now a true
    orthographic projection to the equatorial plane, in 2D axes.
  * nothing in the draft drew the claim the surrounding prose newly makes -- that a hinge's
    own two legs graze at the OTHER TWO hinges' walls and never at its own.  Panel (b) now
    draws exactly that, with hinge 0's pair picked out.
  * the title was duplicated (suptitle + per-axes titles).  A paper figure carries its
    caption in the caption.

And it verifies, rather than assumes, the three facts it draws: tangency at the midpoint,
the causal trichotomy on the fifteen pairs, and the antipodal hinge-to-wall map.

Run: python3 L106_build_U3.py     (numpy, matplotlib)
"""
import os
import itertools
import numpy as np
