"""
p14_dimension_from_flavour.py -- the two conditions this sector places on the spacetime
dimension, gathered into one runnable object.  Parts 1 and 2 are the bodies of
p14_generations_fix_dimension.py and phase5_chart_and_parity.py; those remain the
working scripts and this is the collected form P14 cites.

(1) THE COLLAPSE — the forced fold exists only at D=4 (a 3-fold) and D=5 (a 4-fold).
(2) THE CHART — P3 prop:gnomonic's primary argument is dimension-independent.
(3) THE PARITY — 2M is odd in the signed offset only for even D, so D=5 carries four
    generations and NO chirality.  Four dimensions is the only one carrying both.
