#!/usr/bin/env python3
"""
STEP 3 — THE THREE ITEMS STEP 2 RECORDED INSTEAD OF WORKING.

Step 2 named three questions and left them.  That is the manufactured-deferral shape
the plan indicts at r1907 ("not knowing is a reason to work, not a reason to assign"),
so they are worked here.

  I    THE ANTIPODAL Z_2 vs THE OFFSET PARITY.  QUADRIC §2c records that CR refuses the
       projective quotient because "antipodes are identified ... CR does not identify
       them, and the reason is PHYSICAL -- the oriented future horizon distinguishes
       them."  The offset parity R is also an orientation Z_2.  ARE THEY THE SAME ONE?

  II   FIGURE R3 -- "1+2 -> uud / generations", UNTOUCHED and blocked on a reason the
       ledger admits is "sourced to nothing": *a real-r artifact; off that slice all
       three sheets are alike*.  The row's own rule: "to promote or kill it, the reason
       must first be DERIVED OR WITHDRAWN."  Do that.

  III  (adjudicated in prose, not here) the CATEGORY bake's two ZEROS.

Run: python3 step3_refused_items.py     (numpy, sympy)
"""
