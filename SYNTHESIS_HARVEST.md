...
⑥ NOT OP FOR — **⛔ What the seating fixes is WHICH three; the count is unchanged and so is the dimension argument** — both threes are $D-1$, *because clearing denominators $f=0$ is a polynomial of degree $D-1$ and the two are the two things one does with that single polynomial.* **And the family symmetry proper is CYCLIC**: the turnaround's transpositions are supplied by $T$, the horn swap, which belongs to another sector — *reading it as a family $S_3$ would be counting isospin as flavour --- the within-state index is what the turnaround's deck supplies, and the $S_3$ reading was withdrawn at r2376+c54.31 by P14 `sec:whichthree`.*
...
⑥ NOT OP FOR — **⛔ AT `r4155` I REPORTED ADVANCING TWO OPEN NOT-FOUNDS, AND ONLY ONE OF THEM WAS OPEN — corrected `r4171`.** *The P14-comb item and the map's-own-count item are `B1` and `B5`, **closed at `r1820`–`r1821`**, and this file says so: "section B is clear — all five not-founds closed, and **not one by finding the thing**."* **The A6.4 item is a sixth, logged separately, and it was genuinely open**; its list is now exhausted — `algebroid_paper` zero, `OPEN_PROBLEMS_MAP`'s `orbit structure` hits being $SU(2)_L$'s and a different room in the same words, `MATTER_SECTOR_germ` zero — **so it closes as a sixth negative sourced without finding the thing.** ⌗ ***The cause is worth naming and is a property of the form rather than of anyone's care***: a not-found's **original entry** and its **closure** sit far apart in a long changelog, so a scan that reads the entry without reaching the closure reports a closed item as open. **The three-part form makes an empty search resumable; it does not make the closure findable from the entry.**
