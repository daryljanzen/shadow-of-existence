...
self-check · anti-flinch (feature at weight) · the not claimed census · the completion-shield · the
stale-link prior.
...
(not claimed, walled, abstract-coincidence, "a first-pass account") were *already standing* when the work
...
them. The disciplines below — the not claimed census, the completion-shield, the connection-bakes — are the
bits on that drill.
...
### ★ The not claimed census (Daryl, r1226)

A `not claimed` — and its family, *"poses but does not settle," "candidate," "remains open," "conjectured"*
...
corpus-wide** — because a not claimed is precisely where the work most often lies about its own weight, and
...
single not claimed but a *class of claims* all pinned to a superseded understanding:
- **Old weight (topological).** The substrate's discrete residue is the A₂ root system, which su(3) also
  ...
  fits into su(3)," the continuous colour "walled / not claimed."
...
(abstract coincidence · walled · purely-not claimed · "not yet placed") and upgrade it to the analytic
link — with only the **world-correspondence** (that this geometric su(3) is the physical colour) still
not claimed. It rides alongside the not claimed census (the old "walled/not claimed" phrasings *are* the
...
for stale "walled / not claimed / coincidence" phrasings is only the *defensive minimum* — the tip. It finds
...
open," "not claimed," "not yet," "a further question," "beyond scope," "we do not claim," "a first-pass
...
must be found and stated. This is the **general form** of the not claimed census, and it subsumes it: run it
corpus-wide.
...
known** is itself an assertion the shield governs — and it slips through even more easily than a not claimed,
...
The completion-shield, the not claimed census, and the connection-bakes each *return a verdict*; applying
...
- **L9 · P7 placement** — ✔ VERIFIED at source + underclaim FIXED (r1331–32). P14's built sector is placed in P7 at weight and consistent with every verified result: §1116 (frontiers — one chiral mode/wall, D₆=S₃×ℤ₂=three generations × two chiralities, forced within CR, chirality gauged R=γ⁵ vs family global, propagating sector not claimed, honest edge); §1063–67 (two-sided-closure — C-operator on actual zero-modes, matches L3); §1061 (direct-product/on-tangent, matches L8); §1075 (unification scope). **FIXED (r1332, not flagged — Daryl caught me flagging-not-doing):** the intro §265/§276 underclaimed — named the open propagating sector but stayed silent on the *built* discrete skeleton. Both sites edited to OWN the discrete flavour skeleton (three chiral generations, family, chirality — built and forced within CR, cite JanzenMatter) while keeping the boundary result + the open propagating sector, at weight. P7 recompiled 0 errors / 0 undefined / 50pp (missing figure `dS-SdS-synthesis.pdf` generated from the present .png).
