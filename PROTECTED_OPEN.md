...
| **PO-8** | **The irreducible reassignment remainder** — Kerr-inner, RN-interior | those two reassignments | `CR_framework` P7 `sec:frontiers`, the closing paragraphs | **OPEN — AND ITS GATE IS GONE, WHICH IS NOT THE SAME AS ITS BEING ANSWERED.** ⌗⌗ *Restated r2376+c54.166. It read 'gated on frontier 1'; **frontier 1 — the matter branch-point crossing — closed at c54.113**, and P7's own frontier prose has said so since ('two more have left it since'). **A protected row gated on a closed gate is protected against nothing**, and on this register that costs more than a stale row anywhere else.* ⇒ **What actually remains is a DISPOSITION question and it is Daryl's:** *P9's range places the Kerr-inner and RN interiors outside the slicing operator's reach, so what is left is ordinary interior analysis — which would make this cease to be a frontier of this construction rather than be answered by it.* ⚠ ***That disposition is NOT taken here. The row is restated, not closed*** — a node may not convert 'the gate is gone' into 'the row is done', and the difference is exactly what this register exists for. |
