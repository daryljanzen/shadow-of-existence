...
| 13 | `CR_framework` | *Its one remaining structural step is the descent onto a \emph{propagating} spinor sector, stated without being claimed---what is built is a sector of bound modes of the existent leaf.* |
| 14 | `CR_framework` | *The approach this leaves has a known shape, stated without being claimed.* |
...
| 18 | `CR_framework` | *(withdrawn r6772+66.32 — the sentence quoted here, pairing the alternation with the resolution of the Hubble tension, is no longer in P7; the acoustic scale is computed and $H_0$ is fixed by the angle and the distances together)* |
...
| 36 | `boundary_paper` | *One structural feature sharpens what such a construction would look like, recorded not claimed.* |
| 37 | `boundary_paper` | *That is a definite but clean structure to satisfy, and the reason the lead is stated without being claimed rather than asserted or dismissed.* |
...
| 40 | `boundary_paper` | *What stays unclaimed is only its identification with the physics---whether that geometric $2{+}1$ is the Standard Model's generation hierarchy, or the hierarchy is the ordinary* |
...
| 43 | `boundary_paper` | *A first structural result of that sector, at the level of its bilinears, follows---grounded on the parity's action, not claimed for the content.* |
...
| 50 | `boundary_paper` | *\emph{So the construction is available and has been carried out}, and it delivers what came through it is what this paper says the geometry supplies: three chiral generations and a grading operator, and not th ## 'the gate is open and HAS BEEN WALKED' -- thre* |
...
| 56 | `cosmogenesis_paper` | *And the eager target, held here not claimed, is the derivation---as against the inheritance---of that datum and of the progenitor spectrum: the baryogenesis-analogue of the hando* |
...
| 62 | `geometric_core_paper` | *The orientation-parity $\mathbb{Z}_2$ of \S\ref{sec:shadows} is realised here as the central inversion / double-ruling swap, and the su(3)-wall carried at its careful not claimed* |
...
| 64 | `geometric_core_paper` | *\textbf{Conjecture \textnormal{(not claimed).}} \emph{The Standard Model's three fermion generations are the substrate's own three-fold slicing structure, read on a spinor sector* |
...
| 72 | `groupoid_paper` | *What remains walled is only the \emph{realised colour isometry} on the Lorentzian substrate; what remains unclaimed is only the world-correspondence---that this located $\mathf* |
