...
kind: FORWARD
current: r3542
job: The spectral-theory field bake — what bit, what bounced, and the boundary. Tier B's largest never-thrown field (×189 on the reach measure), verified as NOT covered by the harmonic ledger, which mentions spectral, self-adjoint and deficiency zero times.
sources: [chat]
---

> **▣ FORWARD — the spectral-theory bake.** *Three registers kept apart: **what bit**, **what
> bounced**, **what the boundary is**. Thrown as the first of Tier B, the four fields never thrown at
> all, and the largest of them.*
...
| **`S4` Nariai and Petrov type D are the SAME ALGEBRAIC EVENT on two different cubics** | — | ***LANDED r3591*** — *placed in `ONTOLOGY_FOUNDATION_INDEX` §THE CROSS-PAPER JOINS as “NARIAI AND PETROV TYPE D ARE ONE ALGEBRAIC EVENT”. **A claim about how corpus pieces relate is owned by the map, not by a paper** — the precedent is `K4`, landed there at `r1895`.* |
| **`S2` the wall has a SPECTRAL GAP of the full asymptotic mass, and that is what makes the generation count *stable* where `F14` only made it *defined*** | — | ***LANDED r3545 — row corrected r3561.*** *Placed in P14 at the count-parity passage, and carrying `\ldg{spectral_theory}` since r3558. **Pointer and substance both in place; this row had not been updated after its own landing.*** |
| **`S9` no APS boundary term, because the leaf is CLOSED** | — | ***LANDED r3545 — row corrected r3561.*** *Placed in P14 at the index passage, beside `AtiyahSinger1968`, and carrying `\ldg{spectral_theory}` since r3558. **Pointer and substance both in place; this row had not been updated after its own landing.*** |
...
| **`S3` the corpus relies on TWO spectral gaps doing two jobs, and neither references the other** | — | ***LANDED r3591*** — *placed in `ONTOLOGY_FOUNDATION_INDEX` §THE CROSS-PAPER JOINS as “TWO SPECTRAL GAPS, TWO JOBS”. **A claim about how corpus pieces relate is owned by the map, not by a paper** — the precedent is `K4`, landed there at `r1895`.* |
...
⛔ **The two largest counts are not this field's.**

| term | count | what it is |
|---|---|---|
| `quantisation` / `quantization` | **×66** — `P10` 25, `P07` 13, `P11` 9 | ***canonical quantisation*** — quantum theory, not spectral theory |
| `gap` | **×30** | *mostly a **physical** gap — a mass gap, a gap in a sequence, a gap in an argument* |

⇒ ***The genuinely spectral vocabulary is thinner and sits elsewhere:*** `eigenvalue` *(`P09` 8, `P14`
7, `P03` 4)*, `discrete spectrum` *(`P15` 2)*, `WKB` *(`P15` 3, `P16` 2)*, `resolvent`, `essential
spectrum`, `Sturm–Liouville`.

⌗ **AND THE FIELD IS VERIFIED NOT COVERED BY ITS NEIGHBOUR.** *r3438 checked
`HARMONIC_ANALYSIS_LEDGER` and found `spectral`, `self-adjoint` and `deficiency` mentioned **zero**
times, with the operator half named there as station Ⓗ's. The functional-analysis bake did the
**boundary classification** (`F1`–`F3`: limit-circle at $a=0$, limit-point at $r=0$) and **never asked
what the spectrum IS.***

---

## ⌗ THE ESTIMATE, FROM CONTENTS — *recorded so it can be scored*
