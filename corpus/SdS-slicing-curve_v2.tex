...
% *** THE PHOTON SPHERE IS NARIAI'S FORCING LOCUS.
...
%   appear in this paper); T = time reflection X_0->-X_0.
% AND r6581: TWO USES WERE FREED FROM T, on the r968/r1792 pattern.  (i) P5's
% prop:completeness used T for the sky-angle action of a within-single-geometry morphism g --
% a BOUND VARIABLE in a proof, in the same paper whose footnote reserves T for the time
% reflection -- renamed to \hat{g}, the induced action, which reads itself.  (ii) P14 named the
% COSMIC-time reflection tilde-tau -> -tilde-tau 'T' (r6574): renamed \widetilde{T}.  The tilde
% does the work it does on tilde-tau itself -- T reverses the STATIC time X_0, \widetilde{T} the
% COSMIC time tilde-tau = tau + chi of the non-synchronous slicing.  ** They are not the same map
% and neither restricts to the other: \widetilde{T} holds the mass FIXED where the conjugation
% reverses it. **
% ⌗ NOT RENAMED, and deliberately: (a) T the time-reflection OPERATOR and (b) T the closed-dS
% time COORDINATE in X(T) = alpha cosh(T/alpha) are NOT two referents -- X_0 = alpha sinh(T/alpha),
% so T : X_0 -> -X_0 IS T -> -T.  *** A coordinate and its own reflection, which is the ordinary
% overload and the same relation P has to r. ***  An audit at r6579 first counted that as a
% collision; it is not.  And TEMPERATURE T is disambiguated by its argument and units, covered by
% the corpus's own 'a resonance, NOT a naming-blocker' ruling.
%
% AND r6583: THE SAME AUDIT RUN ON w, WHICH THIS PAPER CARRIED BOTH SENSES OF.
%   (i) THE SKY ANGLE w -- 'the swing read on the observer's celestial sphere', a label for
%       WHICH CUT, inside sin 3w, with the throat angle u and the horizon angle 3w its two
%       companion projections.  18 uses here.  KEEPS w.
%   (ii) THE BEAD'S PHASE 3c tilde-tau / 2 alpha, a label for WHEN along one worldline, inside
%       sinh -- renamed \tilde{w}.  Three uses, all in one paragraph, all in this paper: no
%       other paper named it.  ** The tilde carries through from tilde-tau, as it does for
%       \widetilde{T}. **
%   ⇒ *** CIRCULAR against HYPERBOLIC, one letter, one paper -- and the paragraph itself said
%     'the 2:1 again IN A DIFFERENT VARIABLE' and then reused the letter. ***
%   ⌗ AND THE COMPARISON THIS UNBLOCKS: sigma reflects the SKY angle (w <-> pi/3 - w); 
%     \widetilde{T} reflects the BEAD phase (tilde-w -> -tilde-w).  r6581 nearly composed them
%     into a pi/3 translation on the strength of the shared letter.  ** They act on different
%     circles and no such composition is available until a common variable is established. **
%   ⇒ ** r6591 RESOLVES IT, AND THE BRIDGE IS A SEPARATION RATHER THAN A MAP. **  2M = (2/3sqrt3)
%     sin 3w depends on the CUT alone; r^3 = 2M alpha^2 sinh^2(tilde-w) on the cut AND the bead
%     phase.  *Two independent coordinates of one two-parameter family: w says WHICH CUT,
%     tilde-w says WHEN.*  sigma, R and xi all act on the cut and FIX tilde-w -- R's own
%     statement flips 2M and holds tau~ fixed, and 2M(-w) = -2M(w) places that move on the
%     w-factor.  ** \widetilde{T} alone moves the bead phase, so it is a FOURTH discrete
%     operation, and it COMMUTES with sigma because they move different coordinates. **
%   ⌗ *And that is why the collided reading looked like an answer: on one letter the two
%     compose to a pi/3 translation; on the two variables they commute.  A composition and a
%     commutation are opposite readings.*  ⚠ The paper's "three distinct discrete operations"
%     is untouched -- that clause is about operations on the SLICING, and \widetilde{T} is
%     outside that domain, which is why it is a fourth and not a correction.
%
% ⛔ AND r6593: THE r6581 T-AUDIT'S COUNT WAS WRONG, AND ITS METHOD IS WHY.  That audit searched
%   for FOUR KNOWN SENSES by phrase and reported the result as a census.  ** A sense it was not
%   told about could match no pattern. **  A sense-agnostic sweep -- every bare T in a paper
%   body, clustered by neighbourhood -- returns 173 occurrences and SIX structural senses.  The
%   two missed are:
%     * THE TURNAROUND CUBIC T = r^3 + 2M alpha^2, with H - T = -alpha^2 r -- ten uses, in THIS
%       paper and P7, doing real work separating the two turnings as events;
%     * PAINLEVE-GULLSTRAND TIME T in BH_causality -- a THIRD time coordinate, distinct from the
%       closed-dS one.
%   (And a two-use cut-bend datum in P7, 'a bend of the cut, T != 0: it is content'.)
%   ⇒ *** The two renames r6581 made STAND -- P5's proof variable was a real collision and the
%     freeing of \widetilde{T} was owed.  What does not stand is the CLAIM TO COMPLETENESS. ***
%   ⌗ NOT RENAMED: the turnaround cubic and the PG time are disambiguated by their own equations
%     and by sitting in sections that use no other sense -- the same ground on which temperature
%     is tolerated.  ** They are RECORDED so the next reader meets six and not four. ** R's local radius use (slicing scale 2/sqrt3,
%
% ⛔ AND r6597: sigma IS NOW PROTECTED CORPUS-WIDE FOR THE WEYL ROOT-EXCHANGE INVOLUTION.
%   P14 carried a LOCAL redefinition -- "Let sigma be the involution exchanging the two
%   gamma^5-eigenspaces" -- and then used bare sigma for the CANON sense after it ("sigma and R
%   generate D_6").  ** A shadowed symbol, used both ways on either side of its own redefinition. **
%   ⇒ The local sense is renamed \mathsf{P}: 7 uses in P14, 48 in its receipt W1.  *It is NOT R:
%   R = gamma^5 IS the chirality grading, acting as +-1 on the eigenspaces, where this map
%   EXCHANGES them -- diagonal against off-diagonal.*  The sans-serif marks a parity of a different
%   kind from the areal P : r -> -r; P11's own word for it is parity, 'the fixed reflection
%   symmetry' by which 'parity identifies the two helicities'.
%   ⌗ sigma keeps its canon sense in all eight papers that use it, P14 included -- five uses there,
%     the root transpositions, the diagonal designation, and <sigma,R> = D_6.
...
We exhibit a single radial curve $r(l)$, defined by $dr/dl=\sqrt{|f(r)|}$ with $f(r)=1-2M/r-r^{2}/\alpha^{2}$, whose turning points are exactly the roots of the horizon cubic and whose three regimes are read off the single discriminant $4-3r_{0}^{2}$. The throat radius $\alpha=\sqrt{3/\Lambda}$ is fixed throughout and is the one invariant of the construction: the de~Sitter and Schwarzschild geometries are not two limits of the curve but two \emph{readings} of one slicing of the fixed-$\alpha$ manifold. The cubic factors cleanly when the slicing parameter is taken as one of its own roots, and the map carrying one root to another is an involution with fixed point at the Nariai configuration; this root-exchange is a genuine symmetry of the line element, the mass parameter being invariant under it.

The slicing parameter is fixed by the geometry of observation. The hole's image lies on the observer's celestial sphere, and obtaining a planar chart forces the gnomonic projection; the offset is then $r_{0}=\tfrac{2}{\sqrt3}\sin w$ with $w$ a genuine geometric angle, and the horizon relation is the pure triple-angle $2M=\tfrac{2}{3\sqrt3}\sin 3w$, the slicing scale $2/\sqrt3$ forced as the unique value removing the residual harmonic. That collapse to a pure multiple angle is available in four spacetime dimensions and---up to a parity---in five, and in no other.

Three consequences follow for the geometry. The slicing surface has intrinsic Gaussian curvature $K_{G}=1/\alpha^{2}-M/r^{3}$: finite throughout the static region, invisible to the horizons, and changing sign once, at $r_{\mathrm{HE}}=(M\alpha^{2})^{1/3}$, between a Schwarzschild-like and a de~Sitter-like regime. The mass parameter is the throat radius modulated by the slicing, $2M=\alpha((r_{0}/\alpha)-(r_{0}/\alpha)^{3})$---a turning point, not a coefficient---so that $\alpha$ is the invariant and $M$ its slicing-dependent factor. And the equatorial seam joins a Riemannian spherical piece to a Lorentzian de~Sitter piece by the continuation $\theta\mapsto\pi/2+i\psi$, the signature flip automatic and confined to the two-dimensional slicing surface, the spacetime Lorentzian throughout.

Because the areal radius is signed, the slicing \emph{closes}: run inward to the seam, around the throat through $r=0$---a branch point and not a barrier, the geometry smooth across the locus the chart labels $r=0$---and out onto the conjugate branch, the curve closing on the backward-radial root. The horizon, the curvature singularity and the cosmological horizon are thereby points of one curve: the horizons its turning points, $r=0$ the branch point it passes through.

The slicing curve is intrinsic: moving the observer who charts it changes the description and not the geometry. But the de~Sitter and Schwarzschild descriptions are not two labellings of one swept geometry. The de~Sitter description sweeps the complete arc about the manifold's own axis of symmetry; the Schwarzschild description, viewing the hole from outside in the timelike orientation, cannot use that axis and is forced to pivot its sweep on a selected off-axis point, the critical point $r=0$. \emph{This forced pivot is the geometric origin of the horizon-versus-singularity asymmetry that the companion paper identified algebraically}: the asymmetric standard classification tracks the asymmetric sweep, not the analytic structure of the curve, which is symmetric. Read in the other direction the same fact says how this construction \emph{produces} a curvature singularity at all---by the sweep about the areal origin, and at no other locus.
...
\emph{And the degeneracy stops there for every mass, which makes the classification complete rather than
local}\ldg{catastrophe_singularity}. A triple root of $r^{3}-r+2M$ would force the fixed linear coefficient
to vanish---three equal roots $a$ give $-3a^{2}=-1$ against a coefficient that is $-1$ by construction---so
no mass reaches it. \emph{The turning points of this family are therefore simple or double and never worse},
and the companion's finding that the collapse critical point is a fold rather than a sharper degeneracy is a
statement about the whole family and not about one member~\cite{JanzenCircle}.
...
\begin{figure}[t]
  ...
  $r=3M$ across the family---and the photon sphere meets the horizon locus exactly at Nariai, which is worth stating rather than leaving to the eye. The photon sphere solves $\dd(f/r^{2})/\dd r=0$, giving $r=3M$ independently of $\alpha$; at the Nariai value $M=\alpha/3\sqrt3$ that is $r=\alpha/\sqrt3$, the merged double root, where $f=f'=0$\rcpt{O1_photon_sphere_nariai}. The framework paper forces the same member by a trichotomy on how the reassigned null congruence meets the horizon---transverse, tangent at the merged root, or no horizon~\cite{JanzenCRframework}---and a null direction tangent to a sphere of constant $r$ is a circular null orbit, so the two conditions are one condition reached twice. What the optical reading adds is that the forced member's photon sphere is null-degenerate: sitting on the horizon, its impact parameter $b^{2}=r^{2}/f$ diverges. Carried too is the comoving turnaround $r=(-2M)^{1/3}$---the
...
\paragraph{Why there are three.} The number is not put in by hand, and it is fixed before any of the geometry below is drawn. The horizon cubic of Section~\ref{sec:cubic} has three roots, and a vantage is fixed by which of them it reads as \emph{its own} black-hole horizon---the remaining two being read as the cosmological horizon and its backward-radial partner. Since the three roots are on the same footing, so are the vantages that designate them, and there are exactly as many of the latter as of the former: three. The root-exchange involution $\sigma$ of Section~\ref{sec:cubic} is then the step from one vantage to a neighbour, and the group permuting the roots is the group relating the vantages (Section~\ref{sec:tour}). Their placement follows from the same footing: three vantages symmetric under that permutation and equidistant from the axis stand at the vertices of an equilateral triangle, $120^{\circ}$ apart. \emph{So the count and the spacing come from the cubic, and the distance is what the next paragraph derives.}

The swing-pivot of Section~\ref{sec:slicing}---the hinge---sits at transverse distance $2\alpha$ from the hole's axis, the vantage from which the throat subtends exactly $60^{\circ}$ (angular radius $\arcsin\tfrac12=30^{\circ}$). The three hinges therefore form an equilateral triangle of circumradius $2\alpha$ whose incircle, of radius $\alpha$, is the throat itself: the hole is the incircle of the hinge triangle (Figure~\ref{fig:keplerian}). Its three sides are not imposed but \emph{found}---each is a null generator of the doubly-ruled substrate (Section~\ref{sec:slicing}), tangent to the throat at its midpoint.
...
\item the nine-point circle of the hinge triangle \emph{is} the throat\rcpt{euclid7_nine_point}\ldg{figure_theorem}---by Euler's theorem the nine-point radius is $R/2$ and its centre is the equilateral's own centre;
...
carry weight elsewhere in the programme, and the sweep places them at the same point as the first five.
...
Second, this is an asymmetry between the two three-nesses that runs deeper than their affine inequivalence. The temporal $\mathbb{Z}_{3}$ \emph{factors through} a half-period that exchanges the branches---matter leg to conjugate leg, $r>0$ to $r<0$. The spatial $S_{3}$ carries no exchange inside itself: $f=0$ is a condition on $r$ alone, carries no shift of $\tilde\tau$, and nothing within the permutation of its roots exchanges the branches. \emph{The asymmetry is one of where the exchange sits, not of whether one exists}---the spatial side has its branch exchange in $R$, which carries $r_{0}\mapsto-r_{0}$ and so maps the triple $(r_{N},r_{N},-2r_{N})$ to its $R$-conjugate twin $(-r_{N},-r_{N},2r_{N})$: an exchange \emph{between} the two Nariai members rather than inside one member's $S_{3}$. The temporal case differs precisely there, its exchange being internal to the $\mathbb{Z}_{3}$'s own generator. \emph{The two order-threes differ not only in how their roots sit but in what lives inside them.} What is \emph{not} claimed, here or anywhere in this paper: any identification of the two cubics' root sets, which the framework paper's order-three bridge holds open, and any reading of the turnaround triangle as a weight diagram. The statement above concerns the interior of one three-ness, not a bridge between two.

One more thing the two seams do, and it is the $2{:}1$ again in a different variable. Read along the closed slicing in the phase $\tilde{w}=3c\tilde\tau/2\alpha$\footnote{\emph{The tilde marks it as the bead's, not the sky's.} This paper's $w$ is the \emph{sky angle}---the swing read on the observer's celestial sphere, a label for \emph{which cut}---while $\tilde{w}$ is the phase of the outward law along one worldline, a label for \emph{when}. They are unrelated quantities and the tilde carries through from $\tilde\tau$, as it does for $\widetilde{T}$.} of the bead's outward law, the front seam sits where $\sinh \tilde{w}=1/\sqrt2$ and the backward-radial root where $\cosh \tilde{w}=2$; since $\cosh 2\tilde{w}=1+2\sinh^{2}\tilde{w}$, the first of those is $\cosh 2\tilde{w}=2$, and therefore
\begin{equation}
  \tilde{w}_{\text{back}}=2\,\tilde{w}_{\text{front}}=\operatorname{arccosh}2=\ln(2+\sqrt3).
  \label{eq:seamphases}
\end{equation}
\emph{The $2{:}1$ the two roots carry in areal radius appears as a $1{:}2$ in phase, inverted}, and by an identity rather than a coincidence: the ratio enters the radii as $2^{-1/3}$ against $2^{2/3}$ and leaves the phases as a factor two, because one leg is read on $\sinh^{2/3}$ and the other on $\cosh^{2/3}$.\rcpt{P07_lap_timeline} \emph{What the phases are and are not:} $\tilde{w}$ is set by the proper time of the fundamental observers, so these are intervals of that clock and not of the exterior static one, in which no horizon completes at finite time~\cite{JanzenBHcausality,JanzenCRframework}.
...
\emph{The count that carries the charge is not the count that acts on a bound mode, and the thirds are the difference between them.} Because each vantage's radius vanishes at its own wall alone, a crossing acts on the crossing mode's own component only when the wall is that mode's; over the two routes that own-wall count is $0$ and $-1$, integral, while the graze-point count is $+\tfrac13$ and $-\tfrac23$ of a lap. \emph{An integral label leaves closure satisfied identically and forces no denominator}, so the acting count cannot be what the charge is read from---the quantisation in thirds is available exactly because one count runs over all three walls and the other over one.\rcpt{P03_can_the_charge_be_read_from_the_acting_count}
...
\emph{Closure is not an added rule, because the winding's transformation law is already the corpus's.}\rcpt{P03_sheet_index_and_factors} The areal radius on the lap is a cube root of an invariant---$r^{3}=2M\alpha^{2}\sinh^{2}(3\tilde\tau/2\alpha)$, the bead's law---and Section~\ref{sec:temporal-threeness} derives from cosmic time's single imaginary period that the deck action $\tilde\tau\mapsto\tilde\tau+2\pi i\alpha/3$ sends $r\mapsto e^{2\pi i/3}r$. So $r$ carries a sheet index in $\mathbb{Z}_{3}$ which advances by one at each passage through the $r=0$ branch point and returns after three: exactly the transformation law of a crossing count, obtained for reasons that have nothing to do with charge and before any of this was asked. \emph{To close is therefore to return to the same branch}, and admitting a composite only when its total fractional part vanishes is a statement about the covering rather than a selection rule imposed on it. \emph{The branch in question is per-vantage, and that sharpens the condition rather than loosening it.} The signed radius is defined relative to a vantage, so there are three of them and each vanishes at its own wall and at neither of the other two~\cite{JanzenMatter}; a crossing therefore advances the owner's sheet alone, and return demands it of each separately. \emph{Vanishing total fractional part is what that demands of the total}---necessary, and not on its own sufficient: a single lap crosses each wall once and advances all three sheets together, which is the same fact as its acting by the centre rather than by the identity.
...
One member of the family reads that boundary differently, and the difference is forced rather than coincidental. The condition $K_{G}=0$ is $M/r^{3}=1/\alpha^{2}$, and substituting it into the potential gives $f=1-3r^{2}/\alpha^{2}$: \emph{on the flat locus the mass term is absorbed and $f$ takes the pure de~Sitter form at scale $\alpha/\sqrt3$}. So $f(r_{\mathrm{HE}})=0$ holds precisely when $r_{\mathrm{HE}}=\alpha/\sqrt3$, which is $27M^{2}=\alpha^{2}$---the vanishing of the horizon cubic's discriminant. \emph{The Hubble--Eddington radius lies on a horizon for exactly one member of the family, and it is the double-root member}: below it $f(r_{\mathrm{HE}})>0$ and the crossover sits strictly inside the static region, as Figure~\ref{fig:curvature} shows for the undercritical case; above it $f(r_{\mathrm{HE}})<0$.\rcpt{P03_rstar_merged_horizon} At the degenerate member itself $r_{\mathrm{HE}}=r_{N}=\alpha/\sqrt3$, the merged horizon of Figure~\ref{fig:ellipse}'s Nariai ordinate---so there the boundary between a structure's gravitational hold and the cosmological flow \emph{is} the geometry's own horizon. The same radius carries a third reading on the closed slicing: it is the lap's unique inflection, where the signed radius advances at unit rate against arc length~\cite{JanzenCRframework}. \emph{What is not claimed: nothing here bears on \emph{why} the cosmological reassignment selects the degenerate member---that selection is the framework paper's trichotomy and stands on its own. The statement is that the member so selected is the one whose structure-formation boundary coincides with its own merged horizon.}

\paragraph{The dynamical content of $r_{\mathrm{HE}}$ is exact, not approximate.} The phrase ``boundary between a structure's local hold and the cosmological flow'' invites a coarser reading than the geometry supports, so it is worth writing down what the locus does. For the marginal congruence the areal acceleration is $\dd^{2}r/\dd\tilde\tau^{2}=-f'/2$, and
\begin{equation}\label{eq:fprime-zero}
  f'(r)=\frac{2M}{r^{2}}-\frac{2r}{\alpha^{2}}
\end{equation}
vanishes exactly when $r^{3}=M\alpha^{2}$, that is at $r=r_{\mathrm{HE}}$. \emph{So the areal radius decelerates inside $r_{\mathrm{HE}}$ and accelerates outside it, with the turn precisely there}---and this is general in $M$, holding for every member of the family and not only the degenerate one. It is the same $f'$ whose sign the Gaussian curvature reads, \eqref{eq:KG}: the crossover of $K_{G}$ and the turn of the areal acceleration are one zero seen twice.

\emph{Read in the construction's own direction, this is not a claim about a cause.} Nothing accelerates anything at $r_{\mathrm{HE}}$. The sign of $\dd^{2}r/\dd\tilde\tau^{2}$ is a property of a fixed curve, read off where $f'$ passes through zero, and the deceleration and acceleration are what continuous motion along that curve looks like from within. The curve is not a trajectory a universe follows but the classifier of a family: a member is fixed by where its cut sits, and the whole of its history is already present in the curve's shape. \emph{So the turn is not an event in a member's life; it is a feature of the curve, met by an observer at the moment their own areal radius passes $r_{\mathrm{HE}}$.} Nothing changes at that radius---the curve is smooth there, $f'$ merely passing through zero---and what an observer would record as a transition from deceleration to acceleration is the second derivative of a fixed function changing sign along the path they are carried on. Read in the generating direction the question inverts cleanly: one does not ask why an expansion began to accelerate, but where on the curve $f'$ vanishes, and the answer is $r^{3}=M\alpha^{2}$, fixed by the member's own mass against the throat radius and by nothing else.

\paragraph{And the same length appears at a second scale, in a fixed ratio.} The comoving turnaround---the amplitude at which a collapse's areal radius stops decreasing---stands at $(2M\alpha^{2})^{1/3}$~\cite{JanzenCRframework}, so
\begin{equation}\label{eq:cuberoot2}
  \frac{(2M\alpha^{2})^{1/3}}{r_{\mathrm{HE}}}=\frac{(2M\alpha^{2})^{1/3}}{(M\alpha^{2})^{1/3}}=2^{1/3},
\end{equation}
exactly. \emph{The local balance radius and that amplitude are one length in the fixed ratio $2^{1/3}$}, with no dependence on $M$ or $\alpha$ surviving the quotient---the two differing by the cube root of two and nothing else. That amplitude has a cosmological reading in the companion cosmology, where it is the areal radius at matter--$\Lambda$ equality~\cite{JanzenCRcosmology}, so the local balance radius of a structure and the scale at which the two energy densities cross are the same length in that fixed ratio. On the degenerate member, where $r_{\mathrm{HE}}=\alpha/\sqrt3$ is the front seam, that places the amplitude at $2^{1/3}\alpha/\sqrt3$. \emph{So on that member the boundary between a structure's local hold and the cosmic expansion, the seam at which $r$ changes causal character, and the scale of the turnaround are one locus and one number up to the cube root of two}---a tighter statement than the general member gives, and one belonging to the degenerate member alone.
...
\emph{And the attribution divides into two questions with different answers, which is worth separating because
only one of them is decidable.} \emph{Does the inverse-cube term track the \textbf{selected} off-axis point?}
\emph{No, and that is settled by computation rather than left open.} The Weyl reflection permutes the three
horizon roots at fixed mass, so undercritically one mass is carried by three distinct offsets --- at
$2M=0.30$, by $r_0=-1.125419$, $0.338936$ and $0.786483$, each returning $r_0-r_0^{3}=0.300000$ and each a
genuinely different sky angle. \emph{The slice's curvature is identical on all three}, and it must be:
$K_G=-f'(r)/2r$, and the metric function carries the mass and the throat scale and nothing else, so the
selected offset cannot enter. \emph{The test has content because offset-sensitive quantities sit beside it} ---
the sky angle itself differs across the three while $\sin 3w$ does not, so a quantity carrying the selected
pivot would behave like $w$, and this one behaves like $\sin 3w$.

\emph{Does it track the cut's being off-axis at all?} \emph{That is undecidable, and provably so rather than
for want of a quantity.} Since $2M=r_0-r_0^{3}$, the mass and the offset vanish together, and the reflection
is \emph{one} operation carrying both signs rather than two that agree. No configuration has an off-axis cut
at zero mass or an on-axis cut at non-zero mass, and a discriminating quantity would need one of them.

\emph{So the term is the reflection-odd part of the slice's own curvature, blind to which root the cut sits
on and vanishing exactly when the cut sits on the axis} --- and the two attributions are one datum in two
coordinates, related by $2M=r_0-r_0^{3}$, with no quantity separating them. \emph{At the level where the two
readings differ the mass decides it; at the level where the pivot reading would win, they are not two
readings.} In the companion paper the horizon at $r=2M$ and the curvature singularity at $r=0$ were shown to be non-degenerate critical points of the cycloid $r(z)=M(1+\cos z)$ of \emph{identical} analytic character, the divergence of the Kretschmann scalar at $r=0$ arising as a chain-rule artefact of the chart's labelling of that critical point as $r=0$ rather than as a finite value. The companion paper's identification of both critical points as metric singularities of the same structural kind \cite{JanzenBHcausality,JanzenCircle} is the algebraic counterpart of the present geometric statement. The present construction supplies the reason the labelling falls as it does. The two critical points are \emph{topologically identical on the substrate}---two matching turning points of the one intrinsic curve, exchanged by its involution, two entry points to the conjugate piece continued through the seam. They are emphatically \emph{not} metrically identical: the metric fixes not only the interval but, through its second derivatives, the curvature, and on the swept geometry that curvature is lopsided between the two---divergent at the pivot $r=0$, finite at the horizon. The identity is topological and lives on the substrate; it does not survive onto the geometry, where the sweep---pivoting on one turning point, off the manifold's symmetry axis---renders the two distinct, as endpoint versus crossing and in curvature alike. The topological identity is the invariant of the rigidity result (Proposition~\ref{prop:rigidity}); the distinction is a property of the swept geometry, manufactured by the off-axis pivot---a feature of the vantage-dependent description, not of the substrate. The manufactured singularity at $r=0$ is the chain-rule shadow of the forced pivot: the asymmetric classification tracks the asymmetric sweep, not the analytic structure of the curve, which is symmetric.
...
\paragraph{So the construction \emph{produces} singularities, and this is where.} The reading above is usually stated as a dissolution, and it is worth also stating in the other direction, because it says how a curvature singularity arises here at all. The substrate is maximally symmetric, everywhere finite and smooth, and $r=0$ is a regular point of it---a point \emph{on} the throat circle, at $X_{1}=-\alpha$ (\S\ref{sec:hinge-geometry}). But the geometry generated by slicing and then sweeping \emph{about} that locus carries a curvature singularity there. \emph{A curvature singularity in this construction is therefore not a feature of the substrate but a product of the sweep about the areal origin}---which is why none of them is fundamental, and why the construction has exactly one such locus to produce them at. And that locus has already been met twice in this paper for other reasons: it is where the signed radius branches (\S\ref{sec:lap}) and it is the fixed point of the swing, diametrically opposite the hinge (\S\ref{sec:hinge-geometry}). \emph{These are not three facts about three places.} The construction's singularity-generator and the fixed point its whole family swings about are one place.
...
\emph{And the charged closed loop is placed rather than left hanging.} The loop through $r=0$ is
obstructed by the inner horizon the charge raises, so it is an \emph{interior} reassignment: the range
paper reaches the charged \emph{exterior} as a cut of the substrate~\cite{JanzenRange}, while the
Kerr-inner and Reissner--Nordstr\"om interiors lie outside the symmetry-reducible sector that range
classifies. Adding further charges---electromagnetic, rotational---is otherwise a natural extension not
required for the closed-loop structure of this paper.

\paragraph{Open.} One thing is open, and \S\ref{sec:lap} states its obstruction exactly: with
$Q\neq0$ the term $Q^{2}/r^{2}$ dominates as $r\to0$, so $f\to+\infty$ rather than $-\infty$, an inner
turning point appears, and $r=0$ is a timelike Reissner--Nordstr\"om singularity rather than the branch
point through which the signed radius passes onto the conjugate branch. \emph{So the charged closed loop
has no branch point to close through.} The limit is moreover singular: the inner turning point is
$M-\sqrt{M^{2}-Q^{2}}\to Q^{2}/2M$ and shrinks to zero with the charge, yet at any $Q>0$ the origin is
timelike and only at $Q=0$ exactly is it the branch point, so the loop closes on a set of measure zero in
$Q$. \emph{The function above is the \emph{eternal} Reissner--Nordstr\"om--de~Sitter geometry, and a collapse
is a dynamical one---so whether that stationary structure is reached at all is a separate question, and
it is answerable.} The inner horizon's fate is set by the competition between the blueshift of infalling
perturbations, going as $e^{\kappa_{-}v}$, and their decay: writing $\beta$ for the ratio of the exterior
decay rate to the inner-horizon surface gravity $\kappa_{-}$, the horizon survives only for
$\beta>\tfrac12$. \emph{The setting here is $\Lambda>0$, and that is why the question needs a number
rather than the usual intuition}---with a cosmological horizon the exterior decay is exponential rather
than power-law, so $\beta$ is finite and near extremality the horizon does survive. On a progenitor of
the framework paper's mass it does not, by a wide margin and in \emph{both} readings of a residual
charge: $\beta\simeq10^{-251}$ for a charge of order $e$, and $\simeq10^{-15}$ for a per-baryon asymmetry
at the laboratory bound, computed with a deliberately generous ceiling on the decay rate so that the
figure bounds the horizon's chances from
above\rcpt{P03_the_charged_collapse_does_not_form_the_eternal_inner_horizon}. \emph{So the eternal inner
horizon is not formed, and the obstruction stated above is a property of a stationary solution that the
dynamical problem does not reach.}

\emph{What that does not deliver belongs in the same breath.} A Cauchy horizon failing to form means the
maximal development is inextendible there; \emph{it does not by itself return a spacelike $r=0$} for the
signed radius to pass through. So the charged case is not thereby rejoined to the uncharged loop of
\S\ref{sec:lap}: what stands in the horizon's place, and whether a spacelike origin forms behind it, is
the dynamical interior's question and belongs to the framework paper's bead~\cite{JanzenCRframework}. \emph{That question has a sharper form than it looks, and the distinction just drawn supplies it.} The closed loop is obstructed below because at any $Q>0$ the sign at the origin has switched---and that is read off the same stationary function, with $M$ held constant. \emph{Dynamically the charge term survives the reading and the constancy does not.} Gauss's law makes $Q$ a conserved charge and its contribution to the Misner--Sharp mass is $Q^{2}/2r$ identically, so $Q^{2}/r^{2}$ is present whether or not anything is stationary; but $m$ is then a function of $r$, and the sign at the origin switches only while $2m/r$ stays below it. \emph{So the obstruction holds exactly when the interior mass function is $o(1/r)$ at the origin}---writing $m\sim kr^{-p}$, for $p<1$ it holds, for $p>1$ it fails, and at $p=1$ the coefficient decides\rcpt{P03_the_sign_switch_is_conditional_on_the_interior_mass_function}. \emph{And the interior the companion cosmogenesis solves determines it.} The closed dust-and-radiation ball
has $m(R)=\tfrac12A\sin^{3}\chi+\tfrac12B\sin^{4}\chi/R$ along a comoving shell~\cite{JanzenCosmogenesis},
\emph{so $p=1$ and the coefficient decides} --- the constant term the dust and the $1/R$ the radiation, which
is that interior's parity split read at the branch point. \emph{Written in the quantities the two terms are
made of}, and using the same interior's determination of its radiation content, the sign at the origin
switches only while $Q^{2}/2m_{\rm tot}>a_{\rm eq}\sin\chi$: \emph{the charge's own inner radius against the
progenitor's matter--radiation equality
radius}\rcpt{P03_the_interior_mass_function_is_p_equals_one_along_the_shell_and_the_charge_falls_short_of_the_equality_radius}.
\emph{On a progenitor of the mass this framework's cosmology requires, the first falls short of the second by
three orders in radius}, so the charge does not reach the origin ahead of the radiation and the obstruction is
not present there. \emph{The threshold is a charge-to-mass ratio of about $10^{-2}$}, so what the closed loop
meets is a condition on the progenitor and not a theorem about charge.

\emph{And the obstruction was never total in $\chi$, whatever the charge.} The comparison is not scale-free
across shells: the charge enclosed falls faster than the radiation term as the centre is approached, so
\emph{a central core reaches the origin for any charge at all}, and \emph{obstructed} cannot be read as
obstructed everywhere. \emph{Mass inflation is a statement about the Cauchy horizon at finite radius rather
than about the origin}, and is untouched by either reading.

\emph{What the obstruction does and does not reach is worth separating, because the two have been carried as
one.} \emph{Obstructed} is the closed loop \emph{on the stationary reading}: at any $Q>0$ the sign at the origin
has switched there and there is no branch point to pass through, and since the inner turning point shrinks to
zero with the charge while the sign does not follow it, \emph{the neutral case is not recovered as a limit}.
\emph{Dynamically neither half of that survives}: the obstruction is a condition on the progenitor's charge
rather than a consequence of any charge, and it is in no case total across the shells. \emph{Untouched} is the
framework's cosmogenesis: its hypotheses are the horizon's causal structure, the forced foliation and a
foliation-preserving morphism, none of which mentions charge, and a sub-extremal charged collapse still
carries an event horizon of exactly the structure the first
requires\rcpt{P03_charge_obstructs_the_beads_closure_and_not_the_cosmogenesis}. \emph{So whichever way the
dynamical question falls, what is at stake is the closed-loop reading of the charged case and not the result
that collapse continues as a cosmology.}

\paragraph{Status.} This is the third paper of the sequence. Its constructions are established and verified at the stated scope; its open items are named plainly, each with its reason. The programme of which it is part carries the group-theoretic and algebraic expressions of the same structure in their own papers---the group theory of the description groupoid in the companion groupoid paper~\cite{JanzenGroupoid}, and the constraint-algebra realisation in the algebroid paper~\cite{JanzenAlgebroid}. The slicing curve, the horizon cubic and its throat-angle form, the involution and its conjugacy, the tilted-ellipse locus, and the seam are the load-bearing constructions on which that work builds. These are the geometric core the wider unification is read on. What the companions build on it can be said in the terms established above. The slicing curve is taken up as a generator of a wider class of vacuum geometries rather than a classifier of the SdS family alone~\cite{JanzenOperator,JanzenRange}; the discrete $\mathrm{Aut}(A_{2})=D_{6}$ residue of \S\ref{sec:tour} is the structure a matter sector is built on~\cite{JanzenMatter,JanzenGeometricCore}; and the closed lap of \S\ref{sec:lap} is the object a cosmological reading is carried on~\cite{JanzenCRframework}. \emph{The one curve is where those readings meet, and each of them is argued where it is made.}
