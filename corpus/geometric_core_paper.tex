...
budget\rcpt{U3_the_residue_is_one_and_it_is_already_counted}. \emph{What the count
leaves is not a constant but an initial condition}---the measured
$\rho_r/\rho_m$, which the corpus carries as empirical at no cost to the
dissolution, exactly as flat $\Lambda$CDM carries $\eta$. The source facts are each established and cited, the sector's
...
geometric expansion rate carry no tunable freedom. What the cosmology takes
from the data is its epoch, fixed by the baryon-acoustic distances, and its
onset redshift, fitted to the acoustic angle, of which the measured
$\rho_r/\rho_m$ is the restatement---an $\eta$-analogue \emph{read} not
tuned~\cite{JanzenCRcosmology}. The rigidity is two-sided, and that is its
...
falsification), while the geometric rate resolves the Hubble tension and
...
$\rho_r/\rho_m$ an $\eta$-analogue read not tuned. The full-spectrum
...
should equal the substrate's non-symmetric residue---maximal symmetry, spent, leaving exactly the one
measured cosmological initial condition ($\rho_r/\rho_m$) as the sole tunable datum, and each
constant-relation a locked parameter. Decisive: a hidden geometric freedom in the cosmology, or a genuinely
...
\emph{On the datum side}, $\rho_r/\rho_m$ is a matter initial condition and not a constant, so it is not in
the geometric residue at all, and the programme carries it as a one-parameter accommodation. \emph{It cannot
