...
%  section states its source; conjectures are named as such; stated for
%  reversal.
% =====================================================================
...
carries no phase because it fails to freeze.
\end{remark}
...
geometric expansion rate carry no tunable freedom---the rigidity whose
...
wall closes, but the discrete parity the wall cannot reach. This is not claimed in either direction, and the ordinary route --- a matter
bundle placed by hand --- is untouched by the wall.

\paragraph{The flavour skeleton, as a within-CR result.} One reach on that residue is worth
stating whole, because its parts are grounded in several registers at once. \textbf{The reading, and what
stands behind it.} \emph{The Standard Model's discrete flavour skeleton is the substrate's own three-fold
...
no isometry ($\mathfrak{su}(3)\not\subset\mathfrak{so}(5,1)$), so a discrete
flavour symmetry on this structure is \emph{global}---the Standard Model's own arrangement, gauged
...
\item \textbf{P3, the slicing curve~\cite{JanzenSlicing}} --- forward: P3's substrate-and-cut section now carries the
...
spectrum gives $\chi^{2}=397.13$ against flat $\Lambda$CDM's $206.44$, the
acoustic transfer being what carries that spectrum back to the geometry. The cosmic beginning is the
...
$\omega/|aH|\to0$ independent of $m$---a massive trajectory carries no phase, by failing to freeze rather than
by freezing into one\rcpt{S1_a_massive_mode_freezes_too_and_the_mass_term_vanishes_at_the_branch_point}.
