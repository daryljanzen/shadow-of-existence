...
once $\alpha$ is. \emph{So the branch is not neutral, though not for the reason a blanket reading would give.} A renormalisation condition is a constant of the \emph{quantum} register, and the ledger's own statement is about the geometric constants rather than the tower's regularisation---so this would not be a first free constant, that register already spending one. \emph{What is particular to it is where it becomes visible.} Its only observable is an additive shift in the entropy---and the entropy is the quantity singled out just above as a count taken \emph{across} the register split, where the temperature is not. \emph{So if $S=A/4$ carries, a constant of the quantum register shows up in the one quantity that crosses the registers}, which is the structure this section has already named rather than a new one\rcpt{P17_the_entropy_declination_is_load_bearing_for_the_ledger}. \emph{Whether that is a cost the ledger counts is not settled here}: its statement quantifies over geometric constants, and an additive shift touches the value of a geometric quantity, which are not the same thing.
