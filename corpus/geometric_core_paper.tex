...
The ladder is stated in this paper and the embedding had been stated in none; it is the same pole--polar relation the throat protocol already runs on, read one dimension up. Since the scale is the only
...
CR~\cite{JanzenMatter}; and the descent onto a \emph{propagating} spinor sector is now
built as well, a Dirac field on the unpolarised radiating member propagating on the light
...
it. \textbf{The reading, and what stands behind it.} \emph{The Standard
...
isometry~\cite{JanzenBoundary,JanzenAlgebroid,JanzenGroupoid}.~The audit behind this count, its receipts, and the splits that cut its law are collected in the programme's combinatorics ledger\ldg{combinatorics}.
...
\begin{itemize}
\item \textbf{P1, the metric singularity~\cite{JanzenBHcausality}} --- forward: P1's scope note now points here, reading
...
\item \textbf{P2, the Schwarzschild circle~\cite{JanzenCircle}} --- forward: P2's introduction now points here, its
...
(the geometric base is established; the reading itself is a target rather than a result) (\S\ref{sec:frontiers}).
...
a concrete family.
\item \textbf{P6, the shadow of existence~\cite{JanzenShadowExistence}} --- forward: P6 now points here (from its
...
\item \textbf{P8, the slicing operator~\cite{JanzenOperator}} --- forward: P8's introduction now points here, its
...
entailed by them (P6).
\item \textbf{P9, the range~\cite{JanzenRange}} --- forward: P9's introduction now points here, its range the reach of the
...
\item \textbf{P12, the constraint algebroid~\cite{JanzenAlgebroid}} --- forward: P12's scope section now points here,
...
The structure is established; the reading of it offered here is not claimed. The compact (Wick) face is
...
The item has two sides and they now stand differently, so it is split rather
than carried whole. \emph{The CONSTANT side is a result and
...
input and nothing for a dimensionless tuning to hide in.  \emph{That argument is
about the geometry's own invariants, and it does not reach the regularisation of a
mode sum}: the graviton tower's log-scale coefficient is computed and is not
zero~\cite{JanzenCanonicalTime}, so one dimensionless constant is spent
there. \emph{The structural reason above survives intact --- there is still no second
invariant --- and what it does not establish is that no constant enters by any route
at all.} \emph{And the
...
this list records it as one}: what stands here is not a derivation owed but the
statement that there is none to give. \emph{And the companion
...
%  REGISTER ALIAS: the maximal-symmetry ledger -- register `PO-18`, STRUCK r3097 -- the enumeration was complete and banked in p0 sec:ledger; what was missing was one citation and narrower than stated  [alias repaired r3421]
%  here: U3_the_residue_is_one already answers the second half (no constant maximal symmetry fails to
%  reach), so what is open is the systematic forcing list.
%  This list stays in the paper's own voice; the alias lives in a comment so a reader meets
%  the physics and a node can still trace what is owed.  Unregistered before r2378.
...
constant-side analogue of the discrete-parity matter opening. This is grounded in two proven instances.
%  REGISTER ALIAS: the phase structure at the seam -- register `PO-17`, STRUCK r3097 (DECIDED in both
%  halves; the row had been opened without checking that the second half was already answered at r2566).
%  The item is recorded without being claimed, on the paper's terms; the ALIAS is spent.  [alias repaired r3421]
%  This list stays in the paper's own voice; the alias lives in a comment so a reader meets
%  the physics and a node can still trace what is owed.  Unregistered before r2378.
\end{enumerate}

\emph{A third item stood here and has been decided, in both halves, so it is recorded as a
result rather than carried as a frontier.} The phase structure at the seam is \emph{real
structure and not interpretation} at the level of the object: the antilinear face $K$ and the
reflection $R$ are the two axis-symmetries of one analytic object, whose composite supplies
every kinematic datum of charge conjugation, with the self-conjugate photon congruence as $K$'s
fixed set; the reading is neither a second labelling of matter and antimatter, which $R$ already
performs linearly, nor a continuous interpretive parameter, reality admitting exactly two
values. \emph{And it does not attach to trajectories}: a mass enters the mode equation as
$m^{2}a^{2}$, and the branch point annihilates that term while $|aH|$ diverges, so
$\omega/|aH|\to0$ independent of $m$---a massive trajectory carries no phase, by failing to
freeze rather than by freezing into one\rcpt{S1_a_massive_mode_freezes_too_and_the_mass_term_vanishes_at_the_branch_point}.
