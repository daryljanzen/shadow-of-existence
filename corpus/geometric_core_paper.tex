...
\item \textbf{P5, the description groupoid~\cite{JanzenGroupoid}} --- forward: P5's \texttt{rem:orientation}
...
likelihood is run as well and returns the sharpest datum against: fitted to the
Planck $TT$ likelihood with five free parameters each, the construction's
spectrum gives $\chi^{2}=397.13$ against flat $\Lambda$CDM's $206.44$, and the
acoustic transfer, run end to end, returns the first peak $6.6\%$ low in position
with the heights below the sky's, a mechanism for that deficit being what the
cosmology names as owed. The cosmic beginning is the
