...
\emph{The transpositions are not missing.} The reflection of \emph{cosmic} time, $\widetilde{T}:\tilde\tau\mapsto-\tilde\tau$ at fixed $r$ and mass, is a symmetry of the outward law $r^{3}=2M\alpha^{2}\sinh^{2}(3\tilde\tau/2\alpha)$ in its own right, $\sinh^{2}$ being even---so $\widetilde{T}$ descends to the turnaround.\footnote{\emph{The tilde is doing the same work it does on $\tilde\tau$ itself.} The corpus reserves $T$ for the time reflection $X_{0}\mapsto-X_{0}$ of the \emph{static} time; $\widetilde{T}$ reverses the \emph{cosmic} time $\tilde\tau=\tau+\chi$ of the non-synchronous slicing, whose constant slices sit at $45^{\circ}$ to the fundamental rest frame. \emph{They are not the same map and neither is the other's restriction}: $\widetilde{T}$ holds the mass fixed, where the conjugation of \S\ref{sec:cosmogenesis} reverses it.} But it does \emph{not} commute with the deck action: it conjugates it to its inverse, $\widetilde{T}D\widetilde{T}^{-1}=D^{-1}$, which is the dihedral relation. Hence $\langle D,\widetilde{T}\rangle\cong S_{3}$ and, with the parity adjoined, the turnaround carries $D_{6}$ of order twelve
