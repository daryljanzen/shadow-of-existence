...
%   *** DELIVERED: the generation COUNT (three), the CHIRALITY of each (gamma^5), the within-state S_3 of
%   the walls, and the generations' cyclic Z_3 on the turnaround's deck (sec:whichthree). NOT DELIVERED: the GAUGE REPRESENTATIONS (colour,
%   weak isospin, hypercharge — excluded from the isometry; "they remain the ordinary route, a bundle imposed by hand") and the
...
%   *** THE TEN-AVENUE COMB: the earlier whole-comb was a rubber-stamp, Daryl-caught).
...
%   THE WALLS' S_3: the Z_3 rotation cycles the walls; each of the three reflections swaps the other two
%   and fixes one; together they generate the FULL S_3. With prop:wall's chirality Z_2: D_6 = S_3 x Z_2
%   (the hinge S_3 x chirality) = Aut(A_2). GLOBAL vs GAUGED is the KIND the discrete structure fixes:
%   the Weyl S_3 is the monodromy symmetry of the solution space and NO substrate isometry, whereas the
%   orientation parity Z_2 IS an isometry (gamma^5) -- so chirality descends GAUGED and the walls' S_3
%   GLOBAL. The hinge S_3 is the WITHIN-STATE index; the generations sit on the turnaround's deck Z_3
%   (sec:whichthree), and the wall modes being identical copies is a property the generations share,
%   not what makes the walls them.
...
%   BREAKING STRUCTURE exists (recorded, not claimed); the mass VALUES are the ordinary electroweak route.
...
Three results follow, each stated at the scope it is established. 
...
the bulk index on the non-compact substrate, which the boundary paper's non-compactness escape turns on, is obstructed. \emph{And the finiteness survives the member this programme's cosmology actually is}: at the Nariai double root the two horizons merge, $f$ acquires a double zero, and $1/\sqrt{|f|}$ becomes a simple pole---the non-integrable exponent, so the hypothesis fails \emph{pointwise} exactly there. The length nonetheless converges, to $1.8138\,\alpha$ as the gap closes through four decades, because $\int\dd r/\sqrt{(r-r_b)(r_c-r)}=\pi$ independently of the gap: \emph{the interval closing compensates the integrand blowing up}, so the pointwise divergence never reaches the limit\rcpt{P14_leaf_compactness}\ldg{functional_analysis}. 

The signed-radius flip moreover passes through the $r=0$ \emph{branch point}\rcpt{P14_mode_monodromy_at_the_wall}---the wall and the branch point being one locus with two properties rather than two loci that coincide, which is checked on locus, type, definition, equivariance and separability---rather than a single-valued crossing on a loop, so the even-crossing constraint does not apply and the three modes, being of one chirality, have no cancelling partner: $\dim\ker_+=3$, $\dim\ker_-=0$, the net chirality a $\gamma^5$-graded index. 
...
monodromy symmetry, and the walls' $S_3$ is \emph{global}. The pairing of a gauged chirality with a global index on the three seats is, in this reading, the difference between being \emph{on} the one circle and being \emph{tangent} to it, and this sector is where that difference becomes two physical data. 

The net structure---a count of three, a chirality, and the generations' cyclic $\mathbb{Z}_3$, with a within-state $S_3$ beside them---is the Standard Model's \emph{discrete flavour structure}, obtained here as the maximally-symmetric slicing of a maximally-symmetric substrate. 
...
classes, against the Standard Model's five multiplets per generation, and the difference is
...
inequivalent singlets.  The difference is therefore forced and not merely found, and it reaches
further than one multiplet: the same argument makes the sector's isospin structure vector-like,
...
winding of its turning polarisation plane~\cite{JanzenDynamics}; its Dirac sector is built there
too, and propagates.  \emph{So the two absences are not one}: the polarisation is
supplied and the multiplet is not, and on that chiral member the asymmetric action is permitted and not
selected\rcpt{P14_the_twist_conjugates_T_it_does_not_project_it}, so the multiplet is delivered on neither member.  This sector, built on the polarised cut, matches the
...
This sector fixes the number of such families, their chirality, the $S_3$ relating the three walls and the cyclic $\mathbb{Z}_3$ relating the generations, and fixes nothing about their internal content: not the multiplet count, and not the colour or isospin assignments. 
...
Carried across the branch point at $r=0$ the structure is inherited rather than remade: the reassignment fixes the expansion rate and does not touch the leaf-carried content, so the generations cross unchanged, seated on the comoving turnaround's three rather than on the horizon roots (\S\ref{sec:whichthree}); the crest, where two of the three roots merge, is a fixed point of the roots' $S_3$ but \emph{not} of $R$, so the parity is untouched, the inherited matter stays chiral, and no matter/antimatter asymmetry is a cosmogenesis event---it stays the ordinary route. \emph{Fourth, and reached after the three above, the discrete opening delivers more than a flavour skeleton.} The bundle the operator acts on is not a bundle of the substrate: every ambient candidate is real, and a real bundle's complexification carries a parallel conjugation, so its holonomy lands in the real form and none can carry $\su(3)$. The module is the \emph{branching} itself. 

Because the wall is a wall \emph{of a hinge}, the construction carries three signed areal radii rather than one, so a wall crossing branches only the vantage that owns it and the monodromy is diagonal in the vantage basis and non-abelian across it; the three wall monodromies and the hinge $3$-cycle generate a finite group of order $81$ in $U(3)$, each monodromy carrying determinant $\omega$, whose determinant-one part is $\Delta(27)$ and lies in $SU(3)$, with the lap as its centre. Second quantisation---the ordinary step, available because the three wall modes are one operator's kernel and therefore identical particles---places a three-fermion state in the exterior cube, one-dimensional and generated by $\varepsilon$, and returns baryon $1$, diquark $0$, meson $1$\ldg{representation_theory}: every channel the Standard Model has, with the configuration group \emph{selected} rather than chosen. 
...
The count is forced within CR by the programme's own criterion of necessity, not by an added axiom, and its footing is stated plainly: a framework declining that criterion reads the natural single-hinge index---one---not three. A further consequence is drawn in Section~\ref{sec:scope}, and it runs the other way to the rest of the paper: because the fold the count reads is $D-1$ in a $D$-dimensional cut, and because the horizon relation collapses to a single multiple-angle only at four and five dimensions while the mass-parity that grades chirality exists only in even dimension, four is the only dimension carrying both a generation count and a handedness. Within CR, and on the same footing as the rest, three generations and four-dimensional spacetime are one fact read at two ends.
\end{abstract}

\section{The question, and what is and is not claimed}\label{sec:intro}

One statement in this paper runs the other way to the rest, and its footing is stated here. Section~\ref{sec:scope} draws a consequence about the \emph{dimension} of the cut---four dimensions being the only one in which the sector's own two deliverables, the count and the chirality, both exist. It rests on the same criterion as the count, and on two inputs of its own: that the construction gauge's lock $g_{tt}g_{rr}=-1$ holds at general dimension as the single slicing curve enforces it at four, and that the transverse space is the round sphere. The metric function at general dimension is not among them: the operator returns the Tangherlini--de~Sitter family as its entire solution space~\cite{JanzenSlicing}.
...
\begin{itemize}
\item \textbf{Delivered.} The generation \emph{count} (three), the \emph{chirality} of each generation ($\gamma^5$), and the $S_3$ relating the three walls---a \emph{within-state} index rather than a family symmetry, the generations' own threeness being the turnaround's deck $\mathbb{Z}_3$ (Section~\ref{sec:whichthree}), with the wall structure fixing the \emph{number} at either seat. These are the discrete flavour structure~\cite{IshimoriEtAl2010}. \emph{And, reached after them, the discrete content of \textbf{colour}}: the module the operator's colour structure acts on is the branching rather than any bundle of the substrate; the three wall monodromies with the hinge $3$-cycle generate a finite flat holonomy whose determinant-one part, $\Delta(27)$, lies in $SU(3)$; and second quantisation on the wall kernel returns baryon $1$, diquark $0$, meson $1$, the configuration group selected rather than chosen. With them, the \emph{kind} of each---why the chirality descends gauged and the walls' $S_3$ global---which Section~\ref{sec:twofactors} reads off the two relations the substrate's waist admits.
\item \textbf{Not delivered.} The \emph{coupling}: the bundle above is flat, so the construction supplies colour's exact selection rules and no force---it quantises and does not couple. Weak isospin as a \emph{gauging}: $T$ is a discrete horn swap and delivers a species label, not $SU(2)_L$'s chiral action, the two occupations differing on the right-handed pair (Section~\ref{sec:whichthree}). Nor does any rotation of the substrate supply that action: a compact $\su(2)$ acting on one handedness needs a Euclidean four-space whose chirality is $\gamma^{5}$, and the substrate's one Euclidean four-space carries the chirality of its normal $X_0$, which anticommutes with $\gamma^{5}$, while the four-space whose chirality is $\gamma^{5}$ is Lorentzian, where the chiral split of the rotation algebra is complex\rcpt{P14_no_rotation_of_the_substrate_is_a_compact_chiral_su2}. What the geometry delivers is both ingredients of the weak coupling's chiral projection---the chirality $\gamma^{5}$ itself, as the parity $R$, and a compact $\su(2)$, as the spatial rotations---and not their product, which is not a symmetry of the substrate. Hypercharge's normalisation is fixed by the winding rather than by the anomalies, which are homogeneous. And the mass spectrum: the zero-modes are massless, and their splitting is electroweak physics, external to the geometry.
\end{itemize}
The generation count is \emph{forced within CR}, in the precise sense made explicit in Section~\ref{sec:count}: it follows from the maximal-symmetry principle that defines the programme, applied to the matter construction as the same principle is already applied to the substrate and to the discrete breaking~\cite{JanzenGeometricCore}. It is not a theorem a framework lacking that principle must accept. That is what the word ``forced'' means here.
...
\end{proposition}
This is the $\mathbb{Z}_2$ (chirality) half of the substrate's discrete structure, realised as a bound state, not posited. Reversing the wall reverses the sign of the eigenvalue, flipping the chirality---the parity is the chirality, exactly as the algebroid paper reads it off the constraint algebra. \emph{One feature of $W$ deserves comment, since $\lambda=j+\tfrac12$ appears in it and might be thought to
...
$\lambda\bmod3$. \emph{That divide has the shape of the quark/lepton distinction --- the residue-zero modes
locally readable and free, the others neither --- and what stands between the shape and the distinction is two
identifications this paper holds as assumptions rather than results}: that the residue graded here is the
physical colour, which the boundary paper walls and this paper does not reopen~\cite{JanzenBoundary}, and that
the triality class is the fractional part of the charge, marked as an assumption where it is used below. A further
consequence follows without extra input and is stated on the same footing: since a field on the manifold must be
...
content remains where Section~\ref{sec:correspondence} places it, on the far side of the gauge wall. \emph{That undelivered content is where the programme's cosmological side arrives too, and the two
arrivals are one.} The
...
the wall would settle both, and that neither is settled without it. \emph{It is worth stating plainly where the gap sits, since the index of Section~\ref{sec:count} is easily
...
\emph{And finding it there requires taking this paper's own definition of the wall literally.}\rcpt{P14_the_wall_is_a_wall_of_a_hinge} The wall is where the signed
...
support}''~\cite{JanzenSlicing}.\rcpt{P14_p3_already_said_it} \emph{The two readings agree on the label and differ
only on the group}: with one radius the cover is cyclic and its monodromy abelian, which is why the search
above returned only a $\mathbb{Z}_3$; with three, crossing a wall branches only the vantage that owns it.
\emph{The two are not in conflict but are two constructions}---for the vacuum family the three hinges are equivalent vantages and a single
...
preserves no symmetric bilinear form---so the smallest connected group containing the ratios together with the
hinge three-cycle is $SU(3)$ itself. The monodromies themselves, with the three-cycle, generate a finite group
of order $81$ in $U(3)$; its determinant-one part, of order $27$ and generated by the ratios and the three-cycle,
is $\Delta(27)$, and the remaining factor is a $\mathbb{Z}_3$ of phases carried by the
determinant.\rcpt{P14_the_compact_faces_four_is_three_plus_one_and_the_singlet_has_nothing_to_carry_it} \emph{The two generators are this paper's two threes
...
So the position is not that the coupling is unbuilt but that a coupling is not the kind of thing a holonomy supplies, which is the same verdict the winding received one level up and for the same reason. \emph{What is not excluded here is a mechanism that is neither holonomy nor isometry; the isometry route is excluded separately, and no third mechanism has been named, so naming one remains open.} \emph{What can be said about one before it is named is more than nothing, and it is the dimensional sentence just used, which mentions no route.} 
...
The $\mathbb{Z}_{3}$ is therefore the obstruction to charge being observable rather than a rival to it. \emph{One qualification}: the agreement between ``is a field'' and ``has integer charge'' is not an independent check that could have failed. 
...
Solving for the involution that does it---rather than positing one---returns a unique \emph{affine} map, $w\mapsto\tfrac13-w$, whose constant is fixed by the pair itself. \emph{And it is the Standard Model's own map}: in this paper's own normalisation $Q=I_{3}+Y$, the isospin Weyl reflection at fixed hypercharge is $Q\mapsto 2Y-Q$, and the constant came out equal to \emph{twice} the quark doublet's hypercharge, $2\times\tfrac16=\tfrac13$, with nothing fitted. \emph{The map is the same in the other common normalisation} $Q=I_{3}+Y/2$, where the same reflection reads $Q\mapsto Y-Q$ and the doublet's $Y$ is itself $\tfrac13$---but the two must not be mixed in one sentence, since it is the constant that carries the claim. Its two factors are both already in hand---$w\mapsto-w$ is the slicing paper's independent charge $\mathbb{Z}_{2}$, and $w\mapsto w+\tfrac13$ is one graze-point crossing, which is one wall. \emph{And the exchange sits inside the configuration's own group, so nothing needs adjoining}: it is the horn swap $T$, a substrate isometry belonging to the order-twelve group already in hand. \emph{The offset is no obstacle to that}, the apparent difficulty---that $\mathrm{Aut}(A_{2})$ acts linearly while a map with a non-zero constant does not---arising only if the two are compared in a single space, which they do not share. \emph{Two things
...
\emph{One constraint follows from the construction as it stands.} The wall of
...
\emph{And the condition's reach has a definite edge, which is worth drawing because the sixteenth Weyl
fermion sits exactly on it.} The right-handed neutrino $\nu^{c}:(\mathbf{1},\mathbf{1})_{0}$ carries no
colour, no isospin and no hypercharge, so its contribution to every one of the conditions above is
identically zero: \emph{fifteen is anomaly-free and sixteen is anomaly-free, and the condition returns the
same number in both cases}\rcpt{P14_both_handles_on_content_are_blind_to_the_sixteenth}. \emph{So the one
constraint this construction places on content cannot distinguish them}---and the generation count, being a
graded index of wall-localised zero modes, counts generations rather than the content within one and does not
reach the question either. \emph{Both of this sector's handles on matter content are therefore blind to that
state}, and a preference between fifteen and sixteen must be sourced from outside the construction, which is
the representation-content step this paper declines.
...
undelivered.} Of the three structures a generation's charges are usually listed under---colour, weak isospin, and
hypercharge (\S\ref{sec:correspondence})---\emph{the third is not independent of the other two.} Given the gauge group and the
...
measured ratios, and the fair reading of that pair is a well-posed problem rather than a wall.
...
\caption{The three-plane fermion structure. Three hinges (radius $2\alpha$, polar $0/120/240$) and their three $r=0$ walls (on the throat circle, polar $180/300/60$, each antipodal to its hinge)---a $\mathbb{Z}_3$-orbit. Each plane carries the same three horizon \emph{values} (identical content) around its own wall (distinct loci): three identical bound modes, whose number is what the wall structure fixes (\S\ref{sec:whichthree}). The centre (the $\mathbb{Z}_3$-fixed axis) carries no wall. $\mathbb{Z}_3$ sends wall to wall; $D_6=S_3\times\mathbb{Z}_2$.}
\label{fig:gen}
\end{figure}
...
attains. At the horizons the measure carries an integrable inverse square root and $\psi$ is finite, so the norm converges there as well\rcpt{B67_verdict_the_analytic_sqrt_f_operator}. \emph{One numerical coincidence is worth disowning before anyone builds on it}: the threshold here is $\tfrac34$, and so is the essential-self-adjointness threshold the companion canonical paper meets at $a=0$~\cite{JanzenCanonicalTime}---but they are different computations. That one comes from $\sqrt{\gamma+\tfrac14}=1$ on exponents $\tfrac12\pm\nu$ in $\dd x$, an exponent gap of two; this one from $-2|\lambda|+\tfrac12=-1$ on a density in $\dd\ell$, a gap of three. \emph{Different measures, different gaps, the same number by arithmetic and not by structure.} \emph{Nothing in the count depends on a boundary condition}, the three modes being independent and of definite chirality either way. Each wall's mode is a definite chirality eigenstate $\sigma_y=+1=R=\gamma^5$ (Prop.~\ref{prop:wall}, the conjugate branch rejected), and the signed-radius flip is through the $r=0$ \emph{branch point}\rcpt{P14_mode_monodromy_at_the_wall} rather than a single-valued crossing on a loop~\cite{JanzenSlicing,JanzenCircle}---so the even-crossing constraint that binds a single-valued sign function on a circle does not apply, and the three modes, being of one chirality, have no cancelling partner. Hence $\dim\ker_+=3$, $\dim\ker_-=0$, and the count is the net chirality: a $\gamma^5$-graded index. Index-theoretic stability under deformations preserving the three-wall structure is the expected behaviour of such a graded count and is traced rather than computed here (\rcpt{P14_even_crossing_index} establishes the mechanism---one mode per wall, the even-crossing obstruction on a simple loop, and the necessity of the $r=0$ branch point---and marks the Atiyah--Singer statement on the branched bead as traced); we state it as traced\ldg{index_theory}. \emph{And it is worth saying which half is traced, because the two halves are not equally owed}: the \emph{analytical} index is $\dim\ker_+-\dim\ker_-$ by definition, so the count above is computed and needs no theorem; what the index theorem supplies is its equality with a topological integral, and what that equality buys is exactly the deformation invariance traced here. \emph{So the citation is a pointer rather than load-bearing for the count}, and the corpus's naming follows the same division---\emph{analytical index} appears and \emph{topological index} never does\rcpt{D1_the_traced_half_is_the_theorem_and_the_computed_half_is_not}. It is a leaf (vantage) index, which is why it is well-defined precisely where the spacetime bulk index is not.
\begin{equation}
...
\item \emph{The three are permuted by a global $S_3$}---the turnaround's, acting on its three sheets
(Section~\ref{sec:whichthree})---and it is a family symmetry, not a gauge one. \emph{The part acting on
generations and on nothing else is cyclic}: the deck $\mathbb{Z}_3$, the transpositions being supplied by the
horn swap $T$, which belongs to the isospin sector. The $S_3$ among the hinges (Section~\ref{sec:family}) is a
different three---the within-state index---and the two are related by no covering construction.
\end{itemize}
...
constraints on any proposed content, enters here only as a \emph{condition}: \S\ref{sec:inflow} requires each generation to be anomaly-free on its own, there being no bulk field for inflow to come from, and records that the observed content meets it. \emph{What is not done is the converse}---deriving the content from that requirement. Neither is the mass spectrum, the mixing, nor the
...
an obligation, not a result.} \emph{And the seating sharpens the obligation rather than loosening it, which is
worth drawing because the reverse would be the easier reading.} The group acting on the three families is
$S_3$, so the contact below is with the right group; but only its $\mathbb{Z}_3$ is flavour-only, the
transpositions being $T$ (Section~\ref{sec:whichthree}). \emph{So the breaking is not the Yukawas' business
alone}: the cyclic part is theirs to break, while the transpositions' action is fixed in another sector---and
already recorded there as matching the Standard Model on the left-handed side and differing from it on the
right-handed pair. \emph{A flavour-$S_3$ model may break all of its symmetry with the masses; this sector may
not.} \emph{That obligation can be checked against a literature rather than left standing, and it is worth doing
...
\section{The symmetry of the three walls}\label{sec:family}
...
\begin{equation}
D_6 = S_3 \times \mathbb{Z}_2 \qquad (\text{the hinge }S_3\times\text{chirality}),
\end{equation}
...
That the symmetry the three walls carry is \emph{global} rather than gauged is not a choice made here but the \emph{kind} the discrete structure fixes: the Weyl $S_3$ is the
monodromy symmetry of the solution space and no substrate isometry, whereas the orientation parity $\mathbb{Z}_2$ \emph{is} an isometry, acting on the cut spinor as $\gamma^5$---so chirality descends gauged and the walls' $S_3$ global, which is the flatness of the sector's holonomy read as a kind: exact selection rules and no force~\cite{JanzenGroupoid,JanzenAlgebroid}. 
...
Each plane carries the same three horizon values, so the three wall-modes are identical in content, distinguished only by their wall; this is what makes the count a count of \emph{identical} copies. \emph{That property the generations also have, and having it is not what makes these them}: the seating is settled in \S\ref{sec:whichthree}, and it is not the walls. And the wall is not a bare locus. Each hinge \emph{designates} one of those three roots as its own black-hole horizon, so a transposition of roots is a hop to a neighbouring hinge, and the Weyl $S_3$ \emph{is} the relation among the three hinges rather than an abstract relabelling~\cite{JanzenSlicing}. The $S_3$ the three walls carry and the Weyl $S_3$ of the root system are therefore \emph{one group}, not two that happen to agree in order: which root a vantage takes as its hole is the same distinction as which wall it binds at---the hinge fixes both. This is what makes ``one object read three ways'' a statement about the sector rather than about the cubic alone: the three readings are the three hinges, and the fermion sector is where they become three bound modes. \emph{What they are not is the three generations}---that seating is settled in \S\ref{sec:whichthree}, where the hinge $S_3$ is the within-state index and the generations are carried by the turnaround's deck; what the wall structure fixes is the \emph{number}. At the massless level the three are degenerate and $S_3$ is exact, and this degeneracy is \emph{forced}: the three shared horizons are roots of one cubic $r^3-r+2M=0$, and every root, designated the slicing parameter, returns the \emph{same} $2M=r_0-r_0^3$ (as $r^3=r-2M$ for each), so the three carry one mass parameter---one object read three ways on the fully $D_6$-symmetric horizon locus, a diagonal line and a $45^\circ$ tilted ellipse in the $(r_0,r)$-plane~\cite{JanzenSlicing}, on which the $2{+}1$ of a designated root against the ellipse pair is the diagonal designation $\sigma$ and the sign $2{+}1$ (two horizons of one sign against one) the reflection through the origin ($2M\mapsto-2M$, the chirality parity), each a description structure paired with its symmetric twin---so no asymmetric handle sits in the mass \emph{parameter}. The locus does carry a \emph{breaking structure} for that $S_3$ (the slicing singles one root against the ellipse pair), supplied with the triplet in the boundary paper~\cite{JanzenBoundary}; but the mass \emph{values} are the ordinary electroweak route, and whether that geometric structure bears on the physical hierarchy is not claimed---outside the present scope.

\section{The two factors: why they differ in kind}\label{sec:twofactors}\ldg{combinatorics}

The sector's two data are a chirality and a count, and Section~\ref{sec:family} has just written them as the two factors of one group, $D_6=S_3\times\mathbb{Z}_2$ (the hinge $S_3\times$ chirality; the generations are seated on the deck $\mathbb{Z}_3$ instead, and \S\ref{sec:whichthree} settles why). Two things about that group have been established elsewhere and are used above: that its factors act on independent structures---the Weyl $S_3$ on the three horizon roots and the inversion on the two null rulings, the inversion central~\cite{JanzenAlgebroid}---and that they descend differently, one gauged and one global. \emph{The product's being direct is worth separating from both, because it is forced and therefore carries no information about this substrate}: $\mathrm{Aut}(A_2)$ is a Weyl $S_3$ extended by a diagram $\mathbb{Z}_2$, every automorphism of $S_3$ is inner, and a semidirect product by an inner automorphism is isomorphic to the direct one---so there was never a semidirect alternative for the geometry to have excluded. 
...
which to touch it. \emph{The rate-only reassignment gives that nothing acts on the leaf at all; the
zero-duration argument adds that nothing could.} \emph{The crest:} the Nariai crest~\cite{Nariai1951} is the fixed point of the root-permutation, where two of the three horizon roots merge (the discriminant $4-3r_0^2$ vanishing); that permutation is the within-state index, and the generations are seated on the comoving turnaround's own three, not on the roots (\S\ref{sec:whichthree}). \emph{Chirality:} the crest is a fixed point of $S_3$ but \emph{not} of $R$~\cite{JanzenBoundary}, so the parity $R=\gamma^5$ is untouched---the inherited matter stays chiral, and no matter/antimatter asymmetry is a cosmogenesis event; it stays the ordinary route. 

This is rooted in the substrate's discrete geometry. 

The chirality/mass parity $R=\gamma^5$ is the mass-reflection $r_0\mapsto-r_0$ (equivalently $2M\mapsto-2M$), the orientation/diagram-automorphism parity, realised on the cut spinor as $\gamma^5$; as the $A_2$ diagram automorphism it carries the fundamental $\mathbf 3$ to the antifundamental $\bar{\mathbf 3}$~\cite{JanzenSlicing,JanzenAlgebroid}, and on the wall mode it is an exact operator (Proposition~\ref{prop:wall}). So the $R$-image of a generation is its antifundamental, and within CR that is its \emph{antimatter}: the dual $\bar{\mathbf 3}$ branch is antimatter at exactly the weight the fundamental branch is matter---forced within CR by the same discrete residue. 
...
no affine change of variable identifies the horizon roots with the roots of the comoving turnaround; the marginally-bound congruence integrates to $r^{D-1}=2M\alpha^{2}\sinh^{2}\bigl((D-1)\tilde\tau/2\alpha\bigr)$
...
\emph{The transpositions are not missing.} The horn swap $T$ descends to the turnaround, acting on the
cube-root sheet index as the inversion $k\mapsto-k$, and it does \emph{not} commute with the deck action:
it conjugates it to its inverse, $TDT^{-1}=D^{-1}$, which is the dihedral relation. Hence
$\langle D,T\rangle\cong S_{3}$ and, with the parity adjoined, the turnaround carries $D_{6}$ of order twelve
after all.\footnote{\emph{A second inversion satisfies the same relation on a different three, and the two
must not be run together.} The reflection of \emph{cosmic} time, $\widetilde{T}:\tilde\tau\mapsto-\tilde\tau$
at fixed $r$ and mass, is a symmetry of the outward law in its own right, $\sinh^{2}$ being even, and it too
inverts a three---but the $\mathrm{Im}\,\tilde\tau$ \emph{sectors}, not the cube-root sheets. \emph{Those are
independent}: a sheet rotation leaves $\tilde\tau$ fixed and a sector shift leaves $r$ fixed. The corpus
reserves $T$ for the time reflection $X_{0}\mapsto-X_{0}$ of the \emph{static} time; $\widetilde{T}$ reverses
the \emph{cosmic} time $\tilde\tau=\tau+\chi$, and the tilde does the work it does on $\tilde\tau$ itself.}
\emph{What keeps this from collapsing the distinction drawn above is that the two $S_{3}$'s permute
different triples}---the hinge one the three horizon \emph{roots}, this one the three cube-root
\emph{sheets}---and the companion framework paper establishes that no affine change of
...
Let $\mathsf{P}$ be the involution exchanging the two $\gamma^5$-eigenspaces---the reflection the companion paper's criterion calls the identifier of the helicities.\footnote{\emph{Not $\sigma$, and not $R$.} The corpus reserves $\sigma$ for the Weyl root-exchange involution on the cut---used in that sense in this paper both before and after this passage---and $R=\gamma^{5}$ \emph{is} the chirality grading, acting as $\pm1$ on the eigenspaces where $\mathsf{P}$ \emph{exchanges} them: diagonal against off-diagonal. The sans-serif marks a parity of a different kind from the areal $P:r\mapsto-r$; the companion paper's own word for it is parity, the fixed reflection about a polarisation axis that identifies the two helicities.} 

It moves the ruling datum and not the horn datum, so it commutes with $T$ for the same independence that makes $[T,R]=0$; and then $T|_-=\mathsf{P}\,T|_+\,\mathsf{P}^{-1}$, so the two restrictions are conjugate and have the same orbit structure. \emph{Acting on one eigenspace and trivially on the other is precisely a difference of orbit structure, so it is excluded whenever $\mathsf{P}$ is a realised symmetry}---which, by that same criterion, is exactly the polarised case. 

Enumerating the four actions $T$ may take, the two with orbits $2+1+1$ are the two that admit no commuting $\mathsf{P}$, and the survivors are $2+2$ and $1+1+1+1$: the shape reported above is the only shape available. 

\emph{The statement is sharper as one about invariants}: on the achiral member every invariant is blind to the chirality, since $\mathsf{P}$ exchanges the eigenspaces; on the unpolarised member the conserved twist $c$ is odd under $\mathsf{P}$ and so separates them, and $c=0$ is the polarised cut---the two cases being one charge at two values. 
...
So the chiral member removes the exclusion of \S\ref{sec:whichthree} and leaves the mismatch standing: the fifth multiplet is not a consequence the geometry delivers but a projection it does not carry, and \emph{that} is what a successor must supply. \emph{Two conditions on such a successor can be stated, and they are not the same condition.}\rcpt{P14_S3_against_the_wall_content} A colourless triple is chirality-asymmetric, so it must sit where the $R$-conjugate pairing fails \emph{and} where $R$ does not act on the three seats as the identity. \emph{That second condition is sharper than non-transitivity of the seat symmetry}\rcpt{P14_the_lift_fibre_content_is_two_plus_one_and_R_is_the_square_of_the_frame_exchange}: transitivity together with $R$'s centrality does not by itself exclude a $2{+}1$, since a central $R$ acting as $-1$ on the standard representation splits a transitive triple $1{+}2$. \emph{What excludes it at the wall is the stronger fact that $R$ there is antipodal \emph{within} each wall, so it fixes every seat and is $+1$ times the identity on the mode space.} \emph{The wall sector meets the first and not the second}: normalizability rejects the growing branch at every rung, so the bound content is a single $R$-eigenspace rather than a conjugate pair; but the hinge $S_{3}$ carries the three walls into one another, and since the sector's group is the direct product $D_{6}=S_{3}\times\mathbb{Z}_{2}$ with $R$ generating the second factor, $R$ is central and three equivalent seats carry one eigenvalue. \emph{That is the content of $\dim\ker_{+}=3$, $\dim\ker_{-}=0$}---the split is forced by the transitivity and not settled by which branch normalizes. \emph{And the turnaround, which does meet both conditions, seats the wrong three.}\rcpt{P14_the_two_plus_one_is_on_the_wrong_three} $T$ descends there as the inversion $k\mapsto-k$, an involution with a fixed point whose permutation representation splits $2{+}1$ under $R$; but its three are the generations, where a colourless triple is three fermions \emph{within} one. \emph{So the colourless triple is a third three}, and the two this construction carries are both spoken for and related by no covering construction (\S\ref{sec:whichthree}). \emph{And the candidates can be listed rather than left open.}\rcpt{P14_the_threes_are_exhausted} The graze points, the horizon roots, the triangle sides, the signed radii, the wall monodromies, the three timelike puncture-pairs and the Nariai root triple all reduce to the hinge three by maps this construction already carries; the triality classes and the winding classes reduce to the turnaround's. \emph{One candidate is neither}---the three causal classes---\emph{and it cannot seat the triple either}, since causal character is a metric invariant, so $R$ preserves each class and acts as the identity where a transposition is what a $2{+}1$ requires. \emph{That enumeration covers the structures named in this paper and in the slicing paper's geometry, and it does not close the question}. \emph{Three further places carry threes}---the groupoid's discrete operations, the algebroid's symmetric-pair dimensions, and the charged sector---\emph{and neither of the two genuinely new ones can seat the triple either}. The groupoid's three operations are three distinct maps at three distinct loci, but $\sigma$ and $R$ generate $D_6$ while $\xi$ is a partial involution outside it, so conjugation by $R$ does not preserve the set and there is no action to have eigenvalues at all. The algebroid's symmetric-pair isotropy dimensions $\{6,7,10\}$ are a three of which only two are realised---Type~O at ten and Nariai at six---so a seat would need $R$ to fix an object that does not exist\rcpt{P14_the_named_reopening_is_walked_and_closes}. \emph{The enumeration spans five papers and is not a theorem}, and what it asks of a successor is unchanged: to introduce structure rather than to re-read what is present. \emph{A candidate meeting the two conditions above is computed, and reported with the dependence that qualifies it}\rcpt{P14_the_lift_fibre_content_is_two_plus_one_and_R_is_the_square_of_the_frame_exchange}. On the lift---the region of the cosmogenetic bead where the time coordinate is purely imaginary---the relation $r^{3}=2M\alpha^{2}\sinh^{2}(3\tilde\tau/2\alpha)$ makes the time map three-to-one, and integrating the leaf measure along it gives $\ell=(2\alpha/3)\sin(3\theta/2)$ in the fibre's own angle. \emph{That relation returns the structures the geometry already carries rather than new ones}: $\ell$ vanishes at $\theta=0,120,240^{\circ}$ and is extremal at $60,180,300^{\circ}$, which are the hinge and wall angles; its fold is the monodromy about the binding radius; and a hinge crossing is a half-loop in $\ell$, so it sends $r\mapsto\omega r$ and multiplies the mode by $\omega^{\lambda}$, which is the wall monodromy of \S\ref{sec:chirality} recovered from the lift. \emph{The mode content there is then the same computation as \S\ref{sec:count}'s, run on the lift's own measure}: the growing branch is rejected at each of the three hinges by the same $\lambda<\tfrac34$ that no $\lambda=j+\tfrac12$ attains, the binding radius obstructs neither branch, and each of the three sectors binds one mode of $\sigma_y=+1$. \emph{What differs is not the content but which map moves the seats}: on the lift the involution that acts carries sector one to sector three and fixes sector two, so on the three-dimensional mode space it is a transposition rather than the identity, and \emph{its eigenvalues are $+1,+1,-1$}:
\begin{equation}
\dim\ker_{+}=2,\qquad \dim\ker_{-}=1 .
\end{equation}
\emph{That is the asymmetric split the colourless triple requires, at a locus where the wall gave $3{+}0$.} \emph{And the map that performs it is a named one}\rcpt{P14_the_lifts_two_plus_one_is_not_graded_by_R_after_K_and_the_wrong_C_object_would_have_said_it_was}. It is $\widetilde{T}:\tilde\tau\mapsto-\tilde\tau$ with $r$ and the mass held fixed---a symmetry of the bead relation in its own right, since $\sinh^{2}$ is even---and it is \emph{not} the conjugation of \S\ref{sec:cosmogenesis}. \emph{The distinction is a Clifford identity and not a matter of reading}: built from the Clifford data, $\gamma^{5}\!\circ\!K$ anticommutes with $\gamma^{5}$, so it flips chirality, and since all three lift modes carry $\sigma_y=+1$ while $R$ sends $2M\mapsto-2M$, it carries every one of them off the mode space and onto the conjugate geometry. \emph{It is therefore not an endomorphism there and has no eigenvalues there}---and being antilinear with square $-1$ it has none anywhere, so there is no $2{+}1$ for it to carry even in principle. \emph{This is what conjugation does and not a defect of the lift}: it is what Proposition~\ref{prop:wall}'s mode already does at the wall, where $R$ carries the bound mode to a bound mode of opposite chirality on the reversed wall. \emph{The two looked alike for one reason}: on the lift $\operatorname{Re}\tilde\tau=0$, so $\overline{\tilde\tau}=-\tilde\tau$ \emph{there and nowhere else}---on the Lorentzian wing $\tilde\tau$ is real and the two differ by a sign. \emph{So the split above is graded by the bead's time reversal and not by its conjugation}, and the asymmetry of the colourless seat and the sector's matter/antimatter map are two structures on one locus rather than one structure seen twice. And the three sector modes are counted independent, which is \S\ref{sec:count}'s own ground for counting the three wall modes independent; under the alternative reading, one analytically continued section, the space is one-dimensional and admits the grading only at $\lambda\equiv0\pmod 3$. \emph{And the gradings that would make it the colourless triple can be asked of it one at a time, which is sharper than asking whether it is the triple}\rcpt{P14_the_lift_triple_occupies_one_character_where_the_wall_sector_occupies_four}. \emph{Colour is trivial there, and by indexing rather than by placement}: the three are one per $\operatorname{Im}\tilde\tau$ sector, the sector period $2\pi\alpha/3$ carries no dependence on the cut, and $\sigma$ is not a function of the bead phase at all\rcpt{P14_the_lift_triple_is_colourless_by_indexing}. \emph{Weak isospin is the clause that separates the two loci, and it separates them against the lift.} $T$ commutes with $R$, so on the one-dimensional $R$-eigenspace each lift mode spans it acts by a scalar; it fixes the bead phase where the deck shifts it, so it is diagonal in the sector basis; and a diagonal operator commuting with a transitive $\mathbb{Z}_3$ has all three entries equal. \emph{So $T$ is one common scalar on the whole three-dimensional mode space, and the three modes occupy a single $(T,R)$ character where the sector of \S\ref{sec:whichthree} occupies all four.} \emph{The consequence is the one that matters}: the pair matching the left-handed doublet is the two $T$-values carried within one $R$-eigenspace, and the lift's single $R$-eigenspace carries one value three times, so \emph{it matches for neither sign of that scalar}. \emph{And the right-handed mismatch recorded above does not arise there, by absence rather than by repair}: the lift has no $R$-odd content for the comparison to be about, where the wall sector reaches that structure and differs from the Standard Model by exactly one pair. \emph{Which sign the scalar takes is not fixed by the construction, and none of those three readings depends on it.} \emph{What the lift's own $2{+}1$ grades is therefore not the isospin of a doublet}: in the $\gamma^{5}$ grading the lift is $3{+}0$, as the wall is, that split being $\widetilde{T}$'s. \emph{And the question that failure poses---where in this geometry two values could sit within one $R$-eigenspace---has an answer that is a single fact rather than a survey}\rcpt{P14_the_stratifications_two_valued_data_are_one_involution_and_it_is_R}. The causal stratification of the cosmogenetic contour offers three two-valued data: the horn label $\operatorname{sign}X_{0}$, the leg label $\operatorname{sign}r$, and the sign of the marginal rate, whose square is $1-f$. \emph{They are three names for one involution, the exchange of the contour's two ends, and that involution is $R$ itself.} The rate makes the point exactly: $f$ is even under the mass-reflecting $R$, so the rate's \emph{square} is even while the rate is \emph{odd}, and an $R$-odd grading anticommutes with $R$---it carries one eigenspace to the other rather than splitting either. \emph{So the two values are distributed across the $R$-eigenspaces and never within one}, which is the specific thing a doublet requires. The horn spans show the shape of that failure most clearly: $T$ exchanges them rather than acting within either, so they escape the lift's collapse and occupy \emph{two} characters---but the two are $(+1,+1)$ and $(-1,-1)$, a correlated pair on two one-dimensional eigenspaces. \emph{And the count states the condition in general}: two $T$-values within one $R$-eigenspace requires that eigenspace to be at least two-dimensional, hence at least four states, while every two-valued datum the stratification carries is a two-state exchange. The colourless four of \S\ref{sec:whichthree} is the one object here with a two-dimensional $R$-eigenspace, and that is why it is the one object carrying the pair. \emph{One structure in the construction supplies a two-dimensional $R$-eigenspace without being that four, and it fails the pair for a third reason}\rcpt{P14_the_hexads_hinge_relative_sign_is_the_mass_sign_and_its_three_is_marks_not_modes}. Read from one hinge, the six Nariai marks sort into three pairs: a hinge's two neighbours lie at sky angles $\pm30^{\circ}$ from it, and since $2M=(2/3\sqrt3)\sin3w$ those two carry opposite masses, so each hinge holds one member of the $\mathbf3$ and one of the $\bar{\mathbf3}$. \emph{Three hinges, three $R$-conjugate pairs, and each $R$-eigenspace three-dimensional.} \emph{What closes it is neither dimension nor the deck}: the deck shifts the bead phase while the hexad is indexed by the sky angle, so it does not act here at all, and the transitive $\mathbb{Z}_3$ that does act is the hinge three. \emph{It is closed instead by two commutations}---the hinge $\mathbb{Z}_3$ forcing one entry within each triple, and $T$'s commuting with $R$ forcing those two entries equal---so $T$ is a single scalar across all six and the characters occupied are $(t,+1)$ and $(t,-1)$: both $R$-values, one $T$-value in each. \emph{And the hinge-relative sign is not an independent label}: $+30^{\circ}$ from any hinge has $2M>0$ and $-30^{\circ}$ has $2M<0$, at all three hinges, so the sign that would carry the pair is the mass sign itself. \emph{And the six are marks in the cut parameter space}---which member, at which sky angle---rather than solutions on a leaf, so nothing binds them and nothing is rejected. \emph{They carry both $R$-eigenvalues for the reason a propagating sector does}, and their three is a three of marks. \emph{So this is a structure of the right shape in the right place, computed rather than read off a locus, and it is not an identification}: that the fibre's three elements are the colourless fermions, that the split is the doublet and the singlet rather than some other one, and anything at all about masses are all untouched by it. \emph{And the phrase that placement rests on---that it carries both $R$-eigenvalues for the reason a propagating sector does---asks whether a propagating sector can carry a three at all, which has an answer, and not the one a survey would give.}\rcpt{P14_the_propagating_three_is_inherited_and_the_pairing_fails_only_at_the_bound_energy} This construction carries a second propagating sector beside the unpolarised member, and that one \emph{does} carry a three: under the signature flip the three bound wall-modes continue into three modes propagating in cosmic time (\S\ref{sec:cosmogenesis})---the chiral seats carried across, and a three of modes rather than of marks. \emph{It fails the conjunction because the continuation is a bijection commuting with $\gamma^{5}$}, so the map carrying the three across carries the single $R$-eigenvalue with it, and propagation acquired after binding transports what the binding settled. \emph{And the settling is one event rather than two.} The wall operator is first order and its two branches are the two $R$-eigenspaces; written as $A=\dd/\dd\ell+W$, the partner map $A^{\dagger}$ carries every free eigenfunction to an eigenfunction of the wall problem at the same energy, so both $R$-eigenvalues are occupied at every energy but zero, and those states are reflectionless---\emph{a wall that binds one mode transmits everything else perfectly}. The pairing fails only at $E=0$, which is the bound level, so localisation---which a three requires---and the collapse to one eigenvalue are the same unpaired level. \emph{The deck supplies no third case}: as a phase it is a translation by an imaginary amount, of infinite order on any real frequency and of order three only on the Matsubara tower, and as a permutation it needs the three sectors decoupled, which is \S\ref{sec:count}'s $\lambda<\tfrac34$ rejection read as a condition on the hinges---\emph{one inequality deciding both conjuncts, in opposite senses}. \emph{And what forbids the conjunction is an anticommutation}: the first-order operator's boundary form, the only object that could couple the sectors, is carried by the off-diagonal Clifford element and anticommutes with $R$, so its matrix elements between states of one $R$-eigenvalue vanish identically. \emph{A mode space inside one eigenspace has nothing with which to couple its sectors, and a coupled one is not inside one eigenspace.} \emph{And the native propagating sector, asked for the doublet rather than for a three, gets further than anything else here before it fails.}\rcpt{P14_the_doublet_pairing_on_c50s_member_is_obstructed_by_lambda_and_by_the_twist} On the unpolarised member $T$ is not carried at all: it acts on the twelve's horn factor, and a Gowdy--de Sitter member has no vantage and no horn pair, while the spinorial time reflection reflects one direction and so anticommutes with $\gamma^{5}$---\emph{it exchanges the chirality blocks rather than grading one}, and the expansion $\psi_t=\sqrt{\Lambda/3}\,e^{\psi}$ refuses it a second time over. \emph{So there is no $(T,\text{chirality})$ character table there}, the two labels not commuting, where the lift's failure was a collapse to one of four. \emph{What the member does have is the split.} The operator is massless, so $\gamma^{5}$ decouples it at every momentum, and $\Sigma=\gamma^{0}\gamma^{1}$ commutes with a block exactly when the transverse momenta vanish; there the block is $-E+(k\mp b)\sigma_x$, the commutant inside it is spanned by $1$ and $\Sigma$, and \emph{both} $\Sigma$-values are occupied---the two branches of the dispersion. \emph{What it has not got is the pairing.} The two sit at different momenta at one energy, so a pairing must move $z$; of the sixteen reflections exactly four preserve chirality and flip $\Sigma$, only the two reflecting $z$ also preserve the energy, and both of those send $c\mapsto-c$. \emph{The only operations that could pair two states of one chirality at one energy are exactly the ones that flip the twist, and the twist is what makes the member chiral}---$c=0$ being the polarised, achiral cut~\cite{JanzenDynamics}. \emph{So the member can be chiral or doublet-paired and not both, its chirality and its pairing being carried by one field.} \emph{And the face where a continuous algebra could sit answers too, with a branching that is forced and nothing to carry it.}\rcpt{P14_the_compact_faces_four_is_three_plus_one_and_the_singlet_has_nothing_to_carry_it} The compact face is the round $S^{5}$'s isometry algebra $\so(6)$~\cite{JanzenBoundary}, which is $\su(4)$; and $\su(3)$ has no irreducible of dimension two or four, so its only faithful four-dimensional representation is $\mathbf 3+\mathbf 1$---\emph{a colourless index sits beside the colour triplet by force and not by choice}. \emph{Both branches close.} The spinor of $\so(6)$ is that $\mathbf 4$, and the round face has scalar curvature $n(n-1)=20>0$, so Lichnerowicz's identity leaves no Dirac zero modes to carry the singlet; the vector is $\mathbf 6=\mathbf 3+\bar{\mathbf 3}$, self-conjugate hence real hence vector-like, and carries no colourless weight at all. \emph{So the singlet appears in exactly the representation that face has nothing in.} \emph{And the two objects this construction calls colour are not one}: the holonomy group of the three wall monodromies with the hinge three-cycle is finite, of order $81$, hence zero-dimensional and flat by Ambrose--Singer, where a compact-face $\su(3)$ would be an isometry carrying a curvature. \emph{One consequence places a figure rather than adding one}: in the sky angle the $A_{2}$ roots fall on the zeros of $2M$---the hinge and wall angles---and the weights of $\mathbf 3+\bar{\mathbf 3}$ on the six Nariai marks, set for set, the triples split by the sign of $2M$. \emph{That placement is forced by the root structure already carried}, the zeros and extrema of $\sin 3w$ interleaving at thirty degrees for any three-fold structure whatever; and what would make the six a module rather than a weight diagram---the six-dimensional coset of $\so(6)$ mixing the two triples continuously---is absent, the construction offering a discrete exchange and a sign where a phase would be needed. \emph{And the finite group that holonomy carries is told from the algebra beside it by what it acts on.}\rcpt{P14_the_wall_holonomy_acts_on_the_seat_index_and_the_compact_face_has_no_loops} Its determinant-one part is $\Delta(27)$, generated by $\mathrm{diag}(1,\omega,\omega^{2})$ with the hinge three-cycle; \emph{it acts on the vantage basis}---the three signed areal radii, one per hinge, which are the chiral seats---\emph{and on the generation index not weakly but not at all}. The seat datum $2M=(2/3\sqrt3)\sin 3w$ carries no bead phase; the deck's sector period $2\pi\alpha/3$ carries no sky angle and no mass; and the deck leaves $\sinh^{2}$ invariant, hence $r$, hence every hinge. \emph{The two indices are functions of independent variables, so the group moving one fixes the other identically.} \emph{And $\sigma$ meets it on the same seats without entering it}: $\sigma$ is an involution where $\Delta(27)$ has exponent three, so it lies outside; read on the sky angle it carries each hinge to a wall, and since each wall is antipodal to its own hinge that is the transposition fixing one seat---so $\sigma$ normalises the group and the two together give order $54$, an extension at index two. \emph{What does not follow is an identification with the compact face's algebra.} A holonomy is a representation of the fundamental group of the space it is a holonomy on, and the round $S^{5}$ is simply connected, so every flat bundle there has holonomy of order one; a monodromy group needs a branch locus besides, and a homogeneous face has none where this one has the three zeros of $2M$. \emph{So the two structures this construction calls colour are not one object twice described}, and the reason is not that two embeddings differ: one is a $\pi_{1}$ representation, and the face carrying the other has no loops to have holonomy around. \emph{And the four-dimensional locus where the two faces meet answers too, with a twist it supplies itself.} \emph{The two faces meet at $X_0=0$ on a single $S^4$---the substrate's throat and the compact face's equator---and, being even-dimensional, it carries a chirality of its own: the grading by the Clifford element of its normal $X_0$}. That is not the physical chirality $R=\gamma^{5}$, which is the element of the cut's normal $r_0$; the two anticommute\rcpt{P14_the_shared_four_sphere_twists_itself_and_its_zero_mode_is_a_singlet}. Its spin connection is block diagonal in the sphere's own chirality, so its two blocks are the $\su(2)$ connections on the chiral spinor bundles outright, and their second Chern numbers are $\mp1$: \emph{the self-dual half of the sphere's own connection is the unit instanton, and the radius drops out}. \emph{So Lichnerowicz is evaded by the geometry rather than by an added field}, and the index of the Dirac operator twisted by that bundle is one---computed from the Chern number and again from the Betti numbers, with kernel dimensions $1$ and $0$. \emph{The zero mode is a singlet.} It is the constant function, and not because a doublet sits empty: the chirality carrying it decomposes as $\mathbf1+\mathbf3$ and contains no doublet at all, while the chirality that \emph{is} a doublet is $\Lambda^{1}$, which carries no zero mode because $b_{1}(S^{4})=0$. \emph{The shape is in the bundles and the state is in the singlet}, and the factor supplying the twist is the factor of which the zero mode is a singlet. \emph{Three boundaries belong with it, and the first settles whether either factor could be the weak one}: the factors are chiral with respect to the sphere's chirality, which anticommutes with the physical one, so a state of definite sphere chirality is an equal mixture of physical left and right and neither factor acts on a single physical handedness; they rotate the sphere's tangent space rather than an internal space; and a zero mode on a sphere with no time in it is an index statement about that sphere rather than a state in a spectrum. The causal reassignment carries a three of its own\rcpt{P14_the_reassignment_three}---the ruling bundle promoted to the fundamental timelike congruence, its conjugate read as synchronous flat space, and the at-rest worldlines reinterpreted as the photon congruence~\cite{JanzenCRframework}. \emph{These are congruences of the background}, they are neither of the two threes above, and the standing $R$-conjugation exchanges the first two while the photons stand outside that pair, which splits them $2{+}1$ with chirality's own grading. \emph{And they are not a structure apart from this sector}: the two rulings cross at a hinge\rcpt{P14_the_reassignment_three_at_the_hinge}, and that hinge owns the wall at which Proposition~\ref{prop:wall} binds---so the two are the in-leg and out-leg of one bead, and the exchange of them is the exchange of species at the turn between. \emph{That sharing is what gives the candidate any fermion content and what costs it the colour-freedom in the same move}: read as global bundles the three carry no graze-point index, and carry no built modes either; read as three directions at a hinge they have the bound mode to hand, and the graze-point index is the hinge's own. \emph{Neither reading is both}. The same three-plane structure records where that tension sits: the $\mathbb{Z}_{3}$-fixed axis is the one locus carrying no hinge index, and it is the one the figure says carries no wall. \emph{It carries none because it holds no real point of the substrate}\rcpt{P14_the_fixed_axis_is_not_in_the_substrate}: there the section reads $-X_{0}^{2}=\alpha^{2}$, the hinge triangle's centre being the centre of its incircle and the incircle the throat, so the axis runs through the hole. \emph{That is a statement about real points and not about absence.}\rcpt{P14_the_axis_vantage_is_imaginary_not_absent} The power of a point assigns every planar position a height, $X_{0}=\sqrt{\mathrm{pow}}$ with $\mathrm{pow}=|X|^{2}-\alpha^{2}$, which puts a hinge at $\sqrt{3}\,\alpha$ and the axis at $i\alpha$---imaginary, at the one invariant, and reached by the same continuation that reaches the equatorial seam. \emph{So the seat is at no real point of this manifold}, and the axis vantage is hinge-free by construction, its behaviour under the turnaround's deck being a separate question from its fixity under the rotation---the two threes are related by no covering construction, so neither settles the other. \emph{What the axis does settle is a count.}\rcpt{P14_the_axis_vantage_is_a_pair} The height rule is a square root, so the vantage there has the two branches $\pm i\alpha$, exactly as a hinge has $\pm\sqrt{3}\,\alpha$ and those two are its two horns. \emph{It is a pair, not a triple}---and the triple wanted is itself a doublet and a singlet rather than three symmetric objects. \emph{The singlet is then obstructed, and by a coincidence rather than an absence.}\rcpt{P14_is_there_a_singlet_for_the_axis_pair} A single branch requires $\mathrm{pow}=0$, since the root is two-valued elsewhere, and $\mathrm{pow}$ vanishes exactly on the throat circle; but the three walls lie on that circle, antipodal to their hinges, so the one unbranched locus is the one carrying the colour index. \emph{In this construction, being unbranched and being coloured are the same condition}---and $R$'s sole fixed point, the $r=0$ crossing, is a wall as well. \emph{That welding is forced, and by this paper's own dimension result.}\rcpt{P14_the_coincidence_is_forced_by_the_dimension} A wall is antipodal to its hinge and a graze point lies between two, so the two coincide exactly when a hinge's antipode is a midpoint---which for $n$ equally spaced hinges holds precisely when $n$ is odd. With the count at $D-1$ that is even $D$, and $D=4$ sits squarely inside it; the nearest separations are at $D=3$ and $D=5$, the odd dimensions this section's count and parity jointly exclude. \emph{So the missing singlet is the dimension result seen from the matter side}: the same fact that makes the cut four-dimensional welds unbranchedness to colour, and a successor that separates them is working in a dimension this one has closed. \emph{And one boundary on how the count may be read follows from the same arithmetic, since the temptation runs the other way.}\rcpt{P14_a_labelling_not_a_gauging} Chirality and species each split the four leptons $2+2$, but $SU(2)_L$ does not: it decomposes them as a doublet and \emph{two inequivalent singlets}, $2+1+1$, since it does not distinguish $\nu_R$ from $e_R$ at all. A grading by a one-dimensional character is a function to $\{\pm1\}$ and its blocks are its fibres, so it has at most two, and no \emph{two-valued} character count can produce three. \emph{The restriction is the premise and not a general fact}: a $\mathbb{Z}_3$ character takes three values, and this paper uses one---the centre of $SU(3)$, whose triality grading the quark/lepton distinction rests on below. 
...
\emph{We record that as a mismatch rather than smoothing it}: nothing this sector delivers rests on the clause, and the falsifiable direction is the useful one---a right-handed weak structure distinguishing $\nu_R$ from $e_R$ by an isospin-like $\mathbb{Z}_2$ would make the geometry right and this comparison wrong.
...
residue and not the choice of bridge. The ceiling is established above; the three routes are
enumerated and checked at the receipt, the $\mathbb{Z}_2$-complex identity and the residue's irreducible
dimensions computed there, and the grading-not-exchanging property taken from~\cite{JanzenBoundary}. We make
no claim about a fourth route, and none that the bridge does not exist. \emph{One consequence should be stated plainly, because a reader from the discrete-flavour literature will otherwise mis-place this sector.} That literature's own criterion is that more than one generation can only be explained by a non-abelian symmetry carrying two- and three-dimensional irreducible representations~\cite{IshimoriEtAl2010}, and in its models an accompanying $\mathbb{Z}_3$ is family-\emph{independent}, separating symmetry-breaking sectors while a Froggatt--Nielsen $U(1)$ shapes the hierarchy. This sector's threeness is not a horizontal symmetry of that kind at all: it is the \emph{deck} group of a covering, saying the three generations are one object read three ways rather than three components of a multiplet, and it is not broken and predicts no mixing. \emph{The exchange is a fair one and runs both ways}: this construction cannot claim the explanatory work those models do, and their standing difficulties are correspondingly not its own---it is tested by the count and the chirality, which are what it predicts. Nor is the non-compactness escape of~\cite{JanzenBoundary} in tension: the count is a wall-localized zero-mode index, not a bulk $\dS_5$ index, and localized modes are indifferent to the bulk's non-compactness. The localization is itself a statement in the leaf norm (Section~\ref{sec:chirality}): the fermion is a mode of the existent spatial leaf~\cite{JanzenCRframework}, and in that induced proper measure the horizons sit at finite distance and the modes are bound, whereas in the conserved spacetime Dirac norm the horizons are infinitely distant and the static mode does not normalize\rcpt{P14_dual_norm}. That the wall-localized index is well-defined thus rests on---and is consistent with---the layered ontology: the count is an object on the leaf, where CR locates the existent.

What CR's matter sector delivers, then, is precisely the Standard Model's discrete flavour skeleton: the number of generations, their chirality, and the family symmetry relating them---three, $\gamma^5$, and the turnaround's cyclic $\mathbb{Z}_3$, carried inside an $S_3$ whose transpositions belong to the isospin sector (\S\ref{sec:whichthree})---read as the maximally-symmetric slicing of a maximally-symmetric substrate.

\paragraph{What the matter sector dissolves.} Gathered, and on the same footing as the rest of the paper---\emph{forced within CR}, with the content (gauge representations, masses) external throughout---the discrete flavour skeleton dissolves a small family of the Standard Model's standing structural questions, each in the programme's now-familiar mode: not a mechanism supplied but a replication recognised. The \emph{generation-replication puzzle}---why the fermions come in three families, a number the Standard Model takes as an input---is, within CR, no free count but the fold of the one polynomial $f$: the three walls count it and the comoving turnaround's deck carries it (\S\ref{sec:whichthree}), both being $D-1$ for the same reason, so three is the maximally-symmetric matter construction's own index, not a datum to be fit~\cite{JanzenGeometricCore}. The \emph{origin of the family symmetry}---which flavour model-building posits as an added discrete group---dissolves by identity: the cyclic $\mathbb{Z}_3$ permuting the three generations \emph{is} the turnaround's deck action on the integral of $1-f$, the horizon three being the zero set of $f$ itself, one group and not two resembling one (\S\ref{sec:whichthree}). The \emph{gauged chirality}---carried in the Standard Model as a structural fact---is read here as the difference between being \emph{on} the substrate's waist circle and \emph{tangent} to it: the chirality parity is a substrate isometry and descends gauged, while the walls' $S_3$, a
monodromy symmetry of the solution space, stays global (Section~\ref{sec:twofactors}). And the \emph{matter/antimatter} pairing is that same orientation parity $R=\gamma^5$ carrying each wall-mode to its bound opposite-chirality partner, the discrete residue of the cosmogenesis bead's own $r=0$ crossing~\cite{JanzenBoundary}. Weighed by the programme's criterion of necessity~\cite{JanzenShadowExistence}, these are consolidations---one substrate's discrete structure where the Standard Model carries several independent inputs---but the footing is the narrowest in the corpus and is marked as such: each holds only \emph{within CR}, on the maximal-symmetry principle a framework may decline (declining it, the natural single-hinge index reads one, not three), and none touches the gauge or mass content that stays the ordinary route. The dissolution is a recognition of shape.
...
\bibitem{JanzenBoundary} D.~Janzen, \emph{The de Sitter substrate and the Standard Model: four converging routes to a geometric boundary on what one maximally-symmetric Lorentzian substrate's isometry does and does not force, and the framework and gauge on its two real forms}, companion paper (P13).
...
\bibitem{JanzenCircle} D.~Janzen, \emph{The Schwarzschild and de Sitter circle: the intrinsic geometry of one homogeneous ring---the event horizon and the curvature singularity at $r=0$ its two poles, run through by a single continuation}, companion paper (P2).
