...
the seam datum's own freedoms move the first peak by a factor of $2.26$, so its position is a
statement about a datum the construction does not fix rather than a prediction of it; and the
two-arm transfer that would carry the rest is repaired and validated on its control---which
