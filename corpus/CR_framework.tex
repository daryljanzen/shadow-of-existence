...
fixed is the driving history}: baryon loading displaces the oscillation's zero point, but how much
of the comb that displaces depends on the potential the modes swing against, and the standard shift
is universal only where every mode crosses during radiation domination, which a rate fixed by the
geometry does not provide\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}.  The
