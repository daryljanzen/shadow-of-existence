...
arm}: over the full-range lensed configuration the control returns $1.16$ in $\chi^{2}$ per bin and this arm $2.98$, the arm at $2.57$ times the control's distance from the data~\cite{JanzenCRcosmology}. \emph{The construction's acoustic spectrum is rejected on TT shape}, and what remains is an acoustic phase residual of $2.3\%$ and a fourth peak $0.9\%$ high, the comb itself
