...
the seam datum carries two freedoms, and how far the first peak moves with them---hence whether its
position is a statement about that datum or about the construction---is among the quantities the
transfer is run to measure; and that transfer is repaired and validated on its control, which
reproduces a standard code to $0.14\%$, and is not yet carried to convergence on this
construction's own arm~\cite{JanzenCRcosmology}.  \emph{What remains of this item is accordingly one
computation and not a further argument}: the wavenumber integral run to convergence on the arm whose
answer is not independently known, on the leaf congruence the rate rule assigns the perturbations to.
Until it is run the acoustic comparison is open, and the peak spacing, the acoustic phase and the
peak heights are quantities this construction does not here report.  The exact large-angle shape is now computed on a genuine Boltzmann transfer---a mild, non-discriminating low-multipole deficit consistent with the standard model within cosmic variance.
