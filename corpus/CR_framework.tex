...
Of the three below, the scalar perturbation sector (item~\ref{frontier:scalar}) is \emph{work}: it carries
its transfer and its damping physics and leaves a diagnosis, the acoustic phase misplaced by a computed
amount.  The quantum sector (item~\ref{frontier:quantum}) is \emph{work} with a handover inside it: the
ultraviolet definition of the tower sums remains, while the ordering has been shown external rather than
undecided, and an external datum is not this construction's to supply.  The matter sector
