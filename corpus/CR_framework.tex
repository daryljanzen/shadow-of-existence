...
\emph{What remains of this item is accordingly one thing, and it is neither the transfer nor the
likelihood}: a mechanism for a deficit that is measured. \emph{The likelihood is run, both arms scored on the
same bins by the same code}, and it separates them---the model-to-model distance is $122$ in $\chi^{2}$ per
bin against the control's own $0.11$ from a reference of its own kind, so the statistic arbitrates and is not
near the level at which it could not~\cite{JanzenCRcosmology}.  The exact large-angle shape is now computed on a genuine Boltzmann transfer---a mild, non-discriminating low-multipole deficit consistent with the standard model within cosmic variance.
