...
arm}: over the full-range lensed configuration the control returns $1.16$ in $\chi^{2}$ per bin and this arm $2.98$, the arm at $2.57$ times the control's distance from the data; refitting both arms' own parameters on the full range leaves it at $1.57$ times, and moves this arm's expansion rate and matter density by under a thirtieth of a per cent from where the baryon-acoustic distances put them~\cite{JanzenCRcosmology}. \emph{The construction's acoustic spectrum is rejected on TT shape}, and what remains is an acoustic phase residual of $2.3\%$ and a fourth peak $0.9\%$ high, the comb itself
being right to a part in six hundred. The joint fit leaves the damping-\emph{scale} signature as a Gaussian residual in $\ell$ that
no tilt removes.  \emph{What remains of this item is the phase and the fourth peak}: not the instrument
