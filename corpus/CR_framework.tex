...
$S_{3}$ family symmetry; and each generation is required to be anomaly-free \emph{on its own}, there
...
available from the geometric core}: raising the substrate raises the cut with it, and written at a
general dimension the hinge figure is a regular $(D-1)$-gon whose classical relations---the placement
$R=2\alpha$, the $60^{\circ}$ subtense, the throat as nine-point circle---hold at the equilateral
member and have no analogue at the square; while the mass-parity that grades chirality exists only in
even dimension~\cite{JanzenGeometricCore}.  \emph{So four is the only cut dimension carrying both a
generation count and a chirality}, and a raise taken to buy compact rank spends exactly the two
results this sector delivers.  \emph{The tower is therefore not a route this construction can take
and keep what it has}---which is a sharper statement than each rung being unforced, and it is why the
list above is a boundary rather than a stage.  \emph{The list is a join
