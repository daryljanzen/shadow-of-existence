...
P3 \; slicing & 3 & 7 & -- & . & 14 & 2 & 17 & 3 & 4 & . & . & 3 & 11 & 21 & 2 & 2 & 14 & .\\
...
\textbf{P17 \; core} & \textbf{2} & \textbf{2} & \textbf{25} & \textbf{7} & \textbf{10} & \textbf{4} & \textbf{17} & \textbf{6} & \textbf{6} & \textbf{10} & \textbf{5} & \textbf{13} & \textbf{20} & \textbf{22} & \textbf{11} & \textbf{3} & -- & \textbf{.}\\
...
to, and it returns a deficit: the first peak sits $6.6\%$ low in position, the peak heights fall below
the sky's, and the full-spectrum likelihood rejects the computed temperature spectrum on its
shape~\cite{JanzenCRcosmology}.  \emph{What is open is a mechanism for that deficit}, and it is narrower
for what is already ruled out.  The driving is measured and overshoots rather than falls short, so it
does not supply one.  The seam datum's readings trade position against the comb's alternation---the
sky's peak spacings alternate, $317.5$ then $271.7$, and of the readings placing the first peak within a
grid step of the sky's none shows the contracting second gap, while seven of the seventeen admissible
readings do contract~\cite{JanzenCRcosmology}---and no mechanism for that trade is in hand.  \emph{One control on it is
independent of the census and holds}: with the baryons removed both combs widen, this one more, and giving
it the comparison's no-loading comb with its own loading response returns the observed alternation, so the
loading itself is not what the difference is in. The potential's own evolution is
...
expanding leg inherits $\tfrac{9}{10}\Phi_i$ scale-invariantly~\cite{JanzenCRcosmology}.  What the potential and the photon perturbation both are \emph{at one locus} of the handover is
what the transfer's datum does not yet state, and it is where that mechanism would be sought.  The quantum sector (item~\ref{frontier:quantum}) is a
...
D.~Janzen,
\emph{Reached through the imaginary, real by construction: the maximally symmetric substrate as the geometric--ontological core}, companion paper (P17).
