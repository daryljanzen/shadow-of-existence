...
cause is identified, and the geometry that removes it is now in hand.  No grading available on the
...
on the polarised cut and reversed by the transverse parity~\cite{JanzenDynamics}.  \emph{So the
missing multiplet and the missing polarisation are no longer the same absence}, and what that
leaves should be stated exactly rather than as a closure.  The construction that builds the chiral
member computes no multiplet count---no wall kernel, no zero-mode, no triality grading---so what it
removes is the \emph{cause} P14 identified and not the shortfall itself.  \emph{The remaining step is
therefore a computation and no longer a geometry}: redo the wall construction on the unpolarised
member and read the grading there, which either exhibits the fifth class or shows the grading does
not separate the right-handed pair on that member either.
