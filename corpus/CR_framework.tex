...
% =====================================================================
% CANON NOTE — LIVE OPERATING PHILOSOPHY (added 2026-05-28)
%
...
% *** r1860 (PHASE 8, the OPTICS bake, probe O4): LAMBDA IS IN THE MEASUREMENT, NOT THE PATH.
...
% *** r1859 (PHASE 8, the OPTICS bake, probes O1+O5): THE DEGENERACY HAS AN OPTICAL FACE.
...
%   existence is UNTOUCHED; they merely have no ontological interpretation — landed (r1278+) as a
...
%   landed (r1278+) at full weight as PERSPECTIVAL SHADOW CURVATURE: GR's central quantity (spacetime
...
%   stipulation"), dynamics unchanged — and landed (r1278+) as a STRUCTURAL DISSOLUTION of GR's
...
%   ontological ambiguity" — landed (r1278+) at FULL WEIGHT as a THIRD POSITION: neither substantivalism
...
%   *** NEW APPLICATIONS (r1278+), same axis (named GR problems, only-conjectural patches, dissolved
...
%   daughter cosmology (macro data = progenitor (M,J,Q) no-hair §783; fine-grained carried by the unitary
...
%   ALSO CARRIED FORWARD into the now-comprehensive applications section (r1278+): the remaining P1
...
%   not free-standing coherence. Synthesis now runs cross-links BOTH WAYS (back to P1/P4/P6, forward to
%   cosmology/cosmogenesis/matter).
%   THE CLUSTER now spans ~two roots — manifold-as-existent (problem of time, hole argument, CTCs, + the
...
%   SUBSECTION LANDED (sec:applications-synthesis, r1278+): gathers the cluster, names the two roots + the
...
%   CONSOLIDATION, not just a certification. STILL OWED: urgent propagation of the cluster into P6 itself. ***
...
%   already cites it in prose (§125, §222, both \cite{JanzenBHcausality}); the grounding is §1c. When
...
%   present), synchrony's relativity the shift. This paper cites it at §222 (\cite{JanzenModernParallax});
%   the grounding is §1d. When the cosmic foliation / preferred frame / objective present is drawn on,
...
%
% Contamination audit completed 2026-05-28 against Beyond Space-Time
% (Janzen 2025) as the authoritative source for the framework's
% ontological position. Fourteen prose passages were corrected in
% that pass:
%   - Abstract: "representational framework" → "layered geometric
%     framework"; "augments GR's representational structure" →
%     "structural augmentation... specifying the ontological structure"
%   - §Layered Framework occurrence remark: dropped "purely"
%   - §Layered Framework cosmic-time remark: "augmentation of the
%     representational structure" → "ontological commitment that GR's
%     formalism admits but does not supply"
%   - §FLRW projection closing: synchronous/non-synchronous as
%     "representational choices rather than ontological distinctions"
%     → projections differ in representational structure while
%     preserving ontological layering, with forward reference to SdS
%     selection
%   - §Metric reassignment: "representational change of causal roles"
%     → "implements a reassignment"; "representational structure
%     through which observables are read" → "causal projection of the
%     underlying ontology that the metric encodes"
%   - §Null-boundary correspondence preamble and remark: dropped
%     "purely geometric and representational," "without asserting any
%     physical generative relation," and "No physical generative claim
%     is implied" — first-pass replacement was an acknowledgment that
%     "cosmogenesis is a question raised but not addressed here";
%     subsequent second pass (A6-v2, A9-v2, 2026-05-28) recognised
%     that hedge still backed away from the theorem's actual content
%     (property (iv) of the theorem establishes that the horizon
%     event-of-events and the SdS cosmological initial slice ARE the
%     same ontological layer, not separately related). Both passages
%     now state the structural identification explicitly, with the
%     cosmological expansion law on the post-horizon side determined
%     by the SdS construction as flat ΛCDM ($\sinh^{2/3}$), and the
%     genuine open question reframed as the mechanism of transition
%     for matter and observers crossing the shared ontological layer.
%   - §Null-boundary horizon-orientation remark: "natural
%     representational outcome" → "natural ontological outcome" (twice)
%   - §Synthesis closing: "what CR adds is the representational
%     structure" → "what CR adds is the ontological structure"
%   - §Synthesis Nariai sentence: "natural representational outcome"
%     → "natural ontological outcome"
%
% The single sentence removed earlier in the day ("CR is, accordingly,
% a representational framework rather than a competing theory of
% cosmology...") was the first corrective. The fourteen above complete
% the contamination cleanup against the prose body.
%
% The technical apparatus (axioms, definitions, theorems,
% propositions, the §Metric reassignment section's "representational
% freedom" technical term where the contrast with the ontological
% layer is doing analytic work) was not touched. Some uses of
% "representational" survive intentionally — where the term is used
% in legitimate contrast with "ontological" to do analytic work
% rather than to soften an ontological commitment to a representational
% one. Future passes should leave these alone unless a specific
% structural concern arises.
% =====================================================================
...
\begin{remark}
In CR, spacetime curvature is a property of the projection, not of the underlying ontology. Ontological curvature is encoded exclusively in the intrinsic geometry of $h_{ij}(t)$. A detailed discussion of how distinct Lorentzian metrics on the same manifold may encode different causal assignments while preserving the underlying foliation is provided in the fixed manifold representational freedom and metric reassignment section below.
\end{remark}
...
\emph{And it cuts, which is why it is worth stating rather than leaving as a silence.} A measured Page curve would not be a difficulty for this reading to absorb: the flux whose entropy it tracks is denied here, so observing that entropy rise and turn over would falsify the denial and with it the resolution built on it. \emph{We state the falsifier and not a prediction}: what would distinguish the readings is whether the flux exists at all, which is this paper's own named frontier---the mechanism of the crossing, not its unitarity---and not the shape of a curve neither reading disputes the meaning of.

\emph{And the lift raises, then settles, the obvious objection.} Across it the acoustic modes of the radiation are suppressed by $e^{-kc_s\lvert\Delta\eta\rvert}$---$e^{-152}$ at the first peak---which reads at first sight like loss. \emph{It is not.} On the lift $\operatorname{Re}\tilde\tau$ does not advance: \emph{no proper time elapses there}, so nothing dissipates over an interval. The factor is the mode function's value at the far end of an \emph{analytic continuation}---the imaginary-time segment of one closed contour, the same object read at another point of itself, not a state degraded by evolution. What the continuation selects is which content is \emph{carried} to the expansion leg: the frozen, non-oscillatory part passes untouched; the oscillatory part is not represented there. \emph{No information is destroyed on the realised spacetime; a basis is selected on it.} What remains open is therefore not \emph{whether} the information is preserved---the unbroken unitarity settles that---but the detailed matter dynamics of the branch-point crossing itself, the programme's one named frontier (\S\ref{sec:frontiers}): the mechanism of the crossing, not its unitarity. The black hole thus does not end as a thermal remnant whose purity must be explained; it becomes a universe, by an evolution that never leaves the unitary, globally hyperbolic setting the layered ontology's realised spacetime always occupies.

\subsection{The Laws of Black-Hole Mechanics}\label{sec:CR-mechanics}

The laws of black-hole mechanics---the constancy of the surface gravity over the horizon, the first law $dM=\tfrac{\kappa}{8\pi}\,dA+\Omega\,dJ+\Phi\,dQ$, and the area theorem that the horizon area never decreases~\cite{Bardeen1973,Hawking1971}---are, like the singularity theorem, correct results whose object is a \emph{realised} event horizon carrying a definite area and surface gravity. That object is never instantiated on a finite cosmic layer: the horizon occurs only as the metric singularity of the infinite-time boundary, and no finite layer carries the area whose monotonicity the area theorem asserts~\cite{JanzenBHcausality}. The laws therefore characterise the auxiliary completed geometry, not the realised worldtube---the classical, area-side companions of the horizon-induced Hawking temperature already set aside above; and the Bekenstein--Hawking entropy~\cite{Bekenstein1973}, read as the entropy \emph{of} that horizon, shares their status. What content survives for a perpetually collapsing ultra-compact body---as with the local particle production of the preceding subsections---is not settled by this reading and is not claimed here; the point is only that the horizon-thermodynamic apparatus, area law and entropy alike, has on a finite layer no realised horizon to be defined on, exactly as its temperature has none. The layered ontology thus reads black-hole thermodynamics as the thermodynamics of an idealisation---the completed geometry the collapse asymptotically approaches but never, on any existent layer, becomes.
...
\]
with $S^2$ the cross-sectional sphere and $\mathbb{R}_{\ge 0}$ parametrizing the null ordering along generators. This structure will play a central role in the correspondence established below.
...
whose fibre is the roots of $r^{3}-\alpha^{2}r+2M\alpha^{2}$ with deck $S_{3}$, and the comoving turning as a
...
Two scope notes. This concerns the relation between these two three-folds and says nothing about the separate question, walled elsewhere, of a realised colour isometry on the Lorentzian substrate, $\mathfrak{su}(3)\not\subset\mathfrak{so}(5,1)$, or the world-correspondence held do-not-assert there~\cite{JanzenGroupoid,JanzenBoundary}. And that the exchange occurs at $E=1$---the flat leaf $k=0$, the member the observed cosmology selects~\cite{JanzenOperator}---is recorded as a fact about the family, with no significance claimed for the coincidence.
\end{remark}
...
decreasing---stands at $2^{1/3}r_{\mathrm{HE}}$}, the two differing by exactly the cube root of two. \emph{Two of the readings are the companion epistemic paper's and are not repeated in the list above}---the
...
\section{The lift, and the explanation of the initial rate}\label{sec:lift-initial-rate}
...
\begin{tikzpicture}[font=\scriptsize,>=stealth,
  ...
  \draw[dep] (P1) to[bend right=12] (P3) node[wt,pos=0.6]{2};
  \draw[dep] (P1) -- (P5) node[wt,pos=0.4]{3};
  \draw[dep] (P1) to[bend left=18] (P6) node[wt,pos=0.7]{6};
  \draw[dep] (P2) -- (P3) node[wt]{6};
  \draw[dep] (P2) -- (P5) node[wt,pos=0.4]{9};
  \draw[dep] (P3) -- (P5) node[wt]{37};
  \draw[dep] (P4) to[bend right=14] (P5) node[wt,pos=0.12]{8};
  \draw[dep] (P4) -- (P6) node[wt,pos=0.6]{4};
  \draw[dep] (P5) -- (P6) node[wt]{3};

  % --- all six feed the framework P7 (the compounding) ---
  \draw[feed] (P1) to[bend right=72] node[wt]{32} (P7);
  \draw[feed] (P2) to[bend right=52] node[wt]{7} (P7);
  \draw[feed] (P3) to[bend right=32] node[wt]{16} (P7);
  \draw[feed] (P5) to[bend right=8]  node[wt]{15} (P7);
...
\end{tikzpicture}
\caption{The foundational structure the central theorem rests on (edge labels are citation multiplicities, a proxy for load-bearing weight). The construction stands on \emph{two co-equal foundational forcings, neither depending on the other}: the wedge P1---a \emph{structural} forcing (the event horizon as a metric singularity, local causal structure, proven from general relativity alone)---and modern parallax P4---an \emph{empirical} forcing (the redshift-isotropy floor, precision cosmology and its structural consequences). Their cross-references are \emph{complementary, not dependent}: each cites the other precisely to record that its own result ``stands on causal structure alone and needs none of'' the other's evidence~\cite{JanzenBHcausality,JanzenModernParallax}, P4 the empirical counterpart of P1's conceptual result. From the structural root the geometry chain descends (P1$\to$P2$\to$P3$\to$P5), but the structure is a \emph{dense weave, not a chain}: most papers feed several downstream directly (P2, for instance, into P3, P5, \emph{and} P7), and \emph{all six} feed the framework P7 (F1--F6 of Theorem~\ref{thm:augmentation-p7})---the compounding that makes the corpus a convergence rather than a line. Every dependency edge is drawn; blue arrows are inter-paper dependencies, grey the six feeds into P7, the dashed link the P1--P4 complementarity. P7 leans heaviest on its two foundations, the structural forcing (${\times}32$, reaching P7 both through the chain and directly via the horizon's null structure the null--boundary correspondence uses) and the empirical forcing (${\times}18$). The framework is the convergence of two independent forcings, structural and empirical, of one cosmic foliation. [Full direct feed into P7: P1~${\times}32$, P4~${\times}18$, P3~${\times}16$, P6~${\times}16$, P5~${\times}15$, P2~${\times}7$.]}
...
\hline
P1 \; wedge & -- & 5 & 2 & $\lozenge$2 & . & 2 & 10 & . & . & 2 & 2 & . & . & . & 4 & 2 & 2\\
P2 \; circle & 16 & -- & 28 & 2 & 2 & 4 & 11 & 5 & 4 & . & . & 1 & 3 & 4 & 1 & 3 & 6\\
P3 \; slicing & 3 & 7 & -- & . & 15 & 2 & 20 & 3 & 4 & . & . & 4 & 11 & 24 & 2 & 3 & 16\\
P4 \; mod.\ parallax & $\lozenge$4 & . & . & -- & . & 5 & 8 & 3 & 3 & 1 & . & . & . & . & . & 2 & 1\\
P5 \; groupoid & 3 & 10 & 41 & 8 & -- & 3 & 20 & . & 5 & . & 1 & 6 & 7 & 5 & . & 3 & 4\\
P6 \; shadow & 5 & 4 & 6 & 5 & 3 & -- & 11 & 1 & 1 & 3 & . & 1 & 1 & 4 & 3 & 3 & 3\\
\textbf{P7 \; framework} & \textbf{34} & \textbf{14} & \textbf{25} & \textbf{18} & \textbf{23} & \textbf{18} & -- & \textbf{15} & \textbf{11} & \textbf{18} & \textbf{8} & \textbf{12} & \textbf{17} & \textbf{21} & \textbf{28} & \textbf{12} & \textbf{13}\\
P8 \; operator & 2 & 6 & 8 & 1 & 2 & 4 & 15 & -- & 6 & 3 & 2 & 2 & . & 2 & 3 & 4 & 4\\
P9 \; range & . & 1 & 3 & . & 2 & 1 & 11 & 9 & -- & . & 6 & 3 & 3 & 1 & 1 & 3 & 3\\
P10 \; canon.\ time & 7 & 1 & 3 & 4 & 1 & 3 & 18 & 4 & 2 & -- & 2 & 2 & 1 & . & 1 & 2 & 8\\
P11 \; dynamics & 3 & 1 & 5 & 1 & 3 & 1 & 10 & 4 & 12 & 6 & -- & 4 & 4 & 2 & 2 & 2 & 3\\
P12 \; algebroid & 3 & . & 10 & 3 & 7 & 2 & 4 & 6 & 11 & 5 & 4 & -- & 2 & 4 & . & 2 & 7\\
P13 \; boundary & 1 & 1 & 8 & 1 & 5 & 2 & 13 & 5 & 5 & 6 & 2 & 14 & -- & 29 & 3 & 3 & 8\\
P14 \; matter & . & 3 & 16 & . & 4 & 3 & 10 & 3 & 1 & 1 & 2 & 8 & 21 & -- & 1 & 2 & 9\\
P15 \; cosmology & 4 & 2 & 8 & 8 & 3 & 4 & 17 & 11 & 4 & 4 & 3 & 1 & . & 1 & -- & 16 & 4\\
P16 \; cosmogenesis & 2 & 3 & 2 & 1 & 1 & 3 & 11 & 7 & 2 & 3 & 1 & 1 & 4 & 3 & 11 & -- & 2\\
\textbf{p0 \; core} & \textbf{2} & \textbf{2} & \textbf{24} & \textbf{7} & \textbf{8} & \textbf{4} & \textbf{16} & \textbf{6} & \textbf{5} & \textbf{6} & \textbf{3} & \textbf{12} & \textbf{20} & \textbf{14} & \textbf{10} & \textbf{3} & --\\
\end{tabular}%
}
...
% Rebuilt r895 (G9): 17x17, new numbering, aliases for p0/17's labels (JanzenCausality->P1 etc.) folded in. Every paper cites p0 & P16; p0 cites every paper incl. P16. Two-hub (P7,p0) reciprocity complete.
% Numbers REFRESHED r926 (c37) against the completed Knows-Itself both-ways closure (r896-r923 added forward-pointers/citations the r895 build predated). Authoritative alias-resolved counts recomputed from source; see scripts/depmatrix.py (canonical resolver in BIBKEY_ALIAS_MAP.md). Maintained from here: re-run after any citation change.
% REFRESHED AGAIN r968: recomputed via depmatrix.py after r927-r968 citation changes (DESI/damping edits grew P7->P15=19, P15->P16=10, P16->P15=9; the r968 P-symbol footnotes grew P3->P5=10, P3->P13=5, P6->P5=3; plus P6->P15, P8->P11, P15->P4). Fig:dependency-structure feeds/edges harmonised to match (P1 feed 20->21, P6 feed 7->8, P4->P5 edge 7->8, P5->P6 edge 2->3).
\end{table}
...
The single datum Theorem~\ref{thm:cosmogenesis} does not fix is the \emph{irreducible} interior remainder---the Kerr-inner and Reissner--Nordstr\"om-interior reassignments---which lie across the matter side and are gated on the branch-point crossing dynamics of a concrete matter model (\S\ref{sec:frontiers}). Corollary~\ref{cor:nonspherical} sharpens this into the programme's immediate priority: the exterior correspondence being symmetry-independent, what remains is not a wider class of collapses but the matter dynamics of the one crossing, which the classification isolates as the productive edge.
\end{remark}

\begin{remark}[Scope]
The two theorems are a necessity and sufficiency \emph{internal to the framework's foundations}: given (F1)--(F6), the CR augmentation is the required completion for a description of structural existence (Theorem~\ref{thm:augmentation-p7}), and on it collapse must continue as a cosmology (Theorem~\ref{thm:cosmogenesis}), with (F5) the empirical anchor of the necessary half. What is \emph{not} claimed is that the observed cosmos matches that cosmology in every measured detail; that soundness is held to the two tests the programme keeps open---the structural test of the metric-singularity result, that closed trapped surfaces do not form and collapse does not complete in finite cosmic time~\cite{JanzenBHcausality}, and the empirical test of the cosmology~\cite{JanzenCRcosmology}. Self-consistency is not soundness~\cite{JanzenShadowExistence}.
\end{remark}
...
% See THE_OPEN_PROBLEMS_CENSUS.md. Items migrate OFF as they are worked (the reducible-sector
% classification already has). Goal: this list stays a handful; nothing dangles "open" anywhere else.
% ★ EARNED, NOT INHERITED (r1243, Daryl): each item HERE is itself a suspect until the whole-corpus check
% confirms the corpus works it nowhere. Do not treat this list as settled-open. r1243: the "quantum
% completion" item was checked, found STALE (canonical-time §250 closes the quantization ambiguity at every
% order of the coupling by the horizon thermal state; classical nonlinear regime settled), and reframed to
% done — what remains is the ordinary interacting-QFT problem, not a CR open.
% ⚠ CORRECTED r1609: that r1243 verdict was REVERSED at r1291 (Daryl-flagged) — the interacting-tower
% remainder is a GENUINE open, ledger family 8, and is carried as enumerated item 6 below. The reversal
% reached the enumerated item but not this comment; the r1243 note is kept as the record, corrected here.
% ★ THE LAW (r1244, Daryl; count corrected r1609): the ONLY opens the corpus may carry, anywhere, are the
% six items in this section or a genuinely new miss; every other "open / remains / not yet / deferred /
% frontier" is a false-open, struck with the owed reference to where it is worked. Those six, every mention of each across
% the corpus (the clue-map to each resolution), and the kill list of struck false-opens are tracked in
% THE_OPEN_PROBLEMS_LEDGER.md — the arsenal's product, to conclude with an attack manual per problem.
% ★ SIX HERE, SEVEN LIVE — and the difference is by design, not a shortfall (stated r1609, so the count
% cannot be read as one). The ledger carries EIGHT numbered families, of which item 2 (sheet-to-ruling) was
% KILLED r1280, leaving SEVEN live: 1, 3, 4, 5, 6, 7, 8. Six of those are corpus gaps and are enumerated
% here. The seventh — family 7, the world-correspondence and the empirical grounding of the discipline — is
% NOT a corpus gap but the standing empirical question (coherence is not soundness), and the ledger records
% its home as "no sec:frontiers item (standing empirical, correctly absent)", carried instead at P6
% §199-209 and in the scope remarks of this paper. So: do not restore a seventh item here; a count that
% reads this section short a family has counted live families against corpus gaps, which are not the same set.
% ★ THE BINARY — KILL, or a §972 FAMILY MEMBER (r1247-48, Daryl): every apparent open has two fates only.
% KILL: resolve it dead in place. Otherwise it BELONGS TO A FAMILY: the main §972 statement of an open and
% its every local instance across the corpus are ONE thing (instances catalogued in the ledger, feeding back
% to refine the statement) — that is the whole "open" side, not two options. A genuinely new open family the
% list does not yet hold becomes a NEW §972 item (8th, 9th, ...), growing the list. No third option: "park"
% (a thing left off every family, killed by neither door) is forbidden. Everything open is a family member;
% everything else is killed.
\section{Frontiers and open problems}\label{sec:frontiers}

The structural closure above is what the framework establishes; what it does \emph{not} yet build is, by the same token, the map of where the programme goes next. Each item below is a frontier the construction has carried to a definite edge, listed plainly so it can be worked rather than deferred. \emph{The list is meant to shrink, and it has}: it opened at seven and stands at four. Item seven resolved and was moved below the list; items one and three---the matter branch-point crossing dynamics and the classification of causal reassignments---have since done the same and are recorded there. Of the four that remain, the scalar perturbation sector says inside itself that the bulk of its content is settled, so what it names is a stated remainder rather than the original frontier. \emph{Where an item has been overtaken, the item says so and says by what}, since a frontier list whose entries quietly empty is worse than none: a reader uses it to choose what to work.
\begin{enumerate}
%  r2378 REGISTER ALIAS, re-applied r2393 after absorption: the inherited datum -- register `L-150`, family 3.  The one ID space is the register in
%  THE_LIVE_ARC; this list is the paper's own voice and is NOT rewritten into register voice --
%  the alias lives in a comment so a reader meets the physics and a node can still trace what is
%  owed.  Fold code: L-150.
\item\label{frontier:inherited} \emph{Inherited boundary data, derived rather than measured.} The acoustic scale is met at the directly measured $H_0$ by a single inherited datum, the branch-point radiation amplitude $\rho_r/\rho_m\approx2$ (the structural analogue of the baryon-to-photon ratio), so it stands as a one-parameter accommodation rather than a parameter-free prediction. \emph{The datum proper is the onset redshift, which is $H_0$-independent; the ratio is its restatement in an imported density, so what a derivation must land is an order-unity number in the band $1.7$--$2.0$ and not the figure two to two places.} The open work is the baryogenesis-analogue derivation of the progenitor handover---the composition, and the primordial amplitude and tilt $A_s,n_s$~\cite{JanzenCRcosmology}---with the consistency target the measured primordial abundances (deuterium, the helium-4 fraction, the near-zero metallicity of the oldest systems). The composition half of that target is carried in the cosmogenesis paper~\cite{JanzenCosmogenesis}: the progenitor handover heats the infalling matter above the deuterium bottleneck and re-expands through it at the standard rate, so the cooling leg is a standard nucleosynthesis that \emph{produces} helium-4 and deuterium at their observed values and shares the standard lithium problem---the light-element consistency target met, the derivation of the single inherited datum remaining the open work. Like $\eta$, the datum may remain empirical at no cost to the dissolution; deriving it is the eager target. \emph{The frontier is two data and not one, and the papers should be read together on this.} The handover
...
maximal compact is six-dimensional, while the non-compact form founders on the quaternionic structure
...
declares the representation content---which species, in which multiplets, with which masses---undelivered and
walled~\cite{JanzenMatter, JanzenBoundary}. \emph{That undelivered content is exactly what stands between the
...
\emph{We claim no derivation here and record only the identification}: the eager target of this item and the
walled route of the matter sector are not separate debts.  \emph{Two constraints on that derivation are now established, and both narrow it rather than advance it.}
...
\item\label{frontier:scalar} \emph{The scalar perturbation sector, to a verdict.} The cosmology's completion~\cite{JanzenCRcosmology} carries its remaining open edges: the seam-to-recombination transfer has now been run~\cite{JanzenCRcosmology}: the collapse-phase driving envelope is derived in closed form and is flat above ten times the seam's horizon wavenumber, with its turnover at the equality the inherited datum fixes, so the peak heights verified through the third peak continue to track; the diffusion length and sound horizon follow on the same footing, and the damping-scale signature is settled within the transfer as a parameter-free ratio rather than an entangled estimate. Both of the items formerly named here have since been run, and the result is a disagreement rather
than a closure.  The full-spectrum likelihood comparison against flat $\Lambda$CDM returns
...
$\tfrac34=\tfrac14+\tfrac12$ rather than at a chosen value~\cite{JanzenCanonicalTime}.  The
decomposition is untouched by which branch holds, since it uses only that both sides of the
threshold are occupied and both orderings occupy both sides; the physical content is not.  So what
is open here is narrower than an undefined theory and sharper than a convention: asking which
ordering is asking whether the graviton tower's zero-point energy gravitates at the horizon, which
is the cosmological-constant problem arriving from inside this construction rather than imported
into it.} This is the same wall any interacting quantum field theory meets, and the results established here---the classical dynamics, the deparametrized unitary evolution, and the parameter-free closure of the quantization \emph{ambiguity}---stand independent of its resolution. But the shared character of the wall does not settle it: the quantization ambiguity this framework fixes and the interacting theory's definition it does not are two different things, and the latter is a genuine open frontier of the programme's quantum sector, carried as such and to be worked.

\emph{Two things have since been settled inside that mapping, and they cut opposite ways.  The
confinement condition the residue defines has been tested against the observed hadron spectrum
and agrees on eleven of eleven---meson, baryon, antibaryon, tetraquark, pentaquark and dibaryon
...
Standard-Model input~\cite{JanzenMatterSector}.  Against that, the content count fails, and the
failure is now bounded: the twelve coloured states factor into four classes where the Standard
Model has five multiplets, the shortfall being precisely one pair on the right-handed side.  The
cause is identified.  No grading available on this geometry can act on one chirality eigenspace
and not the other, because the geometry constructed is the polarised---and so achiral---member of
its own range; the unpolarised member, which carries the second propagating mode and with it
handedness, is reachable and has not been built.  \emph{Until it is, the missing multiplet and
the missing polarisation are the same absence, and that build is the concrete next step this
frontier names rather than a general aspiration toward the Standard Model.}}

\end{enumerate}

\paragraph{Two more have left it since, and for opposite reasons.} \emph{The matter branch-point crossing dynamics} stood as item one. It is now answered in both halves, and the halves answer each other. On the field side the segment's selection rule $e^{-kc_s\lvert\Delta\eta\rvert}$ has two factors, and reading it at fixed $k$ rather than fixed $c_s$ makes it a criterion on \emph{species}: a pressureless component has $c_s=0$ identically, so its exponent vanishes at every wavenumber and its crossing is exact rather than adiabatic, while at the acoustic peak the exponent is of order $10^{2}c_s/c$. \emph{Read as a selection rule on species, that was too strong, and the correction is entered here at r2376+c54.162.} The kernel acts on \emph{oscillatory} content, and on the progenitor's own interior no content arrives oscillating: $|aH|$ diverges as $1/x$ at the branch point while $c_s$ saturates at $1/\sqrt3$, so $c_sk/|aH|\to c_skx\to0$ for every $k$, and every mode in the observed range freezes strictly before the crunch---the highest, $\ell\simeq2475$, with a fifth of a per cent of the collapsing leg still to run~\rcpt{P16_every_mode_is_frozen_at_the_crossing}. \emph{So the exponent has nothing to act on, and the crossing is lossless for every species rather than for cold ones alone.} That is the premise \S\ref{sec:lift-quantum}'s fixed-point statement always needed and never had; it also removes a selection rule this paper advertised, which is a loss and is recorded as one. On the worldline side there is nothing to compute: a comoving worldline reaches the branch point at finite proper time with divergent curvature and divergent tidal stretch and terminates there, and what continues is the analytic continuation~\cite{JanzenCircle}. \emph{The two are one geometry read on two clocks}---the segment has zero cosmic duration and finite conformal length $3.32\alpha$---and the asymmetry is why the crossing is lossless for content and fatal for bodies. What the closure also corrected is the description this list carried of the locus: the curvature there diverges, and what is universal is the \emph{approach}, whose Kretschmann in the faller's own proper time is $M$-free.
...
\paragraph{One item has left this list, and its content stays.} \emph{The structure of the lap, and
the physics of the lift} stood as item seven until it resolved; the positive account is
\S\ref{sec:lift-initial-rate}, and the record of how the question closed---the three negative
characterisations the item named, and the positive results that replaced them---is kept with the
programme's working record rather than here, since a frontier list is a map of where the work is and
an item that has been answered is no longer part of that map.
