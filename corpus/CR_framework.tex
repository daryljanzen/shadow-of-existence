...
perturbation sector (item~\ref{frontier:scalar}) carries its transfer and its damping physics, and the
acoustic phase it misplaces by a computed amount is \emph{diagnosed}: not the seam treatment, not the
transfer, and not the geometry the transfer runs on, but the \emph{driving}---and the driving because the
standard shift is universal only where every mode crosses during radiation domination, which a
radiation-free rate does not provide\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}.
\emph{So the offset and the resolution of the Hubble tension are one fact}, and what remains open there is
the coupling's microphysics rather than the location of the discrepancy.  The quantum sector (item~\ref{frontier:quantum}) is a
