...
\emph{Of the three below, exactly one is work.}  The scalar perturbation sector (item~\ref{frontier:scalar}) is that one: it carries
its transfer and its damping physics and leaves a diagnosis, the acoustic phase misplaced by a computed
amount.  The quantum sector (item~\ref{frontier:quantum}) is a
