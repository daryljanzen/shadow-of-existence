...
%   not of WHY the cut bends as it does... the deepest open question the construction raises" (P11 exhibits
%   the confined dynamical case). See also [†ONT-COSMO] §1b, [†ONT-GEOM] §1f, [†ONT-CR] §1e.
...
the mass-dependence of the invariant tracks the slicing being read, not the geometry read through it. The flat synchronous space is the second null ruling of the hyperboloid (the horospheres normal to the common past asymptote), so the lapse-shift, the synchronous slicing, and the second ruling are one object, and the $t=-\infty$/$r=0$ singularity is the artifact of the comoving reading laid over a throat that is the substrate's defining constant $\alpha$. Two clarifications follow. The seam is crossed at \emph{finite proper time}---the ``$t=-\infty$'' is the synchronous coordinate, not the proper cosmic time, whose elapsed value from the seam is the ordinary convergent flat-$\Lambda$CDM age---so taking the chart's $-\infty$ for the physical age is the time-face of the same category error whose radius-face is taking $r=0$ for the source. And the seam is not merely this universe's beginning: being the same species as the event horizon, it \emph{is} one---read inward the future horizon onto which a collapse in the antecedent universe asymptotes, read outward this universe's cosmological beginning, the single $E=1$ geodesic dust collapse read inward and dust cosmology read outward. The flat leaf is the marginal ($E=1$) member of the substrate's three constant-curvature slicings---closed ($E<1$, $S^{3}$), flat ($E=1$), open ($E>1$, $H^{3}$), the full FLRW family $k=+1,0,-1$ at $-k=E^{2}-1$, each carrying pressureless dust through the same bend---the observed cosmology the flat member and the trichotomy over the spatial curvature of the leaf, never the signature of the spacetime. Read inward, the closed member is the cycloid the companion reads as the black-hole \emph{interior}~\cite{JanzenCircle}, so the collapse/cosmology identity holds at the closed energy too---the black-hole interior the inward reading of a closed universe. The structural reading---covariance lifted one level, the de Sitter substrate held invariant under change of geometry as general relativity holds one geometry invariant under change of chart, with the slicing curve the gauge object---is adopted on the strength of these results. The central constructions are established and verified at the stated scope, which is the static, spherically symmetric and spherical-cosmological sector. The range over the rest of general relativity, posed here as the principal open problem, is answered in the companion range paper~\cite{JanzenRange}; and the emergence of the bend (dynamical matter) is since advanced---the confined dynamical case exhibited and the crossing of matter through the cosmogenesis branch point shown well posed (\S\ref{sec:open})---with the general dynamical problem stated as open. \emph{One consequence of the data-separation is worth stating here, since it is cited outside this paper.} Because the energy density is a functional of the leaf alone, a causal reassignment acting on the stacking cannot remake it---the structural transition law of the cosmogenesis account rests on this proposition and on nothing else. And the handover this paper locates, from the sub-marginal bound orbits to the marginally-bound congruence, sits at the radius $r^{3}=M\alpha^{2}$ at which the areal acceleration changes sign, the same radius at which the slicing surface's intrinsic curvature is flat: \emph{one radius, read as a handover, as a flat locus, and as the turn from deceleration to acceleration}. Read through the operator, the boundary between local boundedness and cosmic expansion is one Schwarzschild--de~Sitter geometry at two energies: a structure stays bound only within the Hubble--Eddington radius $r_{\mathrm{HE}}=(M\alpha^{2})^{1/3}$, the handover to the marginally-bound $E=1$ congruence that is the cosmology.
\end{abstract}
...
the arrows as well as the objects\rcpt{K23_functor_and_quotient}. Three properties follow, and each is a
...
of a geometry in the image is induced by a substrate isometry fixing its cut. That equality is stated for the strata tabulated and not in general, and the reason is worth
...
\emph{A second and independent support for the same conclusion has since been given, and the two should not be
...
Two clarifications follow, both established by the foregoing but obscured by the projection coordinate. First, the seam is crossed at \emph{finite proper time}. The ``$t=-\infty$'' above is the synchronous coordinate $\tau$ in which $\eta(X,B)=e^{\tau/\alpha}$, not the proper cosmic time; the proper time elapsed from the seam ($a\to0$, i.e.\ the throat $X=\alpha$) to any later epoch is the ordinary, convergent flat-$\Lambda$CDM age. The cosmological beginning is finite-curvature \emph{and} a finite proper time in the past; only the conformal/synchronous chart sends it to $-\infty$, exactly as the comoving chart sends the finite throat to the spurious $r=0$. Taking the chart's $-\infty$ for the physical age is the time-face of the same doubled category error whose radius-face is taking $r=0$ for the source. Second, the seam is not merely the beginning of this universe: being the same species as the event horizon, it \emph{is} one. Read inward it is the future event horizon onto which a gravitational collapse in the antecedent universe asymptotes---the single $E=1$ geodesic is dust collapse read inward and dust cosmology read outward, the horizon the one congruence crosses; read outward it is this universe's cosmological beginning. The one finite-curvature layer carries both causal readings, joined by the null-boundary correspondence into the single closed \emph{cosmogenetic bead} the framework proves~\cite{JanzenCRframework, JanzenBHcausality}, of which this $E=1$ geodesic is the expansion leg and the operator's straight vacuum cut the curve it bends off: the collapse's event-of-events at the close of the antecedent cosmic time is the big bang at the opening of this one.
...
\paragraph{The emergence of the bend.} The bend-density identity is exact---$\rho=m'/4\pi r^{2}$, the cut's departure from straight is the stress-energy---but it is a statement of \emph{what} the bend is, not of \emph{why} the cut bends as it does. The slicing operator is kinematic: it generates the stress-energy from the curve, not the curve's own dynamics---the matter content's evolution and structure. That dynamics is the deepest question the construction opens onto, the one distinguishing a complete dynamical theory from a kinematical one awaiting its dynamics; and, unlike the range, the corpus has since substantially \emph{worked} it, not merely characterised it. On the geometry side it is the wall of~\cite{JanzenRange}: the boundary at which free gravitational radiation begins, identified as the rigidity of a single global sweep---a swept deformation has one fixed orientation, and the loss of the last isometry is precisely the point at which the wave's polarization must turn from place to place. On the matter side, \emph{what} the bend is extends beyond the spherical case: for a general spatial leaf the energy density is the leaf's intrinsic-curvature departure from the de~Sitter substrate, $16\pi\rho = {}^{3}R + K^{2} - K_{ij}K^{ij} - 2\Lambda$, the Hamiltonian constraint, of which $\rho=m'/4\pi r^{2}$ is the spherical instance. And \emph{why} the cut bends is, in the confined case, now exhibited exactly~\cite{JanzenDynamics}: on a wave that one residual isometry still pins, the leaf's transverse-traceless mode evolves by a wave equation---the first dynamical bend the construction displays, in vacuum, with the wave's energy carried by the shear of the leaf. Since the framework leaves the dynamics of general relativity unchanged, the general inhomogeneous evolution is ordinary dynamical evolution of the leaf rather than a new generative law; its canonical formulation, in which the cut's advance is generated by a true Hamiltonian, is developed in the companion canonical-time paper~\cite{JanzenCanonicalTime}. A further instance of the same frontier is the bend crossing the \emph{cosmogenesis branch point}---the matter counterpart of the geometric branch-point crossing of \S\ref{sec:synchronous}: that crossing is shown well posed on the finite-curvature branch point $r=0$ (a characteristic crossing with no curvature obstruction) and structurally governed---the foliation-preserving reassignment fixing the $\Lambda$-set rate while the leaf-carried density crosses as inherited content---with the detailed worldline dynamics the open depth~\cite{JanzenCRframework}. That worldline dynamics is taken up for a concrete matter model in the cosmogenesis paper~\cite{JanzenCosmogenesis}: the leaf-carried plasma's own thermal history across the branch point---heated by adiabatic compression, cooled on re-expansion through the nuclear window---produces the primordial light-element abundances, the leaf-carried density crossing as inherited content while the composition is produced from that datum on the re-expansion.
