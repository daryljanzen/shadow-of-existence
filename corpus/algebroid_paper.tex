...
%   THE STRATIFICATION: the so(5,1)-action on C is NON-TRANSITIVE — R_ab R^ab = 6M^2/r^6 + 12/alpha^4
...
them.\rcpt{P12_polar_dimension} \emph{Nor can least-arbitrariness close it}: it prefers the structure requiring
...
Rule~3 counts as maximally arbitrary rather than least.} One thing \emph{is} settled, and it is a different
...
\begin{itemize}
\item \emph{energy} (the leaf): matter is the leaf's intrinsic-curvature departure from the round substrate leaf---the Hamiltonian constraint $16\pi\rho={}^3R+K^2-K_{ij}K^{ij}-2\Lambda$, vacuum the round $S^3$ (${}^3R=2\Lambda$), spherically $\rho=m'/4\pi r^2$;
...
$(2,1)$---has a non-trivial holonomy about the branch points, the Klein four-group $V_{4}$ of even sign
changes, whose origin is the per-root resolution of $\sqrt{\Delta}$~\cite{JanzenGroupoid}. Adjoining it to the
deck $S_{3}$ closes a group of order twenty-four; adjoining the orientation parity as well closes one of order
