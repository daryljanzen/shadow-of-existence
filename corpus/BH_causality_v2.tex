...
%   form. Roots downstream: the cosmogenesis branch point, which IS r=0. It is finite-curvature in the
...
A word on scope. The metric singularity defined here---an infinite, ordered set of events carrying no metric separation, the manifold intact and worldlines crossing it---is a locus at which the curvature remains \emph{finite}. It is therefore a different object from the one standardly called the curvature singularity, at $r=0$, where the curvature invariants computed on the areal radius diverge without limit. The present paper establishes the first of these, with the event horizon as its exemplar, and says nothing about the second; that the two admit a common treatment, and what becomes of the divergence at $r=0$ under it, is the subject of a companion paper and is established there, not here~\cite{JanzenCircle}. The identification of this boundary as a metric singularity, reached from general relativity alone, is also what a later companion takes as its structural starting point~\cite{JanzenGeometricCore}. Nothing in the present argument depends on either companion; it stands on standard Lorentzian structure.
...
\emph{One further distinction belongs here, because a companion paper places a thermal state on a horizon and
a reader holding both results together will want the two reconciled.} That paper places the regular
Euclidean---Hartle--Hawking---state at a de~Sitter cosmological horizon, of surface gravity
$\kappa=1/\alpha$ with $\alpha=\sqrt{3/\Lambda}$~\cite{JanzenCanonicalTime}. There is no tension, because the
two horizons are different objects. The horizon whose thermal state is denied above is the \emph{collapse}
horizon of an astrophysical black hole, and the ground of the denial is that it is never completed at finite
exterior time---there is no realised background on which the Bogoliubov construction can be carried out. A
de~Sitter cosmological horizon is not the endpoint of a collapse: it requires no completion and is present on
every slice. \emph{The argument of this paper turns on a horizon that never finishes forming; it
...
stating.} Read across the four horizons at issue, the sorting is clean and it is
...
de~Sitter cosmological horizon (complete, observer-dependent) is thermal, and carries the Euclidean state
...
\emph{In de~Sitter space the accelerated case also says something the flat one cannot, and it is a consequence
of there being a single constant rather than an additional assumption.} The accelerated
...
with $\alpha=\sqrt{3/\Lambda}$ the de~Sitter radius, the geometry's one dimensionful constant, so the rest term
is exactly the $\kappa=1/\alpha$ of the Euclidean state above and
\begin{equation}\label{eq:unruh-ds}
T(a)=\frac{1}{2\pi}\sqrt{\frac{1}{\alpha^{2}}+a^{2}}
\end{equation}
carries no adjustable parameter at all: the observer supplies $a$ and the geometry supplies the rest, and
there is nothing else to supply. \emph{The structural difference from the flat statement is that the rest term
does not vanish.} An unaccelerated observer in Minkowski space sees nothing; an unaccelerated observer in
de~Sitter space is already in a thermal state, and acceleration adds to a bath rather than creating one.
...
is least safe---the near-horizon geometry of the degenerate member is $\mathrm{dS}_{2}\times S^{2}$, which
...
derivation of the Unruh effect from the geometry, or any new value for a temperature. And it does not claim
...
what makes that state available}, and with it the imaginary-time construction the companion papers build on
it~\cite{JanzenCRframework, JanzenCanonicalTime}. The general point is sharper than a single criterion, and
worth stating carefully because the obvious version of it is false. \emph{Finite curvature is sufficient for a
continuation to cross a boundary, but it is not necessary}, and the two boundaries of the Schwarzschild
geometry are of opposite type in exactly this respect. At the horizon the curvature is finite while the
tortoise measure $r_{*}=\int\dd r/f$ diverges. At $r=0$ the reverse holds: $f\to-2M/r$ diverges, so
\emph{$r_{*}$ converges} while the curvature invariants do not. Each is crossable, and for opposite reasons---one
...
of it would have to supply, is not settled here and is not claimed; the companion papers are where the
question has its home~\cite{JanzenCRcosmology, JanzenDynamics}. What is claimed is the
...
\begin{remark}[Consequences for finite-time densities, and what this result supplies]
Because the event horizon occurs, for every exterior observer, only as a metric singularity at the infinite-time null boundary, no finite exterior-time slice contains a horizon or the curvature singularity it would enclose. The densities associated with the central singularity therefore never form at any finite exterior time; mass remains finite in extent on every finite slice, with each infalling element asymptotically approaching its own horizon event. This places the present geometric result within a wider programme. The companion slicing construction for the Schwarzschild--de~Sitter family identifies the metric singularity established here as the geometric cause of the horizon-versus-centre asymmetry of the standard description~\cite{JanzenSlicing}; the existence/occurrence distinction this paper turns on---that the horizon \emph{occurs} without ever \emph{existing} on a finite exterior slice---is, there, the same distinction set at the root of the cosmological ontology, where the enduring cosmic time that existence requires is itself empirically forced~\cite{JanzenModernParallax}, the present paper reaching it from standard general relativity alone. The same companions argue that the collapse does not end at the centre, and continue it past $r=0$ as an expanding cosmology~\cite{JanzenSlicing,JanzenCircle,JanzenCRframework}; the primordial light-element abundances are read there as a record of that history~\cite{JanzenCosmogenesis}. Those are their results and not this paper's, which reaches the metric-singularity structure and stops. What the present result supplies them is one thing, and it is worth naming because it is load-bearing there: a boundary at which the curvature is \emph{finite} gives a continuation across it finite, characteristic initial data, where a curvature singularity would give it a wall. The dynamics of matter and observers crossing such a boundary is not settled here and is not settled by the finiteness alone; it is carried, and left open, in the companions~\cite{JanzenCRcosmology,JanzenDynamics}.
\end{remark}
