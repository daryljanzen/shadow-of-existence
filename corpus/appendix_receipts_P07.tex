...
\textit{the optical face --- **the forced member has no ringdown scale**} \ (P7 \S{}the optical face --- **the forced member has no ringdown scale**). 
\emph{Computes:} run r1863, rc=0
 \emph{Bound:} **bound: the eikonal correspondence is an asymptotic in \$\textbackslash\{\}ell\$ (Cardoso et al. 2009), and the CR reading is stated as a CONSISTENCY, not an independent prediction**
\ \emph{Run:} \texttt{python3 receipts/storyboard\_receipts/O6\_eikonal\_ringdown.py}
\item[\label{rcpt:O4_deflection_perspectival}\texttt{O4\_deflection\_perspectival}]\hfill\textsf{[OK]}\\
\textit{perspectival shadow curvature --- **\ensuremath{\Lambda} is in the measurement, not the path**} \ (P7 \S{}perspectival shadow curvature --- **\ensuremath{\Lambda} is in the measurement, not the path**). 
\emph{Computes:} run r1860, rc=0
 \emph{Bound:} **bound: the physics is STANDARD (Rindler--Ishak 2007) and no new prediction is claimed. What is new is that it instantiates P7's perspectival split exactly, in a place the field itself disputed**
\ \emph{Run:} \texttt{python3 receipts/storyboard\_receipts/O4\_deflection\_perspectival.py}
\item[\label{rcpt:O5_kappa_and_lyapunov}\texttt{O5\_kappa\_and\_lyapunov}]\hfill\textsf{[OK]}\\
\textit{the seam's rigidity --- **\ensuremath{\kappa} and \ensuremath{\lambda} both vanish at the forced member**} \ (P7 \S{}the seam's rigidity --- **\ensuremath{\kappa} and \ensuremath{\lambda} both vanish at the forced member**). 
\emph{Computes:} run r1859, rc=0
 \emph{Bound:} **bound: the coincidence of loci is computed; the causal LINK to P7's reframing-freedom argument is NOT claimed --- that remains P7's, established causally**
\ \emph{Run:} \texttt{python3 receipts/storyboard\_receipts/O5\_kappa\_and\_lyapunov.py}
...
\textit{the locus figure --- **the photon sphere IS Nariai's forcing locus**} \ (P3 \S{}the locus figure --- **the photon sphere IS Nariai's forcing locus** [borrowed from P3]). 
\emph{Computes:} run r1857, rc=0
 \emph{Bound:} **bound, and it records a caught error: the first pass used the SCHWARZSCHILD shadow formula \$b=\textbackslash\{\}sqrt\{27\}M\$ and got \$b=\textbackslash\{\}alpha\$ exactly --- clean and WRONG, since that assumes \$\textbackslash\{\}Lambda=0\$**
\ \emph{Run:} \texttt{python3 receipts/storyboard\_receipts/O1\_photon\_sphere\_nariai.py}
