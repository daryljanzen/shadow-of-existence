...
$1.03$ standard deviations. \emph{A stronger reading is available on these two numbers---that no single damping scale fits both, so
that the residual is the envelope's shape rather than its size---and the split does not support it at its own
error bar. We do not draw it.} What $\chi^{2}$ does resolve is the coefficient in the shear term, and the
comparison is sharp: its minimum sits at $0.865$ against the $0.859$
...
\item The acoustic scale is resolved in the cosmological development above (\S\ref{sec:flatlcdm}), and the radiation-free rate~\eqref{eq:rate} is the standing CR-versus-$\Lambda$CDM discriminator on data in hand (\emph{established}, \S\ref{sec:flatlcdm})---a strength established before the spectrum, not this section's to re-assert.
\end{enumerate}
