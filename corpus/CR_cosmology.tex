...
$1.01$, \emph{while this arm does not move across four instrument
states}\rcpt{P15_the_control_entered_the_regime_and_the_arm_did_not_move}. \emph{Nor is the height pattern an
instrument artefact}: putting the diffusion damping back --- \emph{derived on each arm's own opacity and
rate, neither imported nor fitted} --- brings the heights from $13$--$34\%$ to $2$--$3\%$ and drops the
likelihood's floor by a factor of five \emph{without the verdict turning
over}\rcpt{P15_the_derived_diffusion_damping}. \emph{The polarisation source
