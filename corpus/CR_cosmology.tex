...
\emph{The comb is not the only place this shows, and the second place is where the mechanism is read directly.} Write $Q(k)$ for the accumulated sound phase at first acoustic turnover in units of a half-period---unity for a free oscillator, and calibrated to unity on both arms with no drift in $k$. \emph{The two arms separate in shape and not in normalisation}: the comparison arm returns $Q$ flat at $0.79$, driven and below unity across the sampled range, while this one returns $Q$ \emph{rising} toward the free-oscillator value, $1.28$ to $1.06$\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}. \emph{A uniform comb is an undriven comb, and $Q\to1$ is what undriven means.} Flat against rising is independent of normalisation, which is what makes the second reading a test of the mechanism rather than a restatement of the first.
