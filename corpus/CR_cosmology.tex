...
\emph{The transfer carries the effect directly, and it has since been measured on it rather than
bracketed}: with the anisotropic stress removed at both of its dynamical sites---the neutrino Euler
equation and the shear term in $\Psi$---so that the sector is a perfect fluid at the same background
density, every peak on both arms moves to larger
multipole\rcpt{P15_the_source_decomposition_is_reachable_on_the_reporting_path_and_the_neutrinos_have_a_knob},
and the amplitude moves separately and by about a quarter. \emph{The shift is not constant in $\ell$}:
located sub-bin, by a parabola vertex at each extremum and again by a cross-correlation of the
envelope-normalised oscillation, on both arms and at four times the multipole sampling, it rises
monotonically from about $+3$ near the first peak to about $+12$ by $\ell\sim1500$---so removing the
effect \emph{entire} is not its deep sub-horizon limit, and carries the $k$-dependent change in the
potentials' decay with it. \emph{What the argument
turns on is measured, and is stronger than constancy in $\ell$}: the effect is the same on the two arms. Band by band the two arms' shifts agree to $0.17$ of a multipole and, in units of each arm's own
$\ell_A$, to $1.7\times10^{-4}$; the amplitude change is an envelope rescale of $1.2573$ against $1.2577$
and a peak-to-trough contrast ratio of $0.9875$ against $0.9869$. \emph{So free-streaming cannot be what
distinguishes the two constructions, whatever its shape in
$\ell$}\rcpt{P15_the_free_streaming_knob_is_common_to_both_arms_and_the_switch_sweep_finds_no_further_shadow}.
...
agree, the comb at $298.0$ against a reported scale of $302.9$.
...
No mechanism for the imbalance is offered here.
