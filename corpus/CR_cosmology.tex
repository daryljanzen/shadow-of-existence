...
This cosmology's driving lives on the \emph{contracting} side---the progenitor's radiation-comparable collapse phase, whose potential evolution is the Friedmann time-reverse of an expanding radiation era---so its driving magnitude equals flat $\Lambda$CDM's exactly, and the branch point's faithful transmission (\S\ref{sec:transmission}) carries it across unaltered. 
...
\emph{That argument rests on a premise that does not hold, which leaves its conclusion
unsupported rather than merely unproven.} It requires that a geometrically fixed rate possess no expanding
radiation era at all, so that the contracting side is the only place driving could live. \emph{That census
was taken on the stacking rate, and the three-level rule assigns the perturbations to the leaf} --- whose matter--radiation equality is fixed by the epoch the distance data fix, the plasma's early history being radiation-dominated. \emph{So the leaf
does possess an expanding radiation era}, and the modes carrying the first peak are still super-horizon when
the plasma starts and enter while radiation dominates.

\emph{And the transfer measures what the structural argument predicts, and does not find it.} The height
ratios this arm returns are $P_1/P_2=1.759$ and $P_1/P_3=1.612$ against the sky's $2.217$ and $2.277$ --- a
deficit of $20.7\%$ and $29.2\%$, deepening with peak number. \emph{The heights do not match flat
$\Lambda$CDM's on this construction's own instrument}, and the structural argument above cannot be read as
establishing that they do. \emph{What produces that pattern is not settled here}, and the reason is sharper than an unanswered
question. The transfer never integrates the collapse leg: that leg enters through a handover datum of three
numbers per mode, and \emph{its two halves are read at different points of it} --- the photon amplitude
carries the closed-form transfer this paper derives, and the potential is set to its primordial value. So
the expanding-leg driving is re-applied from a potential the collapse leg has already spent, onto an
amplitude that has not.

\emph{And the factor by which this arm is driven harder than the control is a property of that choice.}
Carrying the decay across to the potential does not halve the driving --- it doubles it again, to five times
the control's, and moves the first peak further from the sky rather than back. Reading both halves at the
same point removes the source altogether and the comb above the first peak with it. \emph{What settles it is a statement of what the photon
perturbation and the potential both are \emph{at one locus}, and the construction supplies one.}
...
sits at equality, and which equality is meant has to be said, because the two differ. The background the driving is integrated on carries its own, $\Omega_m/\Omega_r$ on the leaf rate, which at the geometrically
fixed $H_0$ sits $14\%$ higher in wavenumber and at the rate the baryon-acoustic data prefer sits within a
couple of per cent of flat $\Lambda$CDM's. \emph{It is the second that the envelope turns over at}, and it is
fixed by the matter density rather than assumed\rcpt{C7_equality_and_deficit}.
...
which is ordinary content physics on the observable leg. \emph{The sky's own value, located by parabolic fit on the Planck binned spectrum, is $P_1/P_2=2.2564\pm0.0772$, and flat $\Lambda$CDM's is $2.200$; \emph{what this construction returns for the same quantity is $1.759$, on the transfer of \S\ref{sec:refit-bound} with its wavenumber integral converged, since a peak height is exactly what an unconverged integral misreports}\rcpt{C5b_baryon_term}. \emph{The structural point stands without it}: the loading is the same in both because it is imprinted on the expansion side, and the argument is at the level of contents rather than of the driving---where the loading ratio alone would give $3.5$.} And the free-streaming
...
arm's first peak, and the $6.6\%$ position deficit the transfer measures there, is taken up at
...
be the directly measured one\rcpt{H0_acoustic_angle_and_seam}. \emph{So the acoustic scale is computed rather than accommodated}, and it is the comb of the computed
...
does not presently return. \emph{The end-to-end branch-point-to-recombination transfer is run} (\S\ref{sec:refit-bound}): a two-arm line-of-sight Boltzmann integral, $\Delta_\ell(k)=\int S\,j_\ell(k(\eta_0-\eta))\,d\eta$, carried on this cosmology's own background with the perturbations on the leaf congruence the framework assigns them, and validated on its control to $0.14\%$ in the first-to-second height ratio\rcpt{C59_the_control_reproduces_camb_and_the_height_defect_was_k_truncation}. On this arm it returns a $6.6\%$ deficit in the first peak's position and height ratios $20.7\%$ and $29.2\%$ below the sky's, and the likelihood rejects this arm's spectrum; the diffusion-scale signature it carries is located by the joint fit as a Gaussian residual in $\ell$ that no tilt removes (\S\ref{sec:coherence}). \emph{What remains owed is a mechanism}: the driving's size is measured, and it overshoots rather than falls short, so it does not account for the deficit, and no mechanism for it is in hand;
