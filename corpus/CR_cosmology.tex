...
physical, which is a claim and not a measurement. \emph{The measurement the gate itself names has now been made}: run against a continuum sampling in place of the ladder, the projected peaks move by $0.16\%$ at the first and less than a tenth of a multipole at the fifth, so the answer does not depend on the ladder sampling\rcpt{K1_the_ladder_waiver_is_checked_against_the_continuum}. \emph{A bound replaces the claim}: the ladder's imprint dies at $\ell\simeq k_{2}D_{C}e^{3\sigma}$ with $\sigma$ the transfer's width in $\ln k$, so reaching the first acoustic peak would need a transfer drawing power over a factor of some eight in $k$---far broader than any\rcpt{H1_the_low_multipole_deficit_is_two_effects_and_the_ladder_imprint_dies_where_the_transfer_is_narrow}. \emph{And the direction is the reverse of the naive one}: a broader transfer does not average the ladder away, it reaches further into the ladder's coarse bottom and carries discreteness \emph{up} in $\ell$. \emph{Run on a continuum
...
\emph{And the reason it behaves differently is the rate itself}: the standard driving shift is universal \emph{because} every mode crosses during radiation domination and acquires the same shift, and a radiation-free rate has no such crossing---so the flat driving of the control is a cancellation between two flat channels, and on this rate the cancellation is gone and the two add\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}. \emph{The acoustic-phase offset and the resolution of the Hubble tension are therefore one fact}, and this construction cannot keep the second while disowning the first.

\emph{What that still does not say is the microphysics of the coupling itself, and we mark the difference between an attribution and an explanation.} The candidate is that this cosmology's modes begin already sub-horizon at the
