...
a pure number, so the two candidate scales are one and fall together. \emph{Only the finite duration of the
collapse can enter}, and what it produces oscillates with a decaying envelope rather than tilting ---
sign-alternating across the band, which is a scale and not a tilt. \emph{So a departure from unity has one
channel left}, the mass of whatever field carries the fluctuations: $n_s-1=3-2\sqrt{9/4-m^{2}\alpha^{2}}$,
blue for a positive squared mass. \emph{The measured value is below unity}, so it asks the interior for a
slightly negative $m^{2}\alpha^{2}$ --- \emph{a requirement placed on the progenitor rather than a result
about it}, and nothing here is tuned to it---a distinction that did not exist while the tilt was read off a background fitted to the other arm. \emph{The figure is provisional in one respect and it is stated}: it is measured on the range below the damping tail, which is where a tilt has most of its leverage. \emph{One route to it is closed, and closing it narrows the
