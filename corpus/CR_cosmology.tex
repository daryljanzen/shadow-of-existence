...
guard---where before it moved against nothing at all. \emph{That question has an answer, and it is the rate's own.} The figure above is computed with the
perturbations evolving on the foliation's stacking rate; the framework assigns them to the leaf, since a
process running \emph{in} the content takes the leaf's (\S\ref{sec:properframe}), and the two differ by
thirteen per cent at recombination and more earlier. Evolved on the leaf congruence the first peak's
\emph{position} is no longer in deficit: the first peak spacing comes out $312$ against the sky's $317.5$,
one and seven tenths per cent, and the height ratio $P_1/P_2$ moves from $0.97$ to $2.01$ against the sky's
$2.22$\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}.

\emph{What remains is one feature of the comb, and on this rate it is a prediction rather than an open
number.} The sky's peak spacings alternate---$317.5$ then $271.7$, the second gap contracting---and this
comb's do not: its first two gaps are equal to the resolution at which they are read, under two initial
conditions that move every peak. \emph{The cause is the sentence above, read forward.} The alternation is
the compression--rarefaction asymmetry, and the standard shift that carries it is universal only where
every mode crosses the horizon while there is a plasma to be driven; on this rate the acoustic modes
re-enter at $z\sim3\times10^{4}$ to $3\times10^{5}$, all of them \emph{above} the onset at $z_{\mathrm{onset}}
\simeq6.8\times10^{3}$, so none of them crosses while our plasma exists. \emph{That is measured on the potential's own evolution and not read off the peak positions}: $\Phi$ does \emph{not} sit frozen here---it decays by some forty per cent for the first peak modes, and at a rate per acoustic half-period that is the same in both arms to within the spread across modes---so the ongoing decay is not what separates them. \emph{What separates them is the impulse at crossing}, which the comparison arm's modes receive within the plasma and this one's cannot, their horizon entries lying at $57.7$, $23.2$ and $14.7$~Mpc against an onset at $0.402\,\eta_{\mathrm{rec}}$, so that each is switched on already deep sub-horizon, $k\eta_{\mathrm{onset}}=3.1$, $7.8$, $12.2$\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}. \emph{The uniform comb follows from
the onset lying below the re-entry redshifts, and that ordering is not adjustable}: the nucleosynthesis
plasma is the progenitor's, on the transit's cooling leg, and is complete before the branch
point~\cite{JanzenCosmogenesis}.

\emph{The prediction is calibrated as well as stated, and the calibration is reproducible rather than
reported}\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}. Raising the onset past the
re-entry redshifts
restores the alternation progressively---one mode crossing gives $g_2/g_1=0.92$, all three give $0.895$---
and it saturates at the control's $0.897$, so a comb with crossing-during-plasma is reproduced exactly by
these same equations. Against that control this cosmology's deficit is $0.103$; the further $0.041$ to the
sky's $0.856$ is the instrument's and is carried by both arms alike. \emph{Neither the loading nor the
initial datum carries it}: with the baryons removed both combs widen, this one more, and lending it the
control's no-loading comb with its own loading response returns the observed alternation; giving the modes
their full pre-onset acoustic history relocates every peak and leaves the two gaps equal.

\emph{The comb is not the only place this shows, and the second reading is on different modes.} Write $Q(k)$ for the accumulated sound phase at first acoustic turnover in units of a half-period---unity for a free oscillator, and calibrated to unity on both arms with no drift in $k$. \emph{It is defined only where there is a turnover to accumulate to.} The mode setting the first peak is the one caught at maximal compression at recombination---that is what puts it there---so it has not reached a velocity turnover and $Q$ does not exist for it; the quantity is read on the modes that have turned over by then, the third peak and smaller scales. \emph{So the comb and $Q$ are two independent consequences of the mechanism on different modes, and not one mode's phase measured twice}: the comb's own source extrema project to the spectrum peaks at under two per cent, so the first peak's position is a statement about a density extremum, while $Q$ is a statement about a velocity turnover a quarter-period away. \emph{The two arms separate in shape and not in normalisation}: the comparison arm returns $Q$ flat at $0.79$, driven and below unity across the sampled range, while this one returns $Q$ \emph{rising} toward the free-oscillator value, $1.28$ to $1.06$\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}. \emph{A uniform comb is an undriven comb, and $Q\to1$ is what undriven means.} Flat against rising is independent of normalisation, which is what makes the second reading a test of the mechanism rather than a restatement of the first.
...
shift is a cancellation between two flat terms rather than the signature of one.} On the geometric rate
