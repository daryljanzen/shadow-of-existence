...
A configuration that hands the plasma over at a fitted onset instead carries a radiation amplitude there, $\rho_r/\rho_m=\omega_r(1+z_{\mathrm{onset}})/\omega_m$; but that is the fitted onset read in units of a density, not a second inherited number, and with the handover at the branch point neither the onset nor the amplitude exists to be inherited. \emph{And the epoch the distance data fix lands where the microwave background's own matter density sits}: the rate's $\Omega_m$ at the $H_0$ the angle and the distances return together gives $\omega_m=\Omega_m h^{2}$ within a couple of per cent of the standard inference, so equality falls within a couple of per cent of flat $\Lambda$CDM's with nothing fitted to a spectrum\rcpt{P15_the_ratio_is_the_onset_in_imported_units}. 
...
the branch point unchanged to one part in $2\times10^{9}$: \emph{transmitted, not determined} --- which is
the exact dual of the onset ratio's closure, that one turning on a quantity with no single value to
transmit. \emph{So the neighbouring statement strengthens from may to must: $\eta$ remains a measured
...
was taken on the stacking rate, and the three-level rule assigns the perturbations to the leaf} --- which has
a matter--radiation equality at $z_{\rm eq}=3936$ with the onset at $z=6761$ preceding it. \emph{So the leaf
...
sits at equality, and which equality is meant has to be said, because the two differ. The inherited datum's,
$1+z_{\mathrm{eq}}=(1+z_{\mathrm{onset}})/2$, is essentially flat $\Lambda$CDM's; the background the driving
is actually integrated on carries its own, $\Omega_m/\Omega_r$ on the leaf rate, which at the geometrically
...
point the ratio is a few per cent above flat $\Lambda$CDM's, and with the plasma started at a fitted onset
instead it is some thirty-five per cent above it.
...
against the sky's $298.4$, with the background fixed by the distance data alone. \emph{A configuration that
instead hands the plasma over at a fitted onset reports an angle its own spectrum does not have}---$301.6$
reported against a comb of $313.0$---which is what a parameter repairing a mis-specified handover looks like
from the outside.
...
accumulate}: here the two agree, the comb at $298.0$ against a reported scale of $302.9$, where the pinned
configuration reported $301.6$ against a comb of $313.0$.
...
\emph{A configuration that fits an onset to the acoustic angle instead moves the scale and not the peak.}
Scanning that parameter over a factor of $3.6$ runs $\ell_{A}$ from $360.6$ down to $239.3$ while the first
peak sits between $206$ and $210$ throughout\rcpt{P15_the_one_fitted_number_moves_the_scale_and_not_the_peak};
the fitted number moves the denominator of $\ell_1/\ell_A$ and leaves the numerator where it is\rcpt{P15_zonset_determinations}, which is what
a parameter repairing a mis-specified handover does rather than what a physical locus does.
