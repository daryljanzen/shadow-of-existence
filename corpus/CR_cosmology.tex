...
\emph{Whether the same mechanism can be read directly in the accumulated sound phase at first turnover is not settled here.} A coupled solve on a matter-and-radiation background returns a shape for that quantity, but the shape is controlled by the one datum the construction does not fix---the frozen effective temperature the modes carry at onset---and reverses with its sign: taken negative the phase runs to its free-oscillator value from above, taken at the adiabatic $+\Phi/2$ it does not. \emph{So the comb's uniformity is the measured statement and the phase reading is not a second observable until that datum is determined}\rcpt{P15_the_driving_is_bought_by_decay_during_oscillation}.
