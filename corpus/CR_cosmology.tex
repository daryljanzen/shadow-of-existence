...
\simeq6.8\times10^{3}$, so none of them crosses while our plasma exists. \emph{The uniform comb follows from
...
\emph{The comb is not the only place this shows, and the second place is where the mechanism is read
directly.} The driving shift is bought by potential decay occurring \emph{while a mode oscillates}: solving
the coupled photon--matter--potential system on a matter-and-radiation background, with the potential fixed
by the energy constraint and the acoustic turnover taken at the reversal of the photon velocity, a mode
whose oscillation begins after the potential has settled inherits the \emph{undriven} phase. Writing $Q(k)$
for the accumulated sound phase at first turnover in units of a half-period---unity for a free
oscillator---the prediction is a shape and not a number: $Q(k)\to1$ from above with increasing $k$ on this
rate, where a background carrying a radiation era holds it flat and below unity. \emph{A uniform comb is an
undriven comb, and $Q\to1$ is what undriven means.} Both are measured: the comparison arm returns $Q$ flat
at $0.79$, and this one returns $Q$ rising, $1.28$ to $1.06$ across the sampled
range\rcpt{D1_the_diagnosis_is_the_driving_and_the_driving_is_the_rate}\rcpt{P15_the_driving_is_bought_by_decay_during_oscillation}. \emph{Flat against rising is
independent of normalisation}, which is what makes the second observable a test rather than a restatement
of the first.
