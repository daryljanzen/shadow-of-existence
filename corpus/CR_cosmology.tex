...
%   (where the thermodynamics and BBN run); the two regimes are DISTINCT. THE BEGINNING IS THE BRANCH POINT r=0, at the close
%   of the lift, where the conjugate branch becomes the matter branch. It is finite
%   curvature in the substrate: the throat IS alpha, the substrate's defining constant (not a floor);
%   the divergence of the r-chart invariants there is the areal coordinate degenerating. The seams are
%   elsewhere -- the two unit-speed loci r=-2a/sqrt3 and r=+a/sqrt3 (r2123).
%   There is NO L1/L2 boundary in time: the distinction is kinematic and holds at every epoch.
%
...
%   (sinh^{2/3}, amplitude and rate Lambda-set); the tensions dissolve as consequences of the geometric
%   rate (the load-bearing falsifiable claim). THE PLASMA IS HANDED OVER AT THE BRANCH POINT: every
%   acoustic mode is outside the horizon there, so there is NO onset parameter and the acoustic angle is an
%   OUTPUT; H_0 ~ 68.6 is fixed by BAO and the angle together, and the ladder discrepancy is left where the
%   standard model leaves it. The perturbation sector FACTORIZES: the substrate
%   DETERMINES structure (null boundary coherence, classical inherited character, throat isotropization, the
%   discreteness floor, faithful transmission), the progenitor SUPPLIES content (A_s, n_s, eta and
...
%   Exposed edges: (1) the ACOUSTIC PEAK STRUCTURE -- the two-arm transfer is RUN (sec:refit-bound) at the
%   background the distance data fix: comb 298.0 vs the sky's 298.4, peaks 222/538/818/1134 vs
%   220.4/537.7/817.3/1123.9, heights 2.264/2.298 vs 2.217/2.277 (polarisation path; the fluid path
%   overshoots, so heights are path-specific and positions are not), TT 4.19/bin vs the control's 2.10 --
%   STILL A REJECTION, and what remains is the 2.3% phase and the 4th peak at +0.9%. The third-peak band
%   tracks matter-radiation equality on both routes to it, and equality is omega_m/omega_r with the epoch
%   the distances fix. (2) the damping-SCALE signature -- computed, non-reabsorbable by omega_b; its SIZE is
%   the handover's (a few % at the branch point, ~35% at a fitted onset), read with BOTH lengths carried to
%   the visibility peak (-0.9%); terminating r_D at a sharp recombination cut instead gives +2.2%, and a
%   ratio of integrals to different epochs is not a ratio of anything. The early ISW is present (sec:isw); what supplies none is the substrate rate.
%   GUARD: assuming peaks=LCDM at high-l and applying the extra damping manufactures a false
%   near-refutation. BAO confirmed vs DESI DR2 (chi2/dof~1).
%   Guard: the throat-tower l is NOT the CMB multipole.
%
...
\begin{abstract}
Cosmological Relativity (CR) obtains the observed cosmological expansion by a causal reassignment on a de~Sitter background at the cosmogenesis branch point, with the Einstein field equations unchanged. This paper develops that cosmology and completes it with its scalar perturbation sector. The reassignment selects the Nariai member~\cite{Nariai1951} of Schwarzschild--de~Sitter, whose proper frame is a homogeneous, observationally isotropic, \emph{non-synchronous} cosmology with areal radius $r(\tilde\tau)\propto\sinh^{2/3}$---exactly the flat-$\Lambda$CDM scale factor---its rate fixed by the cosmological constant alone, with radiation and matter inherited content read off the cosmic clock rather than terms that source the expansion. The cosmic beginning is the branch point $r=0$---a genuine curvature singularity of the SdS metric read over the $r$-chart~\cite{JanzenCircle}, and \emph{not} a singularity of the underlying substrate, whose throat is the defining constant $\alpha$; the divergence of the $r$-chart invariants there is the areal coordinate degenerating; the constant-proper-time slices are exactly Euclidean ($\Omega_k=0$) while the cosmological layers are a closed $S^3$---spatial curvature and dark energy one $\Lambda$ read on two slicings. The acoustic scale follows from that rate rather than calibrating it: with the plasma handed over at the branch point there is no early-universe parameter to carry the angle, so the angle is computed, and it and the baryon-acoustic distances fix $H_0$ together at ${\simeq}68.6$. The light-element composition is produced on the same footing: the infalling matter heats above the deuterium bottleneck and the expanding leg cools back through it, so that leg is a standard big-bang nucleosynthesis run from free nucleons, fixing the composition with the inherited baryon-to-photon ratio $\eta$---the one boundary datum of that handover. Helium-4 and deuterium sit at their observed values---the freeze-out temperature the data demands being the bottleneck the standard rate delivers, un-tuned; the near-zero metallicity of the oldest systems follows from the handover; and because the run is standard, lithium-7 carries the standard threefold over-prediction, the lithium problem shared with flat $\Lambda$CDM rather than dissolved.

On this cosmology the primordial scalar spectrum \emph{factorizes}. The de~Sitter substrate \emph{determines} its structure---the geometric coherence of the acoustic phases on the null boundary (not a super-horizon freeze-out), the classical non-vacuum character of the fluctuations (the substrate vacuum lies some $10^{113}$ below the observed $A_s$), the throat's isotropization, a parameter-free low-multipole discreteness floor, and the faithful transmission of the spectral shape---while the progenitor collapse \emph{supplies} its content, the amplitude $A_s$ and tilt $n_s$, inherited as boundary data exactly as flat $\Lambda$CDM inherits the baryon-to-photon ratio. \emph{The transmission is supported by a mechanism rather than by the absence of a scale alone} (\S\ref{sec:what-crosses}): on the contracting leg $aH$ grows without bound as the branch point is approached, so the comoving horizon shrinks to zero and every mode \emph{exits} it and freezes before the crossing; the imaginary-time segment's Euclidean kernel damps oscillatory content and has nothing to act on in a frozen mode, so amplitude and tilt cross unaltered while the collapse-leg acoustic phase does not---which the observed peak positions independently require, excluding a transmitted leg phase by a factor of $21$--$41$ in the required distance~\cite{JanzenCRframework}. 

The decisive result is a \emph{transmission dichotomy}: a non-degenerate horizon's exponential near-horizon approach imprints a scale-invariant spectrum (the inflationary de~Sitter-horizon mechanism), whereas the degenerate Nariai double root's power-law approach carries no scale, and the branch point where the modes cross---a third case, $f$ diverging so that the tortoise coordinate stays finite---accumulates no phase at all; the super-horizon transfer across it is computed and scale-invariant, so the progenitor spectrum is transmitted unaltered and CR has no inflationary scale-invariant attractor, no consistency relation, and no substrate-sourced primordial $B$-modes. 

Because the distance slicing is flat, the discrete closed-$S^3$ source projects through the flat transfer to a parameter-free low-multipole deficit ($\ell\lesssim8$)---mild, and on a genuine Boltzmann transfer a dip whose \emph{minimum falls at $\ell=4$} ($0.36$ of the $\Lambda$CDM expectation there, $0.47$ and $0.41$ at $\ell=2$ and $3$; \emph{the shape and the depth both cross-validated between two independent transfers}, which agree to three per cent at worst once the post-recombination source is carried to the present), and non-discriminating in sum: scored against the measured multipoles with cosmic variance carried, \emph{the quadrupole favours this construction and $\ell=3$ to $5$ favour the standard model}, the two nearly cancelling at $\Delta(-2\ln L)=+1.8$ over $2\le\ell\le10$. 

Taken together, these results carry a reckoning the paper draws in full: what flat $\Lambda$CDM assembles from added, tunable apparatus---inflation for the horizon, flatness, coherence, and near-scale-invariance, a dark-energy component for the late-time acceleration, added early-universe physics for the tensions---this cosmology recovers from the cosmological constant alone, each feature required by structure already present rather than produced by a sector introduced to yield it. By the economy of assumption that distinguishes a required structure from a merely permitted one, that comparison is settled on the theory-choice axis in this cosmology's favour; and on the data axis it is split: the expansion history fits the low-redshift distance ladder on the SDSS~DR12 consensus and, at the current state of the art, the DESI~DR2 dataset (thirteen measurements across seven tracers) at $\chi^2/\text{dof}\simeq1$, which fixes $\Omega_m\simeq0.30$ and $H_0\simeq68.6$ and leaves the discrepancy with the local distance ladder where the standard model leaves it; the light-element abundances are confirmed within $1\sigma$ at the microwave-background baryon density (the cosmogenesis paper~\cite{JanzenCosmogenesis}); and the acoustic peak structure, computed from that same background with nothing fitted to it, reproduces the comb to a part in six hundred and is still rejected on shape---at $2.57$ times the control's distance from the data as computed, and $1.57$ times once both arms' parameters are refitted; what it leaves is about one multipole at each peak. 

The cosmology's exposed edges are named: a signature in the diffusion \emph{scale}, the two rates differing by the radiation term alone so that the plasma's Silk scale and the foliation's distances do not share a rate~\cite{Silk1968}---a computed effect, non-reabsorbable by the baryon density, whose size is fixed by the handover, read with both lengths carried to the peak of the visibility function, and which the joint fit locates as a Gaussian residual in $\ell$ that no tilt removes, a refit absorbing a Gaussian only window-locally (the low multipoles being a mild, non-discriminating
deficit rather than a sharp edge either way); and the acoustic peak structure. \emph{The full-spectrum
likelihood is run on the computed spectrum}: scored against Planck~2018 \texttt{plik\_lite} over the
full-range lensed configuration, with one amplitude fitted so that what is compared is shape, this arm sits
at $2.57$ times the control's distance from the data (\S\ref{sec:refit-bound}). \emph{That is a spectrum
computed on a background fitted to the distances and then confronted}, which is a different statement from a
refit of the spectrum's own parameters to the microwave data; the second is the comparison the standard model
is usually judged by, and it is reported in \S\ref{sec:refit-bound} beside the first. 

We mark maturity throughout. \emph{Established}: the cosmological derivation, the transmission dichotomy, the amplitude floor, the throat, the flat slice, the flat-projection transfer, the data confrontation, and the time-reversal driving equality as a Fourier theorem. \emph{Argued}: the coherence mechanism. \emph{Measured}: the peak \emph{heights}, which this arm's own instrument returns within a few per cent of the sky's first two ratios on the polarisation path and above them on the fluid path, so the heights are the path's and the positions are the construction's. 

\emph{Run, and returning a partial agreement}: the end-to-end branch-point-to-recombination transfer, the standard line-of-sight integral across a visibility of finite width, validated on its control to $0.23\%$ in the first-to-second height ratio in the configuration this construction's arm is read in, and carried through to that arm, which returns the peak spacing to one part in six hundred and the first three peaks within a grid step of the measured ones, with the fourth not resolved against the sky once the likelihood's own covariance is propagated; the likelihood rejects the arm's spectrum on $TT$ shape nonetheless. \emph{Open}: the \emph{derivation}---as against the measurement---of $A_s$ and $n_s$. \emph{The cosmology carries no early-universe parameter, and what it does carry should be stated plainly}: the plasma is handed over at the branch point, where every acoustic mode is outside the horizon and the potential sits at its regular value, so the sound-horizon integral has no lower endpoint to fit and the acoustic angle is an output of the rate rather than a calibration of it. The background is fixed by the late-time data alone---the epoch $x_{0}$ (equivalently $\Omega_m=2/(x_{0}^{3}+2)$) and $H_{0}$, from the baryon-acoustic distances together with that angle, which agree with one another within a twentieth of a unit at $H_{0}\simeq68.6$. \emph{Matter--radiation equality is then fixed by the same epoch}, $1+z_{\mathrm{eq}}=\omega_m/\omega_r$ with $\omega_r$ measured, so one quantity carries the expansion history and the acoustic driving together and the computed spectrum is a test of the background rather than a further fit to it. The substrate's single $\Lambda$ is shown to govern both the global expansion and, per structure, the local scale at which structure resists it---the Hubble--Eddington radius---one constant read at two ranges.
\end{abstract}

\section{Introduction: the cosmological theory and its scalar perturbation sector}\label{sec:intro}

Cosmological Relativity (CR) recovers the flat-$\Lambda$CDM expansion history by a causal reassignment on a de~Sitter background at the cosmogenesis branch point: the expansion rate is fixed by the cosmological constant alone, radiation carries no term in it, and the present matter density is a reading of the cosmic epoch rather than an independent quantity~\cite{JanzenCRframework,JanzenModernParallax}. The cosmic beginning is the branch point $r=0$---a genuine curvature singularity of the SdS metric read over the $r$-chart, and \emph{not} a singularity of the substrate, whose throat is the defining constant $\alpha$, and the construction makes the constant-proper-time slices exactly flat while the cosmological layers are a closed $S^3$. This paper develops that cosmological theory in the form its observational tests require (\S\ref{sec:background}) and completes it with the scalar perturbation sector the framework paper's outlook named open---``perturbations within the Schwarzschild--de~Sitter representation''---the primordial density perturbations and their imprint on the microwave background (\S\S\ref{sec:coherence}--\ref{sec:transmission}), closing on the open frontier the construction sets up (\S\ref{sec:scope}). Across these sections one comparison recurs and is drawn together at the close (\S\ref{sec:predictions}): the causal contact, flatness, coherence, isotropy, and near-scale-invariance that the standard picture obtains from an added, tunable inflationary sector are here read off structure already present and fixed by $\Lambda$ alone---an economy-of-assumption difference the paper argues is decidable on the theory-choice axis, where it falls to this cosmology---and on the data axis the expansion history fits the low-redshift distance ladder, which fixes $H_0$ alongside the epoch, and the light-element abundances are confirmed against the data (\S\ref{sec:tensions}), while the exposed edge is the acoustic peak structure---where the transfer, run on that background with nothing fitted to it, reproduces the comb to a part in six hundred and is still rejected on $TT$ shape (\S\ref{sec:refit-bound})---and the damping-\emph{scale} signature, a computed, non-reabsorbable effect whose size the handover fixes, read with both lengths carried to the peak of the visibility function, and which the joint fit locates as a Gaussian residual no tilt removes, the low multipoles a mild, non-discriminating deficit rather than a sharp edge either way. The companion dynamics paper develops the perturbation sector's \emph{tensor} half---the propagating graviton as the leaf's shear, a massless de~Sitter mode on the cosmic foliation~\cite{JanzenDynamics}; the scalar half is here.
...
\end{equation}
so $\Omega_{m,0}$ records the cosmic epoch $\tilde\tau_0$ at which we observe rather than an independent density, and the coincidence problem becomes the observation that we exist at a time of order the geometry's single timescale. The densities are bookkeeping for one $\Lambda$-set geometry; \emph{radiation, like matter, is inherited content read off that clock, not a term that sources the rate}, and the stacking rate has no radiation-dominated era~\cite{JanzenModernParallax}, the leaf, where radiation gravitates, carrying one (\S\ref{sec:coherence}). The objection that a perturbing radiation fluid ``must gravitate and so alter the rate'' is a category error against a construction that reads~\eqref{eq:rate} leftward: the rate is set, and the fluid's energy is among the contents the set rate carries. The acoustic \emph{scale} follows from this rate, and $H_0$ is fixed with it by the distance data; this is developed next (\S\ref{sec:tensions}).

\subsection{The acoustic scale, the expansion rate, and what the construction dissolves}\label{sec:tensions}
...
This is what the observable cosmology rides at \emph{every} epoch, the branch point included: it is the rate the foliation stacks at, not a regime that begins somewhere. \textbf{(L2) The leaf-level local dynamics}: the expansion scalar of a self-gravitating congruence, the ordinary Friedmann readout $\mathcal H^2=\tfrac{8\pi G}{3}\rho_{\mathrm{tot}}+\tfrac{\Lambda}{3}$ with radiation gravitating normally---the valid phenomenological reading of general relativity on the progenitor's collapse excursion, where nucleosynthesis runs~\cite{JanzenCosmogenesis}; it is the same single $E{=}1$ geodesic read \emph{inward} as dust collapse that L1 reads outward as cosmology~\cite{JanzenOperator,JanzenBHcausality}. \textbf{(L3) The $E=1$ projection}: the reassigned shadow the observer reads, through which an L1 or L2 quantity appears as distance and redshift---the Projection Principle of the framework paper, its geometric origin the \emph{second ruling} of the slicing (the synchronous space is the second ruling)~\cite{JanzenCRframework,JanzenOperator}; reading this appearance as the existent is precisely the naïve-realist collapse the epistemology forbids~\cite{JanzenShadowExistence}. The decision rule is structural, not a choice: a self-gravitating local excursion runs on L2, diffuse content riding the global foliation on L1, and every observable is an L1 or L2 quantity seen through L3; and \emph{there is no L1/L2 boundary in time}. The distinction is kinematic and holds at every epoch: the vacuum kernel of the slicing operator is the SdS family exactly---$\Lambda$ and the cut's offset $2M$, and no more~\cite{JanzenOperator}---so the stacking rate is $c^{2}H_{\text{stack}}^{2}=(c^{2}/\alpha^{2})(1+2(1+z)^{3}/x_0^{3})$, while radiation, requiring $m'(r)\neq0$, is a \emph{bend} of the cut and therefore content. The decomposition is exact, $H_{\text{leaf}}^{2}=H_{\text{stack}}^{2}+H_0^{2}\Omega_r(1+z)^{4}$: the two rates differ by the radiation term alone. A comoving separation read across leaves takes the stacking rate; the scales the plasma itself accumulates---$\rs$ and $r_D$---take the leaf's, together with recombination's microphysics and the perturbations, because they are accumulated by a fluid running \emph{in} the content whose own radiation gravitates in its own expansion. The two must take the same rate, and the peak spacing the construction computes is the leaf accumulation: fitted to the computed peaks, the comb reproduces that integral to two per cent and the stacking one not at all. The assignment looked temporal only because $\Omega_r(1+z)^4$ is negligible below $z\sim10$~\cite{JanzenCRframework,JanzenBHcausality}. Each mis-assignment is quantitatively fatal---the geometric L1 rate carried down into the nuclear window is ${\sim}300\times$ too slow, and the radiation-sourced L2 rate carried into the distances would pin them to a radiation-era density the foliation's own rate does not carry~\cite{JanzenCosmogenesis}. Crucially L1 is the layer's own \emph{ontological} expansion, not the L3 shadow: computing a process against the stacking rate is reading the geometry, not the appearance. Every rate-bearing quantity below carries its level, so the reading is settled by the geometry rather than defaulted to the synchronous machinery.

\emph{The Hubble constant.} The late-time expansion is reproduced exactly, so $H_0$ enters this cosmology as it enters the standard one: through the distances. With the plasma handed over at the branch point there is no early-universe parameter left to carry the acoustic angle, and the angle and the baryon-acoustic distances then fix the rate together, at $H_0\simeq68.6$---the angle alone and the distance data agreeing to a twentieth of a unit. \emph{So this construction does not dissolve the discrepancy with the local distance ladder}: it leaves it where the standard model leaves it, as a disagreement between the ladder and the early-universe inference, and what it supplies in its place is stated in \S\ref{sec:refit-bound}---the acoustic spectrum computed from that background with nothing fitted to it.

\emph{The acoustic scale, computed.} Recombination is observable cosmology from the branch point onward, and the two lengths its angle is built from sit on opposite sides of the rate rule: the comoving distance $D_M$ is a separation read across leaves and rides the stacking rate, while the sound horizon $\rs$ is accumulated by the plasma and rides the leaf's. The observed angle $\theta_*=\rs/D_M$ is their projection. 
...
The sound horizon $\rs=\int_{z_{\mathrm{rec}}}^{\infty}(c_s/H)\,dz$---whose baryon-loaded $c_s$ is ordinary content microphysics and whose expansion is the leaf rate the plasma runs on---carries no early-universe parameter at all once the plasma is handed over at the branch point, where every acoustic mode is outside the horizon and the integral converges at its upper end.
...
geometry~\cite{JanzenCosmogenesis, JanzenGeometricCore}. \emph{And it is the substrate reading of the branch point that makes the integral well posed}: the modes are
outside the horizon there and the plasma's own expansion carries them, so the sound horizon converges at its
upper end without a floor having to be placed under it by hand. Taking $c_s$ the ordinary baryon-loaded value---the matter the plasma carries is pressureless, and the geometric throat ratio $\sqrt3$ does not set it---the acoustic scale is computed rather than met: integrated on the leaf rate from the branch point, it returns an angle whose comb sits at $298.0$ against the sky's $298.4$ with the background fixed by the distance data alone. \emph{This is where the two rates part, and it is not where the standard model's radiation sits.} There the whole expansion carries radiation and the ruler is pinned by the matter density's physical value; here the foliation's own rate carries none, so the distances are radiation-free while the plasma's scales ride a leaf rate that is not, and the two lengths the angle is built from sit on opposite sides of that split. The beginning is well posed: the branch point---finite in the substrate's curvature, whatever the $r$-chart geometry does there (\S\ref{sec:properframe})---carries every acoustic mode outside the horizon and the potential at its regular value, so the sound-travel integral converges at its upper end and needs no lower endpoint placed by hand.

\emph{What the construction fixes, and what it does not.} The empirical forcing of the cosmic foliation bears on the rate, not the matter content---the matter density being the bend of the spatial cut~\cite{JanzenModernParallax,JanzenOperator,JanzenRange}---so the content the plasma carries is measured rather than set by the geometry; the cold seam (Gibbons--Hawking temperature~\cite{GibbonsHawking1977} ${\sim}10^{-30}\,$K, some thirty orders below recombination) confirms there is no geometric source to set it instead. 

The quantities this cosmology actually uses are the epoch $x_{0}$ (equivalently $\Omega_m=2/(x_{0}^{3}+2)$; \emph{$x_{0}$ is the expansion read in units of the geometry's own fixed offset $r_{N}$, not the offset itself, which the Nariai condition fixes at $\alpha/\sqrt3$}), which the rate carries and the baryon-acoustic data fix; $H_{0}$, fixed by the same data together with the acoustic angle; and $\omega_b$ and $\omega_\gamma$, which the sound speed carries and which are measured directly and framework-independently. \emph{There is no early-universe parameter among them}: with the plasma handed over at the branch point the sound horizon has no lower endpoint to place, so the angle is an output of the rate rather than a calibration of it.

\emph{The physical matter density $\omega_m=\Omega_m h^{2}$ then does enter, and it enters once, through equality.} Matter--radiation equality is $1+z_{\mathrm{eq}}=\omega_m/\omega_r$ with $\omega_r$ measured, so the epoch the distance data fix also fixes how long the plasma spends radiation-dominated, and with it the driving that sets the peak heights and the wavenumber at which the envelope turns over. \emph{One quantity carries the expansion history and the acoustic driving together}, which is why the spectrum below is a test of the background rather than a further fit to it.

\emph{One thing about $\Omega_m$ is worth stating plainly, since the construction's matter sector invites the question.} The dust the rate carries exceeds the baryons by about a factor of six, $\Omega_m\simeq0.31$ against $\omega_b/h^{2}\simeq0.05$, so most of it is non-baryonic. \emph{That is not a component this cosmology introduces, and it is not one it derives.} What the matter sector fixes is the number of families, their chirality, the $S_3$ relating the three walls and the cyclic $\mathbb{Z}_3$ relating the generations, and \emph{nothing about their internal content}~\cite{JanzenMatter}; abundances are measured content throughout, the matter density being the bend of the spatial cut. So the non-baryonic fraction stands with $\eta$, $A_s$ and $n_s$: inherited, not set by the geometry. \emph{What the construction does supply is a place rather than an abundance.} Its correspondence with the Standard Model's content carries a right-handed neutrino as the sixteenth Weyl fermion of each generation, and fixes that state's place while fixing none of its couplings~\cite{JanzenMatter}; a species that is never thermalized contributes nothing to $N_{\mathrm{eff}}$, so nucleosynthesis is untouched and no prediction is made or needed here. \emph{The point of the paragraph is the division of labour}: a candidate the construction already contains, an abundance it does not claim, and no third thing wedged in between. 

The radiation amplitude at a start placed by hand, $\omega_r(1+z_{\mathrm{start}})/\omega_m$, is that start read in units of a density rather than an inheritance; with the plasma handed over at the branch point there is no such start and no such amplitude. \emph{And the epoch the distance data fix lands where the microwave background's own matter density sits}: the rate's $\Omega_m$ at the $H_0$ the angle and the distances return together gives $\omega_m=\Omega_m h^{2}$ within a couple of per cent of the standard inference, so equality falls within a couple of per cent of flat $\Lambda$CDM's with nothing fitted to a spectrum\rcpt{P15_the_ratio_is_the_onset_in_imported_units}. 

What this cosmology does carry from outside is the composition: the baryon-to-photon ratio $\eta$, exactly as flat $\Lambda$CDM carries it from a baryogenesis outside the model proper---measured, not derived---and with it the light elements, synthesized from the hot handover of the progenitor collapse, this universe having formed at the event horizon of a black hole in a previous one, equally outside the construction and equally measured. 

That the composition is measured rather than derived is no defect: a framework is credited for \emph{requiring} the structure it explains---here the rate, and the acoustic structure that follows from it---not faulted for measuring the boundary data it does not. \emph{One naturalness argument about the handover fails, because it compares a quantity with itself at two epochs nine orders apart.} 

Complete collapse does liberate binding energy of order the rest mass ($GM/Rc^2=\tfrac12$ at the Schwarzschild radius), and it is tempting to read that as making a handover carrying radiation of order the matter density the natural scale---order unity, rather than the small value $\eta$ takes. But $\rho_r=\rho_m$ occurs at $T\simeq0.8\,$eV, whereas the handover sits four orders \emph{above} the deuterium bottleneck at $0.07\,$MeV, since that is what the nucleosynthesis computed on it requires~\cite{JanzenCosmogenesis}: at the bottleneck the ratio is already ${\sim}9\times10^{4}$ and at the handover proper ${\sim}9\times10^{8}$. A handover carrying radiation merely of order the matter density would be a handover with $\eta\simeq0.5$, which leaves neither light elements nor acoustic peaks. \emph{So the naturalness question runs the other way}: what the progenitor must deliver is a plasma with of order a billion photons per baryon, and the binding-energy scale offers no head start toward it. \emph{One route to that derivation is closed from the geometry's side}: the constituent count the matter sector carries is exactly conserved on this history, its parity-odd source vanishing on the spherically symmetric class the cosmogenesis premise fixes, so what such a derivation needs is a chiral \emph{state} on the spatial layer rather than a chiral member\rcpt{P15_the_history_is_spherically_symmetric_throughout_so_the_chern_simons_number_never_moves}---and that state is not available on this layer, the same thermal regularity that fixes the tower's boundary condition carrying no helicity label, so the two helicity towers are populated alike and the parity-odd content cancels between them\rcpt{P15_the_chiral_state_the_route_needs_is_forced_absent_not_merely_unchosen}. \emph{And the amplitude a start placed by hand would carry is not a feature of this construction either}: given $\eta$ and the measured matter-to-baryon ratio, both of which the composition inherits regardless, it follows from standard thermodynamics alone, $[1+\nu](\pi^4/30\zeta_3)\,T/[\eta\,(\omega_m/\omega_b)\,m_N]$. \emph{So what the handover supplies is one composition datum} (receipt: \texttt{the\_two\_data\_are\_one.py}\rcpt{P15_the_two_data_are_one}). $\eta$ may remain a measured boundary condition indefinitely at no cost to what this paper computes. \emph{And the inheritance has a structural reason rather than being a gap left standing}~\cite{JanzenCRframework,JanzenCosmogenesis}: the infall thermalizes four orders above the deuterium bottleneck, so the progenitor's composition is erased and the abundances are synthesized in the window rather than inherited, while what survives that passage is baryon number, which no dissociation destroys. \emph{So $\eta$ is inherited and the abundances are predicted}, and the asymmetry is worth stating exactly:
baryon number protects the numerator, while what protects the \emph{ratio} is the adiabatic invariance of
the specific entropy --- the peak perturbing it by $5.5\times10^{-10}$ of a standing $1.15\times10^{10}\,k_B$
per baryon, so that $\eta$ survives the erasure because $\eta$ is
small~\cite{JanzenCosmogenesis}. \emph{And a derivation of $\eta$ is not needed, because there is nothing here
for one to be about.} It enters the progenitor from the ambient universe by composition-sharing and leaves
the branch point unchanged to one part in $2\times10^{9}$: \emph{transmitted, not determined} --- which is the exact dual of the radiation amplitude's closure, that one turning on a quantity with no single value to transmit. \emph{So the neighbouring statement strengthens from may to must: $\eta$ remains a measured
boundary condition, at no cost.}

\emph{The synthesized composition.} The same collapse fixes the light-element composition, and the geometry sharpens how. The infalling matter compresses adiabatically---the plasma is optically thick, so the photon bath compresses with it---and heats far above the deuterium bottleneck ($T_{\mathrm{D}}\simeq0.07\,$MeV) for every progenitor: the compression does not stop at horizon crossing but continues to the branch point, and the peak is bounded below by the infall energy at the horizon, of order the rest mass per nucleon, independently of the progenitor's mass (the cosmogenesis paper~\cite{JanzenCosmogenesis}); and across the branch point the expanding leg cools back through the window. \emph{The window is therefore crossed twice}, and at the branch point the temperature exceeds the binding thresholds by any margin, so nothing the infall built survives to be counted. Because the effective rate through the window is the standard Friedmann rate, the expanding leg is a standard big-bang nucleosynthesis run from free nucleons~\cite{JanzenCosmogenesis}. On a first sample this is favourable, and on one element advantageous. The helium-4 fraction $Y_p\approx0.245$ sits at the observed value; the near-zero metallicity of the oldest systems follows from the handover photodissociating the progenitor's processed composition back to light elements; and because the run is a standard nucleosynthesis, the lithium-7 abundance carries its standard threefold over-prediction---the standing lithium problem shared with flat $\Lambda$CDM, neither dissolved nor worsened. Deuterium sits at the observed $D/H\approx2.5\times10^{-5}$: the freeze-out temperature the data demands is the standard bottleneck, and that is exactly the temperature at which the standard rate delivers a freeze-out---target and mechanism coincide un-tuned. The peak calculation that forces this synthesis reading over an inherited one, and the full multi-abundance account, are given in the cosmogenesis paper~\cite{JanzenCosmogenesis}, where the network is integrated explicitly on the cooling leg and returns deuterium, helium-4 and helium-3 at their observed values with lithium-7 at the shared over-prediction, jointly from the single inherited datum---so one progenitor handover does fit the pattern together; what stands as its open edge is the last-percent precision against the measured abundances, a data-confrontation the produced pattern exposes rather than a computation still to be run.

\emph{The load-bearing claim.} The acoustic scale is computed from the rate rather than calibrated against it; the standard $c_s$ it uses presumes the inherited medium to be an ordinary photon--baryon plasma, which the abundance comparison confirms---helium-4 and deuterium at their observed values, lithium-7 the shared standard problem (the cosmogenesis paper). The load-bearing, falsifiable claim remains the prior one---that radiation carries no term in the foliation's own rate---of which the split between the two rates, and with it the whole acoustic reading below, are the consequences. The sound horizon is where a purely geometric expansion history and the standard radiation-governed one are told apart in the data.

\emph{The expansion history, confronted with the late-time data.} The distances and the ruler do not share a common factor: $D_M$ and $D_H=c/H$ carry the stacking rate's $1/H_0$, while the sound horizon is accumulated on the leaf rate, which carries the radiation the stacking rate does not. So the baryon-acoustic observables $D_M/\rs$ and $D_H/\rs$ constrain $H_0$ here as they do in flat $\Lambda$CDM, and the fit returns it: against the DESI~DR2 dataset the rate fits at $\chi^{2}/\mathrm{dof}\simeq1$ with $\Omega_m\simeq0.30$ and $H_0\simeq68.6$, and the acoustic angle alone, from independent data, agrees within a twentieth of a unit. \emph{What the two frameworks still do not share is the ruler's provenance}: there it is pinned by a radiation-era physical density, here it is the plasma's own accumulation on a rate whose geometric part carries no radiation at all.
\emph{The two rulers make the constraint differently, and the difference is where the radiation sits.} In
flat $\Lambda$CDM the ruler is pinned in physical length by the radiation-era matter density, so the same
distance data tie the fit to one $H_0$; here the ruler is the plasma's own accumulation on the leaf rate and
the distances ride the stacking rate, so the two enter the fit through different quantities and the constraint
is the joint one\ldg{convexity_optimisation}.

The confrontation carried to the state-of-the-art DESI~DR2 dataset~\cite{DESI2025}---thirteen distance
measurements across seven tracers ($0.3\lesssim z\lesssim2.3$), with the full per-tracer
$D_M/\rs$--$D_H/\rs$ correlations---returns $\chi^2/\text{dof}\simeq1$ at $\Omega_m\simeq0.30$ and
$H_0\simeq68.6$\rcpt{P15_desi_dr2_confrontation}, and the acoustic angle computed on the same background
agrees with that $H_0$ from independent data.
...
\caption{The expansion history across the baryon-acoustic distance ladder. \emph{Left:} the
...
\begin{proposition}[Every acoustic mode is outside the horizon at the branch point]\label{prop:subhorizon}
Approaching $r=0$ on the contracting leg the comoving Hubble scale $aH$ grows without bound, so the comoving horizon shrinks to zero and every mode leaves it before the crossing, on either rate and for every wavenumber. \emph{What is handed to the expanding leg is therefore the frozen adiabatic state}---the effective temperature at an extremum with no velocity and the potential at its regular value---and the collapse leg's contribution to it is an amplitude and not a phase.
\end{proposition}

\noindent\emph{What the modes then meet on the expanding leg is fixed by the same background.} The leaf congruence carries a radiation term and the perturbations evolve on it, so the wavenumber entering the horizon at equality, $\ell_{\mathrm{eq}}\simeq156$ at the epoch the distance data fix, separates the modes driven through a radiation-dominated entry from those entering later: the first acoustic peak sits above it and is driven, which is what \S\ref{sec:envelope} computes in closed form and \S\ref{sec:refit-bound} measures.

\emph{The two loci a reader may run together are distinct, and the distinction is load-bearing}\rcpt{P15_the_locus_is_wrong_in_six_places_and_the_lint_cannot_see_the_worst}. The branch point is the locus $r=0$, which no finite cosmic-time layer reaches and which therefore carries no finite redshift at all; a configuration that instead starts the plasma at a finite redshift on the expanding leg is starting it somewhere else, and there the census is the opposite one---modes inside the horizon, and a state handed over mid-oscillation.

\noindent\emph{Argument.} The comoving horizon on the contracting leg, $1/(aH)$, vanishes as $r\to0$ because $2M/r$ diverges there, so $k/(aH)\to0$ for every $k$; the equality multipole follows from $1+z_{\mathrm{eq}}=\omega_m/\omega_r$ on the leaf background with the epoch the distance data fix (receipts: \texttt{verify\_numeric.py}, anchor~7\rcpt{P15_verify_numeric}). $\qquad\square$

\emph{What this settles is a premise and not a mechanism}: it fixes what the plasma is handed and where each mode stands when it starts, and it does not measure how much of the comb the subsequent driving supplies. That measurement is the transfer of \S\ref{sec:refit-bound}, which computes the driving directly; the census above is what the transfer is carried from.

\section{What crosses, and what does not}\label{sec:what-crosses}

The transmission dichotomy above is an argument from the absence of an imprinted scale. A mechanism is given for it, and it also settles which content the branch point does \emph{not} carry.
...
is $\simeq1.6\times10^{2}\,c_s/c$, so a component arriving \emph{oscillating} with a per-cent sound speed
would be suppressed by $e^{-1.6}$ and one with ten per cent by $e^{-16}$ (receipt:
\texttt{the\_filter\_is\_a\_sound\_speed\_filter.py}\rcpt{P15_the_filter_is_a_sound_speed_filter}).
\emph{But the exponent has nothing to act on.} The kernel acts on
...
criterion selects nothing.} \emph{The crossing is lossless for every species rather than for cold ones alone},
and the pressureless argument's positive content---that the inheritance is exact rather than
adiabatic---holds of every species and not of dust alone.

\emph{One consequence of that freezing bears on the acoustic sector rather than on the crossing, and it is
computed rather than left as an inference.} Every mode is outside the comoving horizon at the branch point
(Prop.~\ref{prop:subhorizon}), so each re-enters on the expanding leg at its own time, and what it meets
there is fixed by the rate the perturbations run on. \emph{On the leaf rate that re-entry is an event}: the
plasma carries radiation, the potential decays on the sound-crossing time, and the modes entering while
radiation dominates---everything above $\ell_{\mathrm{eq}}\simeq156$ at the epoch the distance data fix---are
driven. \emph{That driving is not an inference about the potential's form but a shape read off the two
arms}: subtracting an undriven reference that calibrates at unity, the control's acoustic phase at first
turnover is flat in wavenumber to $2.1\%$ while this arm's varies by a factor of
$3.9$ across the same band\rcpt{P15_the_driving_shift_by_subtraction}, and \S\ref{sec:envelope} carries the
same driving in closed form.

\emph{What the rate rule closes is the alternative reading}, on which the modes re-enter under pressureless
matter and nothing is imprinted: the potential equation for a pressureless component,
$\Phi''+3\mathcal{H}(1+w)\Phi'+[2\mathcal{H}'+(1+3w)\mathcal{H}^{2}]\Phi+wk^{2}\Phi=0$, \emph{contains no $k$
at all once $w=0$}, so on a radiation-free rate the problem is scale-free and a scale-free problem imprints
nothing\rcpt{P15_the_crossing_exists_and_is_empty}. That is the stacking rate's answer, and the perturbations
do not take it: they take the leaf's, which carries the radiation term and therefore the
driving.

\section{What the substrate determines: coherence from the null boundary}\label{sec:coherence}

The acoustic peaks are sharp because the modes of a given wavenumber share a common phase at last scattering. Inflation produces this coherence dynamically, by freezing modes while super-horizon. CR obtains it instead from the character of the branch point.
...
The equality is a theorem about Fourier magnitudes and holds whichever side the driving lives on. \emph{Which
side it lives on is fixed by the rate rule}: the perturbations run on the leaf congruence, whose matter--radiation
equality is fixed by the epoch the distance data fix, so the plasma's early history is radiation-dominated and
the modes carrying the first peak enter the horizon while it is. \emph{The driving is therefore on the
expanding leg}, and the collapse leg's contribution to the state that meets it is an amplitude and not a
phase.

\emph{What the state at the handover is has to be said at one locus, and the construction supplies one.}

\emph{At the branch point every acoustic mode is outside the horizon and the leg's closed-form potential has
returned to its regular value}, so the state arriving there is the ordinary frozen adiabatic one: the
effective temperature at half the potential with no velocity, the potential unspent, and the collapse leg's
contribution surviving as an amplitude alone---the wavenumber-independent $0.4835\,\Psi_i$, within three per
cent of the frozen value $\Psi_i/2$ the free oscillator carries at every wavenumber. Specified that way the
radiation-era driving is applied exactly once, and the specification carries no free choice, the other three
codings each making one. \emph{It is a limit rather than a parameter}: carried back over two decades in
starting redshift the peaks hold to the grid step and the height ratios to half a per cent, while a start an
order of magnitude later has not converged.

The collapse-side driving that argument relied on carries a qualification of its own receipt: completeness holds for the modes whose entry precedes the horizon maximum at $r_{*}=1.53\,r_{\mathrm{seam}}$, entry on the rising branch being earlier the higher the wavenumber, so the acoustic band is on the safe side of that boundary by construction and the low-$k$ end is where it would bite\rcpt{C2_horizon_limits}; what a Boltzmann reference returns for flat $\Lambda$CDM's own heights is $P_1/P_2\approx2.2$, $P_2/P_1\approx0.45$, $P_3/P_1\approx0.44$ (\texttt{camb\_reference.py}\rcpt{P15_camb_reference})---\emph{those are the reference's values, not this arm's}, and what this construction's own instrument returns is set out above. The \emph{damping} tail ($\ell\gtrsim1000$) is fixed by the three-level rule (\S\ref{sec:tensions}) rather than left to judgement. Recombination is observable cosmology from the branch point onward, and the photon diffusion length $r_D$---like $\rs$---is a scale the plasma itself accumulates, so it takes the leaf's rate: the random walk runs \emph{in} the content, whose own radiation gravitates in its own expansion, its Thomson microphysics ordinary content physics and its expansion the leaf's (\S\ref{sec:tensions}). 

This is the same scoping the cosmogenesis paper derives for the nucleosynthesis window~\cite{JanzenCosmogenesis}, and the two windows fall on the \emph{same} side of it rather than on opposite sides: there the self-gravitating excursion sets the leaf rate radiation is included in, and here the plasma accumulating its own scales takes that same rate. \emph{What differs between the two is not the rate but what is read off it}---an abundance there, an angle here---and the angle alone is divided by a distance read across leaves, which is where the rate radiation is excluded from enters. 

This is forced by consistency: one may not take the rate geometric for the peak spacing and radiation-included for the diffusion---and \emph{the peak spacing this construction computes is the leaf accumulation}, which is measured rather than assumed (\S\ref{sec:tensions}), so the demand puts both lengths on the leaf and not on the foliation's rate. \emph{The two rates are far apart}: the geometric one is ${\sim}13\%$ below the radiation-included one at recombination ($\rho_r/\rho_m\approx0.3$) and further below it earlier, where the inherited radiation is a larger fraction, and the closed-form derivation of \S\ref{sec:envelope-consequence} puts $10.8\%$ of that gap into the diffusion length by itself. \emph{But a rate the two lengths share cancels from their ratio}, and what the sky reads is the ratio, $\theta_D/\theta_*=r_D/\rs$. \emph{So the rate gap is not what the signature is made of.} What it is made of is what \S\ref{sec:diffusion-scale} derives on the leaf assignment---where the plasma is handed over, and how far the diffusion integral is carried---with the handover fixing the size and the common endpoint at the visibility peak fixing the sign. \emph{One consequence of the signature being that small should be stated rather than left implicit}: the reference the instrument is validated against agrees on $\rs$ to $0.28\%$ ($144.0$ against $144.4$), which was a comfortable margin against a signature of several per cent and is a third of this one, so the word \emph{validated} here carries a thinner margin than it did and the figure is quoted with that in view\rcpt{Q1_a_stated_tolerance_is_a_request_and_the_corpus_answers_it}. What a stated tolerance carries anywhere in this corpus's computational layer is measured rather than assumed: every \verb|abs(...)<tol| comparison in the receipts rewritten twice, once to displace the compared quantity above its tolerance and once to collapse it below, so that $243$ of the $245$ receipts that pass are shown to have at least one such comparison that actually gates their verdict, the two exceptions being a display filter and a skip that prints its own reason\rcpt{Q50_the_tolerance_comparisons_gate_the_verdict_and_a_third_of_them_are_not_tolerances}\ldg{numerical_analysis}. 

This is a genuinely CR-specific piece of the height pattern, and \emph{what makes it so is the handover and not a rate}: a rate the two lengths share cancels from their ratio, while the epoch at which the plasma is handed to the expanding leg is fixed by the construction and is not a parameter. So the signature is not an artefact of laying the standard radiation-governed $\rs$ on a rate it was not accumulated on, the calculation belonging to neither framework this paper warns against (\S\ref{sec:intro}). 

One thing about this signature is settled and one is not, and it is essential to keep them apart. 

Settled: the effect in $r_D$ is genuinely CR-specific and does not dissolve under the density that would ordinarily rescale it---$r_D\propto\omega_b^{-0.31}$, so a percent-level $\omega_b$ (fixed by primordial deuterium and the peak-height ratio) shifts it by well under a percent (receipt: \texttt{damping\_reabsorption.py}\rcpt{P15_damping_reabsorption}). \emph{And what the signature does to the observed high-$\ell$ power is measured, with a verdict that is
neither of the two a reader expects.} Scaling the diffusion length by the instrument's own measured ratio
multiplies the damping by a Gaussian in $\ell$, and a tilt is a power law --- so over a restricted high-$\ell$
range the one is well approximated by the other. \emph{Refitting amplitude and tilt therefore absorbs the
signature almost entirely in shape}, and what it costs is a shift in the inferred tilt. \emph{Both scale with
$r^{2}-1$, and at the adjudicated ratio that factor is small}: with both lengths carried to the visibility
peak, $r=0.9918$, the signature costs $1.70$ in $\chi^{2}$ over the $185$ full-range bins---under a hundredth
per bin---and a tilt of $+0.0034$ absorbs seven tenths of it. \emph{Terminating the diffusion integral at a
sharp recombination cut instead}, $r=1.0231$, \emph{gives $12.6$ and a tilt of $-0.0093$}, and a ratio of $1.082$ gives $160$ and $-0.034$ on the same base and the same damping scale\rcpt{P15_the_signature_collapses_at_the_adjudicated_ratio_and_the_endpoint_sets_its_sign}. \emph{So the
hundredfold collapse is the configuration moving and not the fit improving}. \emph{So the signature
does not vanish under a refit --- it moves out of the spectrum's shape and into an inferred parameter.}
\emph{What a refit buys in shape it pays for in parameters}, and that direction is the robust part.

\emph{And how local that approximation is can be stated exactly, which settles what a joint fit would
actually meet.} The signature's own logarithmic slope is
$\dd\ln(\text{ratio})/\dd\ln\ell=-2(r^{2}-1)(\ell/\ell_{D})^{2}$ --- \emph{not a constant, but growing as
$\ell^{2}$}: across the likelihood's own $\ell=30$--$2508$ it runs from $-0.0002$ to $-1.31$, a factor of
seven thousand, which is exactly
$(\ell_{\max}/\ell_{\min})^{2}$\rcpt{P15_the_tilt_is_the_wrong_absorber}. \emph{So a single tilt is a
one-parameter fit to a function whose slope varies by that factor over the data it is scored on}, and the
displacement quoted above is the signature's true slope at $\ell=381$ and at no other multipole.
\emph{The consequence is the useful part}: a joint fit does not encounter a shifted tilt but a residual
whose \emph{shape} is a Gaussian in $\ell$, and no tilt removes a Gaussian --- so what such a fit weighs
is the shape residual, which is the quantity already bounded from above. \emph{And a displacement in
$n_s$ is not by itself a tension here}, the tilt being inherited boundary data rather than predicted: a
different value of an inherited input is a different input. \emph{What could be a tension is a constraint
on $n_s$ that does not run through the damping tail} --- the low- and mid-$\ell$ shape, the polarisation
spectra, lensing --- which is what a joint fit means and is where the question properly sits.

\emph{Two bounds travel with that and neither can be left to a reader.} \emph{The shape residual is an upper
bound}: freeing only the named degeneracy is the least absorption a fuller refit could achieve, so a wash is a
wash \emph{a fortiori}. \emph{The tilt displacement is not a bound}, because more free parameters would share
it --- the baryon density moves the diffusion length itself. \emph{And the magnitude is not robust to a factor
of three}: imposing the ratio on the measured spectrum gives the figures above, while applying it inside the
instrument's own source --- where it also reaches the Doppler term through a derivative --- gives $0.92$ in $\chi^{2}$ per
bin. \emph{The larger comes from the more faithful route, so the wash is stated at the weaker bound} and
closing that factor is named here as work not done.

\emph{The joint fit returns the residual this reading
predicts.} Fitting amplitude and tilt together against the signature-carrying spectrum, on
\texttt{plik\_lite}'s own bandpowers and covariance, the residual left after the fit is the analytic
Gaussian-minus-power-law shape to a correlation of $1.000000$ --- \emph{not merely non-zero, but the
predicted form}. \emph{The residual's form is the finding and its size is the configuration's}: at the adjudicated ratio it is under a hundredth in $\chi^{2}$ per bin, and $1.50$ per bin at a ratio of $1.082$ on the same window.
\emph{And the displacement is not one number}: fitted over $\ell=100$--$1760$ and over
$\ell=1400$--$2508$ it differs by a factor of $6.3$ across windows of the same data, so a
displacement is the value of nothing without the window it was fitted over. \emph{And the tilt takes a fixed share of it rather than a variable one}: seven tenths at the adjudicated ratio and seven tenths at $1.082$, so what the refit can and cannot do is a
property of the shapes and not of the size\rcpt{P15_the_joint_fit_leaves_the_gaussian_and_the_bound_was_chi2_not_sigma}.

\emph{Not} settled, and unchanged by that run: whether a displacement of that size in the inferred
tilt is a \emph{tension}. The tilt is inherited boundary data here, not predicted, so a different
value of it is a different input; what would make it a tension is a constraint on $n_s$ that does
not run through the damping tail, which this fit does not supply. The observable is $\theta_D/\theta_*=r_D/\rs$, and translating it into a spectrum requires the full acoustic transfer, which is run (\S\ref{sec:refit-bound}) and does not return high-$\ell$ peaks tracking flat $\Lambda$CDM's. A leading-order pass that simply \emph{assumes} the high-$\ell$ peaks equal $\Lambda$CDM's and applies the extra damping produces a large high-$\ell$ deficit (receipt: \texttt{full\_transfer\_verdict.py}\rcpt{P15_full_transfer_verdict})---but that assumption may not be imported for free: the same handover that fixes $r_D$ also governs the high-$\ell$ driving envelope, so CR's high-$\ell$ spectrum rests on short-wavelength collapse-phase driving rather than on the boost an expanding radiation era supplies.

\emph{And the observable ratio is read at one epoch on both lengths, which is what fixes it.} The two-arm transfer measures the raw diffusion length at the visibility peak, and the sound horizon is carried to the same place, because a ratio of integrals taken to different epochs is not a ratio of anything (\S\ref{sec:diffusion-scale}). \emph{The endpoint is therefore not a free choice}, and the difference between terminating the two lengths together and terminating them apart is larger than anything the cosmological parameters do to the signature.
...
Diffusion on the leaf rate therefore constrains, but does not by itself determine, the high-$\ell$ shape; the net effect is computed in \S\ref{sec:envelope-consequence}; whether it is a tension, a wash, or a distinctive but consistent feature turns on a parameter refit and on the early integrated Sachs--Wolfe term. \emph{Two confrontations are to be distinguished and both are run.} The spectrum computed on the background the distances fix is scored as it stands, one amplitude fitted so that what is compared is shape, and sits at $2.57$ times the control's distance from the data. \emph{Refitting the spectrum's own parameters to the microwave data}---the comparison the standard model is usually judged by---\emph{leaves it at $1.57$ times}: freedom closes about a third of the gap and leaves the rest (\S\ref{sec:refit-bound}). The control reproduces the sky essentially perfectly, which is what makes the comparison readable at all; and the verdict it returns is that the spectrum derived here is disfavoured against the standard model on this data. We state it as it stands. The effect the fit is run on is separately established: a real, computed, non-reabsorbable
CR-specific diffusion-scale signature, under a per cent at the adjudicated endpoint and fixed by the
handover rather than by a rate (\S\ref{sec:diffusion-scale}), its observable consequence derived rather than entangled
(\S\ref{sec:envelope-consequence}). \emph{What the fit does not reach is the join}: it weighs the
five-parameter spectrum construction, and what carries that construction back to the geometry is the transfer
work of this section, which is where the open end lies. The structural point that stands is the \emph{mechanism of phase}: where inflation freezes the phase dynamically, CR fixes it geometrically on the single null layer; how the amplitudes are driven is what the transfer of \S\ref{sec:refit-bound} measures.
...
the comoving horizon there---a property of the turnaround function's geometry rather than a
numerical accident of the background, and a statement about the seam that Prop.~\ref{prop:subhorizon}, being about the branch point, does not make\rcpt{C2_horizon_limits}. \emph{The locus matters and the two
...
\emph{The transfer carries the effect, and what it carries is a composite}: removing the anisotropic
stress at both of its dynamical sites---the neutrino Euler equation and the shear term in $\Psi$---leaves
the sector a perfect fluid at the same background density, and pushes every peak on both arms to larger
multipole, the direction the free-streaming pull has, with the amplitude moving separately---its envelope
by about a quarter. \emph{But that operation removes the constant phase shift and the change in the potentials' decay
together}, so what it measures is not the deep sub-horizon constant the argument above is about, and no
value for that constant is read off
here\rcpt{P15_the_free_streaming_knob_is_common_to_both_arms_and_the_switch_sweep_finds_no_further_shadow}.
...
\emph{That is a statement about the two rates and not about the observable}: the plasma accumulates both
lengths on the leaf's rate (\S\ref{sec:tensions}), so a difference common to them cancels from
$\theta_D/\theta_*$, and the figure is quoted here because it sizes the rate gap the division of labour
turns on rather than because it sizes the signature. \emph{The elementary route and the code-validated one
agree on it to about a point and a half}, which is the relationship an elementary derivation should bear to
a code-validated one. The sound
horizon is the same kind of integral, and it must be taken \emph{from the branch point}: there is no observable
expansion below it. Doing so returns $r_{s}=146.4$~Mpc against $145.4$ on the radiation-included rate---within $0.7\%$ of each other, the exact crossing sitting a little lower at $z\simeq6.7\times10^{3}$\rcpt{C9_sound_horizon_and_ratio}---the acoustic-scale calibration, met from the
...
is then $\theta_{D}/\theta_{*}$ moved by under a per cent, its size fixed by the handover and its sign by the common endpoint both lengths are carried to (\S\ref{sec:diffusion-scale}), and $\ell_{*}=\pi D_{M}/r_{s}=302.2$
...
=\exp[-(\ell/\ell_{D})^{2}(r^{2}-1)]$ with $r=\theta_{D}/\theta_{*}$---the same ratio the preceding
paragraph derives, and nothing else---so with both lengths carried to the peak of the
visibility function, $r=0.991$, it is a rise of about two per cent at $\ell_{D}$ and four at
$1.5\,\ell_{D}$, where terminating the diffusion integral at a sharp recombination cut instead, $r=1.022$,
turns it into a fall of four and ten\rcpt{R1_the_correction_stopped_at_the_sentence_and_the_number_it_defines_was_left_behind}.
...
everything else fixed while varying one thing. What has changed is the standing of the signature. The signature is a requirement of the construction, following from the level assignment and the handover and nothing else, with its sign fixed by the endpoint the diffusion length is carried to.

\emph{The early integrated Sachs--Wolfe contribution is present here, and the step that once denied it is
worth setting out because the denial was an inference and not a measurement.} The rate at recombination is
matter-dominated to nine orders, the substrate term being utterly negligible
there\rcpt{C11_early_isw}: \emph{that is true, and it is a statement about the rate}. The geometric rate
carries no radiation term, radiation being inherited content read off the clock rather than something that
sources the rate.

\emph{But the effect is driven by the radiation \textbf{content} through the perturbation source, not by the
rate.} The content enters the source under either rate assignment, and \emph{at recombination this
cosmology's content is $21.7\%$ radiation}, $\rho_r/\rho_m=0.2765$ at $z=1088$ --- which no statement about
the rate's composition bears on. \emph{So the inference from a radiationless rate to an absent term does not
hold}, and the term is present.

\emph{Measured, as the integrated $|e^{-\tau}(\Phi'+\Psi')|$ after recombination in units of the
Sachs--Wolfe amplitude there}: $0.348$ on the leaf congruence the rate rule assigns the perturbations to, and
$0.229$ on the stacking rate. \emph{Non-zero on both.} \emph{The leaf assignment makes the term half again
larger; it does not create it.}
...
the $k^{2}\Phi$ term. For the acoustic modes---inside the horizon through the observable leg---that term does not vanish, and the potential does decay on the observable leg, by a factor of order two across the first few
...
Whatever difference in that term remains between the arms cannot offset the diffusion signature, and the reason is structural rather than
...
likelihood confrontation}---a parameter refit, not a further calculation of this kind, and it is run: the
joint fit leaves the Gaussian residual (\S\ref{sec:coherence}) and the full-spectrum likelihood rejects this
arm's spectrum on $TT$ shape (\S\ref{sec:refit-bound}).

\subsection{The diffusion scale, and the error budget}\label{sec:diffusion-scale}

The acoustic scale is computed from the rate, and the diffusion scale is computed from the same one. Both
lengths are accumulated by the plasma on the leaf rate, so the projection cancels from their ratio and what is
left depends on two things only: where the plasma is handed over, and how far the diffusion integral is
carried. \emph{Neither is a free parameter, and the first is the larger.} With the handover at the branch
point the ratio sits within a couple of per cent of flat $\Lambda$CDM's on either endpoint convention; a start placed later on the expanding leg
raises it steeply, reaching some thirty-five per cent by the redshift at which the angle would be met.

\emph{The remaining choice is the epoch both integrals terminate at, and it is settled by the same
consistency the rate assignment is.} $\theta_D/\theta_*$ is a ratio of two lengths the plasma accumulates,
and \emph{a ratio of integrals taken to different epochs is not a ratio of anything}: the observed angle is
read at last scattering, which is the peak of the visibility function, so that is where both the sound
horizon and the diffusion length are carried to. \emph{Terminating them differently is what produces the
spread}: with the sound horizon anchored at the observed angle and the diffusion length taken instead to a
sharp recombination cut, the signature reads $+2.2\%$ where a common endpoint at the visibility peak gives
$-0.9\%$ --- a swing of three points where the cosmological parameters together move it by two
hundredths\rcpt{UNC_error_budget}. \emph{So the size is the handover's and the epoch is not a free
choice}, and the ordering of the four handover-and-rate combinations is unchanged on either reading. \emph{What the signature is not} is
reabsorbable: the baryon density has about one per cent of freedom against it, and the matter density could
only absorb it by moving some thirty-seven per cent, which is a different cosmology rather than a refit.
...
two multipoles rather than by tens\rcpt{ISW_line_of_sight}. \emph{So the early integrated Sachs--Wolfe contribution does not move the first peak}; what sets this
arm's first peak, and the phase residual the transfer measures there, is taken up at
\S\ref{sec:refit-bound}. \emph{The reabsorption of the damping signature, by contrast, can be bounded here}, and bounding it is
...
\rcpt{M2_reabsorption_minimization}. The baryon density has about one per cent of freedom
against it (\S\ref{sec:diffusion-scale}); the matter density could absorb it only by moving some thirty-seven per cent, which is
...
survives it. \emph{The lensed spectrum is therefore what the instrument's remaining floor is made of, and it is
computed rather than fitted.} Applying
...
The angular scale $\ell_{*}=\pi D_{M}/\rs$ is built from two lengths that do not ride the same rate here: the
distance carries the stacking rate's $1/H_{0}$ and the sound horizon is accumulated by the plasma on the leaf
rate, which carries the radiation the other does not. So the measured angle constrains $H_{0}$, as it does in
flat $\Lambda$CDM where the sound horizon is fixed instead by the matter and baryon
densities\rcpt{H0_acoustic_angle_and_seam}. \emph{So the acoustic scale is computed rather than accommodated}, and it is the comb of the computed
spectrum that carries it: fitted to those peaks by the procedure this paper runs on the sky, it is $298.0$
against the sky's $298.4$, with the background fixed by the distance data alone. \emph{A start placed by hand and fitted to the
angle would report a different quantity}: what such a fit sets is the scale reported, not the spacing the
spectrum carries, the two differing by four per cent.

\emph{The datum \S\ref{sec:coherence} states leaves two things to be fixed, and the handover fixes both.} That
datum is one amplitude per mode and a \emph{common} phase; it says that the phase is common without saying
which phase it is, and it does not say what the amplitude is flat in wavenumber \emph{at}. At the branch point
both follow: every mode is outside the horizon there, so the phase is an extremum with no velocity, and the
amplitude is the frozen value the closed-form leg carries, $0.4835\,\Psi_i$.

\emph{How much the comb depends on that choice is measured rather than assumed.} Across the seventeen
readings admitted by a four-peak criterion fixed before the numbers, the first peak spans $148$ to $228$---a
factor of $1.541$, and across the fifteen numeric-phase readings alone $1.118$---with the spacing spanning
$276.0$ to $330.4$, the phase intercept $-0.4044$ to $-0.2161$ and the alternation ratio $0.475$ to $1.088$.
\emph{Within that span position and alternation move against each other}, Spearman $\rho=+0.782$ at
$p=2.1\times10^{-4}$ between the first peak and the gap ratio, with every contracting reading sitting at
$\ell_1\le212$. \emph{The single-locus reading is not inside that trade}: it returns the first peak at $222$
and a contracting second gap together, which is what fixing the state at one locus rather than choosing a
reading of it buys.
...
a standard code's $2.200$---$0.23\%$: \emph{so the instrument is calibrated where this cosmology's number is
taken, and that is the figure quoted for it, the closer $0.14\%$ belonging to a different run and not to this
one}\rcpt{P15_two_arm_control_and_guard}\rcpt{P15_the_line_of_sight_transfer}. \emph{Both arms are converged in the wavenumber integral.} The
...
\emph{On that path this cosmology's arm returns the following, with the plasma handed over at the branch
point and the background fixed by the distance data alone.}
The peaks fall at $222$, $538$, $818$ and $1134$ against the sky's $220.4$, $537.7$, $817.3$ and $1123.9$;
the comb fitted to them by the procedure this paper runs on the sky is $298.0$ against the sky's $298.4$, and
the phase intercept $\varphi/\pi=-0.2349$ against $-0.2405$. The height ratios are $P_{1}/P_{2}=2.264$ and
$P_{1}/P_{3}=2.298$ against the sky's $2.217$ and $2.277$. \emph{The first three peaks land within a grid
step, and the fourth is not resolved against the sky}: located through one procedure with the likelihood's own
covariance propagated, this arm sits $0.9$ to $1.3$ standard deviations from the measured fourth peak across
the admissible window, so \emph{the data cannot tell the two models apart
there}\rcpt{P15_the_skys_own_fourth_peak_cannot_tell_the_arms_apart_and_the_displacement_is_shared_with_the_control}.
\emph{And what offset there is, is mostly not this construction's}: both models sit high of the located sky,
the control by $1.2$ standard deviations and this arm by $2.1$, with well over half of this arm's offset being
the control's own. \emph{Two wavenumber cutoffs agree to every digit, and the positions are
converged in the reported multipole grid}: read on a grid of step $2$ and refined to step $1$, they move by a
single multipole at the first peak and not at all above it.

\emph{And the residual is neither of the two instrument faults a reader would reasonably suspect}, because
both are removed in this configuration: the wavenumber integral is carried past the reported multipole, and
the projection carries the polarisation source rather than a damping
envelope\rcpt{P15_the_height_target_was_below_the_resolution_of_its_own_statistic}. \emph{And the truncation's
removal is measured on the arm whose answer is known}: it was $78\%$ of the control's remaining $\chi^{2}$,
and taking it out brings the control to $1.18$ per degree of freedom over the full range against a reference
$1.01$\rcpt{P15_the_control_entered_the_regime_and_the_arm_did_not_move}. \emph{The diffusion damping is
derived on each arm's own opacity and rate rather than imported or fitted}, and putting it back moves the
height ratios by tens of per cent and drops the likelihood's floor by a factor of five without the verdict
turning over\rcpt{P15_the_derived_diffusion_damping}. \emph{The polarisation source pulls both arms by
comparable amounts}---the control by $8.2\%$ and $20.8\%$ on the two ratios, this arm by $10.9\%$ and
$26.9\%$---and \emph{nothing about either operation differs between the arms}, which is what makes what
survives them attributable to the source rather than to the machinery.

\emph{Two likelihood configurations are quoted in this section and they are not interchangeable, so which is which is stated rather than left to be inferred.} The full-range lensed one, on $185$ bins, is the better-converged: it is where the control reaches $1.18$ per degree of freedom against a reference $1.01$, and it is the configuration in which an instrument fault can be told from a physical one. The $133$-bin unlensed one is where the shape rejection is quoted, and its control sits at $2.10$ per bin because the ceiling cuts the damping tail. \emph{The second is not a worse measurement of the first's quantity}: it is a different configuration, chosen for the comparison it makes, and the two sit beside each other rather than one superseding the other.

\emph{The driving's size on the observable leg is measured rather than inferred, and its wavenumber
dependence is known in closed form}---the $k$-dependence living in the potential's gradient rather than in its
decay, with the turnover at a fixed multiple of the horizon
scale\rcpt{P15_which_coupling_carries_the_k_dependence}\rcpt{P15_the_gradient_coupling_in_closed_form}.
\emph{Subtracting the driving is what measures it}: removing every coupling to the potential moves the
control's first peak from $220$ to $274$ and $\ell_{1}/\ell_{A}$ by $0.1792$, so the modes carrying the first
peak are driven and the shift is the driving's
size\rcpt{P15_the_phase_is_the_driving_and_the_undriven_arms_agree}. \emph{Run on this arm the same
subtraction gives $0.1717$}---\emph{the potential's grip within four per cent of the control's, and slightly
weaker rather than stronger}\rcpt{P15_the_anomalous_driving_belonged_to_the_pin_and_not_to_the_construction}.
\emph{And this one is not path-specific}: the two instrument paths return the same shift to four decimal
places, where the heights do not.

\emph{One of the two position statistics is not a projection quantity, and saying which keeps them apart.}
The ratio of the first peak to the acoustic scale is $\ell_{1}/\ell_{A}=k_{1}\rs/\pi$---\emph{the distance
cancels identically}---which is why removing each source term, the damping envelope and the visibility
function in turn leaves it untouched. What it compares is the sound horizon each arm \emph{reports} against
the phase its modes actually accumulate. \emph{Both arms integrate that horizon from $a\to0$}, the plasma
being handed over at the branch point on this one and the control having no other start, so the comparison
does not confound the rate with the
start\rcpt{P15_the_symmetric_comparison_was_never_runnable_and_the_quarter_was_two_fifths}; and here the two
agree, the comb at $298.0$ against a reported scale of $302.9$. \emph{The two sides of that agreement are
computed independently}: the reported scale reaches the transfer nowhere---no source term, no projection
and no solver reads it---so the comb it is compared against is not a quantity it was used to
set\rcpt{P15_the_free_streaming_knob_is_common_to_both_arms_and_the_switch_sweep_finds_no_further_shadow}.

\emph{And the likelihood is run, both arms scored on the same bins by the same code, with the amplitude
fitted so that what is compared is shape.} \emph{The result is still a rejection of this arm's spectrum and is
stated as one.} On the full-range lensed configuration---$\ell=100$--$1996$, $185$ bins, the lensing carried
by the same derived operator on both arms---the control returns $\chi^{2}=214.1$, or $1.16$ per bin, and this
arm $550.5$, or $2.98$ per bin: \emph{the arm sits at $2.57$ times the control's distance from the
data}\rcpt{P15_where_the_likelihood_sits}. \emph{That is the unfavourable of the two configurations and it is
the one quoted}, the same comparison unlensed over $133$ bins giving $279.4$ against $556.7$, a factor $1.98$;
lensing is worth proportionally more to a spectrum whose peaks are already in place. \emph{The band-by-band comparison is even across the range} --- $1.70$, $2.22$, $4.23$ and $9.29$ per bin against the control's $0.77$, $1.44$, $2.62$ and $3.73$ over the four quarters from $\ell=100$ to $1300$, read on the unlensed configuration --- so the disagreement is not one feature carrying the whole of it. \emph{And the spectrum's own parameters are refitted, which is the comparison the standard model is usually
judged by.} With the expansion rate, the matter and baryon densities and the tilt free in each arm and the
amplitude in closed form, scored on the full range and verified by running the spectra at the minima, the
control settles at $1.01$ in $\chi^{2}$ per bin and this arm at $1.58$: \emph{the arm at $1.57$ times the
control's distance, where the computed spectrum sits at $2.56$ on the same bins}---so \emph{freedom closes
about a third of the gap and leaves the rest}\rcpt{P15_the_refit_leaves_the_background_where_the_distances_put_it_and_does_not_close_the_phase}.

\emph{What the refit does not move is the background.} Given four free parameters this arm's expansion rate
moves by $0.028\%$ and its matter density by $0.031\%$ from the values the baryon-acoustic distances and the
acoustic angle fix with no spectrum involved: \emph{the background the distances fix is the background the
spectrum wants}. \emph{And they are at a sharp minimum rather than unconstrained}---one-step excursions cost
$2527$, $740$, $141$ and $31$ in $\chi^{2}$ on the four parameters, which is the distinction between a
parameter that does not move and one that cannot. \emph{The one parameter that does move is the tilt}: this
arm prefers $0.998$ against the control's $0.954$, bluer by $0.044$, with the damping tail's leverage landing
there and on the baryon density as the shorter range predicted it would.

\emph{One thing the ratio does not carry, and it is stated here rather than folded in.} The two arms fit the
same four parameters, so no information criterion separates them---penalties of about two per fitted parameter
against a difference of some fifty in $\chi^{2}$---and the asymmetry between the frameworks is not in what was
fitted but in what had to be fixed before there was a model to fit. \emph{The standard model reaches flat
$\Lambda$CDM by setting a curvature parameter to zero on inflation's authority, and uses the same added epoch
to supply the coherent phases}; here the flat slices, the expansion law and the coherence are consequences of
the construction (\S\ref{sec:flatlcdm}, \S\ref{sec:coherence}) and there is no curvature parameter to free.
\emph{And it runs the other way too}: both frameworks inherit the composition, the amplitude and the tilt, and
neither derives the spectrum it is scored on. \emph{Two axes, and no single number crosses
them}~\cite{JanzenSynthesis}.

\emph{And the phase intercept is not a residual at all, once the sky's own is located by the same
procedure.} Running one locator over the binned likelihood and both arms alike, the sky's intercept is
$-0.2378\pm0.0099$ on the first three peaks and $-0.2448\pm0.0074$ on all four; this arm's is $-0.2278$ and
$-0.2445$. \emph{So the arm sits one standard deviation from the sky on the one fit and on it in the other},
and the sign of the offset flips between them --- \emph{the shape of a difference carried by a single peak
rather than a phase
error}\rcpt{P15_there_is_no_phase_residual_and_the_fourth_peak_is_not_the_damping_envelope}. \emph{What
dissolves it is the sky's own locating spread and not a correction to the arm}: the binned likelihood cannot
pin that intercept tighter than a hundredth, and the offset is inside it.

\emph{What remains is about one multipole at each peak, and it is not the damping envelope.} Imposing the
control's envelope on this arm moves the fourth peak by a seventh of a multipole against an offset of two,
and the inverse operation returns the same displacement with the opposite sign, so the test is sound and the
answer negative. \emph{The offsets keep their sign and order across all four peaks} --- the shape of a small
common phase rather than one displaced peak, though not a clean constant --- and \emph{at the spreads the
locating itself carries, a finer statement is not available}.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_acoustic_two_arm.pdf}
\caption{Both frameworks refit on the same bins with the same freedom, against the measured temperature
spectrum. \emph{Above}, the acoustic range in $D_\ell$, each arm at its verified refit minimum with the
amplitude taken on the full bandpower covariance; \emph{below}, the same on a logarithmic multipole axis from
$\ell=2$, so the discrete low-multipole floor and its minimum at $\ell=4$ sit in one frame with the peaks ---
that panel's floor being a reconstruction against the control's own low-multipole spectrum, since the sweep
carries it as a ratio. \emph{Whitened residuals beneath each.} The peak positions and the spacing are the
result to read in the upper panels; \emph{the residuals are where the rejection lives}, and the control's own
are drawn beside this arm's so the size of the difference is visible.}\label{fig:acoustic}
\end{figure}

\emph{And what the rejection is made of is measured rather than described.} Whitened against the full
covariance, the two arms' residuals sit at a cosine of $0.855$, and projecting the control's direction out of
this arm's leaves $73\%$ of it shared --- stable across three bin cuts. \emph{That fraction is not an
attribution, and reading it as one would be an error}: both arms are fitted to the same bandpowers, so their
residuals differ by the model difference alone and the fraction measures only how alike the two models are ---
a control deliberately tilted away from its own best fit, which nobody would defend, scores higher still.
\emph{What the decomposition does say runs the other way.} The control already fits these bins at $0.99$ per
bin, so there is no shared defect for this arm's misfit to be an amplification of; this arm exceeds it by
$105$ in $\chi^{2}$ and the model difference alone carries $77$ of that, so \emph{the excess is this
construction's}, and the factor of $1.3$ between the two residuals' sizes is that difference's own
size\rcpt{P15_the_shared_fraction_does_not_mean_what_i_said_and_the_peak_trough_pattern_is_the_arms_alone}.
\emph{Within it, position rather than
amplitude, tilt or damping}: admitting one further parameter at a time, a peak-position rescale buys $4.7$ in
$\chi^{2}$, the damping $1.4$ and the tilt $0.1$ --- and on the control the position direction buys nothing at
all, so \emph{the position part is this construction's}. It is small: the rescale is under a multipole at the
fourth peak, and $4.7$ of $283$ is under two per
cent\rcpt{P15_three_quarters_of_the_arms_residual_is_the_controls_and_what_is_left_is_position_not_amplitude}.
\emph{Amplitude, tilt, damping and position together do not account for the shape rejection}: they carry
$5.7\%$ of the model difference between them. \emph{What does is a contrast difference}, and the sky's noise
does not enter it.

\emph{Taking the realisation out is what makes the difference readable.} With $\Delta$ the whitened
difference of the two arms at their own minima, $\chi^{2}$ decomposes exactly as $282.96=177.88+27.82+77.26$:
\emph{the model difference carries three quarters of the excess and the cross term a quarter}, so the
rejection is not this sky's luck --- set the cross term to its expectation and this arm still scores $1.43$
per bin against the control's $0.99$ on a typical sky. \emph{And $\Delta$ is trough-dominated}: $+0.011$ at
the peaks against $-0.760$ at the troughs, so the two arms agree about where the peaks are and differ between
them. Split about a smooth envelope, the oscillatory part carries seven tenths of it and is peak-high and
trough-low---\emph{antisymmetric about the envelope rather than about each peak, which is a contrast and not
a phase and not an envelope}. This arm's acoustic oscillation is $1.040$ times the control's about its own
envelope, and one direction built from the control alone---raise the oscillation at fixed envelope, with
nothing fitted---carries half of $\lVert\Delta\rVert^{2}$.

\emph{And the three features this arm's residual shows and the control's does not are all $\Delta$'s}: the
peak-and-trough asymmetry, the $\ell\simeq1000$ trough where $\Delta$ carries $-1.074$ of the arm's $-1.177$
in fifteen bins, and the growth with multipole---\emph{which $\Delta$ does monotonically where neither
residual does}, a residual being $\Delta$ on a noise floor of order one that flattens
it\rcpt{P15_the_model_difference_is_an_acoustic_contrast_difference_and_it_is_not_one_skys_luck}.
\emph{The channel is identified and the cause is not}: the contrast direction is the balance between the
monopole and the Doppler terms of the source, bracketed with opposite signs by removing each, on both
instrument paths and with the same signs on both; and \emph{neither deletion returns the two arms to a
common contrast}, so what the bracket locates is where the difference enters rather than what produces
it\rcpt{P15_the_source_decomposition_is_reachable_on_the_reporting_path_and_the_neutrinos_have_a_knob}.
No mechanism for the imbalance is offered here. \emph{And the neutrinos' free-streaming, which the shape
invites, is not the difference either}: removing it moves both arms' peaks by the same amount---band by
band the two agree to a fifth of a multipole, and in units of each arm's own acoustic scale to two parts
in ten thousand---while its large effect falls on the envelope rather than on the contrast. \emph{A
correction the two arms share cannot be what separates
them}\rcpt{P15_the_free_streaming_knob_is_common_to_both_arms_and_the_switch_sweep_finds_no_further_shadow}.

\emph{And the trough-filling a contrast excess would be a shortage of is not short on this arm, which makes
the residual larger than its four per cent rather than smaller.} Deeper troughs are what excess baryon
loading produces, through the displacement of the oscillation's zero point---and measured against each arm's
own loading, \emph{this arm's monopole swings about a point that falls short of its own $-R\Psi$}, by an
effective baryon density of $-2.9$ per cent, which is the wrong sign for that reading. The Doppler term,
which fills troughs, is \emph{stronger} here: the dipole-to-monopole amplitude ratio at the visibility peak
sits two per cent above the control's and rises with wavenumber. And the visibility is \emph{wider},
$43.6$ against $38.0$~Mpc, so this arm averages over more acoustic phase and not less. \emph{So three
channels that would each shallow the troughs are all larger here, and the contrast excess stands on top of
them.} \emph{And the baryon density cannot supply it at any value}: the contrast's response to $\omega_b$
saturates, spanning $0.028$ over the whole range with increments falling monotonically, so four per cent is
beyond its reach rather than away from its fitted
point\rcpt{P15_the_monopole_zero_point_is_not_the_channel_and_the_two_residuals_point_opposite_ways}.

\emph{The comparison between the arms is taken as a distance between
the two models rather than as a difference of two numbers each read against the sky}, which is what makes it
independent of the configuration's own floor\rcpt{P15_the_floor_is_a_distance_between_models_not_a_number_from_the_data}.

\emph{The instrument's own accuracy is measured rather than assumed.} On the full range its control sits at
$1.16$ per bin lensed, within about a seventh of what a standard code returns on the same bins, so the ratio
above is a statement about the arm and not about the machinery. \emph{What the comb does not carry is the
disagreement}: at $298.0$ against $298.4$ the spacing is right to a part in six hundred, on both instrument
paths, so what remains is about one multipole at each peak.

\emph{The result is reproduced independently on a second instrument.} A separate implementation---the
hierarchy path rather than the line-of-sight one, $185$ covered bins rather than $133$, its own wavenumber
grid---returns peaks at $220$, $540$, $820$ and $1132$ against this one's $222$, $538$, $818$ and $1134$,
height ratios $2.273$ and $2.319$ against $2.264$ and $2.298$, and $4.02$ per bin in the band from $\ell=700$
to $1000$ against $4.23$: \emph{within a grid step on every peak and within one per cent on the
heights}. \emph{The fourth peak's excess reproduces there too}, at $1132$, so it is a property of the
spectrum rather than of either implementation.

\emph{The heights are the polarisation path's and the positions are not.} Both paths return the same comb to
four figures; the height ratios differ between them by more than either differs from the sky --- $2.264$ and
$2.298$ on the polarisation path against $2.448$ and $2.934$ on the fluid one --- so every height quoted here
is the polarisation path's and is stated as such, while the position, the phase and the alternation are the
construction's.

\emph{Where the plasma starts moves the scale and not the peak, which is why the start is not free.}
Scanning a hand-placed start over a factor of $3.6$ runs $\ell_{A}$ from $360.6$ down to $239.3$ while the
first peak sits between $206$ and $210$ throughout\rcpt{P15_the_one_fitted_number_moves_the_scale_and_not_the_peak}:
it moves the denominator of $\ell_1/\ell_A$ and leaves the numerator where it
is\rcpt{P15_zonset_determinations}, so the spacing such a start reports is not the spacing its spectrum carries.

\emph{What that leaves is the equality, and it is measured rather than
supposed.} Holding everything else fixed and moving matter--radiation equality alone carries the third peak
and the band it sits in with it: the height ratio $P_1/P_3$ tracks equality on both routes to it --- through
the radiation density and through the matter density, which move the projection in opposite directions ---
agreeing to within a per cent across a $12$ to $24\%$ change in the matter density, so what the band responds
to is the equality and not the route. \emph{And the epoch the distance data fix is what sets it}: equality is
$\omega_m/\omega_r$ with the radiation density measured, so the background that meets the baryon-acoustic
distances is the same background that drives the peaks.

\emph{Two things this section does not claim, and they are separated because a reader would otherwise
supply them.} The alternation figures are read on a multipole grid whose step is the same size as the
sky's own gap contraction is large, so \emph{a contraction of the sky's size is resolved while the smaller
ones at lower $\ell$ are not}\rcpt{P15_the_phase_moves_with_the_datum_and_only_the_spacing_does_not}; a finer
grid is what would settle those. And the equality result accounts for the band it moves and not for the phase
residual: \emph{what the phase is to be read against is the driving subtraction on this arm, and
\S\ref{sec:scope} carries it as such.}
...
With the mass capped at the recollapse threshold $M\le\alpha/3\sqrt3$ and the radiation fraction at the largest value the observed spectrum permits, Eq.~\eqref{eq:transfer-law} on vacuum data returns $A_s\sim1\times10^{-112}$, short of the observed $2\times10^{-9}$ by $\sim10^{103}$. \emph{The figure is read on the potential's scale-free branch}; on the observed band, which lies above
the break the radiation content sets, the residue is some thousands larger in power, which moves the
shortfall by a few orders out of a hundred and three and leaves the conclusion where it is.
\end{proposition}
...
\emph{So ten orders separate the substrate's vacuum from the progenitor's amplified one---Eq.~\eqref{eq:transfer-law} carries the crunch mixing already, so the amplification is the $\rho^{-6}$ and is not a second factor on top of it---and it is still short by a hundred and three}, and the classical character of the primordial statistics is established for the source that actually supplies them rather than only for the one that does not. Two consequences of \emph{character}, not of number, follow. 

First, the observed amplitude is inherited classical content---boundary data from the progenitor, not a quantum fluctuation generated within the Schwarzschild--de Sitter era, nor one generated in the progenitor's collapse. 

Second, there is no inflationary consistency relation: $r=-8n_t$ presupposes that scalar and tensor spectra are one vacuum stretched by one expansion, and with the amplitude not vacuum-sourced that link is absent; the substrate tensor floor is the same $\sim10^{-122}$, so there are no substrate-sourced primordial $B$-modes, and any observed tensors would themselves be inherited content rather than a measure of a substrate inflationary scale. \emph{And the tensor statement can be made unconditionally, which the scalar one cannot.} The scalar law, Eq.~\eqref{eq:transfer-law}, is the true-variable law only through the hydrodynamical $z_S$ of the progenitor's own fluid, which carries that fluid's equation of state and sound speed---and across the progenitor's own equality neither $w$ nor $c_s$ is constant. 
...
A field on $\dS_2\times S^2$ (Fig.~\ref{fig:seam-merger}) decomposes in $S^2$ harmonics; the degree-$\ell$ harmonic becomes a $\dS_2$ field of effective mass $m^2=\ell(\ell+1)/r_{N}^2$, i.e.\ $m^2/H_{\dS_2}^2=\ell(\ell+1)$. With the $\dS_2$ index $\nu^2=\tfrac14-m^2/H^2$, the monopole $\ell=0$ has $\nu=\tfrac12$---a scale-invariant base---while every $\ell\ge1$ has $\ell(\ell+1)\ge2$, hence $\nu^2<0$: the heavy principal series, which oscillate and decay through the throat. We read this as an angular no-hair, and the reading is grounded rather than asserted: the $\dS_2$ factor of the throat is a positive-$\Lambda$ epoch, and the de~Sitter no-hair theorem~\cite{Wald1983} damps exactly the anisotropic content---every $\ell\ge1$ harmonic is heavy principal-series, oscillating and decaying, while only the isotropic monopole $\ell=0$ survives as the $\nu=\tfrac12$ base---so the throat isotropizes what passes through it (receipts: \texttt{verify\_throat\_tower.py}\rcpt{P15_verify_throat_tower}). The geometry is established (Proposition~\ref{prop:throat}) and the mode tower follows from it; the no-hair reading is the standard theorem read on the throat, and we flag one guard as load-bearing: the index $\ell$ of this throat tower is the $S^2$-harmonic degree of the \emph{near-horizon} geometry at areal radius $1/\sqrt\Lambda$, and is \emph{not} the observable microwave-background multipole, which lives on the last-scattering sphere of radius $D_C$. The map between them is \S\ref{sec:largescale}, not an identity; conflating them would manufacture a prediction the geometry does not make. \emph{A second guard belongs beside it, and it bounds what the tower damps.} The computation above is a statement about propagating field modes---degrees of freedom carrying a mass on the $\dS_2$ factor, which is what being heavy principal-series means. \emph{The low multipoles of a perturbation about a spherically symmetric background are not such modes}: $\ell=0$ carries the mass and the axial $\ell=1$ carries the angular momentum, the latter a stationary slow-rotation configuration rather than an oscillation, with the radiative content beginning at $\ell\ge2$---the multipole face of the non-radiative skeleton the range paper describes~\cite{JanzenRange}. \emph{So what the throat isotropises is the radiative content, and a conserved charge sitting in a constrained sector is not among the modes this argument is about}\rcpt{P15_the_angular_momentum_sector_is_not_a_propagating_mode}. \emph{This is a bound on the claim and not a claim in the other direction}: that the angular momentum is not removed \emph{by this mechanism} says nothing about whether it is carried off by matter or torqued on the way, which this paper does not settle. \emph{And the geometry the tower is computed on is itself a non-rotating one}, Proposition~\ref{prop:throat} following directly from $f$ at the Nariai mass. \emph{So it is worth measuring how much the conclusion rests on that.} Writing $\lambda$ for the ratio of the $\dS_2$ curvature radius squared to the $S^2$ one, the index generalises to $\nu^2=\tfrac14-\lambda\,\ell(\ell+1)$, with $\lambda=1$ the equal-radii case above. \emph{The monopole is untouched}: $\nu^2=\tfrac14$ for every $\lambda$, so the scale-invariant base does not depend on the radii being equal. \emph{And the damping persists well away from it}: every $\ell\ge1$ remains heavy principal-series for $\lambda>1/4\ell(\ell+1)$, which at $\ell=1$ is $\lambda>1/8$---the $\dS_2$ radius would have to fall below $0.354$ of the $S^2$ radius, a factor near three, before the dipole ceased to decay, with the higher multipoles safer still\rcpt{P15_the_tower_is_robust_to_the_throat_being_squashed}. \emph{What ratio a rotating progenitor would produce is not computed here}, and a rotating near-horizon geometry is generally warped rather than a direct product, so a single ratio may not describe it; what the margin establishes is that the conclusion does not turn on the equal-radii point itself. \emph{The ratio can in fact be computed on the rotating family, and it runs the helpful way.} Kerr--de~Sitter's radial function $\Delta_r=(r^2+a^2)(1-r^2/\alpha^2)-2Mr$ reduces at $a=0$ to $r^2f(r)$, and its degenerate locus $\Delta_r=\Delta_r'=0$ gives $a^2=r^2(\alpha^2-3r^2)/(\alpha^2+r^2)$, which returns $r_{N}=\alpha/\sqrt3$ and the Nariai mass at $a=0$ rather than presupposing them. \emph{The family is bounded}: $a$ reaches $(2-\sqrt3)\alpha$ at $r/\alpha=0.3933$, where $\Delta_r''$ vanishes and the double root becomes triple---the ultracold member. \emph{Along it the ratio begins at unity and rises}: $\lambda=1$ exactly in the static case and $\lambda>1$ monotonically with $a$, so rotation carries the throat away from the $1/8$ threshold rather than toward it\rcpt{P15_rotation_moves_the_throat_away_from_the_damping_threshold}. \emph{That ratio is read at the pole, where the sphere's radius is $r_N$}; the rotating near-horizon sphere is warped through $r_N^2+a^2\cos^2\theta$ and a single ratio does not describe it, so what the calculation establishes is the direction and its size at one well-defined point, with a factor of eight in hand at the outset.

\emph{The warped problem is done directly, and does not need the proxy.} The
near-horizon limit of the degenerate family, taken as a limit rather than posited, is a warped
product and not a direct one: $\rho_0^{2}=r_{0}^{2}+a^{2}\cos^{2}\theta$ multiplies the $\dS_2$ and
polar directions alike, and the $\dS_2$ is fibred over the sphere by
$\tilde k=2r_{0}a\Xi/(r_{0}^{2}+a^{2})$, which vanishes only at $a=0$. A massless field still
separates on it, the warp cancelling from the relation between the transverse eigenvalue and the
$\dS_2$ operator because it multiplies both. \emph{For the axisymmetric modes it cancels
altogether}: the transverse problem is $(\Delta_\theta\sin\theta\,Y')'+E\sin\theta\,Y=0$ with
$\Delta_\theta=1+(a/\alpha)^{2}\cos^{2}\theta$---a spheroidal deformation of Legendre in which
$\rho_0$ does not appear at all, so the harmonic label survives the warping rather than being
spoiled by it. \emph{The decay condition generalises to}
$\nu^{2}=\tfrac14-L^{2}E-L^{4}m^{2}\tilde k^{2}$, the last term the rotation's own fibration, which
is non-negative and so can only deepen the damping. \emph{Two exact bounds then settle the family
without a scan}: $L^{2}\ge1$ on the physical branch and $E\ge2$ above the monopole, each with
equality only at $a=0$, so $L^{2}E\ge2$ against the threshold $\tfrac14$. \emph{The monopole keeps
$\nu^{2}=\tfrac14$ exactly on the warped sphere too.} \emph{So the conclusion holds across the whole
family $0\le a<a_{\max}$ with the same factor of eight, and the marginal member is the
non-rotating one}---rotation strictly improves the case rather than threatening
it\rcpt{P15_the_warped_throat_keeps_every_mode_above_the_monopole_decaying}.
...
\emph{Three independent routes put the approximations at their limit on the same scale}: the transmission
calculation's boundary at $\ell\approx2.5$, the projection's incompleteness below
$k\sim2\times10^{-4}\,\mathrm{Mpc}^{-1}$ ($\ell\approx3$), and this parameter's reaching order unity at $n=2,3$.
Whether that is a loss of control is decided by computing the filter rather than estimating it.
...
The decomposition's decisive proof is here: \emph{why} the substrate transmits the spectral shape rather than imprinting one of its own. It turns on how a mode approaches the locus it meets. Two horizon cases bound the question: a non-degenerate horizon, and the degenerate \emph{Nariai double root}---the Nariai seam, at $r=+\alpha/\sqrt3\equiv-2\alpha/\sqrt3$, where $f$ and $f'$ vanish together so that $\kappa=0$. \emph{The locus the modes cross is neither}: the branch point sits at $r=0$, carries no surface gravity because $f$ diverges there rather than vanishing, and is not a Killing horizon at all.
...
The proposition shows that a degenerate Killing horizon carries no scale, so it cannot imprint one, and Proposition~\ref{prop:transmit} shows the same of the branch point. The
...
observed is therefore the progenitor's, established from both sides of the branch point rather than one.
...
therefore cannot distinguish a transmitted phase from a reset one}, because a rescaling of $r_{s}$ is absorbed by the angle it is read against. Any discriminant must be a quantity that does not take the same factor---the diffusion scale is one, since $r_{D}$ does not---and that is the discriminant of principle. \emph{The comparison is made, and it does not require $r_{D}$.} The leg's accumulated conformal time is fixed by the geometry rather than fitted: the collapse branch $r=-(2M\alpha^{2})^{1/3}\cosh^{2/3}(3\,\mathrm{Re}\,\tilde\tau/2\alpha)$ has $\cosh\ge1$, so $|r|\ge(2M\alpha^{2})^{1/3}=3766.6$ Mpc and the branch point is reached only across the lift, along which $\mathrm{Re}\,\tilde\tau$ does not advance and no conformal time accumulates. Hence $\eta_{\rm bp}\approx5\times10^{3}$--$10^{4}$ Mpc, and $r_{s}^{\rm eff}=r_{s}+\eta_{\rm bp}/\sqrt3$ is \emph{dominated} by the leg term---$3.0$--$5.9\times10^{3}$ Mpc against $r_{s}=144$ Mpc. The absorption that takes a small rescaling cannot take this one: reproducing the measured $\ell_{A}=301.8$ would require $D\approx2.9$--$5.7\times10^{5}$ Mpc against the actual $1.4\times10^{4}$ Mpc, a factor of $21$--$41$. \emph{The acoustic phase is therefore reset at the branch point and not carried across it}---established on the peak positions alone\rcpt{BRANCHPT_transmission_character}. The result is robust to the open account of the lift: any conformal contribution the lift is found to carry only adds to an already-excluded total.
\end{remark}
...
The result stands on what the substrate determines. What it does \emph{not} yet claim has been carried to a sharp, gradable edge rather than left vague---and those edges are the most interesting part of the programme from here, each a gap to be closed, not papered over. 
...
The buildable ones below are named as debts, not softened into frontiers. \emph{Of the four, three are run and one is a frontier}: the end-to-end branch-point-to-recombination transfer, the large-angle depth and the likelihood are built and confronted below, what the transfer leaves being a phase residual to be read against the driving subtraction on this arm; and the derivation of the progenitor spectrum is the genuine frontier rather than a debt. Four stand out:
\begin{enumerate}
\item the sufficiency of the null-boundary characteristic data for the observed acoustic coherence (\S\ref{sec:coherence}), argued from the structure of the characteristic initial-value problem, with the time-reversal driving equality established there as a Fourier theorem and the handover at the branch point fixing the phase and the amplitude the datum leaves open. \emph{The end-to-end branch-point-to-recombination transfer is run} (\S\ref{sec:refit-bound}): a two-arm line-of-sight Boltzmann integral, $\Delta_\ell(k)=\int S\,j_\ell(k(\eta_0-\eta))\,d\eta$, carried on this cosmology's own background with the perturbations on the leaf congruence the framework assigns them, and validated on its control to $0.23\%$ in the first-to-second height ratio in the configuration the arm's own numbers are taken in\rcpt{C59_the_control_reproduces_camb_and_the_height_defect_was_k_truncation}. On this arm it returns a comb of $298.0$ against the sky's $298.4$ and height ratios $2.264$ and $2.298$ against $2.217$ and $2.277$, with the likelihood still rejecting this arm's spectrum on shape; the diffusion-scale signature it carries is located by the joint fit as a Gaussian residual in $\ell$ that no tilt removes (\S\ref{sec:coherence}). \emph{What the transfer leaves is about one multipole at each peak, and the sky cannot resolve it}: the comb is right to a part in six hundred, the phase intercept sits inside the sky's own locating spread, and the fourth peak is $0.9$ to $1.3$ standard deviations out with most of that offset shared with the control. \emph{What remains is not a position at all}: the difference between this arm and the control is a \emph{contrast} about the acoustic envelope---not a phase and not an envelope---it carries three quarters of the likelihood's excess, and its channel is the balance between the monopole and the Doppler terms of the source, bracketed with opposite signs on both instrument paths, with no mechanism for the imbalance offered (\S\ref{sec:refit-bound});
...
Confronted with the sky the sector is a \emph{wash}: CR's smooth $\sim0.4$--$0.5$ deficit sits above the sharp observed quadrupole ($\sim0.2$) and below the near-$\Lambda$CDM octopole ($\sim0.6$--$1.0$), matching neither, and the exact cosmic-variance likelihood returns $\Delta(-2\ln L)=+1.58$ over $2\le\ell\le8$ and the same to the digit over $2\le\ell\le10$, both arms being at unity by $\ell=8$ (range $+0.2$ to $+3.4$ across the octopole estimator), a small residual inside cosmic variance --- \emph{and it is a near-cancellation rather than a uniform cost}: per multipole the quadrupole rewards the construction by $-2.5$ and $\ell=3$ to $5$ penalise it by $+1.0$, $+2.0$ and $+1.0$, with nothing above (receipt: \texttt{verify\_lowell\_likelihood\_v2.py}\rcpt{P15_verify_lowell_likelihood_v2}). So the low multipoles are a genuine but \emph{non-discriminating} sector, neither a parameter-free correspondence success nor a sharp falsification edge; the deficit's existence and location stand as a real parameter-free feature, but its depth places it consistent with $\Lambda$CDM within the lowest multipoles' cosmic variance.

\emph{The location is robust and the depth is not, and the difference between them is now measured rather
than estimated.} On the background the distance data fix, the discrete scale moves by a quarter of a per cent
and the lowest mode lands at $\ell\simeq7.8$, so \emph{where} the deficit sits is not a background choice.
\emph{The two Boltzmann treatments still differ by a factor of two at $\ell=4$} --- and that factor is not the
geometry: moving the discrete scale alone buys three per cent against a gap of a hundred. \emph{It is the late integrated Sachs--Wolfe term}, which at these multipoles is sourced above the discrete
floor and is therefore retained by the sum, filling the deficit; the treatment that truncated its
line-of-sight integration earlier was deeper at exactly the multipoles where the two disagreed. \emph{Carried
to the present instead, the two treatments agree}: on the exact transfer the deficits are $0.487$, $0.435$,
$0.359$, $0.666$, $0.911$, $0.983$ and $0.998$ of the flat expectation at $\ell=2$ through $8$ ---
\emph{the exact transfer being the one carried here, with the hierarchy agreeing} to within three per cent at
every multipole and one per cent above $\ell=5$\rcpt{P15_the_low_multipole_floor_moves_with_no_background_and_the_factor_two_is_the_late_isw}. \emph{The factor of two was the missing term and
not a difference between the constructions}, and the agreement is stable: it does not move under a doubled
opacity grid, a halved late step count, or a further-extended continuum. \emph{So the shape and the depth
both cross-validate} --- minimum at $\ell=4$, recovery by $\ell=8$ --- and the sector is ready to be scored
against the measured multipoles with cosmic variance carried. 

The exposed edges lie elsewhere---the acoustic contrast difference of the first item and the damping signature it carries;
\item the progenitor spectrum itself---$n_s$ and $A_s$ (\S\ref{sec:transmission})---inherited boundary data fixed by the progenitor collapse, the baryogenesis-analogue computation belonging to the matter sector~\cite{JanzenOperator,JanzenRange}, with the target a consistency between the inherited composition and spectrum and the measured primordial abundances and tilt, not a derivation from the geometry alone---the most ambitious reach of the four, the one that would close the decomposition from both ends and give CR a derivation where the standard model keeps a measured datum. \emph{And the target is now a number rather than a direction}: refitting both arms on this construction's own background prefers a tilt of $0.995$ here against $0.956$ on the standard one (\S\ref{sec:refit-bound}), \emph{so a derivation has to land near the bluer value and not near the one the standard model infers}. \emph{And what supplies it is the progenitor's vacuum and not the collapse leg}: on the leg both the potential and the effective temperature are functions of the single variable $k\eta/\sqrt3$, so a handover at a fixed phase is exactly scale-free---the ratio of amplitudes at two wavenumbers is one---and a handover at a fixed time would contribute not a tilt but a running, red in both channels and growing as $k^{2}$, which no amplitude choice makes look like a constant. \emph{At the branch point it vanishes quadratically}, so the leg is a wavenumber-independent amplitude and
nothing else. \emph{And what the substrate alone fixes is $n_s=1$ exactly}: in conformal time the mode
equation carries no length, so the curvature radius enters only as the prefactor that sets the amplitude and
reaches the tilt for no value of it; and on the forced member the progenitor's mass is that same radius up to
a pure number, so the two candidate scales are one and fall together. \emph{But the fluctuations are not
generated on the substrate}, and the interior they are generated in carries a scale the substrate does not.
\emph{Solved there}, the mode potential interpolates between the scale-free matter form and a radiation form
that is not, so the generated spectrum \emph{breaks} at the wavenumber the radiation content sets: its
log-slope runs from $+0.30$ to $+1.98$ across the observed band, tending to the radiation value $+2$.
\emph{So it is a running and not a tilt}, by the same band test that closes the collapse leg --- and
\emph{blue at every wavenumber}, where the measured value is red by two parts in a
thousand\rcpt{P15_the_interiors_vacuum_is_blue_everywhere_and_a_running_so_the_substrates_mass_template_does_not_apply}. \emph{The flattest slope goes as the
radiation fraction}, so exact scale-invariance is the pure-dust limit and reaching red would need that
fraction negative. \emph{And the running has a domain, which is what reconciles it with exact scale-invariance in the same
interior}: the potential breaks at a wavenumber the radiation content sets, so a band lying wholly below
that break is scale-free to a fifth of a per cent while the observed band lies above it. \emph{One object,
two bands.} \emph{And the transfer from that vacuum to the boundary datum is not a
second place a wavenumber can enter}: it is the same object as the spectrum, and at the crossing the
wavenumber is regular where the potential is singular, entering the analytic solution only at third order
so that the logarithmic coefficient carries no dependence on it at
all\rcpt{P15_the_transfer_is_the_same_object_as_the_vacuum_spectrum_and_the_crossing_carries_no_wavenumber}.
\emph{So the tilt is the vacuum's and nothing downstream of it tilts.} \emph{And the substrate's power-law formula does not apply off the substrate}: its
coefficient is dimensionless, which is what makes it a power law, where the interior's carries a scale---a distinction invisible to a tilt read off a background fitted to the other arm. \emph{The figure is provisional in one respect and it is stated}: it is measured on the range below the damping tail, which is where a tilt has most of its leverage. \emph{One route to it is closed, and closing it narrows the
target rather than the ambition.} The construction is recursive---each progenitor seeds the next
cosmology---so a \emph{stationary} recursion would make $A_s$ a fixed point and determine it. But entropy per
baryon rises strictly at every branch-point crossing, the crossing being an erasure and the increment bounded
below by Landauer's $k_B\ln2$ independently of the network and of $\eta$~\cite{JanzenCosmogenesis}: \emph{a map
that strictly increases a coordinate has no fixed point}, and independently the chain it generates is finite, so
there is no asymptotic limit for a fixed point to be the limit
of\rcpt{P15_the_fixed_point_route_is_closed_by_the_monotone}. \emph{So the derivation must reach the head of a
bounded genealogy rather than a stationary state}---the same terminus the inherited composition reaches, and the
two are one question asked of two quantities;
...
The low multipoles are therefore a genuine but \emph{non-discriminating} test: CR and $\Lambda$CDM are statistically indistinguishable there, the mild deficit neither favouring CR nor refuting it. Decisiveness comes from outside the sector---the geometric expansion rate~\eqref{eq:native-rate}, confronted with the distance ladder (\S\ref{sec:tensions}), as the standing discriminator, and the likelihood across the \emph{full} spectrum, which is run and rejects this arm's spectrum on $TT$ shape (\S\ref{sec:refit-bound}), its amplitude level not established by the time-reversal driving equality (\S\ref{sec:coherence}).
\end{enumerate}
...
The comparison with the standard cosmological model is accordingly not \emph{only} a contest of fits---though the geometric rate fits the joint acoustic-and-distance-ladder data at one $H_0$ (\S\ref{sec:tensions})---but also, and more deeply, a difference in what each must assume to do so. The standard model \emph{assembles} the observed universe: a dark-energy component for the late-time acceleration, an inflationary sector for the causal contact, flatness, coherence, and near-scale-invariance the background dynamics do not themselves supply, and further early-universe physics where the sound horizon and the expansion rate come into tension. Each part is a real solution to a real problem, and each was introduced to yield the feature it produces. \emph{This cosmology adds no such part.} Causal contact, isotropy, flatness, phase coherence, and scale-free transmission are read off structure the construction already contains---the isotropizing throat (\S\ref{sec:throat}), the null boundary on which coherence \emph{is} the characteristic-data condition (\S\ref{sec:coherence}), the exactly Euclidean constant-time slice (\S\ref{sec:largescale}), the scale-free crossing that transmits rather than imprints (\S\ref{sec:transmission})---each fixed by $\Lambda$ alone, with no parameter free to produce it. This is the distinction between a structure that \emph{requires} a phenomenon and one that merely \emph{permits} it through adjustable apparatus~\cite{JanzenShadowExistence,JanzenModernParallax}, and it is the criterion by which competing frameworks are rightly weighed ahead of the measurement that will decide between them.

\emph{The dark-energy component is the plainest case of the difference}: on this cosmology the acceleration is not a sector at all. Since $(\mathrm{d}r/\mathrm{d}\tilde\tau)^{2}=E^{2}-f$ on the energy family, one has $\mathrm{d}^{2}r/\mathrm{d}\tilde\tau^{2}=-f'/2=rK_{G}$: \emph{the comoving acceleration is the existent slice's own Gaussian curvature}, up to the positive factor $r$.\rcpt{P03_acceleration_is_slice_curvature} So the expansion decelerates while the slice is negatively curved, turns over exactly where it is flat---the Hubble--Eddington radius, $\rho_{m}=2\rho_{\Lambda}$, at $\sinh(3c\tilde\tau/2\alpha)=1/\sqrt2$---and accelerates thereafter, with nothing added to make it do so.  \emph{And that locus is a
...
returns $0.99940$ rather than unity. \emph{So what is derived here is the root of the standard
normalisation}, and what remains contingent is that the present epoch lies near it.

\emph{What that figure does \textbf{not} measure is how far the root moves, and saying so matters because it
invites that reading.} Continuing the closed form onto the leaf rate is a closed form applied to a rate it
does not solve. \emph{Solved directly, the root moves the other way and by less} --- the leaf root sitting at
$u_0=1.180039$ against the stacking $1.180309$, a shift of $1.1\%$ of the gap to the measured epoch and
\emph{away} from it, taking the offset from $0.704$ to
$0.713\sigma$\rcpt{P15_the_growth_root_on_the_leaf_rate_is_not_a_moment_balance_and_the_gap_stands}. \emph{So
the root's two-per-cent offset from the measured epoch---$x_{0}=1.6315$ at the leaf root against the measured
$1.6648$---is not an artefact of the rate}: it stands, and wants a mechanism.

The transmission dichotomy (\S\ref{sec:transmission}) is where the distinction between requiring and permitting is sharpest, because there the two frameworks make opposite structural commitments about one object. A non-degenerate horizon's exponential approach imprints a scale-invariant spectrum---the very mechanism by which inflation produces near-scale-invariance---while the degenerate Nariai seam carries no scale and the branch point, where the modes cross, accumulates no phase at all, so the progenitor spectrum is transmitted unaltered. Inflation accommodates the observed tilt because a freely specified potential can be tuned to it; reading that fit as a confirmation mistakes a model with enough adjustable structure to match for one that required what was found (\S\ref{sec:predictions}). This cosmology cannot tune: the finite tortoise coordinate at the branch point \emph{requires} that the crossing not imprint a scale, so the framework has no inflationary scale-invariant attractor, no consistency relation, and no substrate-sourced primordial $B$-modes. The amplitude and tilt it does not derive it inherits, exactly as flat $\Lambda$CDM inherits the baryon-to-photon ratio from a baryogenesis it does not model; on that single datum, of the same kind and count the standard model already carries, the two stand level. A framework is credited for requiring the structure it explains, not faulted for measuring the boundary data it does not---and the asymmetry falls on everything else.

The verdict this licenses must be drawn on the right axis, and confined to it. On the axis of theory-choice---the requiring rather than the permitting of the phenomena, the consolidation of many results under one structure, the absence of a device added per problem---the comparison is decidable from the structural record, and it falls to this cosmology~\cite{JanzenShadowExistence}. On the axis of data-discrimination the picture is split: the geometric rate fits the acoustic scale and the baryon-acoustic distance ladder together at one $H_0$ (\S\ref{sec:tensions}), leaving the discrepancy with the local ladder where the standard model leaves it---while on the light elements and the low multipoles the two frameworks are even. \emph{On the microwave-background peak structure they are not}: the transfer's likelihood comparison rejects this arm's spectrum on TT shape (\S\ref{sec:refit-bound}), and that is the sharpest datum against the construction that the corpus holds. So the standing is stated as it stands: favoured on the structure, level with the standard model on the light elements and the low multipoles, and carrying a spectrum computed from a background fitted to the distances alone---which reproduces the comb to a part in six hundred and is rejected on shape. The data axis is not even, and where it is uneven it runs against the construction, which is the direction a framework that has staked falsifiable commitments---no scale-invariant attractor \emph{reaching the observer}, no consistency relation, no substrate $B$-modes---must be prepared for, precisely where the standard model keeps its freedom\rcpt{P15_the_continuation_is_diagonal}.

The low-multipole deficit is a mild, non-discriminating feature (\S\S\ref{sec:largescale}, \ref{sec:scope}). The flat projection of the discrete closed-$S^3$ source gives a parameter-free deficit below $\ell\approx8$---real, and set by $\Lambda$ through $r_0$ and $D_C$---bottoming at $\ell=4$, at $\approx0.47$, $0.41$, $0.36$ and $0.68$ of the $\Lambda$CDM expectation at $\ell=2,3,4,5$. At that depth CR's smooth deficit matches neither the sharp observed quadrupole-only dip nor the near-$\Lambda$CDM octopole, and under the exact cosmic-variance likelihood the two frameworks are statistically indistinguishable at low $\ell$ ($\Delta(-2\ln L)\approx+1.8$ over $2\le\ell\le10$, inside cosmic variance). So the sector neither confirms nor refutes: it is a genuine parameter-free prediction whose depth places it consistent with $\Lambda$CDM within the lowest multipoles' cosmic variance. The framework's exposed edges are the acoustic peak structure, where the transfer leaves a phase residual and a fourth peak the comb does not carry (\S\ref{sec:refit-bound}), and the damping-\emph{scale} signature (\S\ref{sec:coherence}), a computed, non-reabsorbable effect whose size the handover fixes and whose sign the diffusion integral's endpoint does. Its sharpest test is the geometric expansion rate itself, carried to the distance ladder and confronted with the DESI~DR2 baryon-acoustic dataset at $\chi^2/\text{dof}\simeq1$ (\S\ref{sec:tensions}): the rate that carries the full ladder is the rate the spectrum is computed on, so the two are one commitment.

The framework's standing includes what the standard model has genuinely built and this cosmology has not. Dark matter is inferred from multiple independent, cross-consistent lines---rotation curves, gravitational lensing, the merger offsets, the acoustic-peak structure, the growth of structure---and this cosmology carries the matter density as the bend of the spatial cut without yet removing the component or answering its particle question; that column stands at full weight. Standard nucleosynthesis derives the light-element abundances from one measured parameter, a genuine achievement this cosmology meets on the same footing rather than exceeds: the collapse's cooling leg is a standard nucleosynthesis run on one inherited datum (\S\ref{sec:tensions}; the cosmogenesis paper~\cite{JanzenCosmogenesis}), producing helium-4 and deuterium at their observed values---the freeze-out temperature the data demands being the bottleneck the standard rate delivers---and, because the run is standard, carrying lithium-7's standard threefold over-prediction, the lithium problem shared rather than solved. On the light elements the two frameworks stand even: the same two successes, the same one problem, one progenitor handover fitting the pattern together, and this cosmology's open edge the last-percent precision against the measured abundances. And the framework is young: the \emph{detailed} dynamics of matter crossing the branch point (its crossing shown well posed and structurally governed, \S\ref{sec:scope}), the derivation---as against the measurement---of the inherited data, and the Standard-Model matter \emph{content}---the gauge representations and the mass spectrum, which the framework does not supply, its geometry shown to force only the discrete flavour skeleton~\cite{JanzenMatter}---remain to be built (\S\ref{sec:scope};~\cite{JanzenCRframework}). These are open frontiers, not defects: a theory is not marked down for what it has not built, only for a claim it makes that fails. They are named plainly here because an economy that favours this cosmology on the structure it carries would be worth nothing bought by concealing the structure it does not.

\section{Conclusion}\label{sec:conclusion}

So, plainly, what this paper claims and what it does not. It claims that the observed expansion history, the computation of the acoustic scale, and the structure of the primordial scalar spectrum are consequences of a single de~Sitter geometry read through the causal reassignment its own horizon geometry selects---one seed, grown, the field equations unchanged---and that on the economy of assumption, the criterion by which theory-choice is rightly made ahead of the deciding measurement, this reading is favoured over the assembled standard picture. On the data axis it claims a more limited thing: that the axis is split---the expansion history fitting the distance ladder, which fixes $H_0$ with the acoustic angle, and the abundances confirmed, while the acoustic peak structure, computed from that same background and scored by the full-spectrum likelihood, counts against it---with that structure and the damping-tail signature the edges still to be settled (the low multipoles a mild non-discriminating deficit consistent with $\Lambda$CDM within cosmic variance). It does not claim to be a complete cosmology: its matter sector, the derivation of its progenitor spectrum, and its fermion content remain to be built. It claims to have shown that the universe we observe is the reading of one geometry, and to have staked the falsifiable commitments by which that claim can be tried.
...
\bibitem{JanzenBHcausality} D.~Janzen, \emph{The event horizon is a metric singularity: a missing definition in general relativity, and the black-hole problems it dissolves}, companion paper (P1).
...
\bibitem{JanzenShadowExistence} D.~Janzen, \emph{The shadow of existence: scientific theory-choice as an empirically grounded discipline, calibrated on the historical record and on a live programme}, companion paper (P6).
...
\bibitem{JanzenSynthesis} D.~Janzen, \emph{Cosmological Relativity: what the construction delivers, what it declines, and where its edge is}, companion paper (P18).
...
D.~Janzen,
\emph{The Schwarzschild and de Sitter circle: the intrinsic geometry of one homogeneous ring---the event horizon and the curvature singularity at $r=0$ its two poles, run through by a single continuation}, companion paper (P2).
