...
envelope\rcpt{P15_the_height_target_was_below_the_resolution_of_its_own_statistic}. \emph{And the removal is
measured on the arm whose answer is known}: the truncation was $78\%$ of the control's remaining $\chi^{2}$,
...
rate, neither imported nor fitted} --- brings the heights from $13$--$34\%$ to $2$--$3\%$, and the
likelihood's floor falls by a factor of five \emph{without the verdict turning
...
cutoffs\rcpt{P15_where_the_likelihood_sits}. \emph{So the shortfall is not the configuration.} \emph{And what it is is more specific than a series
uniformly compressed}, which is worth stating because the coarser reading names the wrong quantity.

\emph{At production depth both arms carry eight peaks and the asymptotic spacings agree.} The control's last
four gaps average $302.0$ against its acoustic scale $301.4$; this arm's average $294.0$ against $301.6$
--- \emph{two and a half per cent}\rcpt{P15_the_spacing_is_right_and_the_acoustic_phase_is_wrong}. \emph{And
the disagreement is gone by the fourth gap}: gap by gap this arm runs $0.725$, $0.853$, $0.875$ of the
control's, then $0.973$, $0.949$, $1.000$, $0.974$. \emph{So the two series are parallel lines with different
intercepts.} Fitted on peaks four to eight the slopes agree to $2.6\%$ and the intercepts sit $0.615$ of the
acoustic scale apart --- \emph{a phase offset of $0.62\pi$} --- with a low-multipole transient this arm
carries and the control does not, its first three peaks sitting $142$ above its own asymptotic line where the
control's sit within $16$.

\emph{So the acoustics work and the phase is the disagreement.} The asymptotic spacing tracks the sound
horizon at $98\%$ of the acoustic rate, against the first peak's
$37\%$\rcpt{P15_the_acoustics_work_and_the_phase_is_the_whole_disagreement}; \emph{and the offset is the
construction's rather than the fit's}. Moving the one fitted parameter by $31\%$ shifts it by
$7\%$\rcpt{P15_the_spacing_deficit_survives_the_one_fitted_parameter}; opening both freedoms the seam datum
leaves unspecified spans the first-peak position by a factor of $2.26$ while the spacing holds within
$1.21$\rcpt{P15_both_datum_freedoms_opened_and_the_spacing_still_holds}\rcpt{P15_the_first_peak_is_the_seam_datum_and_the_spacing_is_not};
and over the phases the corpus admits the acoustic phase moves by $0.207$, \emph{a third of the gap and no
more}, with the control's value outside that
band\rcpt{P15_the_admissible_band_is_a_third_and_the_two_entries_differ_against_the_potential}. \emph{So the
disagreement is real and bounded.} \emph{And the driving shift measured model-free by subtraction confirms
where it lives}: the control's acoustic phase at first turnover is flat in wavenumber to $2.1\%$ while this
arm's varies by a factor of
$3.9$\rcpt{P15_the_driving_shift_by_subtraction}. \emph{The earlier reading of a
${\sim}21\%$ spacing deficit was the first three gaps measured where the arm had only four peaks}, and it is
withdrawn.

\emph{What that leaves is a measured
disagreement with no mechanism, and located: a phase.} The account that would have covered it---that the acoustic modes re-enter above
