...
The buildable ones below are named as debts, not softened into frontiers. \emph{Of the four, three are run and one is a frontier}: the end-to-end branch-point-to-recombination transfer, the large-angle depth and the likelihood are built and confronted below, what the transfer leaves owed being a mechanism for the deficit it measures; and the derivation of the progenitor spectrum is the genuine frontier rather than a debt. Four stand out:
\begin{enumerate}
\item the sufficiency of the null-boundary characteristic data for the observed acoustic coherence (\S\ref{sec:coherence}), argued from the structure of the characteristic initial-value problem, with the time-reversal driving equality established there as a Fourier theorem, though what that
equality carries about the peak-height \emph{magnitude} is what \S\ref{sec:coherence}'s own instrument
does not presently return. \emph{The end-to-end branch-point-to-recombination transfer is run} (\S\ref{sec:refit-bound}): a two-arm line-of-sight Boltzmann integral, $\Delta_\ell(k)=\int S\,j_\ell(k(\eta_0-\eta))\,d\eta$, carried on this cosmology's own background with the perturbations on the leaf congruence the framework assigns them, and validated on its control to $0.14\%$ in the first-to-second height ratio\rcpt{C59_the_control_reproduces_camb_and_the_height_defect_was_k_truncation}. On this arm it returns a comb of $298.0$ against the sky's $298.4$ and height ratios $2.264$ and $2.298$ against $2.217$ and $2.277$, with the likelihood still rejecting this arm's spectrum on shape; the diffusion-scale signature it carries is located by the joint fit as a Gaussian residual in $\ell$ that no tilt removes (\S\ref{sec:coherence}). \emph{What the transfer leaves is a phase residual of $2.3\%$ and the fourth peak $0.9\%$ high}, the comb itself being right to a part in six hundred;
...
The exposed edges lie elsewhere---the acoustic deficit of the first item and the damping signature it carries;
