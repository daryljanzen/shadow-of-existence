...
\emph{The transfer carries the effect directly, and its shape is the one the argument needs}: with the
anisotropic stress removed at both of its dynamical sites---the neutrino Euler equation and the shear
term in $\Psi$---so that the sector is a perfect fluid at the same background density, every peak on both
arms moves to larger multipole by the same amount across the first four, which is the signature of a phase
shift and not of a rescaled acoustic scale, while the amplitude moves by about a third and separately.
\emph{The shift's size is at the resolution of the binning and is not read off
here}\rcpt{P15_the_source_decomposition_is_reachable_on_the_reporting_path_and_the_neutrinos_have_a_knob}.
