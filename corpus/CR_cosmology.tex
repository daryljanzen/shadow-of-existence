...
\emph{The acoustic scale, to a one-parameter accommodation.} Recombination is observable cosmology from the branch point onward, so its scales are L1: both the sound horizon $\rs$ and the comoving distance $D_M$ ride the radiation-free foliation rate, and the observed angle $\theta_*=\rs/D_M$ is their L3 projection. 
...
The quantities this cosmology actually uses are $\Omega_m$, which the rate carries and the baryon-acoustic data fix; $\omega_b$ and $\omega_\gamma$, which the sound speed carries and which are measured directly and framework-independently; and $z_{\mathrm{onset}}$, the one fitted number. 
...
Because the radiation-free rate carries the common factor $H_0$ out of every distance and out of the ruler alike---$D_M$, $D_H=c/H$ and the sound horizon $\rs$ each scale as $1/H_0$---the baryon-acoustic-oscillation observables $D_M/\rs$ and $D_H/\rs$ are \emph{independent of $H_0$}, fixed by $\Omega_m$ alone (the same cancellation that fixes $\theta_*$; \S above). 
...
Here they do not: the acoustic data fix $\Omega_m$ and leave $H_0$ to the local distance ladder, with no second value to reconcile. Confronted with the SDSS~DR12 galaxy consensus~\cite{Alam2017} the radiation-free rate at $\Omega_m\simeq0.31$ fits at the directly measured $H_0$ ($\chi^2\simeq1.7$ on six points, at \emph{any} $H_0$ including $73$), where the radiation-governed rate at that same local $H_0$ misses badly ($\chi^2\simeq49$)---that residual is the Hubble tension itself, which the geometric rate structurally lacks. 
...
Evaluating the tightly-coupled mode function at last scattering with a \emph{common} seam phase gives a sharp regular comb, whereas drawing the phase independently per mode---the second, free datum a Cauchy surface would admit---washes it out entirely, leaving flat power (peak-to-trough contrast $\sim1$ against the coherent comb's many orders of magnitude; receipt: \texttt{verify\_coherence\_comb.py}\rcpt{P15_verify_coherence_comb}). \emph{That establishes the mechanism---one characteristic datum per mode is one phase per mode, and coherence follows---and it establishes nothing about where the peaks fall.} The receipt writes the mode function down as $\cos(k\rs)$ with $\rs$ and $D_{C}$ as literals; no mode is propagated in it, as its own scope note says, so the spacing $\Delta\ell\simeq\pi D_{C}/\rs\approx296$ is the arithmetic of the expression assumed rather than an output. \emph{It is not the result of propagating the modes, and is not offered as one}\rcpt{P15_the_first_peak_figure_is_not_stable}. 

Where the modes \emph{are} propagated the comb's first few gaps come out at $0.72$--$0.79$ of that spacing under every initial condition tried, but that is a shallow reading of a four-peak series: at the depth the transfer reaches, the \emph{asymptotic} spacing is $0.975$ of the acoustic scale (\S\ref{sec:refit-bound}). \emph{So the spacing is not the deficit}---what the peak spacing lacks is not agreement but a derivation, being asserted here and computed nowhere in this paper. 
...
This cosmology's driving does not live on an expanding radiation era, which the radiation-free rate does not possess; it lives on the \emph{contracting} side---the progenitor's radiation-comparable collapse phase, whose potential evolution is the Friedmann time-reverse of an expanding radiation era---so its driving magnitude equals flat $\Lambda$CDM's exactly, and the branch point's faithful transmission (\S\ref{sec:transmission}) carries it across unaltered. 
...
This is forced: it is the same L1 rate that dissolves the Hubble tension, and one may not take the rate radiation-free for the peak spacing and radiation-included for the diffusion. Because the radiation-free rate near recombination is ${\sim}13\%$ below the radiation-included one there ($\rho_r/\rho_m\approx0.3$), the diffusion length runs ${\sim}9\%$ longer than $\Lambda$CDM's while $\rs$ is matched to the observed acoustic scale; the projection-independent ratio is therefore $\theta_D/\theta_*\approx1.08\,(\theta_D/\theta_*)_{\Lambda\mathrm{CDM}}$---a $8.2\%$ larger damping angular scale at the onset redshift used there, the value moving with $z_{\mathrm{onset}}$ as \S\ref{sec:diffusion-scale} sets out (receipt: \texttt{damping\_ratio\_clean.py}\rcpt{P15_damping_ratio_clean}, validated against a Boltzmann reference on the radiation-included rate; $\rs=144.0$ vs CAMB $144.4$). 

This is the one genuinely CR-specific piece of the height pattern: a real signature \emph{locked to} the Hubble resolution by their shared L1 rate---not an artefact of laying the standard radiation-governed $\rs$ on the radiation-free rate, the calculation belonging to neither framework this paper warns against (\S\ref{sec:intro}). 
...
datum the radiation-free rate gives a diffusion length $10.8\%$ longer\rcpt{C8_diffusion_length}.
...
\emph{Of the two, one cannot pay at all}\rcpt{P15_what_is_left_is_lensing_and_not_reionisation}. Above $\ell\simeq40$ the
...
says\rcpt{P15_verify_coherence_comb}. \emph{So the peak spacing is asserted in one place and derived in none}---its asymptotic value where
the transfer is run being $0.975$ of the acoustic scale, which is agreement and not a deficit. We record
the missing derivation rather than a disagreement. \emph{The figure of $220$
...
$0.457$ to $0.117$. \emph{That is the acoustic-phase disagreement seen at the level of a single mode rather than of a
spectrum---a driving shift that varies with $k$ where the control's does not}, and it is what a rate
carrying no radiation source predicts: there is no crossing
...
\emph{The ordering is a fact and the ratio is not a $p$-value.} Planck's binned errors are a few
...
its own control}. \emph{Half of that cost is removable, and the removable half is the one that matters for the position}\rcpt{P15_the_line_of_sight_transfer}. 

Evaluated on a delta-function last scattering the transfer computes its source at a
...
$-2.1\%$\rcpt{P15_the_derived_diffusion_damping}. \emph{The envelope and the hierarchy's own tight-coupling damping act on disjoint intervals, and must}:
applied over the same range they double-count, and the control comes out \emph{over}-damped. \emph{On the
instrument's floor that is a factor of five---the likelihood's control at $\chi^{2}/\mathrm{dof}=23$ against
$103$ for a transfer carrying no diffusion damping at all}---without its verdict turning over: two to three per
...
sampling by $1.8$ moves the two ratios by $0.09\%$ and $0.37\%$. \emph{A multipole ceiling truncating the wavenumber
integral beneath the third peak moves them by two and seven per cent, which is why the ceiling is set
above it.}
...
inflated standard deviations. \emph{The fit does not decide it, and must not be allowed to}: a coefficient chosen because it fits is a
fitted parameter whatever it is called, and \emph{what the $1123$ measures is a residual error in this
instrument's damping tail of a size that a shear coefficient could absorb}---which is precisely the reason
it may not. \emph{The coefficient is settled by derivation instead} (\S\ref{sec:shear-coefficient}): the
free-streaming photon hierarchy carrying the polarisation multipoles returns $1.0618$ against $16/15$, and
the polarisation correction itself $1.1996$ against $6/5$, three parts in ten thousand.
...
derived coefficient cost. \emph{The baryons need their own density and their own velocity together, and either alone is worse than
neither}: with a density but the photons' velocity the density they carry is the photons' own and nothing
lets them fall into the potential wells after decoupling, and the control degrades; with both, the residual
halves. \emph{A physical effect taken in half is not a small error but a different and inconsistent model.}

What is arbitrary in the construction is measured rather than argued: doubling the hierarchy depth
...
factor of three}---it carries one massive species and this instrument carries none---so the neutrino mass accounts for
roughly a third of what remains. \emph{And the ordering between the two measures reverses}: the
...
control sat near a hundred times the data's precision. \emph{Carried at a floor small in absolute terms, the test reads.} With both arms
carried at the same wavenumber range---so that the construction is not charged for the instrument's
truncation, which the rule's third clause forbids---the floor is $31$ over the
...
for one order of magnitude and receives three.} \emph{And the separation is physics rather than arithmetic because it is invariant under the
instrument's accuracy while the floor is not}: run at four levels of transfer fidelity---a two-moment
fluid, a fluid carrying the derived shear coefficient, the full polarised hierarchy, and the same with
the wavenumber range opened---the control spans $21.2$ to $1.18$, eighteenfold, while this cosmology's
arm spans $290.7$ to $302.1$, four per cent. \emph{An error the instrument carries moves both
arms; this moved one.}
...
physical, which is a claim and not a measurement. \emph{Run on a continuum
...
\emph{That is a statement about the deficit rather than about the instrument.} On the continuum grid
this cosmology's arm does not move at all---$\ell_{1}/\ell_{A}=0.5703$ on both transfers, to four
...
the first three gaps, which is what a scan at that depth sees; the asymptotic spacing is given below.}
...
\emph{Both freedoms open together give the honest range, and it is the wider one}: a single freedom spans
$1.11$ where the pair spans $1.21$. \emph{Eight of the eighteen readings are excluded from the spacing by a criterion fixed
...
series is not uniformly compressed}: a mean over the first three gaps measures the transient, and the
asymptotic spacing measures the acoustics.
...
nearly a third. \emph{What that scan holds fixed is the mean of a short series, and the short series is
transient-dominated}: the gaps rise along it, and at the depth the transfer reaches the asymptotic spacing
is $0.975$ of $\ell_{A}$ (below). \emph{So what survives the fitted parameter is the transient, not a
deficit in the acoustic spacing}---which is the same conclusion the deeper series returns, read at the
shallow end. The first peak is twice as sensitive and still never approaches the sky's
$0.731$.
...
The two treatments differ in their late-time integrated Sachs--Wolfe handling and in $D_M$, and this paper does not reconcile them: it carries one converged depth table, the first arm's, and a second arm that agrees on where the deficit sits and not on how deep it is. 
...
The buildable ones below are named as debts, not softened into frontiers; the end-to-end transfer and the full-spectrum likelihood are the load-bearing debts, the derivation of the progenitor spectrum the genuine frontier. Four stand out:
\begin{enumerate}
