...
The control's sound horizon is integrated from $a\to0$ whatever its start, and this arm's from its
onset, and the two arms begin in different places --- \emph{a confound the instrument's own comment
names}, that a comparison between the arms confounds the rate with the start. \emph{Run symmetrically
the undriven split largely dissolves, and not monotonically}: with the start matched it does not
shrink but overshoots and \emph{changes sign}, its magnitude growing by a factor of two; with the
sound-horizon convention matched as well it falls to two fifths of its original size and stays on the
far side of zero. \emph{The two mismatches were partly cancelling}, which is why neither was visible
alone\rcpt{P15_the_symmetric_comparison_was_never_runnable_and_the_quarter_was_two_fifths}. \emph{And the
mode itself does not move across that last step}: the first peak reads the same multipole before and
after, and only the scale it is reported against changes.

\emph{That is the shape of the whole comparison, and stripping the last convention shows it again at
the other end.} This arm's onset is solved so that its acoustic scale meets the one fitted number
while the control's is an output, and the instrument was built to ask whether the deficit is
therefore an artefact of where the pin was put. \emph{Scanning the onset over a factor of $3.6$
answers it, and the answer is no}: $\ell_{A}$ runs from $360.6$ down to $239.3$, a range of $51\%$,
while the first peak sits between $206$ and $210$ throughout --- \emph{the one fitted number moves
the denominator and leaves the numerator where it
is}\rcpt{P15_the_one_fitted_number_moves_the_scale_and_not_the_peak}. \emph{So $\ell_{1}/\ell_{A}$
does move with the pin}, steeply enough to cross the sky's $0.7312$ between $z_{\mathrm{onset}}=6761$
and $8600$, \emph{and the deficit does not move at all}: the first peak is $206$ against the sky's
$220.6$ at every onset in the scan.

\emph{The trade the pin appears to offer is then an identity rather than a choice.} With $\ell_{1}$
fixed, pinning $\ell_{A}$ to the measured $301.6$ costs $6.59\%$ on $\ell_{1}/\ell_{A}$, and pinning
$\ell_{1}/\ell_{A}$ to the sky costs $6.59\%$ on $\ell_{A}$ --- \emph{the same number twice, because
it is one deficit booked against either of two observables rather than two deficits.} \emph{Nor can
both arms be pinned}: the control's acoustic scale falls monotonically as its start recedes and
approaches $301.4$ from above, so the target lies below its floor and no onset meets it. \emph{The
control arrives at the scale this arm spends its one fitted number to reach}, and what the pin buys
is which of the two acoustic observables carries the $6.6\%$ --- not whether it is carried.

\emph{And the likelihood is run, both arms scored on the same bins by the same code, with the amplitude
fitted so that what is compared is shape.} \emph{The result is a rejection of this arm's spectrum and is
stated as one.} Over $133$ bins of Planck TT the control returns $\chi^{2}=279.4$, or $2.10$ per bin; this
arm returns $\chi^{2}=15752.0$, or $118.4$ per bin\rcpt{P15_where_the_likelihood_sits}. \emph{The separation
between the two models, measured as a distance between them rather than as a difference of two numbers each
taken against the sky, is $122$ per bin --- eleven standard deviations per
bin}\rcpt{P15_the_floor_is_a_distance_between_models_not_a_number_from_the_data}.
...
spectrum. \emph{What is not configuration-limited is the ratio}: the separation between the arms is $58$
times the control's own distance from the data on the same bins. \emph{And the verdict does not turn on which
measure is used} --- $F_{2}$ and the separation statistic agree to within $5\%$, at $15473$ against
$16261$. \emph{Nor is it a truncation artefact}: cutting at four-fifths of the ceiling the control reads
$1.69$ per bin over $104$ bins and this arm $82.05$, so the disagreement is large at both
cutoffs\rcpt{P15_where_the_likelihood_sits}. \emph{So the deficit is not the configuration's}: it is the
$6.6\%$ position deficit and the height ratios of \S\ref{sec:refit-bound}, carried across every bin.
