...
one part in nine against the position's factor of two---and it is never $1$.
...
absent is not a spacing. \emph{So this construction's acoustic series is some $23\%$ too close at the sky's
angular scale under every reading that produces an acoustic series at all}, and that, rather than the
position of any one peak, is the disagreement that does not depend on a choice.
...
What has not been varied is the background itself, whose starting redshift is solved so that the acoustic
scale comes out at a fixed value---a target rather than an output, and a separate item from the datum.
