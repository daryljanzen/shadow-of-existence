...
%   rate (the load-bearing falsifiable claim). The perturbation sector FACTORIZES: the substrate
...
%   Exposed edges: (1) the ACOUSTIC PEAK STRUCTURE -- the two-arm line-of-sight transfer is RUN
%   (sec:refit-bound): first peak 206 against the sky's 220.6 (6.6% position deficit), heights
%   20.7%/29.2% below the sky's ratios, TT likelihood rejects this arm's spectrum; the driving
%   overshoots, so no mechanism for the deficit is in hand. (2) the damping-SCALE signature --
%   computed, non-reabsorbable by omega_b; the joint fit leaves the predicted Gaussian residual that no
%   tilt removes; whether the tilt displacement is a tension needs an n_s constraint off the damping
%   tail. The early ISW is present (sec:isw); what supplies none is the substrate rate.
