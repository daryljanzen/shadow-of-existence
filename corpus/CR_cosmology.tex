...
likelihood is run}: fitting both arms to the Planck~2018 \texttt{plik\_lite} binned
$TT$ likelihood with five free parameters each returns $\chi^{2}=397.13$ for this construction against
$206.44$ for flat $\Lambda$CDM, so the spectrum derived here is disfavoured against the standard model on that
data. \emph{What that fit weighs is the five-parameter spectrum construction}, and what carries the
construction back to the geometry is the acoustic transfer of \S\ref{sec:refit-bound}, which is where the
open end lies. 
