...
signature almost entirely in shape}, and what it costs is a shift in the inferred tilt. \emph{Both the
residual and the shift scale with $r^{2}-1$}, so they are fixed by the same two choices the ratio is: where
the plasma is handed over, and how far the diffusion integral is carried. At the branch point with the
integral taken to recombination that factor is $0.045$, where a ratio of $1.082$ gives $0.171$---so the
figures quoted for the larger ratio scale down with it, and with the integral taken to the visibility peak
instead the sign reverses. \emph{So the signature
...
predicted form}. \emph{It survives at $1.50$ in $\chi^{2}$ per bin} over the $185$-bin ceiling
window on the instrument's own control spectrum, which is larger than either figure above and
consistent with the ordering they already record, the more faithful route being the larger.
...
displacement is the value of nothing without the window it was fitted over. \emph{In the
likelihood's own units} the signature costs $+357$ in $\chi^{2}$ with amplitude alone and $+85$ over
$171$ bins once the tilt is refitted: \emph{the fit improves the comparison by three quarters of the
cost and does not remove
it}\rcpt{P15_the_joint_fit_leaves_the_gaussian_and_the_bound_was_chi2_not_sigma}.
