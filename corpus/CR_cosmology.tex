...
\subsection{The acoustic comparison: the instrument, and what it does not yet return}\label{sec:refit-bound}
...
\emph{The first peak's position is fixed by the initial datum and not by the transfer, and that is a
statement about what this construction does and does not determine.} The datum \S\ref{sec:coherence} states
is one amplitude per mode and a \emph{common} phase; it fixes that the phase is common without fixing which
phase it is. A second freedom sits beside it: what the amplitude is ``flat in wavenumber'' \emph{at}, the
construction reading its transfer shape at each mode's own horizon crossing where an alternative reads it at
each mode's phase at the seam. \emph{Across the eighteen readings of the two freedoms together the first peak
spans a factor of $2.26$}, and at one phase the peak near $172$ is not present at
all\rcpt{P15_the_first_peak_is_the_seam_datum_and_the_spacing_is_not}\rcpt{P15_both_datum_freedoms_opened_and_the_spacing_still_holds}.
\emph{So the first peak's position is not a statement of this construction}: it is a statement about a seam
datum the construction does not fix, and no reading of it is the construction's prediction.
...
\emph{What the instrument does not yet return is this cosmology's own arm at that convergence, and the paper
records that rather than a figure.} The truncation was found and repaired on the control, whose answer is
known independently and against which a repair can be checked; \emph{the same mechanism truncates this
cosmology's arm, and its acoustic figures are accordingly not in hand.} \emph{What is owed is therefore one
computation and not a further argument}: the two-arm transfer run to convergence in the wavenumber integral,
on the leaf congruence, with the seam datum's two freedoms carried as the range they are. Until it is run,
this cosmology's acoustic comparison is an open computation of this paper's own, and the peak spacing, the
acoustic phase and the peak heights are quantities it does not here report.

\emph{One thing about the inherited datum should be held apart from that, because it does not wait on the
transfer.} The datum taken \emph{alone}---$\rho_{r}/\rho_{m}=2$ exactly at onset---fixes
$1+z_{\mathrm{onset}}=2\omega_{m}/\omega_{r}$, and that determination is \emph{not} $H_{0}$-independent: at
fixed $\Omega_{m}$, with $\omega_{r}$ set by the measured $T_{\mathrm{CMB}}$, it returns
$z_{\mathrm{onset}}=6840$, $7378$ and $8024$ at $H_{0}=67.4$, $70.0$ and $73.0$. It therefore agrees with the
angle-fixed value near $H_{0}=67.4$ and not across the range; at the directly measured $H_{0}$ the
angle-fixed onset carries $\rho_{r}/\rho_{m}=1.69$ rather than $2.0$\rcpt{P15_zonset_determinations}. This
does not bear on the datum's standing---it is inherited and measured, and the argument is that a complete
collapse hands over radiation of \emph{order} the matter density rather than a finely tuned value, which
$1.69$ meets as well as $2.0$ does---but the closeness to $2$ holds at one $H_{0}$ and is not a further
