...
geometry: moving the discrete scale alone buys three per cent against a gap of a hundred. \emph{It is the
late integrated Sachs--Wolfe term}, which at these multipoles is sourced above the discrete floor and is
therefore retained by the sum, filling the deficit; the treatment that truncates its line-of-sight integration
earlier is deeper at exactly the multipoles where the two disagree, and pushing that truncation later moves it
back toward the other. \emph{The shape cross-validates on both treatments and both backgrounds} --- minimum
at $\ell=4$, recovery by $\ell=8$ --- \emph{and it is the depth alone that is open, with its cause named}. 
