...
amplitude in closed form, scored on one set of bins, the control settles at $0.90$ in $\chi^{2}$ per bin and
this arm at $1.30$: \emph{the arm at $1.45$ times the control's distance, where the computed spectrum sits at
$2.22$ on the same bins}---so \emph{freedom closes about a third of the gap and leaves the
rest}\rcpt{P15_the_refit_leaves_the_background_where_the_distances_put_it_and_does_not_close_the_phase}.

\emph{What the refit does not move is the background.} Given four free parameters this arm's expansion rate
moves by $0.011\%$ and its matter density by $0.20\%$ from the values the baryon-acoustic distances and the
...
$1186$, $354$, $84$ and $23$ in $\chi^{2}$ on the four parameters, which is the distinction between a
parameter that does not move and one that cannot. \emph{The one parameter that does move is the tilt}: this
arm prefers $0.9949$ against the control's $0.9559$, bluer by $0.039$.
...
$4.5\%$ out---\emph{worse}, the fit trading it for likelihood elsewhere---and the fourth peak's excess is
unmoved at $0.7\%$. \emph{So both are properties of the construction rather than of the background it is
...
to the present instead, the two treatments agree}: the deficits are $0.477$, $0.430$, $0.349$, $0.659$,
$0.909$, $0.982$ and $0.998$ of the flat expectation at $\ell=2$ through $8$, against the other treatment's
values to within three per cent at every multipole and one per cent above
$\ell=5$\rcpt{P15_the_low_multipole_floor_moves_with_no_background_and_the_factor_two_is_the_late_isw}. \emph{The factor of two was the missing term and
