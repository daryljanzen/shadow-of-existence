...
\emph{The seam datum carries two freedoms, and how far the first peak moves with them is a measurement this
paper does not yet report.} The datum \S\ref{sec:coherence} states is one amplitude per mode and a
\emph{common} phase; it fixes that the phase is common without fixing which phase it is. A second freedom
sits beside it: what the amplitude is ``flat in wavenumber'' \emph{at}, the construction reading its transfer
shape at each mode's own horizon crossing where an alternative reads it at each mode's phase at the seam.
\emph{Whether the first peak's position is therefore a statement about the datum rather than about the
construction turns on how wide that span is}, and the span is one of the quantities the transfer of
\S\ref{sec:refit-bound} is run to measure: it must be read at converged wavenumber and on the leaf
congruence, and no reading taken otherwise settles it either way.
