...
\emph{And the undriven position, which the transfer also reports differently on the two arms, turns out not
to be a physical difference at all.} The ratio of the first peak to the acoustic scale is
$\ell_1/\ell_A=k_1 r_s/\pi$ --- \emph{the distance cancels identically} --- so it was never a projection
quantity, which is why removing each source term, the damping envelope and the visibility function in turn
leaves it untouched. What it compares is the sound horizon each arm \emph{reports} against the phase its
modes actually accumulate.

\emph{Those two are different integrals here, and the difference is a convention rather than a rate.} The
control's sound horizon is taken from the start of its integration and this arm's from the onset, and the two
arms begin in different places --- \emph{a confound the instrument's own comment names}, that a comparison
between the arms confounds the rate with the start. \emph{Run symmetrically it very largely dissolves}: with
the start matched the apparent difference overshoots and \emph{changes sign}, and with the sound-horizon
convention matched as well it falls to a quarter of its original size and stays on the far side of zero. The
two mismatches were partly cancelling.

\emph{One asymmetry is not removed and is stated rather than absorbed}: this arm's onset is solved so that
its acoustic scale meets the one fitted number, while the control's is then an output. \emph{So the matched
comparison still sets a pinned scale against a free one}, and what survives it is not yet a clean physical
number.
