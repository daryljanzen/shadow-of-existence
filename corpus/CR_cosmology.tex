...
%   the handover's (a few % at the branch point, ~35% at a fitted onset) and its SIGN the diffusion
%   integral's endpoint (+2.2% to recombination, -0.9% to the visibility peak). The early ISW is present (sec:isw); what supplies none is the substrate rate.
...
paragraph derives, and nothing else---so with the diffusion length carried to recombination, $r=1.022$, the
ratio is $0.96$ at $\ell_{D}$ and $0.90$ at $1.5\,\ell_{D}$, while carried to the peak of the visibility
function, $r=0.991$, it is a rise of the same order rather than a fall\rcpt{R1_the_correction_stopped_at_the_sentence_and_the_number_it_defines_was_left_behind}.
...
\emph{The residual choice is the diffusion integral's endpoint, and it is stated rather than absorbed.}
Carried to recombination the signature is $+2.2\%$; carried to the peak of the visibility function it is
$-0.9\%$ --- a swing of three points where the cosmological parameters together move it by two
hundredths\rcpt{UNC_error_budget}. \emph{So the sign is the endpoint's and the size is the handover's}, and
the ordering of the four combinations is the same on either endpoint. \emph{What the signature is not} is
