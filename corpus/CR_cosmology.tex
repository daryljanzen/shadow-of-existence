...
%   OUTPUT; H_0 ~ 68.5 is fixed by BAO and the angle together, and the ladder discrepancy is left where the
...
\emph{One thing about that figure should be stated because two of this corpus's instruments do not agree on it, and the disagreement is worth knowing rather than averaging.} The $9\%$ above is the observable $\theta_D/\theta_*$, computed on standard thermodynamics with the sound horizon re-anchored to the observed acoustic scale so that only the damping length moves. \emph{The two-arm transfer, measuring the raw diffusion length at the visibility peak, returns $7.55\%$ on its own arms} --- \emph{a different quantity at a different epoch, and not a correction to this one}. \emph{Neither number is offered as a replacement for the other}, and closing the gap between the two routes is a thing this paper does not do.
...
it}: the $9\%$ runs the ionisation history through a Boltzmann-validated recombination, the closed form takes $x_{e}$ as a sharp cut at $a_{\mathrm{rec}}$ and carries the inherited datum unchanged to
...
is then $\theta_{D}/\theta_{*}$ larger by a few per cent, its size fixed by the handover (\S\ref{sec:diffusion-scale}), and $\ell_{*}=\pi D_{M}/r_{s}=302.2$
...
point the ratio is a few per cent above flat $\Lambda$CDM's; a start placed later on the expanding leg
...
\bibitem{JanzenGeometricCore} D.~Janzen, \emph{Reached through the imaginary, real by construction: the maximally symmetric substrate as the geometric--ontological core}, companion paper (P17).
