...
\emph{So the acoustic statement reaches its final form, and it is one number and one mechanism where this
section began with four items and a ratio.} This cosmology reproduces the acoustic \emph{spacing}---the
quantity its one fitted parameter is fitted to, and which it then tracks at the full acoustic rate when that
parameter is moved---and disagrees with the sky in the acoustic \emph{phase} by $0.62\pi$, a discrepancy
robust to the fitted parameter and only a third reachable by the one freedom the datum leaves open.
