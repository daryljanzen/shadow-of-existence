...
\emph{Under that sits a structural statement rather than a ratio, and it is the sharper way to say what the
comparison registers.} In ordinary acoustics the peak positions \emph{are} set by the sound horizon, so a
$24\%$ fall in $\rs$ should raise $\ell_{1}$ by about $31\%$. \emph{Here it raises it by seven}: this
construction's peaks track its own sound horizon at roughly a quarter of the rate acoustics requires.
\emph{We register that and do not scan it}---whether the peaks \emph{should} be so tied is a question about
the physics and not about a setting, and no freedom this instrument exposes will answer it.
