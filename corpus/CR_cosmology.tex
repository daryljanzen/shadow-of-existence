...
\emph{Subtracting the driving is what measures it}: run on the control, removing every coupling to the
potential moves the first peak from $220$ to $276$ and $\ell_{1}/\ell_{A}$ by $0.1858$, so the modes carrying
the first peak are driven and the shift is the driving's
size\rcpt{P15_the_phase_is_the_driving_and_the_undriven_arms_agree}. \emph{The same subtraction on this arm
is what the phase residual is to be read against}, and the two arms run it on one set of equations.
