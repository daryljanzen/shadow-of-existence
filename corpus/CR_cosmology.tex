...
the branch point and the acoustic modes are inside it at the onset---the reading
Prop.~\ref{prop:subhorizon} returns on the \emph{stacking} rate, which is the rate the boundary below is
computed on---then between
...
$\ell\simeq144$\footnote{\emph{On the leaf congruence the same census moves this boundary.} The rate rule assigns a process running \emph{in} the content to the leaf, where the comoving Hubble wavenumber at the onset is $0.018\,\mathrm{Mpc}^{-1}$ rather than $0.011$ and the corresponding multipole is $237.7$ rather than $144$. \emph{Recomputing the re-entry on that rate is a calculation this paper does not carry out}, and the boundary quoted here is the stacking rate's.}, so every mode at and above the acoustic peaks crossed before the plasma began while
...
\emph{And that difference is measured rather than argued, model-free, by subtraction against an undriven
reference that calibrates at unity}: the control's acoustic phase at first turnover is \emph{flat in
wavenumber to $2.1\%$}, where this arm's varies by a factor of
$3.9$ across the same band\rcpt{P15_the_driving_shift_by_subtraction}. \emph{So the scale-free interval is
not an inference from the potential's form but a shape read off the two arms}, and the contrast is with a
reference whose own answer is known.
