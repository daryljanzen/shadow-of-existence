...
true $\Lambda$CDM's $\chi^{2}$ on the same bins, and \emph{what that remainder is has not been named}.
It is small enough that the comparison can now discriminate and large enough that it is still an
instrument and not a measurement.
...
disagreement with the sky remains the open question it was.
...
one part in nine against the position's factor of two---and it is never $1$. \emph{So this construction's
acoustic series is some $21\%$ too close at the sky's angular scale under every reading of its own datum},
and that, rather than the position of any one peak, is the disagreement that does not depend on a choice.
\emph{And the likelihood separation of the last section survives the freedom that moved the peak}: on
identical bins the arm costs between two hundred and four hundred and fifty times the control at every
phase, so the pre-registered rule fires throughout. \emph{One freedom in the datum has been closed and one
has not}---the amplitude's flatness in wavenumber is not scanned here, and a scan that moved the
\emph{spacing} would be the one that mattered.
