...
\emph{And the position the transfer reports differently on the two arms turns out not to be a
physical difference at all.} The ratio of the first peak to the acoustic scale is
$\ell_{1}/\ell_{A}=k_{1}\rs/\pi$ --- \emph{the distance cancels identically} --- so it was never a
projection quantity, which is why removing each source term, the damping envelope and the visibility
function in turn leaves it untouched. What it compares is the sound horizon each arm \emph{reports}
against the phase its modes actually accumulate.

\emph{Those two are different integrals here, and the difference is a convention rather than a rate.}
Both arms integrate the sound horizon from $a\to0$, the plasma being handed over at the branch point
on this one and the control having no other start, so the comparison does not confound the rate with the
start\rcpt{P15_the_symmetric_comparison_was_never_runnable_and_the_quarter_was_two_fifths}. \emph{And the
quantity that comparison tests is the sound horizon each arm reports against the phase its modes actually
accumulate}: here the two agree, the comb at $298.0$ against a reported scale of $302.9$.
