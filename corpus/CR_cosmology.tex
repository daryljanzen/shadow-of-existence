...
accommodation, and it is spent.} \emph{And the angle the fitted onset reports is not the spacing the computed
spectrum carries}: fitted to the computed peaks by the procedure this paper runs on the sky, that arm's comb
is $313.0$ where the reported scale is $301.6$, so the accommodation and the comb are two quantities and only
the second is what the peak positions measure.
