...
stated as one.} Over $133$ bins of Planck TT the control returns $\chi^{2}=279.4$, or $2.10$ per bin; this
arm returns $556.7$, or $4.19$ per bin\rcpt{P15_where_the_likelihood_sits}. \emph{The band-by-band
comparison is even across the range} --- $1.70$, $2.22$, $4.23$ and $9.29$ per bin against the control's
$0.77$, $1.44$, $2.62$ and $3.73$ over the four quarters from $\ell=100$ to $1300$ --- so the disagreement is
not one feature carrying the whole of it. \emph{The comparison between the arms is taken as a distance between
...
\emph{The configuration limits the absolute values and does not account for the gap.} The control sits at
$2.10$ per bin rather than unity because the multipole ceiling cuts the damping tail on an unlensed
spectrum, and the arm inherits the same limitation. \emph{What the comb does not carry is the
