...
Algebraic type is shown to be no constraint on the range: the operator reaches Petrov type O~\cite{Petrov1954} (de Sitter, Friedmann--Lemaître--Robertson--Walker), type D (Schwarzschild--de Sitter, the Kerr--NUT--(A)dS family), and type I (anisotropic Bianchi cosmologies at three Killing vectors, the Zipoy--Voorhees and Weyl class at two), with the speciality invariant\ldg{spectral_theory}\ldg{algebraic_geometry} verified to separate them. Type D is thereby identified as the separable \emph{corner}, not the edge: the corner where the substrate symmetry surfaces as a Killing tensor~\cite{WalkerPenrose1970}, with the rest of every class generic and needing no such structure. 
