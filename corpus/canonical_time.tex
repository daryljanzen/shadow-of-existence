...
the marginal congruence $(\dd r/\dd\tilde\tau)^{2}+f=1$---the $\gamma=1$, $J=0$ case of the effective-potential form $(\dd r/\dd\tau)^{2}=\gamma^{2}-V_{\mathrm{eff}}$ with $V_{\mathrm{eff}}=f$ derived in~\cite[Eq.~(4.24)]{JanzenThesis}---which is $\tfrac12\dot r^{2}+V=\tfrac12$ with
...
one}\rcpt{S1_the_shear_needs_exactly_one_new_counterterm_and_my_own_count_was_one_too_many}.
