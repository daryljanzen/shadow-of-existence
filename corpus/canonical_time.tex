...
ordinary zero-point degree of a field in four dimensions. \emph{So ``standard'' is the right word and
...
one-parameter family they cannot be told apart. \emph{But the tower does not live on the substrate.}
It lives on the layer, whose observable rate is the radiation-free $\sinh^{2/3}$~\cite{JanzenCRcosmology},
and that background's Ricci scalar runs---divergent at early times, approaching the de~Sitter $12H^{2}$
late---so on it the invariants are independent functions and the basis is not degenerate.
\emph{The open question is therefore no longer whether one dimensionful constant can regulate at all,
but whether that degeneracy survives on a background whose curvature runs}---a question with a stated
object, a known instrument in the sub-leading heat-kernel coefficients, and a decidable answer. The propagating sector itself is a clean, unitary, deparametrized evolution---a true Hamiltonian generating the graviton on the layer, in cosmic time.
