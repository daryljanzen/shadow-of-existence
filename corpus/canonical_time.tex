...
operator $\hat\Gamma$ belongs to. \emph{A counterterm basis is a statement about a class of fixed
backgrounds, and in the coupled sector there is no fixed background to state it on.} \emph{So the open
question is no longer whether one dimensionful constant can regulate at all}---on the free tower's own
geometry it does, and for a structural reason---\emph{but whether the degeneracy survives when the
scale factor is quantized}: a question with a stated object, a known instrument in the sub-leading
heat-kernel coefficients, and a decidable answer. The propagating sector itself is a clean, unitary, deparametrized evolution---a true Hamiltonian generating the graviton on the layer, in cosmic time.
